...
public class AmbienceServer
{
	...
	private void SpawnForcedEntities()
	{
		...
		for (int actuallySelected = _forcedSpawns.Count - 1; actuallySelected >= 0; actuallySelected--)
		{
			AmbienceSpawnInfo num = _forcedSpawns[actuallySelected];
			Player player;
			if (num.targetPlayer == -1)
			{
				FindPlayerThatCanSeeBackgroundAmbience(out player);
			}
			else
			{
				player = Main.player[num.targetPlayer];
			}
			if (player != null && IsPlayerAtRightHeightForType(num.skyEntityType, player))
			{
				SpawnForPlayer(player, num.skyEntityType);
			}
			_forcedSpawns.RemoveAt(actuallySelected);
		}
	}
