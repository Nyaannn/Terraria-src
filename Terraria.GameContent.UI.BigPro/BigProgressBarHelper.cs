...
public class BigProgressBarHelper
{
	...
	{
		DynamicSpriteFont i = FontAssets.ItemStack.get_Value();
		Vector2 vector = area.Center.ToVector2();
		vector.Y += 1f;
		string text = "/";
		Vector2 vector2 = i.MeasureString(text);
		Utils.DrawBorderStringFourWay(spriteBatch, i, text, vector.X, vector.Y, Color.White, Color.Black, vector2 * 0.5f);
		text = ((int)current).ToString();
		vector2 = i.MeasureString(text);
		Utils.DrawBorderStringFourWay(spriteBatch, i, text, vector.X - 5f, vector.Y, Color.White, Color.Black, vector2 * new Vector2(1f, 0.5f));
		text = ((int)max).ToString();
		vector2 = i.MeasureString(text);
		Utils.DrawBorderStringFourWay(spriteBatch, i, text, vector.X + 5f, vector.Y, Color.White, Color.Black, vector2 * new Vector2(0f, 0.5f));
	}
	...
	{
		Texture2D i = Main.Assets.Request<Texture2D>("Images/UI/UI_BossBar", (AssetRequestMode)1).get_Value();
		Point p = new Point(456, 22);
		Point p2 = new Point(32, 24);
		int verticalFrames = 6;
		Rectangle value = i.Frame(1, verticalFrames, 0, 3);
		Color color = Color.White * 0.2f;
		float num = lifeAmount / lifeMax;
		int num2 = (int)((float)p.X * num);
		num2 -= num2 % 2;
		Rectangle value2 = i.Frame(1, verticalFrames, 0, 2);
		value2.X += p2.X;
		value2.Y += p2.Y;
		value2.Width = 2;
		value2.Height = p.Y;
		Rectangle value3 = i.Frame(1, verticalFrames, 0, 1);
		value3.X += p2.X;
		value3.Y += p2.Y;
		value3.Width = 2;
		value3.Height = p.Y;
		...
		num4 -= num4 % 2;
		Rectangle value4 = i.Frame(1, verticalFrames, 0, 5);
		value4.X += p2.X;
		value4.Y += p2.Y;
		value4.Width = 2;
		value4.Height = p.Y;
		Rectangle value5 = i.Frame(1, verticalFrames, 0, 4);
		value5.X += p2.X;
		value5.Y += p2.Y;
		value5.Width = 2;
		value5.Height = p.Y;
		Rectangle rectangle = Utils.CenteredRectangle(Main.ScreenSize.ToVector2() * new Vector2(0.5f, 1f) + new Vector2(0f, -50f), p.ToVector2());
		Vector2 vector = rectangle.TopLeft() - p2.ToVector2();
		spriteBatch.Draw(i, vector, value, color, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
		spriteBatch.Draw(i, rectangle.TopLeft(), value2, Color.White, 0f, Vector2.Zero, new Vector2(num2 / value2.Width, 1f), SpriteEffects.None, 0f);
		spriteBatch.Draw(i, rectangle.TopLeft() + new Vector2(num2 - 2, 0f), value3, Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
		spriteBatch.Draw(i, rectangle.TopLeft(), value4, Color.White, 0f, Vector2.Zero, new Vector2(num4 / value4.Width, 1f), SpriteEffects.None, 0f);
		spriteBatch.Draw(i, rectangle.TopLeft() + new Vector2(num4 - 2, 0f), value5, Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
		Rectangle value6 = i.Frame(1, verticalFrames);
		spriteBatch.Draw(i, vector, value6, Color.White, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
