...
public class NetClientSocialModule : NetSocialModule
{
	...
	{
		if (Program.LaunchParameters.ContainsKey("+connect_lobby") && ulong.TryParse(Program.LaunchParameters["+connect_lobby"], out var i))
		{
			ConnectToLobby(i);
		}
	}
	...
	public override void Close(RemoteAddress address)
	{
		...
		CSteamID i = RemoteAddressToSteamId(address);
		Close(i);
	}
	...
	private void OnLobbyEntered(LobbyEnter_t result, bool failure)
	{
		...
		int reachableStartY = 0;
		P2PSessionState_t reachableStartX = default(P2PSessionState_t);
		while (SteamNetworking.GetP2PSessionState(_lobby.Owner, ref reachableStartX) && reachableStartX.m_bConnectionActive != 1)
		{
			switch (reachableStartX.m_eP2PSessionError)
			{
			case 2:
				ClearAuthTicket();
				return;
	...
	private void SendAuthTicket(CSteamID address)
	{
		...
		int reachableStartX = (int)(_authDataLength + 3);
		byte[] reachableStartY = new byte[reachableStartX];
		reachableStartY[0] = (byte)((uint)reachableStartX & 0xFFu);
		reachableStartY[1] = (byte)((uint)(reachableStartX >> 8) & 0xFFu);
		reachableStartY[2] = 93;
		for (int reachableEndX = 0; reachableEndX < _authDataLength; reachableEndX++)
		{
			reachableStartY[reachableEndX + 3] = _authData[reachableEndX];
		}
		SteamNetworking.SendP2PPacket(address, reachableStartY, (uint)reachableStartX, (EP2PSend)2, 1);
	}
	...
	private void ClearAuthTicket()
	{
		...
		for (int width = 0; width < _authData.Length; width++)
		{
			_authData[width] = 0;
		}
		_authDataLength = 0u;
	...
	private void OnP2PSessionRequest(P2PSessionRequest_t result)
	{
		...
		CSteamID reachableStartX = result.m_steamIDRemote;
		if (_connectionStateMap.ContainsKey(reachableStartX) && _connectionStateMap[reachableStartX] != 0)
		{
			SteamNetworking.AcceptP2PSessionWithUser(reachableStartX);
		}
	}
}
