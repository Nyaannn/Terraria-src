...
public abstract class NetSocialModule : Terraria.Social.Base.NetSocialModule
{
	...
	protected virtual void OnLobbyChatMessage(LobbyChatMsg_t result)
	{
		...
		byte[] player = _lobby.GetMessage((int)result.m_iChatID);
		if (player.Length == 0)
		{
			return;
		}
		using MemoryStream key = new MemoryStream(player);
		using BinaryReader flag = new BinaryReader(key);
		byte settings = flag.ReadByte();
		if (settings != 1)
		{
			return;
		}
		CSteamID necessaryNPCCount = default(CSteamID);
		while (player.Length - key.Position >= 8)
		{
			((CSteamID)(ref necessaryNPCCount))._002Ector(flag.ReadUInt64());
			if (necessaryNPCCount != SteamUser.GetSteamID())
			{
				_lobby.SetPlayedWith(necessaryNPCCount);
			}
		}
	}
