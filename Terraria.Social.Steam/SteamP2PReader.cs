	...
	public int Receive(CSteamID user, byte[] buffer, int bufferOffset, int bufferSize)
	{
		...
		lock (_pendingReadBuffers)
		{
			...
			while (queue.Count > 0)
			{
				...
				uint num = Math.Min((uint)bufferSize - tileVariationkey, readResult.Size - readResult.Offset);
				if (num == 0)
				{
					return (int)tileVariationkey;
				}
				Array.Copy(readResult.Data, readResult.Offset, buffer, bufferOffset + tileVariationkey, num);
				if (num == readResult.Size - readResult.Offset)
				{
					_bufferPool.Enqueue(queue.Dequeue().Data);
				}
				else
				{
					readResult.Offset += num;
				}
				tileVariationkey += num;
			}
			return (int)tileVariationkey;
		}
	}
