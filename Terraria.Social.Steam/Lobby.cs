...
public class Lobby
{
	...
	public void Create(bool inviteOnly, APIDispatchDelegate<LobbyCreated_t> callResult)
	{
		...
		SteamAPICall_t type = SteamMatchmaking.CreateLobby((ELobbyType)((!inviteOnly) ? 1 : 0), 256);
		_lobbyCreatedExternalCallback = callResult;
		_lobbyCreated.Set(type, (APIDispatchDelegate<LobbyCreated_t>)null);
	...
	public void Join(CSteamID lobbyId, APIDispatchDelegate<LobbyEnter_t> callResult)
	{
		...
		if (State == LobbyState.Inactive)
		{
			...
			SteamAPICall_t smartCursorUsageInfo = SteamMatchmaking.JoinLobby(lobbyId);
			_lobbyEnter.Set(smartCursorUsageInfo, (APIDispatchDelegate<LobbyEnter_t>)null);
		}
	}
	...
	public byte[] GetMessage(int index)
	{
		...
		CSteamID inventory = default(CSteamID);
		EChatEntryType num = default(EChatEntryType);
		int i = SteamMatchmaking.GetLobbyChatEntry(Id, index, ref inventory, _messageBuffer, _messageBuffer.Length, ref num);
		byte[] j = new byte[i];
		Array.Copy(_messageBuffer, j, i);
		return j;
	}
