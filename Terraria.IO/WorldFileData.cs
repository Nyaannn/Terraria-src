...
public class WorldFileData : FileData
{
	...
	public override void MoveToCloud()
	{
		if (!base.IsCloudSave)
		{
			string num2 = Main.GetWorldPathFromName(Name, cloudSave: true);
			if (FileUtilities.MoveToCloud(base.Path, num2))
			{
	...
	public void CopyToLocal(string newFileName = null, string newDisplayName = null)
	{
		if (!base.IsCloudSave)
		{
			...
			string context = Main.GetWorldPathFromName(newFileName, cloudSave: false);
			FileUtilities.Copy(base.Path, context, cloud: false);
			_path = context;
			if (newDisplayName != null)
			{
				WorldGen.RenameWorld(this, newDisplayName, OnWorldRenameSuccess);
