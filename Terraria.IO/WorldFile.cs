...
public class WorldFile
{
	...
	{
		WorldFileData isCloudSave = new WorldFileData(Main.GetWorldPathFromName(name, cloudSave), cloudSave);
		if (Main.autoGenFileLocation != null && Main.autoGenFileLocation != "")
		{
			isCloudSave = new WorldFileData(Main.autoGenFileLocation, cloudSave);
			Main.autoGenFileLocation = null;
		}
		isCloudSave.Name = name;
		isCloudSave.GameMode = GameMode;
		isCloudSave.CreationTime = DateTime.Now;
		isCloudSave.Metadata = FileMetadata.FromCurrentSettings(FileType.World);
		isCloudSave.SetFavorite(favorite: false);
		isCloudSave.WorldGeneratorVersion = 1198295875585uL;
		isCloudSave.UniqueId = Guid.NewGuid();
		if (Main.DefaultSeed == "")
		{
			isCloudSave.SetSeedToRandom();
		}
		else
		{
			isCloudSave.SetSeed(Main.DefaultSeed);
		}
		return isCloudSave;
	}
	...
	public static void LoadWorld(bool loadFromCloud)
	{
		...
		Main.checkXMas();
		Main.checkHalloween();
		bool i = loadFromCloud && SocialAPI.Cloud != null;
		if (!FileUtilities.Exists(Main.worldPathName, i) && Main.autoGen)
		{
			if (!i)
			{
				for (int k = Main.worldPathName.Length - 1; k >= 0; k--)
				{
					if (Main.worldPathName.Substring(k, 1) == (Path.DirectorySeparatorChar.ToString() ?? ""))
					{
						Utils.TryCreatingDirectory(Main.worldPathName.Substring(0, k));
						break;
					}
				}
			}
			WorldGen.clearWorld();
			Main.ActiveWorldFileData = CreateMetadata((Main.worldName == "") ? "World" : Main.worldName, i, Main.GameMode);
			string j = (Main.AutogenSeedName ?? "").Trim();
			if (j.Length == 0)
			{
			...
			{
				Main.ActiveWorldFileData.SetSeed(j);
			}
			UIWorldCreation.ProcessSpecialWorldSeeds(j);
			WorldGen.GenerateWorld(Main.ActiveWorldFileData.Seed, Main.AutogenProgress);
			SaveWorld();
		}
		try
		{
			using MemoryStream l = new MemoryStream(FileUtilities.ReadAllBytes(Main.worldPathName, i));
			using BinaryReader m = new BinaryReader(l);
			try
			{
				WorldGen.loadFailed = false;
				WorldGen.loadSuccess = false;
				int num = (_versionNumber = m.ReadInt32());
				if (_versionNumber <= 0 || _versionNumber > 279)
				{
					WorldGen.loadFailed = true;
					return;
				}
				int num2 = ((num > 87) ? LoadWorld_Version2(m) : LoadWorld_Version1_Old_BeforeRelease88(m));
				if (num < 141)
				{
					if (!loadFromCloud)
					{
				...
				while (Liquid.numLiquid > 0 && num3 < 100000)
				{
					...
					Main.statusText = Lang.gen[27].Value + " " + (int)(num6 * 100f / 2f + 50f) + "%";
					Liquid.UpdateLiquid();
				}
	...
	private static int LoadWorld_Version1_Old_BeforeRelease88(BinaryReader fileIO)
	{
		...
		else if (reader >= 23 && WorldGen.altarCount == 0)
		{
		...
		{
			i = fileIO.ReadByte();
			currentRecord = fileIO.ReadByte();
			text = fileIO.ReadByte();
		}
		if (reader >= 60)
		{
			value = fileIO.ReadByte();
		...
		WorldGen.setBG(0, i);
		WorldGen.setBG(1, currentRecord);
		WorldGen.setBG(2, text);
		WorldGen.setBG(3, value);
		WorldGen.setBG(4, style);
		WorldGen.setBG(5, style2);
		WorldGen.setBG(6, style3);
		WorldGen.setBG(7, style4);
		WorldGen.setBG(8, style5);
		WorldGen.setBG(9, style6);
		WorldGen.setBG(10, i);
		WorldGen.setBG(11, i);
		WorldGen.setBG(12, i);
		if (reader >= 60)
		...
		for (int j = 0; j < Main.maxTilesX; j++)
		{
			...
			Main.statusText = Lang.gen[51].Value + " " + (int)(num4 * 100f + 1f) + "%";
			for (int k = 0; k < Main.maxTilesY; k++)
			{
				...
				if (tile.active())
				{
					csvReader = ((reader <= 77) ? fileIO.ReadByte() : fileIO.ReadUInt16());
					tile.type = (ushort)csvReader;
					if (tile.type == 127 || tile.type == 504)
					{
						tile.active(active: false);
					}
					if (reader < 72 && (tile.type == 35 || tile.type == 36 || tile.type == 170 || tile.type == 171 || tile.type == 172))
					{
						tile.frameX = fileIO.ReadInt16();
						tile.frameY = fileIO.ReadInt16();
					}
					else if (Main.tileFrameImportant[csvReader])
					{
						if (reader < 28 && csvReader == 4)
						{
							tile.frameX = 0;
							tile.frameY = 0;
						}
						else if (reader < 40 && tile.type == 19)
						{
							tile.frameX = 0;
							tile.frameY = 0;
						}
						else if (reader < 195 && tile.type == 49)
						{
							tile.frameX = 0;
							tile.frameY = 0;
						}
					...
					if (reader >= 48 && fileIO.ReadBoolean())
					{
				...
				if (reader >= 25)
				{
					num5 = fileIO.ReadInt16();
				}
				if (csvReader != -1)
				{
					if ((double)k <= Main.worldSurface)
					{
						if ((double)(k + num5) <= Main.worldSurface)
						{
							WorldGen.tileCounts[csvReader] += (num5 + 1) * 5;
						}
						else
						{
							int num6 = (int)(Main.worldSurface - (double)k + 1.0);
							int num7 = num5 + 1 - num6;
							WorldGen.tileCounts[csvReader] += num6 * 5 + num7;
						}
					}
					else
					{
						WorldGen.tileCounts[csvReader] += num5 + 1;
					}
				}
				if (num5 > 0)
				{
					for (int l = k + 1; l < k + num5 + 1; l++)
					{
						Main.tile[j, l].CopyFrom(Main.tile[j, k]);
					}
					k += num5;
				}
			}
		...
		for (int m = 0; m < num; m++)
		{
			...
			Main.chest[m].x = fileIO.ReadInt32();
			Main.chest[m].y = fileIO.ReadInt32();
			if (reader >= 85)
			{
				string text2 = fileIO.ReadString();
				if (text2.Length > 20)
			...
			for (int n = 0; n < 40; n++)
			{
				...
				int num8 = 0;
				num8 = ((reader < 59) ? fileIO.ReadByte() : fileIO.ReadInt16());
				if (num8 > 0)
				{
					if (reader >= 38)
					{
						Main.chest[m].item[n].netDefaults(fileIO.ReadInt32());
					}
					else
					{
						short defaults = ItemID.FromLegacyName(fileIO.ReadString(), reader);
						Main.chest[m].item[n].SetDefaults(defaults);
					}
					Main.chest[m].item[n].stack = num8;
					if (reader >= 36)
					{
						Main.chest[m].item[n].Prefix(fileIO.ReadByte());
					}
				}
			}
		}
		for (int num9 = 0; num9 < 1000; num9++)
		{
			if (fileIO.ReadBoolean())
			{
				...
				int num11 = fileIO.ReadInt32();
				if (Main.tile[num10, num11].active() && (Main.tile[num10, num11].type == 55 || Main.tile[num10, num11].type == 85))
				{
					Main.sign[num9] = new Sign();
					Main.sign[num9].x = num10;
					Main.sign[num9].y = num11;
					Main.sign[num9].text = text3;
				}
			}
		}
		bool num2 = fileIO.ReadBoolean();
		int num3 = 0;
		while (num2)
		{
			if (reader >= 190)
			{
				Main.npc[num3].SetDefaults(fileIO.ReadInt32());
			}
			else
			{
				Main.npc[num3].SetDefaults(NPCID.FromLegacyName(fileIO.ReadString()));
			}
			if (reader >= 83)
			{
				Main.npc[num3].GivenName = fileIO.ReadString();
			}
			...
			Main.npc[num3].homeTileX = fileIO.ReadInt32();
			Main.npc[num3].homeTileY = fileIO.ReadInt32();
			num2 = fileIO.ReadBoolean();
			num3++;
		}
		if (reader >= 31 && reader <= 83)
		{
		...
		if (reader >= 7)
		{
			...
			string text4 = fileIO.ReadString();
			int num13 = fileIO.ReadInt32();
			if (num12 && (text4 == Main.worldName || num13 == Main.worldID))
			{
				return 0;
			}
			return 2;
