...
public class PlayerFileData : FileData
{
	...
	{
		PlayerFileData array = new PlayerFileData();
		array.Metadata = FileMetadata.FromCurrentSettings(FileType.Player);
		array.Player = player;
		array._isCloudSave = SocialAPI.Cloud != null && SocialAPI.Cloud.EnabledByDefault;
		array._path = Main.GetPlayerPathFromName(player.name, array.IsCloudSave);
		(array.IsCloudSave ? Main.CloudFavoritesData : Main.LocalFavoriteData).ClearEntry(array);
		Player.SavePlayer(array, skipMapSave: true);
		return array;
	}
	...
	public override void MoveToLocal()
	{
		...
		string address = Main.GetPlayerPathFromName(Name, cloudSave: false);
		if (!FileUtilities.MoveToLocal(base.Path, address))
		{
			return;
		}
		string fileName = GetFileName(includeExtension: false);
	...
	{
		bool socket = Main.gamePaused && !Main.hasFocus;
		bool flag = Main.instance.IsActive && !socket;
		if (Main.gameMenu)
		{
			flag = false;
		}
