...
public class Preferences
{
	...
	public bool Load()
	{
		...
		lock (_lock)
		{
			...
			try
			{
				if (!UseBson)
				{
					string item = File.ReadAllText(_path);
					_data = JsonConvert.DeserializeObject<Dictionary<string, object>>(item, _serializerSettings);
				}
				else
				{
					using FileStream value = File.OpenRead(_path);
					BsonReader num = new BsonReader((Stream)value);
					try
					{
						JsonSerializer val = JsonSerializer.Create(_serializerSettings);
						_data = val.Deserialize<Dictionary<string, object>>((JsonReader)(object)num);
					}
					finally
					{
	...
	public T Get<T>(string name, T defaultValue)
	{
		...
		lock (_lock)
		{
			try
			{
				if (_data.TryGetValue(name, out var scaleIfSelected))
				{
					if (scaleIfSelected is T)
					{
						return (T)scaleIfSelected;
					}
					if (scaleIfSelected is JObject)
					{
						return JsonConvert.DeserializeObject<T>(((object)(JObject)scaleIfSelected).ToString());
					}
					return (T)Convert.ChangeType(scaleIfSelected, typeof(T));
				}
				return defaultValue;
			}
			catch
