...
public class LockOnHelper
{
	...
	public static Vector2 PredictedPosition
	{
		get
		{
			NPC collection = AimedTarget;
			if (collection == null)
			{
				return Vector2.Zero;
			}
			Vector2 list = collection.Center;
			if (NPC.GetNPCLocation(_targets[_pickedTarget], seekHead: true, averageDirection: false, out var num, out var color))
			{
				list = color;
				list += Main.npc[num].Distance(Main.player[Main.myPlayer].Center) / 2000f * Main.npc[num].velocity * 45f;
			}
			Player black = Main.player[Main.myPlayer];
			int colorOnlyShaderIndex = ItemID.Sets.LockOnAimAbove[black.inventory[black.selectedItem].type];
			while (colorOnlyShaderIndex > 0 && list.Y > 100f)
			{
				Point vector = list.ToTileCoordinates();
				vector.Y -= 4;
				if (!WorldGen.InWorld(vector.X, vector.Y, 10) || WorldGen.SolidTile(vector.X, vector.Y))
				{
					break;
				}
				list.Y -= 16f;
			...
			float? num2 = ItemID.Sets.LockOnAimCompensation[black.inventory[black.selectedItem].type];
			if (num2.HasValue)
			{
				list.Y -= collection.height / 2;
				Vector2 i = list - black.Center;
				Vector2 value = i.SafeNormalize(Vector2.Zero);
				value.Y -= 1f;
				float j = i.Length();
				j = (float)Math.Pow(j / 700f, 2.0) * 700f;
				list.Y += value.Y * j * num2.Value * 1f;
				list.X += (0f - value.X) * j * num2.Value * 1f;
			}
			return list;
		}
	}
	...
	{
		int i = _targets[_pickedTarget];
		RefreshTargets(Main.MouseWorld, 2000f);
		if (_targets.Count < 1 || (_targets.Count == 1 && i == _targets[0]))
		{
			SetActive(on: false);
			return;
		}
	...
	public static void Draw(SpriteBatch spriteBatch)
	{
		...
		Texture2D color = TextureAssets.LockOnCursor.get_Value();
		Rectangle rectangle = new Rectangle(0, 0, color.Width, 12);
		Rectangle rectangle2 = new Rectangle(0, 16, color.Width, 12);
		...
		Color color3 = t.MultiplyRGBA(new Color(0.8f, 0.8f, 0.8f, 0.8f));
		...
		for (int i = 0; i < _drawProgress.GetLength(0); i++)
		{
			...
			if (num9 > 0f)
			{
				...
				spriteBatch.Draw(color, pos, rectangle, color2 * num9, 0f, rectangle.Size() / 2f, new Vector2(0.58f, 1f) * num2 * num4 * (1f + num9) / 2f, SpriteEffects.None, 0f);
				spriteBatch.Draw(color, pos, rectangle2, color3 * num9 * num9, 0f, rectangle2.Size() / 2f, new Vector2(0.58f, 1f) * num2 * num4 * (1f + num9) / 2f, SpriteEffects.None, 0f);
			}
			...
			if (num11 > 0f)
			{
				...
				for (int j = 0; (float)j < num13; j++)
				{
					...
					spriteBatch.Draw(color, pos3, rectangle, t * num11, rotation, rectangle.Size() / 2f, new Vector2(0.58f, 1f) * num2 * num5 * (1f + num11) / 2f, SpriteEffects.None, 0f);
					spriteBatch.Draw(color, pos3, rectangle2, t2 * num11 * num11, rotation, rectangle2.Size() / 2f, new Vector2(0.58f, 1f) * num2 * num5 * (1f + num11) / 2f, SpriteEffects.None, 0f);
				}
			}
		}
	}
