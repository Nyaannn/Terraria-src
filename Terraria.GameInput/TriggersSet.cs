...
public class TriggersSet
{
	...
	{
		bool i = Main.gameMenu || Main.ingameOptionsWindow || Main.editChest || Main.editSign || ((Main.playerInventory || Main.LocalPlayer.talkNPC != -1) && PlayerInput.CurrentProfile.UsingDpadMovekeys());
		bool linkedListNode = Up || (i && MenuUp);
		bool value2 = Right || (i && MenuRight);
		bool value = Down || (i && MenuDown);
		bool next = Left || (i && MenuLeft);
		return new Vector2(value2.ToInt() - next.ToInt(), value.ToInt() - linkedListNode.ToInt());
	}
	...
	public void CopyInto(Player p)
	{
		...
		if (SmartSelect && LatestInputMode.TryGetValue("SmartSelect", out var flag) && IsInputFromGamepad(flag))
		{
			PlayerInput.SettingsForUI.SetCursorMode(CursorMode.Gamepad);
		}
		bool i = PlayerInput.Triggers.Current.HotbarPlus || PlayerInput.Triggers.Current.HotbarMinus;
		if (i)
		{
			HotbarHoldTime++;
		}
		...
		if (HotbarScrollCD > 0 && (!(HotbarScrollCD == 1 && i) || PlayerInput.CurrentProfile.HotbarRadialHoldTimeRequired <= 0))
		{
			HotbarScrollCD--;
		}
	}
