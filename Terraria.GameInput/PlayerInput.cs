...
public class PlayerInput
{
	...
	{
		bool num = Main.instance.IsActive;
		if (_lastActivityState != num)
		{
			...
			Triggers.Reset();
			string text = "";
			List<Keys> activeSky = GetPressedKeys();
			for (int i = 0; i < activeSky.Count; i++)
			{
				text = string.Concat(text, activeSky[i], ", ");
			}
			_invalidatorCheck = text;
		}
		_lastActivityState = num;
	}
	...
	public static void EnterBuildingMode()
	{
		...
		{
			int num = _UIPointForBuildingMode;
			if (num < 50 && num >= 0 && Main.player[Main.myPlayer].inventory[num].stack > 0)
			{
				Utils.Swap(ref Main.mouseItem, ref Main.player[Main.myPlayer].inventory[num]);
			}
		}
	}
	...
	public static void Initialize()
	{
		...
		PlayerInputProfile notRetro = new PlayerInputProfile("Redigit's Pick");
		notRetro.Initialize(PresetProfiles.Redigit);
		Profiles.Add(notRetro.Name, notRetro);
		notRetro = new PlayerInputProfile("Yoraiz0r's Pick");
		notRetro.Initialize(PresetProfiles.Yoraiz0r);
		Profiles.Add(notRetro.Name, notRetro);
		notRetro = new PlayerInputProfile("Console (Playstation)");
		notRetro.Initialize(PresetProfiles.ConsolePS);
		Profiles.Add(notRetro.Name, notRetro);
		notRetro = new PlayerInputProfile("Console (Xbox)");
		notRetro.Initialize(PresetProfiles.ConsoleXBox);
		Profiles.Add(notRetro.Name, notRetro);
		notRetro = new PlayerInputProfile("Custom");
		notRetro.Initialize(PresetProfiles.Redigit);
		Profiles.Add(notRetro.Name, notRetro);
		notRetro = new PlayerInputProfile("Redigit's Pick");
		notRetro.Initialize(PresetProfiles.Redigit);
		OriginalProfiles.Add(notRetro.Name, notRetro);
		notRetro = new PlayerInputProfile("Yoraiz0r's Pick");
		notRetro.Initialize(PresetProfiles.Yoraiz0r);
		OriginalProfiles.Add(notRetro.Name, notRetro);
		notRetro = new PlayerInputProfile("Console (Playstation)");
	...
	{
		PlayerInputProfile fileStream = Profiles["Custom"];
		...
		for (int i = 0; i < array.GetLength(0); i++)
		{
			...
			{
				fileStream.InputModes[InputMode.Keyboard].KeyStatus[array[i, 1]] = new List<string> { currentValue };
				fileStream.InputModes[InputMode.KeyboardUI].KeyStatus[array[i, 1]] = new List<string> { currentValue };
			}
		}
	}
	...
	public static void LockGamepadButtons(string TriggerName)
	{
		...
		KeyConfiguration value = null;
		if (CurrentProfile.InputModes.TryGetValue(CurrentInputMode, out value) && value.KeyStatus.TryGetValue(TriggerName, out num))
		{
			_buttonsLocked.AddRange(num);
		}
	}
	...
	private static bool GamePadInput()
	{
		...
		GamePadState text = default(GamePadState);
		bool i = false;
		for (int j = 0; j < 4; j++)
		{
		...
		Player texture2D = Main.player[Main.myPlayer];
		bool num2 = UILinkPointNavigator.Available && !InBuildingMode;
		InputMode vector = InputMode.XBoxGamepad;
		if (Main.gameMenu || num2 || texture2D.talkNPC != -1 || texture2D.sign != -1 || IngameFancyUI.CanCover())
		{
			vector = InputMode.XBoxGamepadUI;
		}
		if (!Main.gameMenu && InBuildingMode)
		{
			vector = InputMode.XBoxGamepad;
		}
		if (CurrentInputMode == InputMode.XBoxGamepad && vector == InputMode.XBoxGamepadUI)
		{
			num = true;
		}
		if (CurrentInputMode == InputMode.XBoxGamepadUI && vector == InputMode.XBoxGamepad)
		{
			num = true;
		}
		if (num)
		{
			CurrentInputMode = vector;
		}
		KeyConfiguration color = CurrentProfile.InputModes[vector];
		int flag = 2145386496;
		for (int k = 0; k < ButtonsGamepad.Length; k++)
		{
			if ((int)((uint)flag & (uint)ButtonsGamepad[k]) > 0)
			{
				continue;
			}
			string text2 = ButtonsGamepad[k].ToString();
			bool flag2 = _buttonsLocked.Contains(text2);
			if (text.IsButtonDown(ButtonsGamepad[k]))
			{
				if (!flag2)
				{
					if (CheckRebindingProcessGamepad(text2))
					{
						return false;
					}
					color.Processkey(Triggers.Current, text2, vector);
					num = true;
				}
			}
			else
			{
				_buttonsLocked.Remove(text2);
			}
		}
		GamepadThumbstickLeft = text.ThumbSticks.Left * new Vector2(1f, -1f) * new Vector2(CurrentProfile.LeftThumbstickInvertX.ToDirectionInt() * -1, CurrentProfile.LeftThumbstickInvertY.ToDirectionInt() * -1);
		GamepadThumbstickRight = text.ThumbSticks.Right * new Vector2(1f, -1f) * new Vector2(CurrentProfile.RightThumbstickInvertX.ToDirectionInt() * -1, CurrentProfile.RightThumbstickInvertY.ToDirectionInt() * -1);
		Vector2 gamepadThumbstickRight = GamepadThumbstickRight;
		Vector2 gamepadThumbstickLeft = GamepadThumbstickLeft;
		Vector2 vector2 = gamepadThumbstickRight;
		if (vector2 != Vector2.Zero)
		{
			vector2.Normalize();
		}
		Vector2 vector3 = gamepadThumbstickLeft;
		...
		if (vector == InputMode.XBoxGamepadUI)
		{
			num3 = 0.4f;
		...
		if (Vector2.Dot(-Vector2.UnitX, vector3) >= num3 && gamepadThumbstickLeft.X < 0f - CurrentProfile.LeftThumbstickDeadzoneX)
		{
			if (CheckRebindingProcessGamepad(Buttons.LeftThumbstickLeft.ToString()))
			{
				return false;
			}
			color.Processkey(Triggers.Current, Buttons.LeftThumbstickLeft.ToString(), vector);
			num = true;
		}
		if (Vector2.Dot(Vector2.UnitX, vector3) >= num3 && gamepadThumbstickLeft.X > CurrentProfile.LeftThumbstickDeadzoneX)
		{
			if (CheckRebindingProcessGamepad(Buttons.LeftThumbstickRight.ToString()))
			{
				return false;
			}
			color.Processkey(Triggers.Current, Buttons.LeftThumbstickRight.ToString(), vector);
			num = true;
		}
		if (Vector2.Dot(-Vector2.UnitY, vector3) >= num3 && gamepadThumbstickLeft.Y < 0f - CurrentProfile.LeftThumbstickDeadzoneY)
		{
			if (CheckRebindingProcessGamepad(Buttons.LeftThumbstickUp.ToString()))
			{
				return false;
			}
			color.Processkey(Triggers.Current, Buttons.LeftThumbstickUp.ToString(), vector);
			num = true;
		}
		if (Vector2.Dot(Vector2.UnitY, vector3) >= num3 && gamepadThumbstickLeft.Y > CurrentProfile.LeftThumbstickDeadzoneY)
		{
			if (CheckRebindingProcessGamepad(Buttons.LeftThumbstickDown.ToString()))
			{
				return false;
			}
			color.Processkey(Triggers.Current, Buttons.LeftThumbstickDown.ToString(), vector);
			num = true;
		}
		if (Vector2.Dot(-Vector2.UnitX, vector2) >= num3 && gamepadThumbstickRight.X < 0f - CurrentProfile.RightThumbstickDeadzoneX)
		{
			if (CheckRebindingProcessGamepad(Buttons.RightThumbstickLeft.ToString()))
			{
				return false;
			}
			color.Processkey(Triggers.Current, Buttons.RightThumbstickLeft.ToString(), vector);
			num = true;
		}
		if (Vector2.Dot(Vector2.UnitX, vector2) >= num3 && gamepadThumbstickRight.X > CurrentProfile.RightThumbstickDeadzoneX)
		{
			if (CheckRebindingProcessGamepad(Buttons.RightThumbstickRight.ToString()))
			{
				return false;
			}
			color.Processkey(Triggers.Current, Buttons.RightThumbstickRight.ToString(), vector);
			num = true;
		}
		if (Vector2.Dot(-Vector2.UnitY, vector2) >= num3 && gamepadThumbstickRight.Y < 0f - CurrentProfile.RightThumbstickDeadzoneY)
		{
			if (CheckRebindingProcessGamepad(Buttons.RightThumbstickUp.ToString()))
			{
				return false;
			}
			color.Processkey(Triggers.Current, Buttons.RightThumbstickUp.ToString(), vector);
			num = true;
		}
		if (Vector2.Dot(Vector2.UnitY, vector2) >= num3 && gamepadThumbstickRight.Y > CurrentProfile.RightThumbstickDeadzoneY)
		{
			if (CheckRebindingProcessGamepad(Buttons.RightThumbstickDown.ToString()))
			{
				return false;
			}
			color.Processkey(Triggers.Current, Buttons.RightThumbstickDown.ToString(), vector);
			num = true;
		}
		if (text.Triggers.Left > triggersDeadzone)
		{
			if (CheckRebindingProcessGamepad(Buttons.LeftTrigger.ToString()))
			{
				return false;
			}
			color.Processkey(Triggers.Current, Buttons.LeftTrigger.ToString(), vector);
			num = true;
		}
		if (text.Triggers.Right > triggersDeadzone)
		{
			...
			if (vector == InputMode.XBoxGamepadUI && SteamDeckIsUsed && SettingsForUI.CurrentCursorMode == CursorMode.Mouse)
			{
				Triggers.Current.MouseLeft = true;
			}
			else
			{
				color.Processkey(Triggers.Current, newKey, vector);
				num = true;
			}
		}
		bool flag3 = ItemID.Sets.GamepadWholeScreenUseRange[texture2D.inventory[texture2D.selectedItem].type] || texture2D.scope;
		Item item = texture2D.inventory[texture2D.selectedItem];
		int num4 = item.tileBoost + ItemID.Sets.GamepadExtraRange[item.type];
		if (texture2D.yoyoString && ItemID.Sets.Yoyo[item.type])
		{
			num4 += 5;
		}
		...
		else if (texture2D.controlTorch)
		{
			num4++;
		}
		if (item.createWall > 0 || item.createTile > 0 || item.tileWand > 0)
		{
			num4 += texture2D.blockRange;
		}
		if (flag3)
		{
			num4 += 30;
		}
		if (texture2D.mount.Active && texture2D.mount.Type == 8)
		{
			num4 = 10;
		}
		bool flag4 = false;
		bool flag5 = !Main.gameMenu && !num2 && Main.SmartCursorWanted;
		if (!CursorIsBusy)
		{
			bool flag6 = Main.mapFullscreen || (!Main.gameMenu && !num2);
			int num5 = Main.screenWidth / 2;
			int num6 = Main.screenHeight / 2;
			if (!Main.mapFullscreen && flag6 && !flag3)
			{
				Point point = Main.ReverseGravitySupport(texture2D.Center - Main.screenPosition).ToPoint();
				num5 = point.X;
				num6 = point.Y;
			}
			if (texture2D.velocity == Vector2.Zero && gamepadThumbstickLeft == Vector2.Zero && gamepadThumbstickRight == Vector2.Zero && flag5)
			{
				num5 += texture2D.direction * 10;
			}
			...
			if (gamepadThumbstickRight != Vector2.Zero && flag6)
			{
				Vector2 vector4 = new Vector2(8f);
				if (!Main.gameMenu && Main.mapFullscreen)
				{
					vector4 = new Vector2(16f);
				}
				if (flag5)
				{
					vector4 = new Vector2(Player.tileRangeX * 16, Player.tileRangeY * 16);
					if (num4 != 0)
					{
						vector4 += new Vector2(num4 * 16, num4 * 16);
					}
					if (flag3)
					{
						vector4 = new Vector2(Math.Max(Main.screenWidth, Main.screenHeight) / 2);
					}
				}
				else if (!Main.mapFullscreen)
				{
					if (texture2D.inventory[texture2D.selectedItem].mech)
					{
						vector4 += Vector2.Zero;
					}
					else
					{
						vector4 += new Vector2(num4) / 4f;
					}
				}
				float m2 = Main.GameViewMatrix.ZoomMatrix.M11;
				Vector2 vector5 = gamepadThumbstickRight * vector4 * m2;
			...
			if (gamepadThumbstickLeft != Vector2.Zero && flag6)
			{
				float num9 = 8f;
				if (!Main.gameMenu && Main.mapFullscreen)
				{
				...
				{
					Vector2 vector6 = gamepadThumbstickLeft * num9;
					Main.mapFullscreenPos += vector6 * num9 * (1f / Main.mapFullscreenScale);
					num = true;
				}
				else if (!flag4 && Main.SmartCursorWanted && allowSecondaryGamepadAim)
				{
					float m3 = Main.GameViewMatrix.ZoomMatrix.M11;
					Vector2 vector7 = gamepadThumbstickLeft * new Vector2(Player.tileRangeX * 16, Player.tileRangeY * 16) * m3;
					if (num4 != 0)
					{
						vector7 = gamepadThumbstickLeft * new Vector2((Player.tileRangeX + num4) * 16, (Player.tileRangeY + num4) * 16) * m3;
					}
					if (flag3)
					{
						vector7 = new Vector2(Math.Max(Main.screenWidth, Main.screenHeight) / 2) * gamepadThumbstickLeft;
					}
					int num10 = (int)vector7.X;
					int num11 = (int)vector7.Y;
					MouseX = num10 + num5;
					MouseY = num11 + num6;
					flag4 = true;
				}
			...
			if (CurrentInputMode == InputMode.XBoxGamepad)
			{
				...
				if (SettingsForUI.AllowSecondaryGamepadAim)
				{
					...
					if (!Main.gameMenu && !num2)
					{
						if (flag3 && !Main.mapFullscreen)
						{
							float num14 = 1f;
						...
						{
							float num17 = 0f;
							if (texture2D.HeldItem.createTile >= 0 || texture2D.HeldItem.createWall > 0 || texture2D.HeldItem.tileWand >= 0)
							{
								num17 = 0.5f;
							}
							...
							num18 -= (float)(texture2D.height / 16 / 2 * 16);
		...
		if (CurrentInputMode != InputMode.XBoxGamepadUI && num)
		{
		...
		if (CurrentInputMode == InputMode.XBoxGamepadUI && num && !PreventCursorModeSwappingToGamepad)
		{
			SettingsForUI.SetCursorMode(CursorMode.Gamepad);
		}
		return num;
	...
	private static bool KeyboardInput()
	{
		...
		for (int l = 0; l < list.Count; l++)
		{
			...
			if (!(list[l] == Keys.Tab.ToString()) || !((offScreenTiles && SocialAPI.Mode == SocialMode.Steam) || flag))
			{
	...
	{
		string num = "Redigit's Pick";
		if (OriginalProfiles.ContainsKey(_selectedProfile))
		{
			num = _selectedProfile;
		}
		CurrentProfile.InputModes[inputMode].KeyStatus[trigger].Clear();
		CurrentProfile.InputModes[inputMode].KeyStatus[trigger].AddRange(OriginalProfiles[num].InputModes[inputMode].KeyStatus[trigger]);
	}
	...
	{
		string[] settings = text.Split(new string[1] { "\r\n" }, StringSplitOptions.None);
		foreach (string text2 in settings)
		{
			if (text2.Contains(": {"))
			{
	...
	public static void SetZoom_Test()
	{
		...
		Vector2 knownTrigger = Main.screenPosition + new Vector2(_originalMouseX, _originalMouseY);
		Vector2 vector2 = Main.screenPosition + new Vector2(_originalLastMouseX, _originalLastMouseY);
		Vector2 vector3 = Main.screenPosition + new Vector2(0f, 0f);
		Vector2 vector4 = Main.screenPosition + new Vector2(Main.screenWidth, Main.screenHeight);
		Vector2 vector5 = knownTrigger - vector;
		Vector2 vector6 = vector2 - vector;
		Vector2 vector7 = vector3 - vector;
		_ = vector4 - vector;
		float num = 1f / Main.GameViewMatrix.Zoom.X;
		float num2 = 1f;
		Vector2 vector8 = vector - Main.screenPosition + vector5 * num;
		Vector2 vector9 = vector - Main.screenPosition + vector6 * num;
		Vector2 screenPosition = vector + vector7 * num2;
		Main.mouseX = (int)vector8.X;
		Main.mouseY = (int)vector8.Y;
		Main.lastMouseX = (int)vector9.X;
		Main.lastMouseY = (int)vector9.Y;
	...
	{
		Vector2 vector = Main.screenPosition + new Vector2(Main.screenWidth, Main.screenHeight) / 2f;
		Vector2 item = Main.screenPosition + new Vector2(_originalMouseX, _originalMouseY);
		Vector2 vector2 = Main.screenPosition + new Vector2(_originalLastMouseX, _originalLastMouseY);
		Vector2 vector3 = item - vector;
		Vector2 vector4 = vector2 - vector;
		float num = 1f / Main.GameViewMatrix.Zoom.X;
		Vector2 vector5 = vector - Main.screenPosition + vector3 * num;
		Main.mouseX = (int)vector5.X;
		Main.mouseY = (int)vector5.Y;
		Vector2 vector6 = vector - Main.screenPosition + vector4 * num;
		Main.lastMouseX = (int)vector6.X;
		Main.lastMouseY = (int)vector6.Y;
	}
