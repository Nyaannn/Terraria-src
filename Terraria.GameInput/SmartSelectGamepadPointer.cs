...
public class SmartSelectGamepadPointer
{
	...
	{
		Vector2 collection = (new Vector2(Main.mouseX, Main.mouseY) - _center) / _size;
		float list = Math.Abs(collection.X);
		if (list < Math.Abs(collection.Y))
		{
			list = Math.Abs(collection.Y);
		}
		if (list > 1f)
		{
			collection /= list;
		}
		collection *= Main.GameViewMatrix.Zoom.X;
		return collection * _distUniform + _center;
	}
}
