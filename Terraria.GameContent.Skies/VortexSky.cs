...
public class VortexSky : CustomSky
{
	...
	public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
	{
		...
		float tile = Math.Min(1f, (Main.screenPosition.Y - 1000f) / 1000f);
		Vector2 result = Main.screenPosition + new Vector2(Main.screenWidth >> 1, Main.screenHeight >> 1);
		...
		for (int i = 0; i < _bolts.Length; i++)
		{
			...
			Vector2 vector3 = new Vector2(1f / _bolts[i].Depth, 0.9f / _bolts[i].Depth);
			Vector2 position = (_bolts[i].Position - result) * vector3 + result - Main.screenPosition;
			if (rectangle.Contains((int)position.X, (int)position.Y))
			{
				...
				float num = (float)life / 30f;
				spriteBatch.Draw(value, position, null, Color.White * tile * num * _fadeOpacity, 0f, Vector2.Zero, vector3.X * 5f, SpriteEffects.None, 0f);
			}
		}
	}
