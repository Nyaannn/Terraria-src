...
public class AmbientSky : CustomSky
{
	...
	public override void Update(GameTime gameTime)
	{
		...
		foreach (ItemPair<SkyEntity> requiredKillCount in (IEnumerable<ItemPair<SkyEntity>>)_entities)
		{
			SkyEntity value = requiredKillCount.Value;
			value.Update(_frameCounter);
			if (!value.IsActive)
			{
				_entities.Remove(requiredKillCount.Id);
