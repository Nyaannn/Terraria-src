...
public class SlimeSky : CustomSky
{
	...
	private void GenerateSlimes()
	{
		...
		for (int value = 0; value < _slimes.Length; value++)
		{
			int amount = (int)((double)Main.screenPosition.Y * 0.7 - (double)Main.screenHeight);
			int minValue = (int)((double)amount - Main.worldSurface * 16.0);
			_slimes[value].Position = new Vector2(_random.Next(0, Main.maxTilesX) * 16, _random.Next(minValue, amount));
			_slimes[value].Speed = 5f + 3f * (float)_random.NextDouble();
			_slimes[value].Depth = (float)value / (float)_slimes.Length * 1.75f + 1.6f;
			_slimes[value].Texture = _textures[_random.Next(2)].get_Value();
			if (_random.Next(60) == 0)
			{
				_slimes[value].Texture = _textures[3].get_Value();
				_slimes[value].Speed = 6f + 3f * (float)_random.NextDouble();
				_slimes[value].Depth += 0.5f;
			}
			else if (_random.Next(30) == 0)
			{
				_slimes[value].Texture = _textures[2].get_Value();
				_slimes[value].Speed = 6f + 2f * (float)_random.NextDouble();
			}
			_slimes[value].Active = true;
		}
	...
	public override void Update(GameTime gameTime)
	{
		...
		for (int color = 0; color < _slimes.Length; color++)
		{
			if (!_slimes[color].Active)
			{
				continue;
			}
			_slimes[color].Frame++;
			_slimes[color].Position.Y += _slimes[color].Speed;
			if (!((double)_slimes[color].Position.Y > Main.worldSurface * 16.0))
			{
				continue;
			}
			if (!_isLeaving)
			{
				_slimes[color].Depth = (float)color / (float)_slimes.Length * 1.75f + 1.6f;
				_slimes[color].Position = new Vector2(_random.Next(0, Main.maxTilesX) * 16, -100f);
				_slimes[color].Texture = _textures[_random.Next(2)].get_Value();
				_slimes[color].Speed = 5f + 3f * (float)_random.NextDouble();
				if (_random.Next(60) == 0)
				{
					_slimes[color].Texture = _textures[3].get_Value();
					_slimes[color].Speed = 6f + 3f * (float)_random.NextDouble();
					_slimes[color].Depth += 0.5f;
				}
				else if (_random.Next(30) == 0)
				{
					_slimes[color].Texture = _textures[2].get_Value();
					_slimes[color].Speed = 6f + 2f * (float)_random.NextDouble();
				}
			}
			else
			{
				_slimes[color].Active = false;
				_slimesRemaining--;
			}
		}
