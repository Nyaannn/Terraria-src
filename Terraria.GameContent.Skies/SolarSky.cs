...
public class SolarSky : CustomSky
{
	...
	public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
	{
		...
		for (int j = center; j < num; j++)
		{
			...
			{
				spriteBatch.Draw(_meteorTexture.get_Value(), position, new Rectangle(0, num3 * (_meteorTexture.Height() / 4), _meteorTexture.Width(), _meteorTexture.Height() / 4), Color.White * num2 * _fadeOpacity, 0f, Vector2.Zero, vector4.X * 5f * _meteors[j].Scale, SpriteEffects.None, 0f);
			}
		}
	}
	...
	public override void Activate(Vector2 position, params object[] args)
	{
		...
		for (int sandTileCount = 0; sandTileCount < _meteors.Length; sandTileCount++)
		{
			float player = (float)sandTileCount / (float)_meteors.Length;
			_meteors[sandTileCount].Position.X = player * ((float)Main.maxTilesX * 16f) + _random.NextFloat() * 40f - 20f;
			_meteors[sandTileCount].Position.Y = _random.NextFloat() * (0f - ((float)Main.worldSurface * 16f + 10000f)) - 10000f;
			if (_random.Next(3) != 0)
			{
				_meteors[sandTileCount].Depth = _random.NextFloat() * 3f + 1.8f;
			}
			else
			{
				_meteors[sandTileCount].Depth = _random.NextFloat() * 5f + 4.8f;
			}
			_meteors[sandTileCount].FrameCounter = _random.Next(12);
			_meteors[sandTileCount].Scale = _random.NextFloat() * 0.5f + 1f;
			_meteors[sandTileCount].StartX = _meteors[sandTileCount].Position.X;
		}
