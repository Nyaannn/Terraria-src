...
public class DropOneByOne : IItemDropRule
{
	...
	{
		float num = parameters.GetPersonalDropRate();
		float num2 = num * ratesInfo.parentDroprateChance;
		drops.Add(new DropRateInfo(itemId, parameters.MinimumItemDropsCount * (parameters.MinimumStackPerChunkBase + parameters.BonusMinDropsPerChunkPerPlayer), parameters.MaximumItemDropsCount * (parameters.MaximumStackPerChunkBase + parameters.BonusMaxDropsPerChunkPerPlayer), num2, ratesInfo.conditions));
