...
public class ItemDropDatabase
{
	...
	public void PrepareNPCNetIDsByTypeDictionary()
	{
		...
		foreach (KeyValuePair<int, NPC> num2 in ContentSamples.NpcsByNetId.Where((KeyValuePair<int, NPC> x) => x.Key < 0))
		{
			if (!_npcNetIdsByType.ContainsKey(num2.Value.type))
			{
				_npcNetIdsByType[num2.Value.type] = new List<int>();
			}
			_npcNetIdsByType[num2.Value.type].Add(num2.Value.netID);
		}
	}

	public void TrimDuplicateRulesForNegativeIDs()
	{
		for (int dollPlayer = -65; dollPlayer < 0; dollPlayer++)
		{
			if (_entriesByNpcNetId.TryGetValue(dollPlayer, out var value))
			{
				_entriesByNpcNetId[dollPlayer] = value.Distinct().ToList();
			}
		}
	}
	...
	{
		List<IItemDropRule> inv = new List<IItemDropRule>();
		if (includeGlobalDrops)
		{
			inv.AddRange(_globalEntries);
		}
		if (_entriesByNpcNetId.TryGetValue(npcNetId, out var num))
		{
			inv.AddRange(num);
		}
		return inv;
	...
	{
		for (int items = 0; items < npcNetIds.Length; items++)
		{
			RegisterToNPC(npcNetIds[items], new LeadingConditionRule(new Conditions.RemixSeed())).OnSuccess(entry);
		}
		return entry;
	}
	...
	public IItemDropRule RemoveFromNPC(int type, IItemDropRule entry)
	{
		...
		if (type > 0 && _npcNetIdsByType.TryGetValue(type, out var item))
		{
			for (int i = 0; i < item.Count; i++)
			{
				RemoveFromNPCNetId(item[i], entry);
			}
		}
		return entry;
	}
	...
	private void RegisterBloodMoonFishing()
	{
		...
		Conditions.IsBloodMoonAndNotFromStatue item = new Conditions.IsBloodMoonAndNotFromStatue();
		RegisterToMultipleNPCs(ItemDropRule.ByCondition(item, 4271, 100), 489, 490);
		RegisterToMultipleNPCs(ItemDropRule.ByCondition(item, 4271, 25), 587, 586, 621, 620);
	}
	...
	private void RegisterEclipse()
	{
		...
		Conditions.DownedAllMechBosses defaults = new Conditions.DownedAllMechBosses();
		Conditions.DownedPlantera stack = new Conditions.DownedPlantera();
		IItemDropRule rule = RegisterToNPC(477, new LeadingConditionRule(defaults));
		IItemDropRule pre = rule.OnSuccess(new LeadingConditionRule(stack));
		rule.OnSuccess(ItemDropRule.ExpertGetsRerolls(1570, 4, 1));
		pre.OnSuccess(ItemDropRule.ExpertGetsRerolls(2770, 20, 1));
		pre.OnSuccess(ItemDropRule.ExpertGetsRerolls(3292, 3, 1));
		RegisterToNPC(253, new LeadingConditionRule(defaults)).OnSuccess(ItemDropRule.ExpertGetsRerolls(1327, 40, 1));
		RegisterToNPC(460, new LeadingConditionRule(stack)).OnSuccess(ItemDropRule.ExpertGetsRerolls(3098, 40, 1));
		...
		RegisterToNPC(468, new LeadingConditionRule(stack)).OnSuccess(ItemDropRule.ExpertGetsRerolls(3105, 40, 1));
		RegisterToNPC(468, ItemDropRule.ExpertGetsRerolls(4738, 50, 1));
		RegisterToNPC(468, ItemDropRule.ExpertGetsRerolls(4739, 50, 1));
		RegisterToNPC(466, new LeadingConditionRule(stack)).OnSuccess(ItemDropRule.ExpertGetsRerolls(3106, 40, 1));
		RegisterToNPC(467, new LeadingConditionRule(stack)).OnSuccess(ItemDropRule.ExpertGetsRerolls(3249, 30, 1));
		IItemDropRule item = ItemDropRule.Common(3107, 25);
		IItemDropRule itemDropRule = ItemDropRule.WithRerolls(3107, 1, 25);
		item.OnSuccess(ItemDropRule.Common(3108, 1, 100, 200), hideLootReport: true);
		itemDropRule.OnSuccess(ItemDropRule.Common(3108, 1, 100, 200), hideLootReport: true);
		RegisterToNPC(463, new LeadingConditionRule(stack)).OnSuccess(new DropBasedOnExpertMode(item, itemDropRule));
	}
	...
	private void RegisterHardmodeDungeonDrops()
	{
		...
		RegisterToMultipleNPCs(ItemDropRule.ExpertGetsRerolls(1183, 400, 1), num);
		RegisterToMultipleNPCs(ItemDropRule.ExpertGetsRerolls(1266, 300, 1), num);
		RegisterToMultipleNPCsNotRemixSeed(ItemDropRule.ExpertGetsRerolls(671, 200, 1), num);
		RegisterToMultipleNPCsRemixSeed(ItemDropRule.ExpertGetsRerolls(2273, 200, 1), num);
		RegisterToMultipleNPCs(ItemDropRule.ExpertGetsRerolls(4679, 200, 1), num);
	...
	{
		Conditions.PumpkinMoonDropGatingChance number = new Conditions.PumpkinMoonDropGatingChance();
		Conditions.PumpkinMoonDropGateForTrophies condition = new Conditions.PumpkinMoonDropGateForTrophies();
		new Conditions.IsPumpkinMoon();
		new Conditions.FromCertainWaveAndAbove(15);
		RegisterToNPC(315, ItemDropRule.ByCondition(number, 1857, 20));
		int[] npcNetIds = new int[10] { 305, 306, 307, 308, 309, 310, 311, 312, 313, 314 };
		RegisterToMultipleNPCs(new LeadingConditionRule(number), npcNetIds).OnSuccess(ItemDropRule.OneFromOptions(10, 1788, 1789, 1790));
		IItemDropRule rule = RegisterToNPC(325, new LeadingConditionRule(number));
		...
		rule.OnSuccess(ItemDropRule.ByCondition(condition, 1855));
		...
		IItemDropRule rule2 = RegisterToNPC(327, new LeadingConditionRule(number));
		...
		rule2.OnSuccess(ItemDropRule.ByCondition(condition, 1856));
	...
	{
		short tEItemFrame = 262;
		RegisterToNPC(tEItemFrame, ItemDropRule.BossBag(3328));
		RegisterToNPC(tEItemFrame, ItemDropRule.MasterModeCommonDrop(4934));
		RegisterToNPC(tEItemFrame, ItemDropRule.MasterModeDropOnAllPlayers(4806, _masterModeDropRng));
		LeadingConditionRule leadingConditionRule = new LeadingConditionRule(new Conditions.NotExpert());
		RegisterToNPC(tEItemFrame, leadingConditionRule);
	...
	{
		LeadingConditionRule value = new LeadingConditionRule(new Conditions.MissingTwin());
		LeadingConditionRule leadingConditionRule = new LeadingConditionRule(new Conditions.NotExpert());
		value.OnSuccess(ItemDropRule.BossBag(3326));
		value.OnSuccess(leadingConditionRule);
		leadingConditionRule.OnSuccess(ItemDropRule.Common(2106, 7));
		leadingConditionRule.OnSuccess(ItemDropRule.Common(1225, 1, 15, 30));
		leadingConditionRule.OnSuccess(ItemDropRule.Common(549, 1, 25, 40));
		value.OnSuccess(ItemDropRule.MasterModeCommonDrop(4931));
		value.OnSuccess(ItemDropRule.MasterModeDropOnAllPlayers(4804, _masterModeDropRng));
		RegisterToMultipleNPCs(value, 126, 125);
		RegisterToMultipleNPCs(ItemDropRule.ByCondition(new Conditions.MechdusaKill(), 5382), 126, 125);
	}

	private void RegisterBoss_EOC()
	{
		Conditions.NotExpert value = new Conditions.NotExpert();
		Conditions.IsCrimsonAndNotExpert condition = new Conditions.IsCrimsonAndNotExpert();
		Conditions.IsCorruptionAndNotExpert condition2 = new Conditions.IsCorruptionAndNotExpert();
		short type = 4;
		...
		RegisterToNPC(type, ItemDropRule.ByCondition(value, 2112, 7));
		RegisterToNPC(type, ItemDropRule.ByCondition(value, 1299, 40));
		RegisterToNPC(type, ItemDropRule.ByCondition(condition, 880, 1, 30, 90));
		RegisterToNPC(type, ItemDropRule.ByCondition(condition, 2171, 1, 1, 3));
		RegisterToNPC(type, ItemDropRule.ByCondition(condition2, 47, 1, 20, 50));
		RegisterToNPC(type, ItemDropRule.ByCondition(condition2, 56, 1, 30, 90));
		RegisterToNPC(type, ItemDropRule.ByCondition(condition2, 59, 1, 1, 3));
	}
	...
	{
		Conditions.NotExpert num = new Conditions.NotExpert();
		short tEItemFrame = 113;
		RegisterToNPC(tEItemFrame, ItemDropRule.BossBag(3324));
		RegisterToNPC(tEItemFrame, ItemDropRule.MasterModeCommonDrop(4930));
		RegisterToNPC(tEItemFrame, ItemDropRule.MasterModeDropOnAllPlayers(4795, _masterModeDropRng));
		RegisterToNPC(tEItemFrame, ItemDropRule.ByCondition(num, 2105, 7));
		RegisterToNPC(tEItemFrame, ItemDropRule.ByCondition(num, 367));
		RegisterToNPC(tEItemFrame, new LeadingConditionRule(num)).OnSuccess(ItemDropRule.OneFromOptionsNotScalingWithLuck(1, 490, 491, 489, 2998));
		RegisterToNPC(tEItemFrame, new LeadingConditionRule(num)).OnSuccess(ItemDropRule.OneFromOptionsNotScalingWithLuck(1, 426, 434, 514, 4912));
	}

	private void RegisterBoss_AncientCultist()
	{
		short num = 439;
		RegisterToNPC(num, ItemDropRule.MasterModeCommonDrop(4937));
		RegisterToNPC(num, ItemDropRule.MasterModeDropOnAllPlayers(4809, _masterModeDropRng));
		RegisterToNPC(num, ItemDropRule.Common(3372, 7));
		RegisterToNPC(num, ItemDropRule.Common(3549));
	}
	...
	private void RegisterBoss_LunarTowers()
	{
		...
		DropOneByOne.Parameters num = parameters;
		DropOneByOne.Parameters parameters2 = num;
		parameters2.BonusMinDropsPerChunkPerPlayer = 1;
		parameters2.BonusMaxDropsPerChunkPerPlayer = 1;
		parameters2.MinimumStackPerChunkBase = (int)((float)num.MinimumStackPerChunkBase * 1.5f);
		parameters2.MaximumStackPerChunkBase = (int)((float)num.MaximumStackPerChunkBase * 1.5f);
		RegisterToNPC(517, new DropBasedOnExpertMode(new DropOneByOne(3458, num), new DropOneByOne(3458, parameters2)));
		RegisterToNPC(422, new DropBasedOnExpertMode(new DropOneByOne(3456, num), new DropOneByOne(3456, parameters2)));
		RegisterToNPC(507, new DropBasedOnExpertMode(new DropOneByOne(3457, num), new DropOneByOne(3457, parameters2)));
		RegisterToNPC(493, new DropBasedOnExpertMode(new DropOneByOne(3459, num), new DropOneByOne(3459, parameters2)));
	}
	...
	private void RegisterBoss_DukeFishron()
	{
		...
		RegisterToNPC(type, ItemDropRule.ByCondition(num, 2588, 7));
		RegisterToNPC(type, ItemDropRule.ByCondition(num, 2609, 15));
		RegisterToNPC(type, new LeadingConditionRule(new Conditions.NotRemixSeed())).OnSuccess(new LeadingConditionRule(num)).OnSuccess(ItemDropRule.OneFromOptions(1, 2611, 2624, 2622, 2621, 2623));
		RegisterToNPC(type, new LeadingConditionRule(new Conditions.RemixSeed())).OnSuccess(new LeadingConditionRule(num)).OnSuccess(ItemDropRule.OneFromOptions(1, 2611, 2624, 2622, 2621, 157));
	}
	...
	{
		Conditions.LegacyHack_IsABoss result = new Conditions.LegacyHack_IsABoss();
		RegisterToNPC(4, ItemDropRule.ByCondition(result, 1360, 10));
		RegisterToNPC(13, ItemDropRule.ByCondition(result, 1361, 10));
		RegisterToNPC(14, ItemDropRule.ByCondition(result, 1361, 10));
		RegisterToNPC(15, ItemDropRule.ByCondition(result, 1361, 10));
		RegisterToNPC(266, ItemDropRule.ByCondition(result, 1362, 10));
		RegisterToNPC(35, ItemDropRule.ByCondition(result, 1363, 10));
		RegisterToNPC(222, ItemDropRule.ByCondition(result, 1364, 10));
		RegisterToNPC(113, ItemDropRule.ByCondition(result, 1365, 10));
		RegisterToNPC(134, ItemDropRule.ByCondition(result, 1366, 10));
		RegisterToNPC(127, ItemDropRule.ByCondition(result, 1367, 10));
		RegisterToNPC(262, ItemDropRule.ByCondition(result, 1370, 10));
		RegisterToNPC(245, ItemDropRule.ByCondition(result, 1371, 10));
		RegisterToNPC(50, ItemDropRule.ByCondition(result, 2489, 10));
		RegisterToNPC(370, ItemDropRule.ByCondition(result, 2589, 10));
		RegisterToNPC(439, ItemDropRule.ByCondition(result, 3357, 10));
		RegisterToNPC(395, ItemDropRule.ByCondition(result, 3358, 10));
		RegisterToNPC(398, ItemDropRule.ByCondition(result, 3595, 10));
		RegisterToNPC(636, ItemDropRule.ByCondition(result, 4783, 10));
		RegisterToNPC(657, ItemDropRule.ByCondition(result, 4958, 10));
		RegisterToNPC(668, ItemDropRule.ByCondition(result, 5108, 10));
