...
public class BestiaryDatabaseNPCsPopulator
{
	...
	private void TryGivingEntryFlavorTextIfItIsMissing(BestiaryEntry entry)
	{
		...
		SpawnConditionBestiaryInfoElement condition = null;
		int? num = null;
		foreach (IBestiaryInfoElement item in entry.Info)
		{
			if (item is BestiaryPortraitBackgroundProviderPreferenceInfoElement bestiaryPortraitBackgroundProviderPreferenceInfoElement && bestiaryPortraitBackgroundProviderPreferenceInfoElement.GetPreferredProvider() is SpawnConditionBestiaryInfoElement spawnConditionBestiaryInfoElement)
			{
				condition = spawnConditionBestiaryInfoElement;
				break;
			}
			if (item is SpawnConditionBestiaryInfoElement spawnConditionBestiaryInfoElement2)
			{
				int displayTextPriority = spawnConditionBestiaryInfoElement2.DisplayTextPriority;
				if (!num.HasValue || displayTextPriority >= num)
				{
					condition = spawnConditionBestiaryInfoElement2;
		...
		if (condition != null)
		{
			string displayNameKey = condition.GetDisplayNameKey();
