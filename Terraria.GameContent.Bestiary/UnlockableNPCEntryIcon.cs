...
public class UnlockableNPCEntryIcon : IEntryIcon
{
	...
	public void Update(BestiaryUICollectionInfo providedInfo, Rectangle hitbox, EntryIconDrawSettings settings)
	{
		...
		if (NPCID.Sets.NPCBestiaryDrawOffset.TryGetValue(_npcNetId, out var result))
		{
			_npcCache.rotation = result.Rotation;
			_npcCache.scale = result.Scale;
			if (result.PortraitScale.HasValue && settings.IsPortrait)
			{
				_npcCache.scale = result.PortraitScale.Value;
			}
			positionOffsetCache = result.Position;
			num = result.Frame;
			num2 = result.Direction;
			num3 = result.SpriteDirection;
			num4 = result.Velocity;
			wet = result.IsWet;
			if (result.PortraitPositionXOverride.HasValue && settings.IsPortrait)
			{
				positionOffsetCache.X = result.PortraitPositionXOverride.Value;
			}
			if (result.PortraitPositionYOverride.HasValue && settings.IsPortrait)
			{
				positionOffsetCache.Y = result.PortraitPositionYOverride.Value;
			}
			if (result.CustomTexturePath != null)
			{
				val = Main.Assets.Request<Texture2D>(result.CustomTexturePath, (AssetRequestMode)1);
			}
	...
	{
		if (NPCID.Sets.SpecialSpawningRules.TryGetValue(_npcNetId, out var num) && num == 0)
		{
			Point num2 = (_npcCache.position - _npcCache.rotation.ToRotationVector2() * -1600f).ToTileCoordinates();
			_npcCache.ai[0] = num2.X;
			_npcCache.ai[1] = num2.Y;
		}
		switch (_npcNetId)
		{
		case 244:
