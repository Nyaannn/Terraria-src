...
public class TileObjectData
{
	...
	public int DrawYOffset
	{
		...
		set
		{
			...
			if (_linkedAlternates)
			{
				for (int text = 0; text < _alternates.data.Count; text++)
				{
					_alternates.data[text].DrawYOffset = value;
				}
			}
		}
	}
	...
	public bool DrawFlipVertical
	{
		...
		set
		{
			...
			if (_linkedAlternates)
			{
				for (int value2 = 0; value2 < _alternates.data.Count; value2++)
				{
					_alternates.data[value2].DrawFlipVertical = value;
				}
			}
		}
	}
	...
	public int StyleLineSkip
	{
		...
		set
		{
			...
			if (_linkedAlternates)
			{
				for (int vec = 0; vec < _alternates.data.Count; vec++)
				{
					_alternates.data[vec].StyleLineSkip = value;
				}
			}
		}
	}
	...
	public int StyleMultiplier
	{
		...
		set
		{
			...
			if (_linkedAlternates)
			{
				for (int num = 0; num < _alternates.data.Count; num++)
				{
					_alternates.data[num].StyleMultiplier = value;
				}
			}
		}
	}
	...
	public int Height
	{
		...
		set
		{
			...
			if (_linkedAlternates)
			{
				for (int vector = 0; vector < _alternates.data.Count; vector++)
				{
					_alternates.data[vector].Height = value;
				}
			}
		}
	}
	...
	public bool FlattenAnchors
	{
		...
		set
		{
			...
			if (_linkedAlternates)
			{
				for (int num = 0; num < _alternates.data.Count; num++)
				{
					_alternates.data[num].FlattenAnchors = value;
				}
			}
		}
	}
	...
	public bool isValidTileAnchor(int type)
	{
		...
		{
			num = _anchorTiles.tileValid;
			packetView = _anchorTiles.tileInvalid;
		}
		if (packetView != null)
		{
			for (int i = 0; i < packetView.Length; i++)
			{
				if (type == packetView[i])
				{
					return false;
				}
			}
	...
	{
		int[] dst = ((_anchorTiles != null) ? _anchorTiles.wallValid : null);
		if (dst == null)
		{
			if (type == 0)
			{
		...
		for (int stringBuilder = 0; stringBuilder < dst.Length; stringBuilder++)
		{
			if (type == dst[stringBuilder])
			{
				return true;
			}
		}
	...
	public bool isValidAlternateAnchor(int type)
	{
		...
		int[] dateTime = _anchorTiles.tileAlternates;
		if (dateTime == null)
		{
			return false;
		}
		for (int Name = 0; Name < dateTime.Length; Name++)
		{
	...
	public static TileObjectData GetTileData(Tile getTile)
	{
		...
		int panel = getTile.type;
		if (panel < 0 || panel >= _data.Count)
		{
			throw new ArgumentOutOfRangeException("Function called with a bad tile type");
		}
		TileObjectData scrollbar = _data[panel];
		if (scrollbar == null)
		{
			return null;
		}
		int localizedBackTextPanel = getTile.frameX / scrollbar.CoordinateFullWidth;
		int num = getTile.frameY / scrollbar.CoordinateFullHeight;
		int cheat = scrollbar.StyleWrapLimit;
		if (cheat == 0)
		{
			cheat = 1;
		}
		int localizedCheatsTextPanel = ((!scrollbar.StyleHorizontal) ? (localizedBackTextPanel * cheat + num) : (num * cheat + localizedBackTextPanel));
		int num2 = localizedCheatsTextPanel / scrollbar.StyleMultiplier;
		int num3 = localizedCheatsTextPanel % scrollbar.StyleMultiplier;
		int styleLineSkip = scrollbar.StyleLineSkip;
		if (styleLineSkip > 1)
		{
			if (scrollbar.StyleHorizontal)
			{
				num2 = num / styleLineSkip * cheat + localizedBackTextPanel;
				num3 = num % styleLineSkip;
			}
			else
			{
				num2 = localizedBackTextPanel / styleLineSkip * cheat + num;
				num3 = localizedBackTextPanel % styleLineSkip;
			}
		}
		if (scrollbar.SubTiles != null && num2 >= 0 && num2 < scrollbar.SubTiles.Count)
		{
			TileObjectData tileObjectData = scrollbar.SubTiles[num2];
			if (tileObjectData != null)
			{
				scrollbar = tileObjectData;
			}
		}
		if (scrollbar._alternates != null)
		{
			for (int i = 0; i < scrollbar.Alternates.Count; i++)
			{
				TileObjectData tileObjectData2 = scrollbar.Alternates[i];
				if (tileObjectData2 != null && num3 >= tileObjectData2.Style && num3 <= tileObjectData2.Style + tileObjectData2.RandomStyleRange)
				{
	...
	{
		TileObjectData sortableElementIndex = GetTileData(type, style, data.alternate);
		if (sortableElementIndex == null || sortableElementIndex._placementHooks == null || sortableElementIndex._placementHooks.postPlaceMyPlayer.hook == null)
		{
			return false;
		}
		PlacementHook panel = sortableElementIndex._placementHooks.postPlaceMyPlayer;
		if (panel.processedCoordinates)
		{
			tileX -= sortableElementIndex.Origin.X;
			tileY -= sortableElementIndex.Origin.Y;
		}
		return panel.hook(tileX, tileY, type, style, dir, data.alternate) == panel.badReturn;
	}

	public static void OriginToTopLeft(int type, int style, ref Point16 baseCoords)
	{
		TileObjectData i = GetTileData(type, style);
		if (i != null)
		{
			baseCoords = new Point16(baseCoords.X - i.Origin.X, baseCoords.Y - i.Origin.Y);
		}
	}
}
