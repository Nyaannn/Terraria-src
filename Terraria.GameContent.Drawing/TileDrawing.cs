...
public class TileDrawing
{
	...
	private void MakeExtraPreparations(Tile tile, int x, int y)
	{
		...
		case 5:
		{
			...
			if (WorldGen.GetCommonTreeFoliageData(x, y, xoffset2, ref i, ref treeStyle2, out floorY2, out topTextureFrameWidth2, out topTextureFrameHeight2))
			{
		...
		case 589:
		{
			...
			if (WorldGen.GetGemTreeFoliageData(x, y, xoffset3, ref treeFrame2, ref treeStyle3, out floorY3, out topTextureFrameWidth3, out topTextureFrameHeight3))
			{
				TilePaintSystemV2.TreeFoliageVariantKey treeFoliageVariantKey = default(TilePaintSystemV2.TreeFoliageVariantKey);
		...
		case 634:
		{
			...
			if (WorldGen.GetAshTreeFoliageData(x, y, xoffset4, ref treeFrame3, ref treeStyle4, out floorY4, out topTextureFrameWidth4, out topTextureFrameHeight4))
			{
				TilePaintSystemV2.TreeFoliageVariantKey treeFoliageVariantKey = default(TilePaintSystemV2.TreeFoliageVariantKey);
	...
	public void Update()
	{
		if (!Main.dedServ)
		{
			double value = Math.Abs(Main.WindForVisuals);
			value = Utils.GetLerpValue(0.08f, 1.2f, (float)value, clamped: true);
			_treeWindCounter += 1.0 / 240.0 + 1.0 / 240.0 * value * 2.0;
			_grassWindCounter += 1.0 / 180.0 + 1.0 / 180.0 * value * 4.0;
			_sunflowerWindCounter += 1.0 / 420.0 + 1.0 / 420.0 * value * 5.0;
			_vineWindCounter += 1.0 / 120.0 + 1.0 / 120.0 * value * 0.40000000596046448;
			UpdateLeafFrequency();
			EnsureWindGridSize();
			_windGrid.Update();
	...
	{
		bool i = intoRenderTargets || Lighting.UpdateEveryFrame;
		if (!solidLayer && i)
		{
			_specialsCount[5] = 0;
			_specialsCount[4] = 0;
	...
	private void DrawSingleTile(TileDrawInfo drawData, bool solidLayer, int waterStyleOverride, Vector2 screenPosition, Vector2 screenOffset, int tileX, int tileY)
	{
		...
		Rectangle i = Rectangle.Empty;
		Color canvasPositionOfIndex = Color.Transparent;
		if (TileID.Sets.HasOutlines[drawData.typeCache])
		{
			GetTileOutlineInfo(tileX, tileY, drawData.typeCache, ref drawData.tileLight, ref value, ref canvasPositionOfIndex);
		}
		...
		color &= IsVisible(drawData.tileCache);
		CacheSpecialDraws_Part1(tileX, tileY, drawData.typeCache, drawData.tileFrameX, drawData.tileFrameY, !color);
		CacheSpecialDraws_Part2(tileX, tileY, drawData, !color);
		...
		case 692:
		{
			...
			{
				color2 = drawData.tileCache.actColor(color2);
			}
			drawData.finalColor = color2;
			break;
		}
		case 129:
		{
		...
		if (Main.tileGlowMask[drawData.tileCache.type] != -1)
		{
			...
			{
			case 633:
				color3 = Color.Lerp(Color.White, drawData.finalColor, 0.75f);
				break;
			case 659:
			case 667:
				color3 = LiquidRenderer.GetShimmerGlitterColor(top: true, tileX, tileY);
				break;
			case 350:
				color3 = new Color(new Vector4((float)((0.0 - Math.Cos(((int)(num9 / 6.283) % 3 == 1) ? num9 : 0.0)) * 0.2 + 0.2)));
				break;
			case 381:
			case 517:
			case 687:
				color3 = _lavaMossGlow;
				break;
			case 534:
			case 535:
			case 689:
				color3 = _kryptonMossGlow;
				break;
			case 536:
			case 537:
			case 690:
				color3 = _xenonMossGlow;
				break;
			case 539:
			case 540:
			case 688:
				color3 = _argonMossGlow;
				break;
			case 625:
			case 626:
			case 691:
				color3 = _violetMossGlow;
				break;
			case 627:
			case 628:
			case 692:
			...
			case 391:
				...
				break;
			case 209:
				color3 = PortalHelper.GetPortalColor(Main.myPlayer, (drawData.tileCache.frameX >= 288) ? 1 : 0);
				break;
			case 429:
			case 445:
				drawData.drawTexture = GetTileDrawTexture(drawData.tileCache, tileX, tileY);
			...
			case 129:
			{
				...
				drawData.drawTexture = GetTileDrawTexture(drawData.tileCache, tileX, tileY);
				color3 = Main.hslToRgb(0.7f + (float)Math.Sin((float)Math.PI * 2f * Main.GlobalTimeWrappedHourly * 0.16f + (float)tileX * 0.3f + (float)tileY * 0.7f) * 0.16f, 1f, 0.5f);
				color3.A /= 2;
				color3 *= 0.3f;
				int num10 = 72;
				for (float num11 = 0f; num11 < (float)Math.PI * 2f; num11 += (float)Math.PI / 2f)
				{
					Main.spriteBatch.Draw(drawData.drawTexture, vector + num11.ToRotationVector2() * 2f, new Rectangle(drawData.tileFrameX + drawData.addFrX, drawData.tileFrameY + drawData.addFrY + num10, drawData.tileWidth, drawData.tileHeight), color3, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				}
			...
			if (!flag)
			{
				...
				{
					Main.spriteBatch.Draw(drawData.drawTexture, vector, new Rectangle(drawData.tileFrameX + drawData.addFrX, drawData.tileFrameY + drawData.addFrY, drawData.tileWidth, drawData.tileHeight), color3, 0f, Vector2.Zero, 1f, SpriteEffects.None, 0f);
				}
				else if (drawData.tileCache.halfBrick())
				{
					Main.spriteBatch.Draw(drawData.drawTexture, vector, rectangle, color3, 0f, _zero, 1f, SpriteEffects.None, 0f);
				}
				else if (TileID.Sets.Platforms[drawData.tileCache.type])
				{
					Main.spriteBatch.Draw(drawData.drawTexture, vector, rectangle, color3, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					...
					{
						Rectangle value2 = new Rectangle(198, drawData.tileFrameY, 16, 16);
						if (TileID.Sets.Platforms[Main.tile[tileX + 1, tileY + 1].type] && Main.tile[tileX + 1, tileY + 1].slope() == 0)
						{
							value2.X = 324;
						}
						Main.spriteBatch.Draw(drawData.drawTexture, vector + new Vector2(0f, 16f), value2, color3, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					...
					{
						Rectangle value3 = new Rectangle(162, drawData.tileFrameY, 16, 16);
						if (TileID.Sets.Platforms[Main.tile[tileX - 1, tileY + 1].type] && Main.tile[tileX - 1, tileY + 1].slope() == 0)
						{
							value3.X = 306;
						}
						Main.spriteBatch.Draw(drawData.drawTexture, vector + new Vector2(0f, 16f), value3, color3, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
				}
				else if (TileID.Sets.HasSlopeFrames[drawData.tileCache.type])
				{
					Main.spriteBatch.Draw(drawData.drawTexture, vector, new Rectangle(drawData.tileFrameX + drawData.addFrX, drawData.tileFrameY + drawData.addFrY, 16, 16), color3, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
				}
				else
				{
					...
					for (int j = 0; j < 8; j++)
					{
						...
						Main.spriteBatch.Draw(drawData.drawTexture, vector + new Vector2(num17, j * num13 + num14), new Rectangle(drawData.tileFrameX + drawData.addFrX + num17, drawData.tileFrameY + drawData.addFrY + num16, num13, num15), color3, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					int num18 = ((num12 <= 2) ? 14 : 0);
					Main.spriteBatch.Draw(drawData.drawTexture, vector + new Vector2(0f, num18), new Rectangle(drawData.tileFrameX + drawData.addFrX, drawData.tileFrameY + drawData.addFrY + num18, 16, 2), color3, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
				}
			}
		}
		...
		if (value != null)
		{
			i = new Rectangle(drawData.tileFrameX + drawData.addFrX, drawData.tileFrameY + drawData.addFrY, drawData.tileWidth, drawData.tileHeight);
			int num19 = 0;
			int num20 = 0;
			Main.spriteBatch.Draw(value, new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + (float)num19, tileY * 16 - (int)screenPosition.Y + drawData.tileTop + num20) + screenOffset, i, canvasPositionOfIndex, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
		}
	}

	private bool IsVisible(Tile tile)
	{
		bool vector = tile.invisibleBlock();
		switch (tile.type)
		{
		case 19:
	...
	{
		Texture2D i = TextureAssets.Tile[tile.type].get_Value();
		int canvasPositionOfIndex = 0;
		int backgroundColor = tile.type;
		switch (tile.type)
		{
		case 5:
			canvasPositionOfIndex = GetTreeBiome(tileX, tileY, tile.frameX, tile.frameY);
			break;
		case 323:
			canvasPositionOfIndex = GetPalmTreeBiome(tileX, tileY);
			break;
		case 83:
			...
			Main.instance.LoadTiles(backgroundColor);
			break;
		}
		Texture2D num = _paintSystem.TryGetTileAndRequestIfNotReady(backgroundColor, canvasPositionOfIndex, tile.color());
		if (num != null)
		{
			i = num;
		}
	...
	private void DrawSingleTile_Flames(Vector2 screenPosition, Vector2 screenOffset, int tileX, int tileY, TileDrawInfo drawData)
	{
		...
		{
			int num = 15;
			Color num2 = new Color(255, 255, 255, 0);
			switch (drawData.tileFrameX / 54)
			{
			case 5:
				num2 = new Color((float)Main.DiscoR / 255f, (float)Main.DiscoG / 255f, (float)Main.DiscoB / 255f, 0f);
				break;
			case 14:
				num2 = new Color(50, 50, 100, 20);
				break;
			case 15:
				num2 = new Color(255, 255, 255, 200);
				break;
			}
			Main.spriteBatch.Draw(TextureAssets.Flames[num].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f, tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY + drawData.addFrY, drawData.tileWidth, drawData.tileHeight), num2, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
		}
		if (drawData.typeCache == 85)
		{
			float array2 = Main.GraveyardVisualIntensity;
			if (array2 > 0f)
			{
				ulong i = Main.TileFrameSeed ^ (ulong)(((long)tileX << 32) | (uint)tileY);
				TileFlameData j = GetTileFlameData(tileX, tileY, drawData.typeCache, drawData.tileFrameY);
				if (i == 0L)
				{
					i = j.flameSeed;
				}
				j.flameSeed = i;
				Vector2 k = new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f, tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + screenOffset;
				Rectangle l = new Rectangle(drawData.tileFrameX + drawData.addFrX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight);
				for (int m = 0; m < j.flameCount; m++)
				{
					Color color = j.flameColor * array2;
					float x = (float)Utils.RandomInt(ref j.flameSeed, j.flameRangeXMin, j.flameRangeXMax) * j.flameRangeMultX;
					float y = (float)Utils.RandomInt(ref j.flameSeed, j.flameRangeYMin, j.flameRangeYMax) * j.flameRangeMultY;
					for (float num3 = 0f; num3 < 1f; num3 += 0.25f)
					{
						Main.spriteBatch.Draw(j.flameTexture, k + new Vector2(x, y) + Vector2.UnitX.RotatedBy(num3 * ((float)Math.PI * 2f)) * 2f, l, color, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					Main.spriteBatch.Draw(j.flameTexture, k, l, Color.White * array2, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
				}
			}
		}
		if (drawData.typeCache == 356 && Main.sundialCooldown == 0)
		{
			Texture2D value = TextureAssets.GlowMask[325].get_Value();
			Rectangle value2 = new Rectangle(drawData.tileFrameX, drawData.tileFrameY + drawData.addFrY, drawData.tileWidth, drawData.tileHeight);
			...
			for (int n = 0; n < 7; n++)
			{
				...
				Main.spriteBatch.Draw(value, new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num6, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num7) + screenOffset, value2, color2, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
			}
		}
		if (drawData.typeCache == 663 && Main.moondialCooldown == 0)
		{
			Texture2D value3 = TextureAssets.GlowMask[335].get_Value();
			Rectangle value4 = new Rectangle(drawData.tileFrameX, drawData.tileFrameY + drawData.addFrY, drawData.tileWidth, drawData.tileHeight);
			value4.Y += 54 * Main.moonPhase;
			Main.spriteBatch.Draw(value3, new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f, tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + screenOffset, value4, Color.White * ((float)(int)Main.mouseTextColor / 255f), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
		}
		...
		else if (Main.tileFlame[drawData.typeCache])
		{
			...
			case 7:
			{
				for (int num96 = 0; num96 < 4; num96++)
				{
					...
					float num98 = (float)Utils.RandomInt(ref seed2, -10, 10) * 0.15f;
					num97 = 0f;
					num98 = 0f;
			...
			case 1:
				...
				case 10:
				{
					for (int num69 = 0; num69 < 7; num69++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num70, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num71) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 8:
				{
					for (int num75 = 0; num75 < 7; num75++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num76, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num77) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 12:
				{
					...
					{
						float num64 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.1f;
						float num65 = (float)Utils.RandomInt(ref seed2, -10, 1) * 0.15f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num64, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num65) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 14:
				{
					...
					{
						float num73 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.1f;
						float num74 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.1f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num73, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num74) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(75, 75, 75, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 16:
				{
					for (int num66 = 0; num66 < 4; num66++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num67, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num68) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(75, 75, 75, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 27:
				...
				default:
				{
					...
					{
						float num61 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.15f;
						float num62 = (float)Utils.RandomInt(ref seed2, -10, 1) * 0.35f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num61, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num62) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(100, 100, 100, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				}
			...
			case 2:
				...
				case 3:
				{
					for (int num87 = 0; num87 < 3; num87++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num88, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num89) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 6:
				{
					for (int num93 = 0; num93 < 5; num93++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num94, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num95) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(75, 75, 75, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 9:
				{
					for (int num81 = 0; num81 < 7; num81++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num82, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num83) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(100, 100, 100, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 11:
				{
					for (int num90 = 0; num90 < 7; num90++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num91, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num92) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 13:
				{
					for (int num84 = 0; num84 < 8; num84++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num85, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num86) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(75, 75, 75, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 28:
				...
				default:
				{
					for (int num78 = 0; num78 < 7; num78++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num79, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num80) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(100, 100, 100, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				}
			...
			case 3:
				...
				case 8:
				{
					for (int num31 = 0; num31 < 7; num31++)
					{
						...
						float num33 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.075f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num32, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num33) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 9:
				{
					...
					{
						float num20 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.05f;
						float num21 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.15f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num20, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num21) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 11:
				{
					for (int num25 = 0; num25 < 7; num25++)
					{
						...
						float num27 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.3f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num26, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num27) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 15:
				{
					for (int num34 = 0; num34 < 7; num34++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num35, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num36) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 17:
				case 20:
				{
					for (int num28 = 0; num28 < 7; num28++)
					{
						...
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num29, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num30) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 18:
				{
					for (int num22 = 0; num22 < 8; num22++)
					{
						...
						float num24 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.1f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num23, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num24) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(75, 75, 75, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 34:
			...
			case 4:
				...
				case 1:
				{
					...
					{
						float num55 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.15f;
						float num56 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.15f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num55, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num56) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 2:
				case 4:
				{
					...
					{
						float num41 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.075f;
						float num42 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.075f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num41, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num42) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 3:
				{
					...
					{
						float num49 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.2f;
						float num50 = (float)Utils.RandomInt(ref seed2, -20, 1) * 0.35f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num49, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num50) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(100, 100, 100, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 5:
				{
					...
					{
						float num58 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.3f;
						float num59 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.3f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num58, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num59) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 9:
				{
					...
					{
						float num52 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.1f;
						float num53 = (float)Utils.RandomInt(ref seed2, -10, 1) * 0.15f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num52, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num53) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 13:
				{
					...
					{
						float num46 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.1f;
						float num47 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.1f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num46, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num47) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(75, 75, 75, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 12:
				{
					float num43 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.01f;
					float num44 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.01f;
					Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num43, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num44) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(Utils.RandomInt(ref seed2, 90, 111), Utils.RandomInt(ref seed2, 90, 111), Utils.RandomInt(ref seed2, 90, 111), 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					break;
				}
				case 28:
				case 29:
				...
				default:
				{
					...
					{
						float num38 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.15f;
						float num39 = (float)Utils.RandomInt(ref seed2, -10, 1) * 0.35f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num38, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num39) + screenOffset, new Rectangle(drawData.tileFrameX + drawData.addFrX, drawData.tileFrameY + drawData.addFrY, drawData.tileWidth, drawData.tileHeight), new Color(100, 100, 100, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				}
			...
			case 13:
			{
				int num99 = drawData.tileFrameY / 36;
				switch (num99)
				{
				case 1:
				case 3:
				...
				case 39:
				{
					...
					{
						float num107 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.15f;
						float num108 = (float)Utils.RandomInt(ref seed2, -10, 1) * 0.35f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num107, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num108) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(100, 100, 100, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				case 2:
				...
				case 25:
				{
					for (int num103 = 0; num103 < 7; num103++)
					{
						...
						float num105 = (float)Utils.RandomInt(ref seed2, -10, 1) * 0.1f;
						Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num104, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num105) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(50, 50, 50, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
					}
					break;
				}
				default:
					...
					case 29:
					{
						...
						{
							float num101 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.15f;
							float num102 = (float)Utils.RandomInt(ref seed2, -10, 1) * 0.15f;
							Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num101, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num102) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), new Color(25, 25, 25, 0), 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
						}
						break;
					}
					case 34:
			...
			default:
			{
				...
				if (drawData.tileCache.type == 4)
				{
					...
					case 14:
						...
						break;
					case 22:
						color3 = new Color(50, 50, 100, 20);
						break;
					case 23:
						color3 = new Color(255, 255, 255, 200);
						break;
					}
				}
				if (drawData.tileCache.type == 646)
				{
					color3 = new Color(100, 100, 100, 150);
				}
				for (int num13 = 0; num13 < 7; num13++)
				{
					float num14 = (float)Utils.RandomInt(ref seed2, -10, 11) * 0.15f;
					float num15 = (float)Utils.RandomInt(ref seed2, -10, 1) * 0.35f;
					Main.spriteBatch.Draw(TextureAssets.Flames[num12].get_Value(), new Vector2((float)(tileX * 16 - (int)screenPosition.X) - ((float)drawData.tileWidth - 16f) / 2f + num14, (float)(tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + num15) + screenOffset, new Rectangle(drawData.tileFrameX, drawData.tileFrameY, drawData.tileWidth, drawData.tileHeight), color3, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
				}
				break;
			}
			}
		...
		int num109 = drawData.tileFrameY / 54;
		if (num109 != 2)
		{
			Texture2D value5 = TextureAssets.GlowMask[334].get_Value();
			Vector2 vector = new Vector2(0f, -10f);
			Vector2 position = new Vector2((float)(tileX * 16 - (int)screenPosition.X) - (float)drawData.tileWidth / 2f, tileY * 16 - (int)screenPosition.Y + drawData.tileTop) + screenOffset + vector;
			Rectangle value6 = value5.Frame();
			Color color4 = new Color(Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor, 0);
			if (num109 == 0)
			{
				color4 *= 0.75f;
			}
			Main.spriteBatch.Draw(value5, position, value6, color4, 0f, _zero, 1f, drawData.tileSpriteEffect, 0f);
		}
	}
	...
	private void DrawSingleTile_SlicedBlock(Vector2 normalTilePosition, int tileX, int tileY, TileDrawInfo drawData)
	{
		...
		Rectangle i = default(Rectangle);
		Vector3 tileLight = default(Vector3);
		Vector2 ledCanvasPosition = default(Vector2);
		if (drawData.tileLight.R > _highQualityLightingRequirement.R || drawData.tileLight.G > _highQualityLightingRequirement.G || drawData.tileLight.B > _highQualityLightingRequirement.B)
		{
			...
			for (int j = 0; j < 9; j++)
			{
				...
				tileLight.X = (slices[j].X + vector.X) * 0.5f;
				tileLight.Y = (slices[j].Y + vector.Y) * 0.5f;
				tileLight.Z = (slices[j].Z + vector.Z) * 0.5f;
				GetFinalLight(drawData.tileCache, drawData.typeCache, ref tileLight, ref tint);
				ledCanvasPosition.X = normalTilePosition.X + (float)i.X;
				ledCanvasPosition.Y = normalTilePosition.Y + (float)i.Y;
				i.X += drawData.tileFrameX + drawData.addFrX;
				i.Y += drawData.tileFrameY + drawData.addFrY;
				...
				Main.spriteBatch.Draw(drawData.drawTexture, ledCanvasPosition, i, color, 0f, origin, 1f, drawData.tileSpriteEffect, 0f);
			}
		}
		else if (drawData.tileLight.R > _mediumQualityLightingRequirement.R || drawData.tileLight.G > _mediumQualityLightingRequirement.G || drawData.tileLight.B > _mediumQualityLightingRequirement.B)
		{
			...
			for (int k = 0; k < 4; k++)
			{
				...
				tileLight.X = (slices2[k].X + vector2.X) * 0.5f;
				tileLight.Y = (slices2[k].Y + vector2.Y) * 0.5f;
				tileLight.Z = (slices2[k].Z + vector2.Z) * 0.5f;
	...
	private void DrawTile_MinecartTrack(Vector2 screenPosition, Vector2 screenOffset, int tileX, int tileY, TileDrawInfo drawData)
	{
		...
		Minecart.TrackColors(tileX, tileY, drawData.tileCache, out var canvasPositionOfIndex, out var staticNoise);
		drawData.drawTexture = GetTileDrawTexture(drawData.tileCache, tileX, tileY, canvasPositionOfIndex);
		Texture2D i = GetTileDrawTexture(drawData.tileCache, tileX, tileY, staticNoise);
		if (WorldGen.IsRope(tileX, tileY) && Main.tile[tileX, tileY - 1] != null)
		{
			_ = Main.tile[tileX, tileY - 1].type;
			int color = (tileY + tileX) % 3 * 18;
			Texture2D tileDrawTexture = GetTileDrawTexture(Main.tile[tileX, tileY - 1], tileX, tileY);
			Main.spriteBatch.Draw(tileDrawTexture, new Vector2(tileX * 16 - (int)screenPosition.X, tileY * 16 - (int)screenPosition.Y) + screenOffset, new Rectangle(90, color, 16, 16), drawData.tileLight, 0f, default(Vector2), 1f, drawData.tileSpriteEffect, 0f);
		}
		drawData.tileCache.frameNumber();
		if (drawData.tileFrameY != -1)
		{
			Main.spriteBatch.Draw(i, new Vector2(tileX * 16 - (int)screenPosition.X, tileY * 16 - (int)screenPosition.Y) + screenOffset, Minecart.GetSourceRect(drawData.tileFrameY, Main.tileFrame[314]), drawData.tileLight, 0f, default(Vector2), 1f, drawData.tileSpriteEffect, 0f);
		}
		...
		{
			Main.spriteBatch.Draw(i, new Vector2(tileX * 16 - (int)screenPosition.X, (tileY + 1) * 16 - (int)screenPosition.Y) + screenOffset, Minecart.GetSourceRect(36), drawData.tileLight, 0f, default(Vector2), 1f, drawData.tileSpriteEffect, 0f);
		}
		...
		{
			Main.spriteBatch.Draw(i, new Vector2(tileX * 16 - (int)screenPosition.X, (tileY + 1) * 16 - (int)screenPosition.Y) + screenOffset, Minecart.GetSourceRect(37, Main.tileFrame[314]), drawData.tileLight, 0f, default(Vector2), 1f, drawData.tileSpriteEffect, 0f);
		}
	...
	private void CacheSpecialDraws_Part1(int tileX, int tileY, int tileType, int drawDataTileFrameX, int drawDataTileFrameY, bool skipDraw)
	{
		if (tileType == 395)
		{
			...
			if (!_itemFrameTileEntityPositions.ContainsKey(i))
			{
				_itemFrameTileEntityPositions[i] = TEItemFrame.Find(i.X, i.Y);
				if (_itemFrameTileEntityPositions[i] != -1)
				{
					AddSpecialLegacyPoint(i);
				}
			}
		}
		if (tileType == 520)
		{
			Point canvasPositionOfIndex = new Point(tileX, tileY);
			if (!_foodPlatterTileEntityPositions.ContainsKey(canvasPositionOfIndex))
			{
				_foodPlatterTileEntityPositions[canvasPositionOfIndex] = TEFoodPlatter.Find(canvasPositionOfIndex.X, canvasPositionOfIndex.Y);
				if (_foodPlatterTileEntityPositions[canvasPositionOfIndex] != -1)
				{
					AddSpecialLegacyPoint(canvasPositionOfIndex);
				}
			}
		}
		if (tileType == 471)
		{
			Point color = new Point(tileX, tileY);
			color.X -= drawDataTileFrameX % 54 / 18;
			color.Y -= drawDataTileFrameY % 54 / 18;
			if (!_weaponRackTileEntityPositions.ContainsKey(color))
			{
				_weaponRackTileEntityPositions[color] = TEWeaponsRack.Find(color.X, color.Y);
				if (_weaponRackTileEntityPositions[color] != -1)
				{
					AddSpecialLegacyPoint(color);
				}
			}
		}
		if (tileType == 470)
		{
			...
			if (!_displayDollTileEntityPositions.ContainsKey(point))
			{
				_displayDollTileEntityPositions[point] = TEDisplayDoll.Find(point.X, point.Y);
				if (_displayDollTileEntityPositions[point] != -1)
				{
		...
		if (tileType == 475)
		{
			...
			point2.X -= drawDataTileFrameX % 54 / 18;
			point2.Y -= drawDataTileFrameY % 72 / 18;
			if (!_hatRackTileEntityPositions.ContainsKey(point2))
			{
				_hatRackTileEntityPositions[point2] = TEHatRack.Find(point2.X, point2.Y);
				if (_hatRackTileEntityPositions[point2] != -1)
				{
	...
	private void CacheSpecialDraws_Part2(int tileX, int tileY, TileDrawInfo drawData, bool skipDraw)
	{
		if (TileID.Sets.BasicChest[drawData.typeCache])
		{
			...
			if (!_chestPositions.ContainsKey(i))
			{
				_chestPositions[i] = Chest.FindChest(i.X, i.Y);
			}
			int canvasPositionOfIndex = drawData.tileFrameX / 18;
			int num = drawData.tileFrameY / 18;
			int staticNoise = drawData.tileFrameX / 36;
			int baseColor = canvasPositionOfIndex * 18;
			drawData.addFrX = baseColor - drawData.tileFrameX;
			int num2 = num * 18;
			if (_chestPositions[i] != -1)
			{
				int frame = Main.chest[_chestPositions[i]].frame;
				if (frame == 1)
				{
					num2 += 38;
				}
			...
			drawData.addFrY = num2 - drawData.tileFrameY;
			if (num != 0)
			{
				drawData.tileHeight = 18;
			}
			if (drawData.typeCache == 21 && (staticNoise == 48 || staticNoise == 49))
			{
				drawData.glowSourceRect = new Rectangle(16 * (canvasPositionOfIndex % 2), drawData.tileFrameY + drawData.addFrY, drawData.tileWidth, drawData.tileHeight);
			}
		}
		...
		{
			key.Y -= drawData.tileFrameY / 18;
		}
		if (!_trainingDummyTileEntityPositions.ContainsKey(key))
		{
			_trainingDummyTileEntityPositions[key] = TETrainingDummy.Find(key.X, key.Y);
		}
		if (_trainingDummyTileEntityPositions[key] != -1)
		{
			int npc = ((TETrainingDummy)TileEntity.ByID[_trainingDummyTileEntityPositions[key]]).npc;
			if (npc != -1)
			{
				int num3 = Main.npc[npc].frame.Y / 55;
				num3 *= 54;
				num3 += drawData.tileFrameY;
				drawData.addFrY = num3 - drawData.tileFrameY;
			}
		}
	}
	...
	{
		int player = (int)((float)(tileLight.R * tint.R) / 255f);
		int num = (int)((float)(tileLight.G * tint.G) / 255f);
		int num2 = (int)((float)(tileLight.B * tint.B) / 255f);
		if (player > 255)
		{
			player = 255;
		}
		...
		num2 <<= 16;
		num <<= 8;
		tileLight.PackedValue = (uint)(player | num | num2) | 0xFF000000u;
	...
	{
		if (Main.InSmartCursorHighlightArea(x, y, out var i))
		{
			int canvasPositionOfIndex = (tileLight.R + tileLight.G + tileLight.B) / 3;
			if (canvasPositionOfIndex > 10)
			{
				highlightTexture = TextureAssets.HighlightMask[typeCache].get_Value();
				highlightColor = Colors.GetSelectionGlowColor(i, canvasPositionOfIndex);
			}
		}
	}
	...
	{
		int i = tileCache.slope();
		bool dynamicNoise = !TileID.Sets.BlocksWaterDrawingBehindSelf[tileCache.type];
		if (!behindBlocks)
		{
			dynamicNoise = false;
		}
	...
	private bool IsWindBlocked(int x, int y)
	{
		...
		if (num.wall > 0 && !WallID.Sets.AllowsWind[num.wall])
		{
			return true;
		}
		if ((double)y > Main.worldSurface)
	...
	private int GetWaterAnimalCageFrame(int x, int y, int tileFrameX, int tileFrameY)
	{
		...
		int i = y - tileFrameY / 18;
		return num / 2 * (i / 3) % Main.cageFrames;
	}
	...
	private void DrawTiles_EmitParticles(int j, int i, Tile tileCache, ushort typeCache, short tileFrameX, short tileFrameY, Color tileLight)
	{
		...
		if (typeCache == 565 && tileFrameX == 0 && tileFrameY == 18 && _rand.Next(3) == 0 && ((Main.drawToScreen && _rand.Next(4) == 0) || !Main.drawToScreen))
		{
			...
			velocity *= 4f;
			Gore.NewGorePerfect(position, velocity, color, scale);
		}
		...
		else if (typeCache == 406 && tileFrameY == 54 && tileFrameX == 0 && _rand.Next(3) == 0)
		{
			...
			int type = _rand.Next(825, 828);
			if (_rand.Next(4) == 0)
			{
				Gore.NewGore(position2, velocity2, type, _rand.NextFloat() * 0.2f + 0.2f);
			}
			else if (_rand.Next(2) == 0)
			{
				Gore.NewGore(position2, velocity2, type, _rand.NextFloat() * 0.3f + 0.3f);
			}
			else
			{
				Gore.NewGore(position2, velocity2, type, _rand.NextFloat() * 0.4f + 0.4f);
			}
		}
		else if (typeCache == 452 && tileFrameY == 0 && tileFrameX == 0 && _rand.Next(3) == 0)
		{
			...
			int type2 = 907 + num4 / 5;
			if (_rand.Next(2) == 0)
			{
				Gore.NewGore(position3, velocity3, type2, _rand.NextFloat() * 0.4f + 0.4f);
			}
		}
		if (typeCache == 192 && _rand.Next(k) == 0)
		{
			EmitLivingTreeLeaf(i, j, 910);
		}
		if (typeCache == 384 && _rand.Next(k) == 0)
		{
	...
	private void EmitLivingTreeLeaf_Sideways(int x, int y, int leafGoreType)
	{
		...
		Tile canvasPositionOfIndex = Main.tile[x + i, y];
		if (!WorldGen.SolidTile(canvasPositionOfIndex) && canvasPositionOfIndex.liquid <= 0)
		{
			int staticNoise = 0;
			if (i == -1)
			{
			...
			Gore.NewGorePerfect(new Vector2(x * 16 + 8 + 4 * i + staticNoise, y * 16 + 8), Vector2.Zero, leafGoreType).Frame.CurrentColumn = Main.tile[x, y].color();
		}
	}
	...
	private float GetWindCycle(int x, int y, double windCounter)
	{
		...
		float vector = (float)Math.Cos(windCounter * 6.2831854820251465 + (double)num) * 0.5f;
		if (Main.remixWorld)
		{
			if (!((double)y > Main.worldSurface))
			{
		...
		float value = Utils.GetLerpValue(0.08f, 0.18f, Math.Abs(Main.WindForVisuals), clamped: true);
		return vector * value;
	}
	...
	{
		Vector2 i = Main.Camera.UnscaledPosition;
		Vector2 canvasPositionOfIndex = new Vector2(Main.offScreenRange, Main.offScreenRange);
		if (Main.drawToScreen)
		{
			canvasPositionOfIndex = Vector2.Zero;
		}
		GetScreenDrawArea(i, canvasPositionOfIndex, out var staticNoise, out var value, out var color, out var lastTileY);
		_windGrid.SetSize(value - staticNoise, lastTileY - color);
	}
	...
	private void EmitTreeLeaves(int tilePosX, int tilePosY, int grassPosX, int grassPosY)
	{
		...
		int canvasPositionOfIndex = grassPosY - tilePosY;
		Tile value = Main.tile[tilePosX, tilePosY];
		if (value.liquid > 0)
		{
			return;
		}
		WorldGen.GetTreeLeaf(tilePosX, value, Main.tile[grassPosX, grassPosY], ref canvasPositionOfIndex, out var _, out var i);
		int num;
		switch (i)
		{
		case -1:
		...
		bool color = (byte)num != 0;
		int num2 = _leafFrequency;
		bool flag = tilePosX - grassPosX != 0;
		if (color)
		{
			num2 /= 2;
		}
		...
		{
			Gore.NewGoreDirect(vector, Utils.RandomVector2(Main.rand, -num3, num3), i, 0.7f + Main.rand.NextFloat() * 0.6f).Frame.CurrentColumn = Main.tile[tilePosX, tilePosY].color();
		}
	}
	...
	private void DrawEntities_DisplayDolls()
	{
		...
		foreach (KeyValuePair<Point, int> i in _displayDollTileEntityPositions)
		{
			if (i.Value != -1 && TileEntity.ByPosition.TryGetValue(new Point16(i.Key.X, i.Key.Y), out var canvasPositionOfIndex))
			{
				(canvasPositionOfIndex as TEDisplayDoll).Draw(i.Key.X, i.Key.Y);
			}
		}
		Main.spriteBatch.End();
	}
	...
	{
		Main.spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, Main.DefaultSamplerState, DepthStencilState.None, Main.Rasterizer, null, Main.Transform);
		foreach (KeyValuePair<Point, int> value in _hatRackTileEntityPositions)
		{
			if (value.Value != -1 && TileEntity.ByPosition.TryGetValue(new Point16(value.Key.X, value.Key.Y), out var i))
			{
				(i as TEHatRack).Draw(value.Key.X, value.Key.Y);
			}
		}
		Main.spriteBatch.End();
	}
	...
	{
		Texture2D i = _paintSystem.TryGetTreeTopAndRequestIfNotReady(treeTextureIndex, treeTextureStyle, tileColor);
		if (i == null)
		{
			i = TextureAssets.TreeTop[treeTextureIndex].get_Value();
		}
		return i;
	}
	...
	{
		Texture2D vector = _paintSystem.TryGetTreeBranchAndRequestIfNotReady(treeTextureIndex, treeTextureStyle, tileColor);
		if (vector == null)
		{
			vector = TextureAssets.TreeBranch[treeTextureIndex].get_Value();
		}
		return vector;
	}
	...
	private void DrawAnimatedTile_AdjustForVisionChangers(int i, int j, Tile tileCache, ushort typeCache, short tileFrameX, short tileFrameY, ref Color tileLight, bool canDoDust)
	{
		if (_localPlayer.dangerSense && IsTileDangerous(_localPlayer, tileCache, typeCache))
		{
			...
			{
				int k = Dust.NewDust(new Vector2(i * 16, j * 16), 16, 16, 60, 0f, 0f, 100, default(Color), 0.3f);
				_dust[k].fadeIn = 1f;
				_dust[k].velocity *= 0.1f;
				_dust[k].noLight = true;
				_dust[k].noGravity = true;
			}
		}
		if (_localPlayer.findTreasure && Main.IsTileSpelunkable(typeCache, tileFrameX, tileFrameY))
		{
			...
			{
				int dynamicNoise = Dust.NewDust(new Vector2(i * 16, j * 16), 16, 16, 204, 0f, 0f, 150, default(Color), 0.3f);
				_dust[dynamicNoise].fadeIn = 1f;
				_dust[dynamicNoise].velocity *= 0.1f;
				_dust[dynamicNoise].noLight = true;
			}
		}
		if (!_localPlayer.biomeSight)
		{
		...
		Color dynamicNoise2 = Color.White;
		if (Main.IsTileBiomeSightable(typeCache, tileFrameX, tileFrameY, ref dynamicNoise2))
		{
			if (tileLight.R < dynamicNoise2.R)
			{
				tileLight.R = dynamicNoise2.R;
			}
			if (tileLight.G < dynamicNoise2.G)
			{
				tileLight.G = dynamicNoise2.G;
			}
			if (tileLight.B < dynamicNoise2.B)
			{
				tileLight.B = dynamicNoise2.B;
			}
			if (_isActiveAndNotPaused && canDoDust && _rand.Next(480) == 0)
			{
				Color value = dynamicNoise2;
				int num = Dust.NewDust(new Vector2(i * 16, j * 16), 16, 16, 267, 0f, 0f, 150, value, 0.3f);
				_dust[num].noGravity = true;
				_dust[num].fadeIn = 1f;
				_dust[num].velocity *= 0.1f;
				_dust[num].noLightEmittence = true;
			}
		}
	}
	...
	{
		_windGrid.GetWindTime(i, j, pushAnimationTimeTotal, out var k, out var dynamicNoise, out var _);
		if (k >= pushAnimationTimeTotal / 2)
		{
			return (float)(pushAnimationTimeTotal - k) * pushForcePerFrame * (float)dynamicNoise;
		}
		return (float)k * pushForcePerFrame * (float)dynamicNoise;
	}
	...
	{
		_windGrid.GetWindTime(i, j, pushAnimationTimeTotal, out var flag, out var k, out var gridPositionOfIndex);
		if (flag >= pushAnimationTimeTotal / 2)
		{
			pushX = (float)(pushAnimationTimeTotal - flag) * pushForcePerFrame * (float)k;
			pushY = (float)(pushAnimationTimeTotal - flag) * pushForcePerFrame * (float)gridPositionOfIndex;
		}
		else
		{
			pushX = (float)flag * pushForcePerFrame * (float)k;
			pushY = (float)flag * pushForcePerFrame * (float)gridPositionOfIndex;
		}
	}
	...
	{
		int scaleFactor = 11;
		int color = _specialsCount[scaleFactor];
		for (int i = 0; i < color; i++)
		{
			Point p = _specialPositions[scaleFactor][i];
			Tile tile = Main.tile[p.X, p.Y];
			if (tile != null && tile.active())
			{
				Texture2D value = TextureAssets.Extra[198].get_Value();
				int frameY = tile.frameX / 54;
				bool num = tile.frameY / 72 != 0;
				...
				float num2 = (float)Math.Sin(Main.GlobalTimeWrappedHourly * ((float)Math.PI * 2f) / 5f);
				Vector2 vector2 = vector + new Vector2(0f, -40f) + new Vector2(0f, num2 * 4f);
				Color color2 = Lighting.GetColor(p.X, p.Y);
				SpriteEffects effects = (num ? SpriteEffects.FlipHorizontally : SpriteEffects.None);
				Main.spriteBatch.Draw(value, vector2 - Main.screenPosition, rectangle, color2, 0f, origin, 1f, effects, 0f);
				float num3 = (float)Math.Sin(Main.GlobalTimeWrappedHourly * ((float)Math.PI * 2f) / 2f) * 0.3f + 0.7f;
				Color color3 = color2;
				color3.A = 0;
				color3 = color3 * 0.1f * num3;
				for (float num4 = 0f; num4 < 1f; num4 += 1f / 6f)
				{
					Main.spriteBatch.Draw(value, vector2 - Main.screenPosition + ((float)Math.PI * 2f * num4).ToRotationVector2() * (6f + num2 * 2f), rectangle, color3, 0f, origin, 1f, effects, 0f);
				}
			}
		}
	}
	...
	{
		int i = 0;
		int dynamicNoise = originy;
		while (dynamicNoise > 10)
		{
			Tile color = Main.tile[originx, dynamicNoise];
			if (!color.active() || color.type != 519)
			{
				break;
			}
			if (color.frameX >= 180)
	...
	{
		Vector2 i = Main.Camera.UnscaledPosition;
		Vector2 slimeColor = Vector2.Zero;
		int staticNoise = 5;
		int num = _specialsCount[staticNoise];
		for (int j = 0; j < num; j++)
		{
			Point point = _specialPositions[staticNoise][j];
			int x = point.X;
			int y = point.Y;
			int sizeX = 1;
			int sizeY = 1;
			...
			if (tile != null && tile.active())
			{
				...
				DrawMultiTileVinesInWind(i, slimeColor, x, y, sizeX, sizeY);
			}
		}
	}
	...
	{
		Vector2 i = Main.Camera.UnscaledPosition;
		Vector2 staticNoise = Vector2.Zero;
		int color = 9;
		int num = _specialsCount[color];
		for (int j = 0; j < num; j++)
		{
			Point point = _specialPositions[color][j];
			int x = point.X;
			int y = point.Y;
			DrawRisingVineStrip(i, staticNoise, x, y);
		}
	}
	...
	{
		float i = GetWindCycle(topLeftX, topLeftY, _sunflowerWindCounter);
		new Vector2((float)(sizeX * 16) * 0.5f, sizeY * 16);
		Vector2 canvasPositionOfIndex = new Vector2((float)(topLeftX * 16 - (int)screenPosition.X) + (float)sizeX * 16f * 0.5f, topLeftY * 16 - (int)screenPosition.Y + 16 * sizeY) + offSet;
		float vector = 0.07f;
		int dynamicNoise = Main.tile[topLeftX, topLeftY].type;
		Texture2D num = null;
		Color num2 = Color.Transparent;
		bool staticNoise = InAPlaceWithWind(topLeftX, topLeftY, sizeX, sizeY);
		switch (dynamicNoise)
		{
		case 27:
			num = TextureAssets.Flames[14].get_Value();
			num2 = Color.White;
			break;
		case 519:
			staticNoise = InAPlaceWithWind(topLeftX, topLeftY, sizeX, 1);
			break;
		default:
			vector = 0.15f;
			break;
		...
		for (int value = topLeftX; value < topLeftX + sizeX; value++)
		{
			for (int amount = topLeftY; amount < topLeftY + sizeY; amount++)
			{
				Tile tile = Main.tile[value, amount];
				ushort type = tile.type;
				if (type != dynamicNoise || !IsVisible(tile))
				{
					continue;
				}
				Math.Abs(((float)(value - topLeftX) + 0.5f) / (float)sizeX - 0.5f);
				short tileFrameX = tile.frameX;
				short tileFrameY = tile.frameY;
				float num3 = 1f - (float)(amount - topLeftY + 1) / (float)sizeY;
				if (num3 == 0f)
				{
					num3 = 0.1f;
				}
				...
				GetTileDrawData(value, amount, tile, type, ref tileFrameX, ref tileFrameY, out var tileWidth, out var tileHeight, out var tileTop, out var halfBrickHeight, out var addFrX, out var addFrY, out var tileSpriteEffect, out var _, out var _, out var _);
				bool flag = _rand.Next(4) == 0;
				Color tileLight = Lighting.GetColor(value, amount);
				DrawAnimatedTile_AdjustForVisionChangers(value, amount, tile, type, tileFrameX, tileFrameY, ref tileLight, flag);
				tileLight = DrawTiles_GetLightOverride(amount, value, tile, type, tileFrameX, tileFrameY, tileLight);
				if (_isActiveAndNotPaused && flag)
				{
					DrawTiles_EmitParticles(amount, value, tile, type, tileFrameX, tileFrameY, tileLight);
				}
				Vector2 vector2 = new Vector2(value * 16 - (int)screenPosition.X, amount * 16 - (int)screenPosition.Y + tileTop) + offSet;
				...
				Vector2 vector3 = new Vector2(i * 1f, Math.Abs(i) * 2f * num3);
				Vector2 origin = canvasPositionOfIndex - vector2;
				Texture2D tileDrawTexture = GetTileDrawTexture(tile, value, amount);
				if (tileDrawTexture != null)
				{
					Main.spriteBatch.Draw(tileDrawTexture, canvasPositionOfIndex + new Vector2(0f, vector3.Y), new Rectangle(tileFrameX + addFrX, tileFrameY + addFrY, tileWidth, tileHeight - halfBrickHeight), tileLight, i * vector * num3, origin, 1f, tileSpriteEffect, 0f);
					if (num != null)
					{
						Main.spriteBatch.Draw(num, canvasPositionOfIndex + new Vector2(0f, vector3.Y), new Rectangle(tileFrameX + addFrX, tileFrameY + addFrY, tileWidth, tileHeight - halfBrickHeight), num2, i * vector * num3, origin, 1f, tileSpriteEffect, 0f);
					}
				}
			}
		}
	...
	private void DrawVineStrip(Vector2 screenPosition, Vector2 offSet, int x, int startY)
	{
		...
		for (int j = startY; j < Main.maxTilesY - 10; j++)
		{
			...
			if (Main.remixWorld)
			{
				if (WallID.Sets.AllowsWind[tile.wall] && (double)j > Main.worldSurface)
				{
					color++;
				}
			}
			else if (WallID.Sets.AllowsWind[tile.wall] && (double)j < Main.worldSurface)
			{
				color++;
			}
			float windGridPush = GetWindGridPush(x, j, 20, 0.01f);
			num2 = ((windGridPush != 0f || num3 == 0f) ? (num2 - windGridPush) : (num2 * -0.78f));
			num3 = windGridPush;
			short tileFrameX = tile.frameX;
			short tileFrameY = tile.frameY;
			Color color2 = Lighting.GetColor(x, j);
			GetTileDrawData(x, j, tile, type, ref tileFrameX, ref tileFrameY, out var tileWidth, out var tileHeight, out var tileTop, out var halfBrickHeight, out var addFrX, out var addFrY, out var tileSpriteEffect, out var glowTexture, out var glowSourceRect, out var glowColor);
			Vector2 position = new Vector2(-(int)screenPosition.X, -(int)screenPosition.Y) + offSet + i;
			if (tile.fullbrightBlock())
			{
				color2 = Color.White;
			}
			float num4 = (float)color * num * windCycle + num2;
			if (_localPlayer.biomeSight)
			{
				Color sightColor = Color.White;
				if (Main.IsTileBiomeSightable(type, tileFrameX, tileFrameY, ref sightColor))
				{
					if (color2.R < sightColor.R)
					{
						color2.R = sightColor.R;
					}
					if (color2.G < sightColor.G)
					{
						color2.G = sightColor.G;
					}
					if (color2.B < sightColor.B)
					{
						color2.B = sightColor.B;
					}
					if (_isActiveAndNotPaused && _rand.Next(480) == 0)
					{
						Color newColor = sightColor;
						int num5 = Dust.NewDust(new Vector2(x * 16, j * 16), 16, 16, 267, 0f, 0f, 150, newColor, 0.3f);
						_dust[num5].noGravity = true;
						_dust[num5].fadeIn = 1f;
						_dust[num5].velocity *= 0.1f;
						_dust[num5].noLightEmittence = true;
					}
				}
			}
			Texture2D tileDrawTexture = GetTileDrawTexture(tile, x, j);
			if (tileDrawTexture == null)
			{
				break;
			}
			if (IsVisible(tile))
			{
				Main.spriteBatch.Draw(tileDrawTexture, position, new Rectangle(tileFrameX + addFrX, tileFrameY + addFrY, tileWidth, tileHeight - halfBrickHeight), color2, num4, new Vector2(tileWidth / 2, halfBrickHeight - tileTop), 1f, tileSpriteEffect, 0f);
				if (glowTexture != null)
				{
					Main.spriteBatch.Draw(glowTexture, position, glowSourceRect, glowColor, num4, new Vector2(tileWidth / 2, halfBrickHeight - tileTop), 1f, tileSpriteEffect, 0f);
				}
			}
			i += (num4 + (float)Math.PI / 2f).ToRotationVector2() * 16f;
		}
	}
	...
	{
		float i = GetWindCycle(topLeftX, topLeftY, _sunflowerWindCounter);
		float canvasPositionOfIndex = i;
		int gridPositionOfIndex = 60;
		float amount = 1.26f;
		float num = GetHighestWindGridPushComplex(topLeftX, topLeftY, sizeX, sizeY, gridPositionOfIndex, amount, 3, swapLoopDir: true);
		i += num;
		new Vector2((float)(sizeX * 16) * 0.5f, 0f);
		Vector2 color = new Vector2((float)(topLeftX * 16 - (int)screenPosition.X) + (float)sizeX * 16f * 0.5f, topLeftY * 16 - (int)screenPosition.Y) + offSet;
		float num2 = 0.07f;
		...
		for (int j = topLeftX; j < topLeftX + sizeX; j++)
		{
			for (int k = topLeftY; k < topLeftY + sizeY; k++)
			{
				...
				GetTileDrawData(j, k, tile2, type2, ref tileFrameX, ref tileFrameY, out var tileWidth, out var tileHeight, out var tileTop, out var halfBrickHeight, out var addFrX, out var addFrY, out var tileSpriteEffect, out var _, out var _, out var _);
				bool flag3 = _rand.Next(4) == 0;
				Color tileLight = Lighting.GetColor(j, k);
				DrawAnimatedTile_AdjustForVisionChangers(j, k, tile2, type2, tileFrameX, tileFrameY, ref tileLight, flag3);
				tileLight = DrawTiles_GetLightOverride(k, j, tile2, type2, tileFrameX, tileFrameY, tileLight);
				if (_isActiveAndNotPaused && flag3)
				{
					DrawTiles_EmitParticles(k, j, tile2, type2, tileFrameX, tileFrameY, tileLight);
				}
				Vector2 vector2 = new Vector2(j * 16 - (int)screenPosition.X, k * 16 - (int)screenPosition.Y + tileTop) + offSet;
				vector2 += vector;
				Vector2 vector3 = new Vector2(i * num4, Math.Abs(i) * num5 * num7);
				Vector2 vector4 = color - vector2;
				Texture2D tileDrawTexture = GetTileDrawTexture(tile2, j, k);
				if (tileDrawTexture != null)
				{
					Vector2 vector5 = color + new Vector2(0f, vector3.Y);
					Rectangle rectangle = new Rectangle(tileFrameX + addFrX, tileFrameY + addFrY, tileWidth, tileHeight - halfBrickHeight);
					float rotation = i * num2 * num7;
					if (type2 == 660 && k == topLeftY + sizeY - 1)
					{
						Texture2D value = TextureAssets.Extra[260].get_Value();
						_ = ((float)((j + k) % 200) * 0.11f + (float)Main.timeForVisualEffects / 360f) % 1f;
						Color white = Color.White;
						Main.spriteBatch.Draw(value, vector5, rectangle, white, rotation, vector4, 1f, tileSpriteEffect, 0f);
					}
					Main.spriteBatch.Draw(tileDrawTexture, vector5, rectangle, tileLight, rotation, vector4, 1f, tileSpriteEffect, 0f);
					if (type2 == 660 && k == topLeftY + sizeY - 1)
					{
						Texture2D value2 = TextureAssets.Extra[260].get_Value();
						Color color3 = Main.hslToRgb(((float)((j + k) % 200) * 0.11f + (float)Main.timeForVisualEffects / 360f) % 1f, 1f, 0.8f);
						color3.A = 127;
						Rectangle value3 = rectangle;
						Vector2 position = vector5;
						Vector2 origin = vector4;
						Main.spriteBatch.Draw(value2, position, value3, color3, rotation, origin, 1f, tileSpriteEffect, 0f);
					}
					if (texture2D != null)
					{
						Main.spriteBatch.Draw(texture2D, vector5, rectangle, color2, rotation, vector4, 1f, tileSpriteEffect, 0f);
					}
					TileFlameData tileFlameData = GetTileFlameData(j, k, type2, tileFrameY);
					if (num6 == 0L)
					{
					...
					for (int l = 0; l < tileFlameData.flameCount; l++)
					{
						...
						Main.spriteBatch.Draw(tileFlameData.flameTexture, vector5 + new Vector2(x, y), rectangle, tileFlameData.flameColor, rotation, vector4, 1f, tileSpriteEffect, 0f);
					}
				}
			}
		}
	...
	private void EmitAlchemyHerbParticles(int j, int i, int style)
	{
		...
		if (style == 3)
		{
			if (_rand.Next(200) == 0)
			{
				int k = Dust.NewDust(new Vector2(i * 16, j * 16), 16, 16, 14, 0f, 0f, 100, default(Color), 0.2f);
				_dust[k].fadeIn = 1.2f;
			}
			if (_rand.Next(75) == 0)
			{
				int canvasPositionOfIndex = Dust.NewDust(new Vector2(i * 16, j * 16), 16, 16, 27, 0f, 0f, 100);
				_dust[canvasPositionOfIndex].velocity.X /= 2f;
				_dust[canvasPositionOfIndex].velocity.Y /= 2f;
			}
		}
		if (style == 4 && _rand.Next(150) == 0)
		{
			int vector = Dust.NewDust(new Vector2(i * 16, j * 16), 16, 8, 16);
			_dust[vector].velocity.X /= 3f;
			_dust[vector].velocity.Y /= 3f;
			_dust[vector].velocity.Y -= 0.7f;
			_dust[vector].alpha = 50;
			_dust[vector].scale *= 0.1f;
			_dust[vector].fadeIn = 0.9f;
			_dust[vector].noGravity = true;
		}
		if (style == 5 && _rand.Next(40) == 0)
		{
			int num2 = Dust.NewDust(new Vector2(i * 16, j * 16 - 6), 16, 16, 6, 0f, 0f, 0, default(Color), 1.5f);
			_dust[num2].velocity.Y -= 2f;
			_dust[num2].noGravity = true;
		}
		if (style == 6 && _rand.Next(30) == 0)
		{
			int num3 = Dust.NewDust(newColor: new Color(50, 255, 255, 255), Position: new Vector2(i * 16, j * 16), Width: 16, Height: 16, Type: 43, SpeedX: 0f, SpeedY: 0f, Alpha: 254, Scale: 0.5f);
			_dust[num3].velocity *= 0f;
		}
	}
