...
public class WallDrawing
{
	...
	protected bool FullTile(int x, int y)
	{
		...
		if (i.active())
		{
			if (i.type < TileID.Sets.DrawsWalls.Length && TileID.Sets.DrawsWalls[i.type])
			{
				return false;
			}
			if (i.invisibleBlock() && !_shouldShowInvisibleWalls)
			{
				return false;
			}
			if (Main.tileSolid[i.type] && !Main.tileSolidTop[i.type])
			{
				int dynamicNoise = i.frameX;
				int color = i.frameY;
				if (Main.tileLargeFrames[i.type] > 0)
				{
					if (color == 18 || color == 108)
					{
						if (dynamicNoise >= 18 && dynamicNoise <= 54)
						{
							return true;
						}
						if (dynamicNoise >= 108 && dynamicNoise <= 144)
						{
							return true;
						}
					}
				...
				else if (color >= 90 && color <= 196)
				{
					if (dynamicNoise <= 70)
					{
						return true;
					}
					if (dynamicNoise >= 144 && dynamicNoise <= 232)
					{
						return true;
					}
				}
