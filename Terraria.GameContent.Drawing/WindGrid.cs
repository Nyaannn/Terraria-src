...
public class WindGrid
{
	...
	{
		Player vector = Main.player[i];
		if (!vector.active || vector.dead || (vector.velocity.X == 0f && vector.velocity.Y == 0f) || !Utils.CenteredRectangle(Main.Camera.Center, Main.Camera.UnscaledSize).Intersects(vector.Hitbox) || vector.velocity.HasNaNs())
		{
			return;
		}
		int point = Math.Sign(vector.velocity.X);
		int vector2 = Math.Sign(vector.velocity.Y);
		foreach (Point item in Collision.GetTilesIn(vector.TopLeft, vector.BottomRight))
		{
			SetWindTime(item.X, item.Y, point, vector2);
		}
	}
}
