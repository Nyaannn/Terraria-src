...
public class WoodHouseBuilder : HouseBuilder
{
	...
	{
		for (int type = 0; type < room.Width * room.Height / 16; type++)
		{
			int x = WorldGen.genRand.Next(1, room.Width - 1) + room.X;
			int leadingConditionRule = WorldGen.genRand.Next(1, room.Height - 1) + room.Y;
			WorldUtils.Gen(new Point(x, leadingConditionRule), new Shapes.Rectangle(2, 2), Actions.Chain(new Modifiers.Dither(), new Modifiers.Blotches(2, 2), new Modifiers.IsEmpty(), new Actions.SetTile(51, setSelfFrames: true)));
		}
	...
	private void RainbowifyOnTenthAnniversaryWorlds()
	{
		...
		foreach (Rectangle type in base.Rooms)
		{
			WorldUtils.Gen(new Point(type.X, type.Y), new Shapes.Rectangle(type.Width, type.Height), new Actions.SetTileAndWallRainbowPaint());
		}
	}
}
