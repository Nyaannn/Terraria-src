...
public class LegacyLighting : ILightingEngine
{
	...
	public void AddLight(int x, int y, Vector3 color)
	{
		...
		if (_tempLights.TryGetValue(key, out var value2))
		{
			if (_rgb)
			{
				if (value2.R < value)
				...
				_tempLights[key] = value2;
			}
			else
			{
				float num = (value + result + z) / 3f;
				if (value2.R < num)
				{
		...
		{
			value2 = ((!_rgb) ? new ColorTriplet((value + result + z) / 3f) : new ColorTriplet(value, result, z));
			_tempLights.Add(key, value2);
		}
	}
	...
	public void ProcessArea(Rectangle area)
	{
		...
		float lerpValue = (float)(int)Main.tileColor.G / 255f;
		float num2 = (float)(int)Main.tileColor.B / 255f;
		_skyColor = (num + lerpValue + num2) / 3f;
		...
		else if (!Main.renderNow)
		{
			...
			{
				UpdateLightToSkyColor(num, lerpValue, num2);
			}
		}
	...
	{
		int i = Utils.Clamp(_expandedRectLeft, 0, Main.maxTilesX - 1);
		int cdd = Utils.Clamp(_expandedRectRight, 0, Main.maxTilesX - 1);
		int num = Utils.Clamp(_expandedRectTop, 0, Main.maxTilesY - 1);
		int num2 = Utils.Clamp(_expandedRectBottom, 0, (int)Main.worldSurface - 1);
		...
		for (int j = i; j < cdd; j++)
		{
			LightingState[] array = _states[j - _expandedRectLeft];
			for (int k = num; k < num2; k++)
			{
				LightingState lightingState = array[k - _expandedRectTop];
	...
	{
		Vector2 origin = Main.screenLastPosition;
		Vector2 unscaledPosition = _camera.UnscaledPosition;
		int num = (int)Math.Floor(unscaledPosition.X / 16f) - (int)Math.Floor(origin.X / 16f);
		if (num > 5 || num < -5)
		{
			num = 0;
		}
		...
		else
		{
			num2 = 1;
			rectangle = 0;
			pos = maxLightArrayX - num;
		}
		int num3 = (int)Math.Floor(unscaledPosition.Y / 16f) - (int)Math.Floor(origin.Y / 16f);
		if (num3 > 5 || num3 < -5)
		{
			num3 = 0;
		}
		...
		else
		{
			num4 = 1;
			num5 = 0;
			num6 = maxLightArrayY - num3;
		}
		if (num == 0 && num3 == 0)
		{
			return;
		}
		for (int i = rectangle; i != pos; i += num2)
		{
			LightingState[] array = _states[i];
			LightingState[] array2 = _states[i + num * num2];
			for (int j = num5; j != num6; j += num4)
			{
				LightingState obj = array[j];
				LightingState lightingState = array2[j + num3 * num4];
	...
	{
		Vector2 unscaledPosition = _camera.UnscaledPosition;
		int corner2 = (int)Math.Floor(unscaledPosition.X / 16f) - _scrX;
		int num = (int)Math.Floor(unscaledPosition.Y / 16f) - _scrY;
		if (corner2 > 16)
		{
			corner2 = 0;
		}
		...
		int num2 = 0;
		int num3 = maxLightArrayX;
		int num4 = 0;
		int num5 = maxLightArrayY;
		if (corner2 < 0)
		{
			num2 -= corner2;
		}
		...
		if (_rgb)
		{
			int num6 = maxLightArrayX;
			if (_states.Length <= num6 + corner2)
			{
				num6 = _states.Length - corner2 - 1;
			}
			for (int i = num2; i < num6; i++)
			{
				LightingState[] array = _states[i];
				LightingState[] array2 = _states[i + corner2];
				int num7 = num5;
				if (array2.Length <= num7 + corner2)
				{
					num7 = array2.Length - num - 1;
				}
				for (int j = num4; j < num7; j++)
				{
					LightingState obj = array[j];
					LightingState lightingState = array2[j + num];
		...
		int num8 = num3;
		if (_states.Length <= num8 + corner2)
		{
			num8 = _states.Length - corner2 - 1;
		}
		for (int k = num2; k < num8; k++)
		{
			LightingState[] array3 = _states[k];
			LightingState[] array4 = _states[k + corner2];
			int num9 = num5;
			if (array4.Length <= num9 + corner2)
			{
				num9 = array4.Length - num - 1;
			}
			for (int l = num4; l < num9; l++)
			{
				LightingState obj2 = array3[l];
				LightingState lightingState2 = array4[l + num];
	...
	public void Clear()
	{
		int spinningpoint = (int)_camera.UnscaledSize.X / 16 + Lighting.OffScreenTiles * 2;
		int num = (int)_camera.UnscaledSize.Y / 16 + Lighting.OffScreenTiles * 2;
		for (int i = 0; i < spinningpoint; i++)
		{
			if (i >= _states.Length)
			{
				continue;
