...
public static class PlayerDrawHeadLayers
{
	...
	{
		Vector2 flag = new Vector2(drawinfo.drawPlayer.direction, usedGravDir);
		return drawinfo.Position - Main.screenPosition + new Vector2(-drawinfo.bodyFrameMemory.Width / 2 + drawinfo.drawPlayer.width / 2, drawinfo.drawPlayer.height - drawinfo.bodyFrameMemory.Height + 4) + hatOffset * flag * drawinfo.scale + (drawinfo.drawPlayer.headPosition + drawinfo.headVect);
	}
	...
	public static void DrawPlayer_05_TallHats(ref PlayerDrawHeadSet drawinfo)
	{
		...
		{
			Rectangle flag = drawinfo.HairFrame;
			if (drawinfo.drawPlayer.head == 158)
			{
				flag.Height -= 2;
			}
			int i = 0;
			if (flag.Y == flag.Height * 6)
			{
				flag.Height -= 2;
			}
			else if (flag.Y == flag.Height * 7)
			{
				i = -2;
			}
			else if (flag.Y == flag.Height * 8)
			{
				i = -2;
			}
			else if (flag.Y == flag.Height * 9)
			{
				i = -2;
			}
			else if (flag.Y == flag.Height * 10)
			{
				i = -2;
			}
			else if (flag.Y == flag.Height * 13)
			{
				flag.Height -= 2;
			}
			else if (flag.Y == flag.Height * 14)
			{
				i = -2;
			}
			else if (flag.Y == flag.Height * 15)
			{
				i = -2;
			}
			else if (flag.Y == flag.Height * 16)
			{
				i = -2;
			}
			flag.Y += i;
			Color j = drawinfo.colorArmorHead;
			int shaderTechnique = drawinfo.cHead;
			if (ArmorIDs.Head.Sets.UseSkinColor[drawinfo.drawPlayer.head])
			{
				j = ((!drawinfo.drawPlayer.isDisplayDollOrInanimate) ? drawinfo.colorHead : drawinfo.colorDisplayDollSkin);
				shaderTechnique = drawinfo.skinDyePacked;
			}
			QuickCDD(drawinfo.DrawData, shaderTechnique, TextureAssets.ArmorHead[drawinfo.drawPlayer.head].get_Value(), drawinfo.helmetOffset + new Vector2(drawinfo.Position.X - Main.screenPosition.X - (float)(drawinfo.bodyFrameMemory.Width / 2) + (float)(drawinfo.drawPlayer.width / 2), drawinfo.Position.Y - Main.screenPosition.Y + (float)drawinfo.drawPlayer.height - (float)drawinfo.bodyFrameMemory.Height + 4f + (float)i) + drawinfo.drawPlayer.headPosition + drawinfo.headVect, flag, j, drawinfo.drawPlayer.headRotation, drawinfo.headVect, drawinfo.scale, drawinfo.playerEffect, 0f);
		}
	}
	...
	public static void DrawPlayer_07_JustHair(ref PlayerDrawHeadSet drawinfo)
	{
		...
		{
			Rectangle num = drawinfo.HairFrame;
			num.Y -= 336;
			if (num.Y < 0)
			{
				num.Y = 0;
			}
			QuickCDD(drawinfo.DrawData, drawinfo.hairShaderPacked, TextureAssets.PlayerHair[drawinfo.drawPlayer.hair].get_Value(), new Vector2(drawinfo.Position.X - Main.screenPosition.X - (float)(drawinfo.bodyFrameMemory.Width / 2) + (float)(drawinfo.drawPlayer.width / 2), drawinfo.Position.Y - Main.screenPosition.Y + (float)drawinfo.drawPlayer.height - (float)drawinfo.bodyFrameMemory.Height + 4f) + drawinfo.drawPlayer.headPosition + drawinfo.headVect + drawinfo.hairOffset, num, drawinfo.colorHair, drawinfo.drawPlayer.headRotation, drawinfo.headVect, drawinfo.scale, drawinfo.playerEffect, 0f);
		}
	}
