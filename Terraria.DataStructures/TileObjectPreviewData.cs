...
public class TileObjectPreviewData
{
	...
	public void CopyFrom(TileObjectPreviewData copy)
	{
		...
		{
			int i = ((copy._dataSize.X > _dataSize.X) ? copy._dataSize.X : _dataSize.X);
			int j = ((copy._dataSize.Y > _dataSize.Y) ? copy._dataSize.Y : _dataSize.Y);
			_data = new int[i, j];
			_dataSize = new Point16(i, j);
		}
		for (int k = 0; k < copy._dataSize.X; k++)
		{
			for (int l = 0; l < copy._dataSize.Y; l++)
			{
				_data[k, l] = copy._data[k, l];
			}
		}
	}
