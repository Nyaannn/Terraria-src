...
public abstract class AWorkshopEntry
{
	...
	{
		JToken i = default(JToken);
		if (!JObject.Parse(jsonText).TryGetValue("ContentType", ref i))
		{
			return null;
		}
		return i.ToObject<string>();
	}
	...
	protected static string CreateHeaderJson(string contentTypeName, ulong workshopEntryId, string[] tags, WorkshopItemPublicSettingId publicity, string previewImagePath)
	{
		...
		i["WorkshopPublishedVersion"] = 1;
		i["ContentType"] = contentTypeName;
		i["SteamEntryId"] = workshopEntryId;
		if (tags != null && tags.Length != 0)
		{
			i["Tags"] = JArray.FromObject((object)tags);
		}
		i["Publicity"] = publicity;
		return JsonConvert.SerializeObject((object)i, SerializerSettings);
	}
	...
	protected static bool TryGet<T>(Dictionary<string, object> dict, string name, out T outputValue)
	{
		...
		try
		{
			if (dict.TryGetValue(name, out var pt))
			{
				if (pt is T)
				{
					outputValue = (T)pt;
				...
				if (pt is JObject)
				{
					outputValue = JsonConvert.DeserializeObject<T>(((object)(JObject)pt).ToString());
					return true;
				}
				outputValue = (T)Convert.ChangeType(pt, typeof(T));
				return true;
			}
			return false;
		}
