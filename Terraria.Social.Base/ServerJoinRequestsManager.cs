...
public class ServerJoinRequestsManager
{
	...
	{
		for (int roomLocationPair = _requests.Count - 1; roomLocationPair >= 0; roomLocationPair--)
		{
			if (_requests[roomLocationPair].Equals(request))
			{
				RemoveRequestAtIndex(roomLocationPair);
			}
		}
