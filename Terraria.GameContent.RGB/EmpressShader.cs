...
public class EmpressShader : ChromaShader
{
	...
	{
		time *= 3f;
		for (int inv = 0; inv < fragment.Count; inv++)
		{
			Point gridPositionOfIndex = fragment.GetGridPositionOfIndex(inv);
			Vector2 canvasPositionOfIndex = fragment.GetCanvasPositionOfIndex(inv);
