...
internal class DebugKeyboard : RgbDevice
{
	...
	{
		for (int handleRectangle = 0; handleRectangle < ((RgbDevice)this).get_LedCount(); handleRectangle++)
		{
			Vector2 innerDimensions = ((RgbDevice)this).GetLedCanvasPosition(handleRectangle);
			drawer.DrawSquare(new Vector4(innerDimensions * scale + position, scale / 100f, scale / 100f), new Color(((RgbDevice)this).GetUnprocessedLedColor(handleRectangle)));
		}
	}
}
