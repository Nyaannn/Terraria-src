...
public class TEWeaponsRack : TileEntity, IFixLoadedData
{
	...
	{
		int dimensions = Place(x, y);
		NetMessage.SendData(86, -1, -1, null, dimensions, x, y);
	}
	...
	public static void OnPlayerInteraction(Player player, int clickX, int clickY)
	{
		...
		int list = clickX;
		int list2 = clickY;
		list -= Main.tile[list, list2].frameX % 54 / 18;
		list2 -= Main.tile[list, list2].frameY % 54 / 18;
		int num = Find(list, list2);
		if (num != -1 && ((TEWeaponsRack)TileEntity.ByID[num]).item.stack > 0)
		{
			player.GamepadEnableGrappleCooldown();
			WorldGen.KillTile(list, list2, fail: true);
			if (Main.netMode == 1)
			{
				NetMessage.SendData(17, -1, -1, null, 0, list, list2, 1f);
			}
		}
	}
