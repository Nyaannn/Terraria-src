...
public class TEFoodPlatter : TileEntity, IFixLoadedData
{
	...
	{
		int uIPanel = Place(x, y);
		NetMessage.SendData(86, -1, -1, null, uIPanel, x, y);
	}
	...
	{
		TEFoodPlatter canScroll = new TEFoodPlatter();
		canScroll.Position = new Point16(x, y);
		canScroll.ID = TileEntity.AssignNewID();
		canScroll.type = _myEntityID;
		lock (TileEntity.EntityCreationLock)
		{
			TileEntity.ByID[canScroll.ID] = canScroll;
			TileEntity.ByPosition[canScroll.Position] = canScroll;
		}
		return canScroll.ID;
	}
	...
	{
		if (TileEntity.ByPosition.TryGetValue(new Point16(x, y), out var uICollectionInfo) && uICollectionInfo.type == _myEntityID)
		{
			lock (TileEntity.EntityCreationLock)
			{
				TileEntity.ByID.Remove(uICollectionInfo.ID);
	...
	public static void TryPlacing(int x, int y, int netid, int prefix, int stack)
	{
		...
		int widthPerButton = Find(x, y);
		if (widthPerButton == -1)
		{
			int widthWithSpacing = Item.NewItem(new EntitySource_TileBreak(x, y), x * 16, y * 16, 16, 16, 1);
			Main.item[widthWithSpacing].netDefaults(netid);
			Main.item[widthWithSpacing].Prefix(prefix);
			Main.item[widthWithSpacing].stack = stack;
			NetMessage.SendData(21, -1, -1, null, widthWithSpacing);
			return;
		}
		TEFoodPlatter heightPerButton = (TEFoodPlatter)TileEntity.ByID[widthPerButton];
		if (heightPerButton.item.stack > 0)
		{
			heightPerButton.DropItem();
		}
		heightPerButton.item = new Item();
		heightPerButton.item.netDefaults(netid);
		heightPerButton.item.Prefix(prefix);
		heightPerButton.item.stack = stack;
		NetMessage.SendData(86, -1, -1, null, heightPerButton.ID, x, y);
	}
