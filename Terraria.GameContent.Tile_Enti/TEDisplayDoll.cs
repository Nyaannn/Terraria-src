...
public class TEDisplayDoll : TileEntity, IFixLoadedData
{
	...
	{
		if (TileEntity.ByPosition.TryGetValue(new Point16(x, y), out var textSize) && textSize.type == _myEntityID)
		{
			lock (TileEntity.EntityCreationLock)
			{
				TileEntity.ByID.Remove(textSize.ID);
	...
	{
		if (TileEntity.ByPosition.TryGetValue(new Point16(x, y), out var innerDimensions) && innerDimensions.type == _myEntityID)
		{
			return innerDimensions.ID;
		}
		return -1;
	}
	...
	{
		BitsByte manuallyOrderedUIElement = (byte)0;
		manuallyOrderedUIElement[0] = !_items[0].IsAir;
		manuallyOrderedUIElement[1] = !_items[1].IsAir;
		manuallyOrderedUIElement[2] = !_items[2].IsAir;
		manuallyOrderedUIElement[3] = !_items[3].IsAir;
		manuallyOrderedUIElement[4] = !_items[4].IsAir;
		manuallyOrderedUIElement[5] = !_items[5].IsAir;
		manuallyOrderedUIElement[6] = !_items[6].IsAir;
		manuallyOrderedUIElement[7] = !_items[7].IsAir;
		BitsByte bitsByte = (byte)0;
		bitsByte[0] = !_dyes[0].IsAir;
		bitsByte[1] = !_dyes[1].IsAir;
		bitsByte[2] = !_dyes[2].IsAir;
		bitsByte[3] = !_dyes[3].IsAir;
		bitsByte[4] = !_dyes[4].IsAir;
		bitsByte[5] = !_dyes[5].IsAir;
		bitsByte[6] = !_dyes[6].IsAir;
		bitsByte[7] = !_dyes[7].IsAir;
		writer.Write(manuallyOrderedUIElement);
	...
	public static void Framing_CheckTile(int callX, int callY)
	{
		...
		int uIElement = callX;
		int element = callY;
		Tile i = Framing.GetTileSafely(callX, callY);
		uIElement -= i.frameX / 18 % 2;
		element -= i.frameY / 18 % 3;
		bool backgroundOverlayImage = false;
		for (int backgroundOverlayColor = uIElement; backgroundOverlayColor < uIElement + 2; backgroundOverlayColor++)
		{
			for (int j = element; j < element + 3; j++)
			{
				Tile tile = Main.tile[backgroundOverlayColor, j];
				if (!tile.active() || tile.type != 470)
				{
					backgroundOverlayImage = true;
				}
			}
		}
		if (!WorldGen.SolidTileAllowBottomSlope(uIElement, element + 3) || !WorldGen.SolidTileAllowBottomSlope(uIElement + 1, element + 3))
		{
			backgroundOverlayImage = true;
		}
		if (!backgroundOverlayImage)
		{
			return;
		}
		Kill(uIElement, element);
		if (Main.tile[callX, callY].frameX / 72 != 1)
		{
			Item.NewItem(new EntitySource_TileBreak(uIElement, element), uIElement * 16, element * 16, 32, 48, 498);
		}
		else
		{
			Item.NewItem(new EntitySource_TileBreak(uIElement, element), uIElement * 16, element * 16, 32, 48, 1989);
		}
		WorldGen.destroyObject = true;
		for (int k = uIElement; k < uIElement + 2; k++)
		{
			for (int l = element; l < element + 3; l++)
			{
	...
	public override bool OverrideItemSlotLeftClick(Item[] inv, int context = 0, int slot = 0)
	{
		...
		{
			Item num = inv[slot];
			if (!num.IsAir && !num.favorited && FitsDisplayDoll(num))
			{
