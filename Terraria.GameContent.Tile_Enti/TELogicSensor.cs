...
public class TELogicSensor : TileEntity
{
	...
	{
		bool uIElement = GetState(Position.X, Position.Y, logicCheck, this);
		switch (logicCheck)
		{
		...
		case LogicCheckType.Liquid:
			if (On != uIElement)
			{
				ChangeState(uIElement, TripWire: true);
			}
			break;
		}
	}
	...
	public static LogicCheckType FigureCheckType(int x, int y, out bool on)
	{
		...
		Tile uIText = Main.tile[x, y];
		if (uIText == null)
		{
			return LogicCheckType.None;
		}
		LogicCheckType itemIdChecked = LogicCheckType.None;
		switch (uIText.frameY / 18)
		{
		case 0:
			itemIdChecked = LogicCheckType.Day;
			break;
		case 1:
			itemIdChecked = LogicCheckType.Night;
			break;
		case 2:
			itemIdChecked = LogicCheckType.PlayerAbove;
			break;
		case 3:
			itemIdChecked = LogicCheckType.Water;
			break;
		case 4:
			itemIdChecked = LogicCheckType.Lava;
			break;
		case 5:
			itemIdChecked = LogicCheckType.Honey;
			break;
		case 6:
			itemIdChecked = LogicCheckType.Liquid;
			break;
		}
		on = GetState(x, y, itemIdChecked);
	...
	public static bool GetState(int x, int y, LogicCheckType type, TELogicSensor instance = null)
	{
		...
		case LogicCheckType.PlayerAbove:
		{
			bool itemIdChecked = false;
			Rectangle amountWeHave = new Rectangle(x * 16 - 32 - 1, y * 16 - 160 - 1, 82, 162);
			{
				foreach (KeyValuePair<int, Rectangle> item in playerBox)
				{
					if (item.Value.Intersects(amountWeHave))
					{
						return true;
					}
				}
	...
	{
		if (TileEntity.ByPosition.TryGetValue(new Point16(x, y), out var localPlayerCreativeTracker) && localPlayerCreativeTracker.type == _myEntityID)
		{
			return localPlayerCreativeTracker.ID;
		}
		return -1;
	}
