...
public class TETrainingDummy : TileEntity
{
	...
	{
		TETrainingDummy startX = new TETrainingDummy();
		startX.Position = new Point16(x, y);
		startX.ID = TileEntity.AssignNewID();
		startX.type = _myEntityID;
		lock (TileEntity.EntityCreationLock)
		{
			TileEntity.ByID[startX.ID] = startX;
			TileEntity.ByPosition[startX.Position] = startX;
		}
		return startX.ID;
	}
	...
	{
		if (TileEntity.ByPosition.TryGetValue(new Point16(x, y), out var rectangle) && rectangle.type == _myEntityID)
		{
			lock (TileEntity.EntityCreationLock)
			{
				TileEntity.ByID.Remove(rectangle.ID);
