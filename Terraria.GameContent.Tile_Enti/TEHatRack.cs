...
public class TEHatRack : TileEntity, IFixLoadedData
{
	...
	{
		BitsByte num = (byte)0;
		num[0] = !_items[0].IsAir;
		num[1] = !_items[1].IsAir;
		num[2] = !_dyes[0].IsAir;
		num[3] = !_dyes[1].IsAir;
		writer.Write(num);
		for (int i = 0; i < 2; i++)
	...
	public static void Framing_CheckTile(int callX, int callY)
	{
		...
		int uICharacterListItem = callX;
		int num = callY;
		Tile tileSafely = Framing.GetTileSafely(callX, callY);
		uICharacterListItem -= tileSafely.frameX / 18 % 3;
		num -= tileSafely.frameY / 18 % 4;
		bool flag = false;
		for (int i = uICharacterListItem; i < uICharacterListItem + 3; i++)
		{
		...
		if (!WorldGen.SolidTileAllowBottomSlope(uICharacterListItem, num + 4) || !WorldGen.SolidTileAllowBottomSlope(uICharacterListItem + 1, num + 4) || !WorldGen.SolidTileAllowBottomSlope(uICharacterListItem + 2, num + 4))
		{
			flag = true;
		}
		if (!flag)
		...
		Kill(uICharacterListItem, num);
		Item.NewItem(new EntitySource_TileBreak(uICharacterListItem, num), uICharacterListItem * 16, num * 16, 48, 64, 3977);
		WorldGen.destroyObject = true;
		for (int k = uICharacterListItem; k < uICharacterListItem + 3; k++)
		{
		...
		for (int i = 0; i < slotsToShowLine; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				...
				if (Utils.FloatIntersect(Main.mouseX, Main.mouseY, 0f, 0f, num2, num3, (float)TextureAssets.InventoryBack.Width() * Main.inventoryScale, (float)TextureAssets.InventoryBack.Height() * Main.inventoryScale) && !PlayerInput.IgnoreMouseInterface)
				{
					...
					ItemSlot.Handle(dimensions, num, i + slotsArrayOffset);
				}
				ItemSlot.Draw(spriteBatch, dimensions, num, i + slotsArrayOffset, new Vector2(num2, num3));
			}
		}
	}
	...
	public override bool OverrideItemSlotLeftClick(Item[] inv, int context = 0, int slot = 0)
	{
		...
		{
			Item dimensions = inv[slot];
			if (Main.cursorOverride == 9 && !dimensions.IsAir && !dimensions.favorited && context == 0 && FitsHatRack(dimensions))
			{
	...
	{
		int dimensions = clickX;
		int num = clickY;
		dimensions -= Main.tile[dimensions, num].frameX % 54 / 18;
		num -= Main.tile[dimensions, num].frameY / 18;
		int num2 = Find(dimensions, num);
		if (num2 != -1)
		{
			return !(TileEntity.ByID[num2] as TEHatRack).ContainsItems();
		}
		return true;
	}
