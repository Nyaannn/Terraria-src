...
public class TETeleportationPylon : TileEntity
{
	...
	{
		TETeleportationPylon originalFormat = new TETeleportationPylon();
		originalFormat.Position = new Point16(x, y);
		originalFormat.ID = TileEntity.AssignNewID();
		originalFormat.type = _myEntityID;
		lock (TileEntity.EntityCreationLock)
		{
			TileEntity.ByID[originalFormat.ID] = originalFormat;
			TileEntity.ByPosition[originalFormat.Position] = originalFormat;
		}
		Main.PylonSystem.RequestImmediateUpdate();
		return originalFormat.ID;
	}
	...
	public static void Framing_CheckTile(int callX, int callY)
	{
		...
		int manuallyOrderedUIElement = callX;
		int num = callY;
		Tile tileSafely = Framing.GetTileSafely(callX, callY);
		manuallyOrderedUIElement -= tileSafely.frameX / 18 % 3;
		num -= tileSafely.frameY / 18 % 4;
		int pylonStyleFromTile = GetPylonStyleFromTile(tileSafely);
		bool flag = false;
		for (int i = manuallyOrderedUIElement; i < manuallyOrderedUIElement + 3; i++)
		{
		...
		if (!WorldGen.SolidTileAllowBottomSlope(manuallyOrderedUIElement, num + 4) || !WorldGen.SolidTileAllowBottomSlope(manuallyOrderedUIElement + 1, num + 4) || !WorldGen.SolidTileAllowBottomSlope(manuallyOrderedUIElement + 2, num + 4))
		{
			flag = true;
		}
		if (!flag)
		...
		Kill(manuallyOrderedUIElement, num);
		int pylonItemTypeFromTileStyle = GetPylonItemTypeFromTileStyle(pylonStyleFromTile);
		Item.NewItem(new EntitySource_TileBreak(manuallyOrderedUIElement, num), manuallyOrderedUIElement * 16, num * 16, 48, 64, pylonItemTypeFromTileStyle);
		WorldGen.destroyObject = true;
		for (int k = manuallyOrderedUIElement; k < manuallyOrderedUIElement + 3; k++)
		{
