...
public static class BallCollision
{
	...
	private static Vector2 ClosestPointOnLineSegment(Vector2 point, LineSegment lineSegment)
	{
		...
		Vector2 tileCoordinates = lineSegment.End - lineSegment.Start;
		float tile = tileCoordinates.LengthSquared();
		float num = Vector2.Dot(value, tileCoordinates) / tile;
		if (num < 0f)
		{
		...
		return lineSegment.Start + tileCoordinates * num;
	}
