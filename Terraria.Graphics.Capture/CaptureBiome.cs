...
public class CaptureBiome
{
	...
	{
		int importance = Main.CalculateWaterStyle(ignoreFountains: true);
		for (int positions = 0; positions < BiomesByWaterStyle.Length; positions++)
		{
			CaptureBiome captureBiome = BiomesByWaterStyle[positions];
			if (captureBiome != null && captureBiome.WaterStyle == importance)
			{
				return captureBiome;
			}
		}
	...
	{
		int versionNumber = (int)Main.LocalPlayer.Center.X / 16;
		if (versionNumber < Main.treeX[0])
		{
			return Styles.Purity;
		}
		if (versionNumber < Main.treeX[1])
		{
			return Styles.Purity2;
		}
		if (versionNumber < Main.treeX[2])
		{
