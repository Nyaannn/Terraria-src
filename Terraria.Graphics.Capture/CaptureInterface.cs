...
public class CaptureInterface
{
	...
	public static Rectangle GetArea()
	{
		...
		int num = Math.Min(EdgeA.Y, EdgeB.Y);
		int i = Math.Abs(EdgeA.X - EdgeB.X);
		int list = Math.Abs(EdgeA.Y - EdgeB.Y);
		return new Rectangle(x, num, i + 1, list + 1);
	}
	...
	public void Update()
	{
		...
		bool num = PlayerInput.Triggers.Current.ToggleCameraMode;
		if (num && !KeyToggleActiveHeld && (Main.mouseItem.type == 0 || Active) && !Main.CaptureModeDisabled && !Main.player[Main.myPlayer].dead && !Main.player[Main.myPlayer].ghost)
		{
			ToggleCamera(!Active);
		}
		KeyToggleActiveHeld = num;
		if (!Active)
		{
			return;
		}
		...
		Vector2 i = new Vector2(Main.mouseX, Main.mouseY);
		if (UpdateButtons(i) && Main.mouseLeft)
		{
			return;
		}
	...
	{
		Point baseStream = Main.ViewPosition.ToTileCoordinates();
		Point num = (Main.ViewPosition + Main.ViewSize).ToTileCoordinates();
		StartCamera(new CaptureSettings
		{
			Area = new Rectangle(baseStream.X, baseStream.Y, num.X - baseStream.X + 1, num.Y - baseStream.Y + 1),
			Biome = CaptureBiome.GetCaptureBiome(Settings.BiomeChoiceIndex),
			CaptureBackground = !Settings.TransparentBackground,
			CaptureEntities = Settings.IncludeEntities,
			UseScaling = Settings.PackImage,
	...
	private void DrawButtons(SpriteBatch sb)
	{
		...
		int buffer = 9;
		for (int input = 0; input < buffer; input++)
		{
			Texture2D binaryReader = TextureAssets.InventoryBack.get_Value();
			float num3 = 0.8f;
			Vector2 vector = new Vector2(24 + 46 * input, 24f);
			Color color = Main.inventoryBack * 0.8f;
			if (SelectedMode == 0 && input == 2)
			{
				binaryReader = TextureAssets.InventoryBack14.get_Value();
			}
			else if (SelectedMode == 1 && input == 3)
			{
				binaryReader = TextureAssets.InventoryBack14.get_Value();
			}
			else if (SelectedMode == 2 && input == 6)
			{
				binaryReader = TextureAssets.InventoryBack14.get_Value();
			}
			else if (input >= 2 && input <= 3)
			{
				binaryReader = TextureAssets.InventoryBack2.get_Value();
			}
			sb.Draw(binaryReader, vector, null, color, 0f, default(Vector2), num3, SpriteEffects.None, 0f);
			switch (input)
			{
			case 0:
				binaryReader = TextureAssets.Camera[7].get_Value();
				break;
			case 1:
				binaryReader = TextureAssets.Camera[0].get_Value();
				break;
			case 2:
			case 3:
			case 4:
				binaryReader = TextureAssets.Camera[input].get_Value();
				break;
			case 5:
				binaryReader = (Main.mapFullscreen ? TextureAssets.MapIcon[0].get_Value() : TextureAssets.MapIcon[4].get_Value());
				break;
			case 6:
				binaryReader = TextureAssets.Camera[1].get_Value();
				break;
			case 7:
				binaryReader = TextureAssets.Camera[6].get_Value();
				break;
			case 8:
				binaryReader = TextureAssets.Camera[5].get_Value();
				break;
			}
			sb.Draw(binaryReader, vector + new Vector2(26f) * num3, null, Color.White, 0f, binaryReader.Size() / 2f, 1f, SpriteEffects.None, 0f);
			bool flag = false;
			switch (input)
			{
			case 1:
			...
			if (flag)
			{
				sb.Draw(TextureAssets.Cd.get_Value(), vector + new Vector2(26f) * num3, null, Color.White * 0.65f, 0f, TextureAssets.Cd.get_Value().Size() / 2f, 1f, SpriteEffects.None, 0f);
			}
		}
		string num2 = "";
		switch (HoveredMode)
		{
		case 0:
			num2 = Lang.inter[111].Value;
			break;
		case 1:
			num2 = Lang.inter[67].Value;
			break;
		case 2:
			num2 = Lang.inter[69].Value;
			break;
		case 3:
			num2 = Lang.inter[70].Value;
			break;
		case 4:
			num2 = Lang.inter[78].Value;
			break;
		case 5:
			num2 = (Main.mapFullscreen ? Lang.inter[109].Value : Lang.inter[108].Value);
			break;
		case 6:
			num2 = Lang.inter[68].Value;
			break;
		case 7:
			num2 = Lang.inter[110].Value;
			break;
		case 8:
			num2 = Lang.inter[71].Value;
			break;
		default:
			num2 = "???";
			break;
		...
		if (num2 != "")
		{
			Main.instance.MouseText(num2, 0, 0);
		}
	}
	...
	private static bool GetMapCoords(int PinX, int PinY, int Goal, out Point result)
	{
		...
		float num = 10f;
		float num2 = 10f;
		float num3 = Main.maxTilesX - 10;
		float num4 = Main.maxTilesY - 10;
		i = 200f;
		j = 300f;
		tile = Main.mapFullscreenScale;
		...
		float num6 = x * tile;
		y *= tile;
		i = 0f - num6 + (float)(Main.screenWidth / 2);
		j = 0f - y + (float)(Main.screenHeight / 2);
		i += num * tile;
		j += num2 * tile;
		float num7 = Main.maxTilesX / 840;
		num7 *= Main.mapFullscreenScale;
		float num8 = i;
		float num9 = j;
		float num10 = TextureAssets.Map.Width();
		float num11 = TextureAssets.Map.Height();
		...
		case 0:
		{
			int num12 = (int)((0f - i + (float)PinX) / tile + num);
			int num13 = (int)((0f - j + (float)PinY) / tile + num2);
			bool flag = false;
			if ((float)num12 < num)
			{
				flag = true;
		...
		case 1:
		{
			...
			Vector2 vector2 = new Vector2(PinX, PinY) * tile - new Vector2(10f * tile);
