...
public static class GolfHelper
{
	...
	public static bool IsPlayerHoldingClub(Player player)
	{
		...
		int value = player.HeldItem.type;
		if (value == 4039 || (uint)(value - 4092) <= 2u || (uint)(value - 4587) <= 11u)
		{
			return true;
		}
		return false;
	...
	{
		Vector2 value = golfBall.Center - player.Bottom;
		if (player.direction == -1)
		{
			value.X *= -1f;
			shotVector.X *= -1f;
		}
		float x = shotVector.ToRotation();
		if (x > 0f)
		{
			shotVector = shotVector.Length() * new Vector2((float)Math.Cos(0.0), (float)Math.Sin(0.0));
		}
		else if (x < -1.52079642f)
		{
		...
		if (value.X >= -16f && value.X <= 32f && value.Y <= 16f)
		{
			return value.Y >= -16f;
		}
		return false;
	...
	{
		Vector2 i = entity.Bottom;
		i.Y += 1f;
		Point point = i.ToTileCoordinates();
