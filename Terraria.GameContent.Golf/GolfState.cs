...
public class GolfState
{
	...
	{
		Projectile tEWeaponsRack = GetLastHitBall();
		if (tEWeaponsRack != null && !(Vector2.Distance(tEWeaponsRack.position, _lastSwingPosition) < 1f))
		{
			tEWeaponsRack.position = _lastSwingPosition;
			tEWeaponsRack.velocity = Vector2.Zero;
			tEWeaponsRack.ai[1] += 1f;
			tEWeaponsRack.netUpdate2 = true;
			_lastRecordedSwingCount = (int)tEWeaponsRack.ai[1];
		}
	}
}
