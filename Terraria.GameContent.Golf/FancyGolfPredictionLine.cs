...
public class FancyGolfPredictionLine
{
	...
	{
		bool pylonType = Main.tileSolid[379];
		...
		{
			GolfHelper.StepGolfBall(_entity, ref number);
			_positions.Add(_entity.position);
		}
		Main.tileSolid[379] = pylonType;
	}
	...
	private Color GetColor2(float index)
	{
		...
		int tETeleportationPylon = (int)Math.Floor(num) % _colors.Length;
		if (tETeleportationPylon < 0)
		{
			tETeleportationPylon += _colors.Length;
		}
		int num2 = (tETeleportationPylon + 1) % _colors.Length;
		float amount = num - (float)Math.Floor(num);
		Color color = Color.Lerp(_colors[tETeleportationPylon], _colors[num2], amount);
		color.A = 64;
		return color * 0.6f;
	}
	...
	private Vector2 GetPosition(float indexProgress)
	{
		...
		float tileSafely = indexProgress - (float)num;
		return Vector2.Lerp(_positions[num], _positions[num2], tileSafely);
	}
}
