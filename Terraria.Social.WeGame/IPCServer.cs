...
public class IPCServer : IPCBase
{
	...
	private void ConnectionCallback(IAsyncResult result)
	{
		...
		catch (IOException tileCoords)
		{
			_pipeBrokenFlag = true;
			WeGameHelper.WriteDebugString("ConnectionCallback Exception, {0}", tileCoords.Message);
		}
	}
	...
	public void ContinueReadOrWait()
	{
		...
		{
			_pipeBrokenFlag = true;
			WeGameHelper.WriteDebugString("ContinueReadOrWait Exception, {0}", num.Message);
		}
	}
