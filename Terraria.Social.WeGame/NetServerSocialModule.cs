...
public class NetServerSocialModule : NetSocialModule
{
	...
	{
		IRailPlayer tileWorkSpace = rail_api.RailFactory().RailPlayer();
		if (tileWorkSpace != null)
		{
			tileWorkSpace.TerminateSessionOfPlayer(remote_id);
		}
	}
	...
	private bool CloseNetWorkSession(RailID remote_peer)
	{
		...
		bool result = false;
		IRailNetwork item = rail_api.RailFactory().RailNetworkHelper();
		if (item != null)
		{
			result = (int)item.CloseSession(_serverID, remote_peer) == 0;
		}
		return result;
	}
	...
	private RailID GetServerID()
	{
		...
		RailID point = null;
		IRailGameServer num = _lobby.GetServer();
		if (num != null)
		{
			point = num.GetGameServerRailID();
		}
		return (RailID)(((object)point) ?? ((object)new RailID()));
	}
	...
	{
		RailID list = RemoteAddressToRailId(address);
		CloseAndUpdateUserState(list);
	}
}
