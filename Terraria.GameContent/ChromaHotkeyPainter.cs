...
public class ChromaHotkeyPainter
{
	private class ReactiveRGBKey
	{
		...
		public void Update()
		{
			float packet = (float)Utils.GetLerpValue(_startTime.TotalSeconds, _expireTime.TotalSeconds, Main.gameTimeCache.TotalGameTime.TotalSeconds, clamped: true);
			_rgbKey.SetSolid(Color.Lerp(_color, Color.Black, packet));
		}
	...
	private void Step_Movement()
	{
		...
		{
			SetGroupColorBase(text, PainterColors.DangerKeyBlocked);
		}
		else
		{
			SetGroupColorBase(text, PainterColors.MovementKeys);
		}
	}
