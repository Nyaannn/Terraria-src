...
public class MinecartDiggerHelper
{
	...
	{
		digDirectionY = 0;
		Point vector = trackWorldPosition.ToTileCoordinates();
		if (Framing.GetTileSafely(vector).type != 314 || (double)vector.Y < Main.worldSurface)
		{
			return;
		}
		Point point = vector;
		point.X += digDirectionX;
		point.Y += digDirectionY;
		if (AlreadyLeadsIntoWantedTrack(vector, point) || (digDirectionY == 0 && (AlreadyLeadsIntoWantedTrack(vector, new Point(point.X, point.Y - 1)) || AlreadyLeadsIntoWantedTrack(vector, new Point(point.X, point.Y + 1)))))
		{
			return;
		}
		int num = 5;
		...
		point3.Y -= num - 1;
		int x = point3.X;
		for (int i = point3.Y; i <= point2.Y; i++)
		{
		...
		{
			int x2 = point3.X;
			for (int j = point3.Y; j <= point2.Y; j++)
			{
				MineTheTileIfNecessary(x2, j);
			}
			ConsumeATrackItem(player);
			PlaceATrack(point.X, point.Y);
			player.velocity.X = MathHelper.Clamp(player.velocity.X, -1f, 1f);
			if (!DoTheTracksConnectProperly(vector, point))
			{
				CorrectTrackConnections(vector, point);
			}
		}
	}
	...
	private bool CanGetPastTile(int x, int y)
	{
		...
		if (uIElement.active() && (TileID.Sets.Falling[uIElement.type] || (uIElement.type == 26 && !Main.hardMode) || !WorldGen.CanKillTile(x, y)))
		{
			return false;
		}
		return true;
