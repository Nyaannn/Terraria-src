...
public class PlayerTitaniumStormBuffTextureContent : ARenderTargetContentByRequest
{
	...
	protected override void HandleUseReqest(GraphicsDevice device, SpriteBatch spriteBatch)
	{
		...
		Asset<Texture2D> value = TextureAssets.Projectile[908];
		UpdateSettingsForRendering(0.6f, 0f, Main.GlobalTimeWrappedHourly, 0.3f);
		PrepareARenderTarget_AndListenToEvents(ref _target, device, value.Width(), value.Height(), RenderTargetUsage.PreserveContents);
		device.SetRenderTarget(_target);
		device.Clear(Color.Transparent);
		DrawData value2 = new DrawData(value.get_Value(), Vector2.Zero, Color.White);
		spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend);
		_shaderData.Apply(value2);
