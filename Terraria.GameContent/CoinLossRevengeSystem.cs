...
public class CoinLossRevengeSystem
{
	public class RevengeMarker
	{
		...
		{
			Vector2 num = _location / 16f - mapTopLeft;
			num *= mapScale;
			num += mapX2Y2AndOff;
			if (mapRect.HasValue && !mapRect.Value.Contains(num.ToPoint()))
			{
				return false;
			}
			Texture2D packet = TextureAssets.MapDeath.get_Value();
			packet = ((_coinsValue < 100) ? TextureAssets.Coin[0].get_Value() : ((_coinsValue < 10000) ? TextureAssets.Coin[1].get_Value() : ((_coinsValue >= 1000000) ? TextureAssets.Coin[3].get_Value() : TextureAssets.Coin[2].get_Value())));
			Rectangle rectangle = packet.Frame(1, 8);
			spriteBatch.Draw(packet, num, rectangle, Color.White, 0f, rectangle.Size() / 2f, drawScale, SpriteEffects.None, 0f);
			return Utils.CenteredRectangle(num, rectangle.Size() * drawScale).Contains(Main.MouseScreen.ToPoint());
		}
