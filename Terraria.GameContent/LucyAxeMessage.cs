...
public static class LucyAxeMessage
{
	...
	{
		for (int randomColorVector = 0; randomColorVector < _messageCooldownsByType.Length; randomColorVector++)
		{
			if (_messageCooldownsByType[randomColorVector] > 0)
			{
				_messageCooldownsByType[randomColorVector]--;
			}
		}
	}
	...
	private static void ItemSlot_OnItemTransferred(ItemSlot.ItemTransferInfo info)
	{
		...
		bool colorPickers = CountsAsStorage(info.FromContenxt);
		bool i = CountsAsStorage(info.ToContext);
		if (colorPickers != i)
		{
			MessageSource messageSource = ((!colorPickers) ? MessageSource.Storage : MessageSource.PickedUp);
			if (_messageCooldownsByType[(int)messageSource] <= 0)
			{
