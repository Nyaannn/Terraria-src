...
public class SmartCursorHelper
{
	...
	public static void SmartCursorLookup(Player player)
	{
		...
		SmartCursorUsageInfo list = new SmartCursorUsageInfo
		{
			player = player,
		...
		list.screenTargetX = Utils.Clamp(tileTargetX, 10, Main.maxTilesX - 10);
		list.screenTargetY = Utils.Clamp(tileTargetY, 10, Main.maxTilesY - 10);
		if (Main.tile[list.screenTargetX, list.screenTargetY] == null)
		{
			return;
		}
		bool num = IsHoveringOverAnInteractibleTileThatBlocksSmartCursor(list);
		TryFindingPaintInplayerInventory(list, out list.paintLookup, out list.paintCoatingLookup);
		int tileBoost = list.item.tileBoost;
		list.reachableStartX = (int)(player.position.X / 16f) - tileRangeX - tileBoost + 1;
		list.reachableEndX = (int)((player.position.X + (float)player.width) / 16f) + tileRangeX + tileBoost - 1;
		list.reachableStartY = (int)(player.position.Y / 16f) - tileRangeY - tileBoost + 1;
		list.reachableEndY = (int)((player.position.Y + (float)player.height) / 16f) + tileRangeY + tileBoost - 2;
		list.reachableStartX = Utils.Clamp(list.reachableStartX, 10, Main.maxTilesX - 10);
		list.reachableEndX = Utils.Clamp(list.reachableEndX, 10, Main.maxTilesX - 10);
		list.reachableStartY = Utils.Clamp(list.reachableStartY, 10, Main.maxTilesY - 10);
		list.reachableEndY = Utils.Clamp(list.reachableEndY, 10, Main.maxTilesY - 10);
		if (!num || list.screenTargetX < list.reachableStartX || list.screenTargetX > list.reachableEndX || list.screenTargetY < list.reachableStartY || list.screenTargetY > list.reachableEndY)
		{
			...
			{
				Step_Axe(list, ref fX, ref fY);
			}
			Step_ForceCursorToAnyMinableThing(list, ref fX, ref fY);
			Step_Pickaxe_MineShinies(list, ref fX, ref fY);
			Step_Pickaxe_MineSolids(player, list, _grappleTargets, ref fX, ref fY);
			if (Player.SmartCursorSettings.SmartAxeAfterPickaxe)
			{
				Step_Axe(list, ref fX, ref fY);
			}
			Step_ColoredWrenches(list, ref fX, ref fY);
			Step_MulticolorWrench(list, ref fX, ref fY);
			Step_Hammers(list, ref fX, ref fY);
			Step_ActuationRod(list, ref fX, ref fY);
			Step_WireCutter(list, ref fX, ref fY);
			Step_Platforms(list, ref fX, ref fY);
			Step_MinecartTracks(list, ref fX, ref fY);
			Step_Walls(list, ref fX, ref fY);
			Step_PumpkinSeeds(list, ref fX, ref fY);
			Step_GrassSeeds(list, ref fX, ref fY);
			Step_Pigronata(list, ref fX, ref fY);
			Step_Boulders(list, ref fX, ref fY);
			Step_Torch(list, ref fX, ref fY);
			Step_LawnMower(list, ref fX, ref fY);
			Step_BlocksFilling(list, ref fX, ref fY);
			Step_BlocksLines(list, ref fX, ref fY);
			Step_PaintRoller(list, ref fX, ref fY);
			Step_PaintBrush(list, ref fX, ref fY);
			Step_PaintScrapper(list, ref fX, ref fY);
			Step_Acorns(list, ref fX, ref fY);
			Step_GemCorns(list, ref fX, ref fY);
			Step_EmptyBuckets(list, ref fX, ref fY);
			Step_Actuators(list, ref fX, ref fY);
			Step_AlchemySeeds(list, ref fX, ref fY);
			Step_PlanterBox(list, ref fX, ref fY);
			Step_ClayPots(list, ref fX, ref fY);
			Step_StaffOfRegrowth(list, ref fX, ref fY);
			if (fX != -1 && fY != -1)
			{
	...
	private static void Step_PlanterBox(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		if (!num)
		{
			for (int num2 = providedInfo.reachableStartX; num2 <= providedInfo.reachableEndX; num2++)
			{
				for (int snapPoints = providedInfo.reachableStartY; snapPoints <= providedInfo.reachableEndY; snapPoints++)
				{
					Tile uILinkPoint2 = Main.tile[num2, snapPoints];
					if (uILinkPoint2.active() && uILinkPoint2.type == 380)
					{
						if (!Main.tile[num2 - 1, snapPoints].active() || Main.tileCut[Main.tile[num2 - 1, snapPoints].type] || TileID.Sets.BreakableWhenPlacing[Main.tile[num2 - 1, snapPoints].type])
						{
							_targets.Add(new Tuple<int, int>(num2 - 1, snapPoints));
						}
						if (!Main.tile[num2 + 1, snapPoints].active() || Main.tileCut[Main.tile[num2 + 1, snapPoints].type] || TileID.Sets.BreakableWhenPlacing[Main.tile[num2 + 1, snapPoints].type])
						{
							_targets.Add(new Tuple<int, int>(num2 + 1, snapPoints));
		...
		{
			float num3 = -1f;
			Tuple<int, int> list = _targets[0];
			for (int array = 0; array < _targets.Count; array++)
			{
				float num4 = Vector2.Distance(new Vector2(_targets[array].Item1, _targets[array].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
			...
			if (Collision.InTileBounds(list.Item1, list.Item2, providedInfo.reachableStartX, providedInfo.reachableStartY, providedInfo.reachableEndX, providedInfo.reachableEndY) && num3 != -1f)
			{
				focusedX = list.Item1;
				focusedY = list.Item2;
			}
	...
	private static void Step_Actuators(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		_targets.Clear();
		for (int scaleFactor = providedInfo.reachableStartX; scaleFactor <= providedInfo.reachableEndX; scaleFactor++)
		{
			for (int minimum = providedInfo.reachableStartY; minimum <= providedInfo.reachableEndY; minimum++)
			{
				Tile maximum = Main.tile[scaleFactor, minimum];
				if ((maximum.wire() || maximum.wire2() || maximum.wire3() || maximum.wire4()) && !maximum.actuator() && maximum.active())
				{
					_targets.Add(new Tuple<int, int>(scaleFactor, minimum));
				}
			}
		}
		if (_targets.Count > 0)
		{
			...
			{
				float num = Vector2.Distance(new Vector2(_targets[j].Item1, _targets[j].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
				if (i == -1f || num < i)
				{
					i = num;
					tuple = _targets[j];
	...
	private static void Step_EmptyBuckets(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		_targets.Clear();
		for (int num = providedInfo.reachableStartX; num <= providedInfo.reachableEndX; num++)
		{
			for (int num2 = providedInfo.reachableStartY; num2 <= providedInfo.reachableEndY; num2++)
			{
				Tile snapPoints = Main.tile[num, num2];
				if (snapPoints.liquid <= 0)
				{
					continue;
				}
				int uILinkPoint = snapPoints.liquidType();
				int uILinkPoint2 = 0;
				for (int scaleFactor = num - 1; scaleFactor <= num + 1; scaleFactor++)
				{
					for (int minimum = num2 - 1; minimum <= num2 + 1; minimum++)
					{
						if (Main.tile[scaleFactor, minimum].liquidType() == uILinkPoint)
						{
							uILinkPoint2 += Main.tile[scaleFactor, minimum].liquid;
						}
					}
				}
				if (uILinkPoint2 > 100)
				{
					_targets.Add(new Tuple<int, int>(num, num2));
				}
			}
		}
		if (_targets.Count > 0)
		{
			float maximum = -1f;
			Tuple<int, int> num3 = _targets[0];
			for (int num4 = 0; num4 < _targets.Count; num4++)
			{
				float num5 = Vector2.Distance(new Vector2(_targets[num4].Item1, _targets[num4].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
				if (maximum == -1f || num5 < maximum)
				{
					maximum = num5;
					num3 = _targets[num4];
				}
			}
			if (Collision.InTileBounds(num3.Item1, num3.Item2, providedInfo.reachableStartX, providedInfo.reachableStartY, providedInfo.reachableEndX, providedInfo.reachableEndY))
			{
				focusedX = num3.Item1;
				focusedY = num3.Item2;
			}
	...
	{
		int num = providedInfo.item.type;
		if (num < 0 || num >= ItemID.Count || !Player.SmartCursorSettings.SmartBlocksEnabled || providedInfo.item.createTile <= -1 || num == 213 || num == 5295 || ItemID.Sets.GrassSeeds[num] || !Main.tileSolid[providedInfo.item.createTile] || Main.tileSolidTop[providedInfo.item.createTile] || Main.tileFrameImportant[providedInfo.item.createTile] || focusedX != -1 || focusedY != -1)
		{
			return;
		}
		_targets.Clear();
		...
		if (!array)
		{
			for (int i = providedInfo.reachableStartX; i <= providedInfo.reachableEndX; i++)
			{
				for (int num2 = providedInfo.reachableStartY; num2 <= providedInfo.reachableEndY; num2++)
				{
					Tile num3 = Main.tile[i, num2];
					if (!num3.active() || Main.tileCut[num3.type] || TileID.Sets.BreakableWhenPlacing[num3.type])
					{
						bool flag = false;
						if (Main.tile[i - 1, num2].active() && Main.tileSolid[Main.tile[i - 1, num2].type] && !Main.tileSolidTop[Main.tile[i - 1, num2].type])
						{
							flag = true;
						}
						if (Main.tile[i + 1, num2].active() && Main.tileSolid[Main.tile[i + 1, num2].type] && !Main.tileSolidTop[Main.tile[i + 1, num2].type])
						...
						if (Main.tile[i, num2 - 1].active() && Main.tileSolid[Main.tile[i, num2 - 1].type] && !Main.tileSolidTop[Main.tile[i, num2 - 1].type])
						{
							flag = true;
						}
						if (Main.tile[i, num2 + 1].active() && Main.tileSolid[Main.tile[i, num2 + 1].type] && !Main.tileSolidTop[Main.tile[i, num2 + 1].type])
						...
						if (flag)
						{
							_targets.Add(new Tuple<int, int>(i, num2));
						}
					}
				}
			}
		...
		{
			float j = -1f;
			Tuple<int, int> k = _targets[0];
			for (int uILinkPoint = 0; uILinkPoint < _targets.Count; uILinkPoint++)
			{
				if (Collision.EmptyTile(_targets[uILinkPoint].Item1, _targets[uILinkPoint].Item2))
				{
					float uILinkPoint2 = Vector2.Distance(new Vector2(_targets[uILinkPoint].Item1, _targets[uILinkPoint].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
					if (j == -1f || uILinkPoint2 < j)
					{
						j = uILinkPoint2;
						k = _targets[uILinkPoint];
					}
				}
			}
			if (Collision.InTileBounds(k.Item1, k.Item2, providedInfo.reachableStartX, providedInfo.reachableStartY, providedInfo.reachableEndX, providedInfo.reachableEndY) && j != -1f)
			{
				focusedX = k.Item1;
				focusedY = k.Item2;
			}
	...
	private static void Step_Boulders(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		for (int num = providedInfo.reachableStartX; num <= providedInfo.reachableEndX; num++)
		{
			for (int num2 = providedInfo.reachableStartY; num2 <= providedInfo.reachableEndY; num2++)
			{
				Tile i = Main.tile[num, num2 + 1];
				Tile j = Main.tile[num + 1, num2 + 1];
				bool k = true;
				if (!j.nactive() || !i.nactive())
				{
					k = false;
				}
				if (j.slope() > 0 || i.slope() > 0 || j.halfBrick() || i.halfBrick())
				{
					k = false;
				}
				if ((!Main.tileSolid[j.type] && !Main.tileTable[j.type]) || (!Main.tileSolid[i.type] && !Main.tileTable[i.type]))
				{
					k = false;
				}
				if (Main.tileNoAttach[j.type] || Main.tileNoAttach[i.type])
				{
					k = false;
				}
				for (int l = num; l <= num + 1; l++)
				{
					for (int m = num2 - 1; m <= num2; m++)
					{
	...
	private static void Step_Pigronata(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		for (int num = providedInfo.reachableStartX; num <= providedInfo.reachableEndX; num++)
		{
			for (int num2 = providedInfo.reachableStartY; num2 <= providedInfo.reachableEndY && !((double)num2 > Main.worldSurface - 2.0); num2++)
			{
				bool i = true;
		...
		if (_targets.Count > 0)
		{
			...
			{
				float num4 = Vector2.Distance(new Vector2(_targets[l].Item1, _targets[l].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
				if (num3 == -1f || num4 < num3)
				{
					num3 = num4;
					tuple = _targets[l];
	...
	private static void Step_PumpkinSeeds(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		for (int scaleFactor = providedInfo.reachableStartX; scaleFactor <= providedInfo.reachableEndX; scaleFactor++)
		{
			for (int minimum = providedInfo.reachableStartY; minimum <= providedInfo.reachableEndY; minimum++)
			{
				Tile maximum = Main.tile[scaleFactor, minimum + 1];
				Tile i = Main.tile[scaleFactor + 1, minimum + 1];
				if ((double)minimum > Main.worldSurface - 2.0)
				{
					break;
				}
				bool flag = true;
				if (!i.active() || !maximum.active())
				{
					flag = false;
				}
				if (i.slope() > 0 || maximum.slope() > 0 || i.halfBrick() || maximum.halfBrick())
				{
					flag = false;
				}
				if (i.type != 2 && i.type != 477 && i.type != 109 && i.type != 492)
				{
					flag = false;
				}
				if (maximum.type != 2 && maximum.type != 477 && maximum.type != 109 && maximum.type != 492)
				{
					flag = false;
				}
				for (int j = scaleFactor; j <= scaleFactor + 1; j++)
				{
					for (int k = minimum - 1; k <= minimum; k++)
					{
						Tile tile = Main.tile[j, k];
						if (tile.active() && (tile.type < 0 || tile.type >= TileID.Count || Main.tileSolid[tile.type] || !WorldGen.CanCutTile(j, k, TileCuttingContext.TilePlacement)))
						{
							flag = false;
						}
					}
		...
		if (_targets.Count > 0)
		{
			...
			for (int l = 0; l < _targets.Count; l++)
			{
				float num2 = Vector2.Distance(new Vector2(_targets[l].Item1, _targets[l].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
				if (num == -1f || num2 < num)
				{
					num = num2;
					tuple = _targets[l];
	...
	{
		int num = providedInfo.player.width;
		...
		for (int i = providedInfo.reachableStartX; i <= providedInfo.reachableEndX; i++)
		{
			...
			{
				Tile tile = Main.tile[i, j];
				if (tile.wall == 0 && (!tile.active() || !Main.tileSolid[tile.type] || Main.tileSolidTop[tile.type]) && Collision.CanHitWithCheck(providedInfo.position, num, height, new Vector2(i, j) * 16f, 16, 16, DelegateMethods.NotDoorStand))
				{
					bool flag = false;
	...
	private static void Step_MinecartTracks(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		if ((providedInfo.item.type == 2340 || providedInfo.item.type == 2739) && focusedX == -1 && focusedY == -1)
		{
			...
			float num2 = Vector2.Dot(value, -Vector2.UnitY);
			bool num = num2 >= 0.5f;
			bool flag = num2 <= -0.5f;
			float num3 = Vector2.Dot(value, Vector2.UnitX);
			bool flag2 = num3 >= 0.5f;
			bool flag3 = num3 <= -0.5f;
			bool flag4 = num && flag3;
			bool flag5 = num && flag2;
			...
			if (!flag8)
			{
				for (int i = providedInfo.reachableStartX; i <= providedInfo.reachableEndX; i++)
				{
					for (int j = providedInfo.reachableStartY; j <= providedInfo.reachableEndY; j++)
					{
						...
						{
							bool flag9 = Main.tile[i + 1, j + 1].active() && Main.tile[i + 1, j + 1].type == 314;
							...
							if (flag4 && (!Main.tile[i - 1, j - 1].active() || Main.tileCut[Main.tile[i - 1, j - 1].type] || TileID.Sets.BreakableWhenPlacing[Main.tile[i - 1, j - 1].type]) && !(!flag9 && flag10) && !flag11)
							{
							...
							if (flag6 && (!Main.tile[i - 1, j + 1].active() || Main.tileCut[Main.tile[i - 1, j + 1].type] || TileID.Sets.BreakableWhenPlacing[Main.tile[i - 1, j + 1].type]) && !(!flag10 && flag9) && !flag12)
							{
								_targets.Add(new Tuple<int, int>(i - 1, j + 1));
							}
							if (flag5 && (!Main.tile[i + 1, j - 1].active() || Main.tileCut[Main.tile[i + 1, j - 1].type] || TileID.Sets.BreakableWhenPlacing[Main.tile[i + 1, j - 1].type]) && !(!flag11 && flag12) && !flag9)
							{
								_targets.Add(new Tuple<int, int>(i + 1, j - 1));
							}
							if (flag2 && (!Main.tile[i + 1, j].active() || Main.tileCut[Main.tile[i + 1, j].type] || TileID.Sets.BreakableWhenPlacing[Main.tile[i + 1, j].type]))
							{
								_targets.Add(new Tuple<int, int>(i + 1, j));
							}
							if (flag7 && (!Main.tile[i + 1, j + 1].active() || Main.tileCut[Main.tile[i + 1, j + 1].type] || TileID.Sets.BreakableWhenPlacing[Main.tile[i + 1, j + 1].type]) && !(!flag12 && flag11) && !flag10)
							{
								_targets.Add(new Tuple<int, int>(i + 1, j + 1));
							}
						}
			...
			if (_targets.Count > 0)
			{
				...
				if (Collision.InTileBounds(tuple.Item1, tuple.Item2, providedInfo.reachableStartX, providedInfo.reachableStartY, providedInfo.reachableEndX, providedInfo.reachableEndY) && num4 != -1f)
				{
					focusedX = tuple.Item1;
					focusedY = tuple.Item2;
				}
		...
		if (_targets.Count > 0)
		{
			...
			for (int n = 0; n < _targets.Count; n++)
			{
				...
				{
					float num7 = Vector2.Distance(new Vector2(_targets[n].Item1, _targets[n].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
					if (num6 == -1f || num7 < num6)
					{
						num6 = num7;
						tuple2 = _targets[n];
					}
			...
			if (Collision.InTileBounds(tuple2.Item1, tuple2.Item2, providedInfo.reachableStartX, providedInfo.reachableStartY, providedInfo.reachableEndX, providedInfo.reachableEndY) && num6 != -1f)
			{
	...
	private static void Step_WireCutter(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		_targets.Clear();
		for (int array = providedInfo.reachableStartX; array <= providedInfo.reachableEndX; array++)
		{
			for (int i = providedInfo.reachableStartY; i <= providedInfo.reachableEndY; i++)
			{
				Tile tile = Main.tile[array, i];
				if (tile.wire() || tile.wire2() || tile.wire3() || tile.wire4() || tile.actuator())
				{
					_targets.Add(new Tuple<int, int>(array, i));
				}
			}
		}
		if (_targets.Count > 0)
		{
			...
			for (int k = 0; k < _targets.Count; k++)
			{
				float num = Vector2.Distance(new Vector2(_targets[k].Item1, _targets[k].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
				if (j == -1f || num < j)
				{
					j = num;
					tuple = _targets[k];
	...
	{
		bool array = providedInfo.player.ActuationRodLock;
		bool i = providedInfo.player.ActuationRodLockSetting;
		if (providedInfo.item.type != 3620 || focusedX != -1 || focusedY != -1)
		{
			return;
		}
		_targets.Clear();
		for (int k = providedInfo.reachableStartX; k <= providedInfo.reachableEndX; k++)
		{
			for (int j = providedInfo.reachableStartY; j <= providedInfo.reachableEndY; j++)
			{
				Tile tile = Main.tile[k, j];
				if (tile.active() && tile.actuator() && (!array || i == tile.inActive()))
				{
					_targets.Add(new Tuple<int, int>(k, j));
				}
			}
		}
		if (_targets.Count > 0)
	...
	{
		int list = providedInfo.player.width;
		int i = providedInfo.player.height;
		if (providedInfo.item.hammer > 0 && focusedX == -1 && focusedY == -1)
		{
			...
			int num4 = (int)((providedInfo.position.X + (float)(list / 2) + (float)((list / 2 - 1) * num)) / 16f);
			int num5 = (int)(((double)providedInfo.position.Y + 0.1) / 16.0);
			if (num3 == -1)
			{
				num5 = (int)((providedInfo.position.Y + (float)i - 1f) / 16f);
			}
			int num6 = list / 16 + ((list % 16 != 0) ? 1 : 0);
			int num7 = i / 16 + ((i % 16 != 0) ? 1 : 0);
			if (num != 0)
			{
				for (int j = 0; j < num7; j++)
				{
					if (Main.tile[num4, num5 + j * num3] != null)
					{
						_points.Add(new Tuple<int, int>(num4, num5 + j * num3));
					}
				}
			}
			if (num2 != 0)
			{
				...
				{
					if (Main.tile[(int)(providedInfo.position.X / 16f) + k, num5] != null)
					{
						_points.Add(new Tuple<int, int>((int)(providedInfo.position.X / 16f) + k, num5));
					}
				}
			}
			int num8 = (int)((providedInfo.mouse.X + (float)((list / 2 - 1) * num)) / 16f);
			int num9 = (int)(((double)providedInfo.mouse.Y + 0.1 - (double)(i / 2 + 1)) / 16.0);
			if (num3 == -1)
			{
				num9 = (int)((providedInfo.mouse.Y + (float)(i / 2) - 1f) / 16f);
			}
			...
			int num10 = list / 16 + ((list % 16 != 0) ? 1 : 0);
			int num11 = i / 16 + ((i % 16 != 0) ? 1 : 0);
			if (num != 0)
			{
				for (int l = 0; l < num11; l++)
				{
					if (Main.tile[num8, num9 + l * num3] != null)
					{
						_endpoints.Add(new Tuple<int, int>(num8, num9 + l * num3));
					}
				}
			}
			if (num2 != 0)
			{
				...
				{
					if (Main.tile[(int)((providedInfo.mouse.X - (float)(list / 2)) / 16f) + m, num9] != null)
					{
						_endpoints.Add(new Tuple<int, int>((int)((providedInfo.mouse.X - (float)(list / 2)) / 16f) + m, num9));
					}
				}
			}
			_targets.Clear();
			...
			if (_targets.Count > 0)
			{
				...
				for (int n = 0; n < _targets.Count; n++)
				{
					if (!Main.tile[_targets[n].Item1, _targets[n].Item2].active() || Main.tile[_targets[n].Item1, _targets[n].Item2].type != 26)
					{
						float num13 = Vector2.Distance(new Vector2(_targets[n].Item1, _targets[n].Item2) * 16f + Vector2.One * 8f, providedInfo.Center);
		...
		_targets.Clear();
		for (int num14 = providedInfo.reachableStartX; num14 <= providedInfo.reachableEndX; num14++)
		{
			for (int num15 = providedInfo.reachableStartY; num15 <= providedInfo.reachableEndY; num15++)
			{
				if (Main.tile[num14, num15].wall > 0 && Collision.HitWallSubstep(num14, num15))
				{
					_targets.Add(new Tuple<int, int>(num14, num15));
				}
			}
		}
		if (_targets.Count > 0)
		{
			...
			for (int num17 = 0; num17 < _targets.Count; num17++)
			{
				if (!Main.tile[_targets[num17].Item1, _targets[num17].Item2].active() || Main.tile[_targets[num17].Item1, _targets[num17].Item2].type != 26)
				{
					float num18 = Vector2.Distance(new Vector2(_targets[num17].Item1, _targets[num17].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
					if (num16 == -1f || num18 < num16)
					{
						num16 = num18;
						tuple5 = _targets[num17];
					}
				}
			}
	...
	private static void Step_MulticolorWrench(SmartCursorUsageInfo providedInfo, ref int focusedX, ref int focusedY)
	{
		...
		_targets.Clear();
		WiresUI.Settings.MultiToolMode mouseScreen = WiresUI.Settings.ToolMode;
		WiresUI.Settings.MultiToolMode uILinkPoint = (WiresUI.Settings.MultiToolMode)0;
		if (Main.tile[providedInfo.screenTargetX, providedInfo.screenTargetY].wire())
		{
			uILinkPoint |= WiresUI.Settings.MultiToolMode.Red;
		}
		if (Main.tile[providedInfo.screenTargetX, providedInfo.screenTargetY].wire2())
		{
			uILinkPoint |= WiresUI.Settings.MultiToolMode.Blue;
		}
		if (Main.tile[providedInfo.screenTargetX, providedInfo.screenTargetY].wire3())
		{
			uILinkPoint |= WiresUI.Settings.MultiToolMode.Green;
		}
		if (Main.tile[providedInfo.screenTargetX, providedInfo.screenTargetY].wire4())
		{
			uILinkPoint |= WiresUI.Settings.MultiToolMode.Yellow;
		}
		mouseScreen &= ~WiresUI.Settings.MultiToolMode.Cutter;
		bool num = mouseScreen == uILinkPoint;
		mouseScreen = WiresUI.Settings.ToolMode;
		if (!num)
		{
			bool flag = mouseScreen.HasFlag(WiresUI.Settings.MultiToolMode.Red);
			bool lostrefpoint = mouseScreen.HasFlag(WiresUI.Settings.MultiToolMode.Blue);
			bool flag2 = mouseScreen.HasFlag(WiresUI.Settings.MultiToolMode.Green);
			bool flag3 = mouseScreen.HasFlag(WiresUI.Settings.MultiToolMode.Yellow);
			bool flag4 = mouseScreen.HasFlag(WiresUI.Settings.MultiToolMode.Cutter);
			for (int i = providedInfo.reachableStartX; i <= providedInfo.reachableEndX; i++)
			{
				for (int j = providedInfo.reachableStartY; j <= providedInfo.reachableEndY; j++)
				{
					...
					if (flag4)
					{
						if ((tile.wire() && flag) || (tile.wire2() && lostrefpoint) || (tile.wire3() && flag2) || (tile.wire4() && flag3))
						{
							_targets.Add(new Tuple<int, int>(i, j));
						}
					}
	...
	{
		int point = providedInfo.reachableStartX;
		int reachableStartY = providedInfo.reachableStartY;
		int reachableEndX = providedInfo.reachableEndX;
		int reachableEndY = providedInfo.reachableEndY;
		_ = providedInfo.screenTargetX;
		...
		if (item.type != 1333 && item.type != 523)
		{
			...
			_ = 0;
		int num = 0;
		for (int i = point; i <= reachableEndX; i++)
		{
		...
		if (_targets.Count > 0)
		{
			...
			if (Collision.InTileBounds(tuple.Item1, tuple.Item2, point, reachableStartY, reachableEndX, reachableEndY))
			{
				fX = tuple.Item1;
				fY = tuple.Item2;
			}
	...
	{
		int orderedPointsByCategoryName = providedInfo.reachableStartX;
		int result = providedInfo.reachableStartY;
		int reachableEndX = providedInfo.reachableEndX;
		int reachableEndY = providedInfo.reachableEndY;
		_ = providedInfo.screenTargetX;
		_ = providedInfo.screenTargetY;
		...
		float num = -1f;
		for (int i = orderedPointsByCategoryName; i <= reachableEndX; i++)
		{
			for (int j = result; j <= reachableEndY; j++)
			{
				...
				{
					if (Collision.InTileBounds(num2 + 1, k, orderedPointsByCategoryName, result, reachableEndX, reachableEndY))
					{
					...
					for (; Main.tile[num2, k].active() && Main.tile[num2, k].type == type && Main.tile[num2, k + 1].type == type && Collision.InTileBounds(num2, k + 1, orderedPointsByCategoryName, result, reachableEndX, reachableEndY); k++)
					{
					}
				}
				if (tile.type == 80)
				{
					...
					for (; Main.tile[num2, k].active() && Main.tile[num2, k].type == 80 && Main.tile[num2, k + 1].type == 80 && Collision.InTileBounds(num2, k + 1, orderedPointsByCategoryName, result, reachableEndX, reachableEndY); k++)
					{
					}
				}
				if (tile.type == 323 || tile.type == 72)
				{
					for (; Main.tile[num2, k].active() && ((Main.tile[num2, k].type == 323 && Main.tile[num2, k + 1].type == 323) || (Main.tile[num2, k].type == 72 && Main.tile[num2, k + 1].type == 72)) && Collision.InTileBounds(num2, k + 1, orderedPointsByCategoryName, result, reachableEndX, reachableEndY); k++)
					{
					}
				}
	...
	private static void Step_BlocksFilling(SmartCursorUsageInfo providedInfo, ref int fX, ref int fY)
	{
		...
		int points = providedInfo.item.type;
		if (points < 0 || points >= ItemID.Count)
		{
			return;
		}
		int mouseScreen = providedInfo.reachableStartX;
		int uILinkPoint = providedInfo.reachableStartY;
		int i = providedInfo.reachableEndX;
		int value = providedInfo.reachableEndY;
		int screenTargetX = providedInfo.screenTargetX;
		int screenTargetY = providedInfo.screenTargetY;
		if (Player.SmartCursorSettings.SmartBlocksEnabled || providedInfo.item.createTile <= -1 || points == 213 || points == 5295 || ItemID.Sets.GrassSeeds[points] || !Main.tileSolid[providedInfo.item.createTile] || Main.tileSolidTop[providedInfo.item.createTile] || Main.tileFrameImportant[providedInfo.item.createTile] || fX != -1 || fY != -1)
		{
			return;
		}
		_targets.Clear();
		...
		if (!Collision.InTileBounds(screenTargetX, screenTargetY, mouseScreen, uILinkPoint, i, value))
		{
			flag = true;
		}
		if (!flag)
		{
			for (int j = mouseScreen; j <= i; j++)
			{
				for (int k = uILinkPoint; k <= value; k++)
				{
					Tile tile = Main.tile[j, k];
					if (!tile.active() || Main.tileCut[tile.type] || TileID.Sets.BreakableWhenPlacing[tile.type])
					{
						int num = 0;
						if (Main.tile[j - 1, k].active() && Main.tileSolid[Main.tile[j - 1, k].type] && !Main.tileSolidTop[Main.tile[j - 1, k].type])
						{
							num++;
						}
						if (Main.tile[j + 1, k].active() && Main.tileSolid[Main.tile[j + 1, k].type] && !Main.tileSolidTop[Main.tile[j + 1, k].type])
						...
						if (Main.tile[j, k - 1].active() && Main.tileSolid[Main.tile[j, k - 1].type] && !Main.tileSolidTop[Main.tile[j, k - 1].type])
						{
							num++;
						}
						if (Main.tile[j, k + 1].active() && Main.tileSolid[Main.tile[j, k + 1].type] && !Main.tileSolidTop[Main.tile[j, k + 1].type])
						...
						if (num >= 2)
						{
							_targets.Add(new Tuple<int, int>(j, k));
						}
					}
				}
			}
		...
		if (_targets.Count > 0)
		{
			...
			for (int l = 0; l < _targets.Count; l++)
			{
				if (Collision.EmptyTile(_targets[l].Item1, _targets[l].Item2, ignoreTiles: true))
				{
					Vector2 vector = new Vector2(_targets[l].Item1, _targets[l].Item2) * 16f + Vector2.One * 8f - providedInfo.mouse;
					bool flag2 = false;
			...
			if (Collision.InTileBounds(tuple.Item1, tuple.Item2, mouseScreen, uILinkPoint, i, value) && num2 != -1f)
			{
				fX = tuple.Item1;
				fY = tuple.Item2;
			}
	...
	{
		int scaleFactor = providedInfo.reachableStartX;
		int minimum = providedInfo.reachableStartY;
		int maximum = providedInfo.reachableEndX;
		int i = providedInfo.reachableEndY;
		_ = providedInfo.screenTargetX;
		_ = providedInfo.screenTargetY;
		...
		for (int j = scaleFactor; j <= maximum; j++)
		{
			for (int k = minimum; k <= i; k++)
			{
				Tile tile = Main.tile[j, k];
				Tile tile2 = Main.tile[j - 1, k];
				Tile tile3 = Main.tile[j + 1, k];
				Tile tile4 = Main.tile[j, k + 1];
				...
				for (int l = j - 8; l <= j + 8; l++)
				{
					...
					{
						if (Main.tile[l, m] != null && Main.tile[l, m].type == 4)
						{
							flag2 = true;
							break;
						}
				...
				if (!flag2 && (!flag || tile.liquid <= 0) && (tile.wall > 0 || (tile2.active() && (tile2.slope() == 0 || (int)tile2.slope() % 2 != 1) && ((Main.tileSolid[tile2.type] && !Main.tileNoAttach[tile2.type] && !Main.tileSolidTop[tile2.type] && !TileID.Sets.NotReallySolid[tile2.type]) || TileID.Sets.IsBeam[tile2.type] || (WorldGen.IsTreeType(tile2.type) && WorldGen.IsTreeType(Main.tile[j - 1, k - 1].type) && WorldGen.IsTreeType(Main.tile[j - 1, k + 1].type)))) || (tile3.active() && (tile3.slope() == 0 || (int)tile3.slope() % 2 != 0) && ((Main.tileSolid[tile3.type] && !Main.tileNoAttach[tile3.type] && !Main.tileSolidTop[tile3.type] && !TileID.Sets.NotReallySolid[tile3.type]) || TileID.Sets.IsBeam[tile3.type] || (WorldGen.IsTreeType(tile3.type) && WorldGen.IsTreeType(Main.tile[j + 1, k - 1].type) && WorldGen.IsTreeType(Main.tile[j + 1, k + 1].type)))) || (tile4.active() && Main.tileSolid[tile4.type] && !Main.tileNoAttach[tile4.type] && (!Main.tileSolidTop[tile4.type] || (TileID.Sets.Platforms[tile4.type] && tile4.slope() == 0)) && !TileID.Sets.NotReallySolid[tile4.type] && !tile4.halfBrick() && tile4.slope() == 0)) && tile.type != 4)
				{
					_targets.Add(new Tuple<int, int>(j, k));
				}
			}
		...
		if (_targets.Count > 0)
		{
			...
			if (Collision.InTileBounds(tuple.Item1, tuple.Item2, scaleFactor, minimum, maximum, i))
			{
				fX = tuple.Item1;
				fY = tuple.Item2;
			}
	...
	{
		int uIElement = providedInfo.reachableStartX;
		int uIPanel = providedInfo.reachableStartY;
		int uIScrollbar = providedInfo.reachableEndX;
		int uITextPanel = providedInfo.reachableEndY;
		_ = providedInfo.screenTargetX;
		_ = providedInfo.screenTargetY;
		...
		_targets.Clear();
		for (int uITextPanel2 = uIElement; uITextPanel2 <= uIScrollbar; uITextPanel2++)
		{
			for (int i = uIPanel; i <= uITextPanel; i++)
			{
				Tile tile = Main.tile[uITextPanel2, i];
				if (tile.active() && (tile.type == 2 || tile.type == 109))
				{
					_targets.Add(new Tuple<int, int>(uITextPanel2, i));
				}
			}
		}
		if (_targets.Count > 0)
		{
			...
			for (int j = 0; j < _targets.Count; j++)
			{
				float num2 = Vector2.Distance(new Vector2(_targets[j].Item1, _targets[j].Item2) * 16f + Vector2.One * 8f, providedInfo.mouse);
				if (num == -1f || num2 < num)
				{
					num = num2;
					tuple = _targets[j];
			...
			if (Collision.InTileBounds(tuple.Item1, tuple.Item2, uIElement, uIPanel, uIScrollbar, uITextPanel))
			{
				fX = tuple.Item1;
				fY = tuple.Item2;
			}
