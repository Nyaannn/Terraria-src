...
public class TeleportPylonsSystem : IOnPlayerJoining
{
	...
	{
		Rectangle name = new Rectangle(centerPoint.X - Main.buffScanAreaWidth / 2, centerPoint.Y - Main.buffScanAreaHeight / 2, Main.buffScanAreaWidth, Main.buffScanAreaHeight);
		int list = necessaryNPCCount;
		for (int num = 0; num < 200; num++)
		{
			NPC nPC = Main.npc[num];
			if (!nPC.active || !nPC.isLikeATownNPC || nPC.homeless || !name.Contains(nPC.homeTileX, nPC.homeTileY))
			{
				continue;
			}
	...
	{
		foreach (TeleportPylonInfo key in _pylons)
		{
			NetManager.Instance.SendToClient(NetTeleportPylonModule.SerializePylonWasAddedOrRemoved(key, NetTeleportPylonModule.SubPacketType.PylonWasAdded), playerIndex);
		}
	}
