...
public struct PlayerSleepingHelper
{
	...
	public void UpdateState(Player player)
	{
		...
		Point mainCategory = (player.Bottom + new Vector2(0f, -2f)).ToTileCoordinates();
		if (!GetSleepingTargetInfo(mainCategory.X, mainCategory.Y, out var list, out var _, out var request))
		{
			StopSleeping(player);
			return;
		}
		if (player.controlLeft || player.controlRight || player.controlUp || player.controlDown || player.controlJump || player.pulley || player.mount.Active || list != player.direction)
		{
			StopSleeping(player);
		}
		bool groupOptionButton = false;
		if (player.itemAnimation > 0)
		{
			Item groupOptionButton2 = player.HeldItem;
			if (groupOptionButton2.damage > 0 && !groupOptionButton2.noMelee)
			{
				groupOptionButton = true;
			}
			if (groupOptionButton2.fishingPole > 0)
			{
				groupOptionButton = true;
			}
			bool? groupOptionButton3 = ItemID.Sets.ForcesBreaksSleeping[groupOptionButton2.type];
			if (groupOptionButton3.HasValue)
			{
				groupOptionButton = groupOptionButton3.Value;
			}
		}
		...
		if (Main.sleepingManager.GetNextPlayerStackIndexInCoords(mainCategory) >= 2)
		{
			StopSleeping(player);
		}
		if (isSleeping)
		{
			visualOffsetOfBedBase = request;
			Main.sleepingManager.AddPlayerAndGetItsStackedIndexInCoords(player.whoAmI, mainCategory, out sleepingIndex);
		}
	}
