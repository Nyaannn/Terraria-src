...
public class PortalHelper
{
	...
	{
		Vector2 num = velocity / velocity.Length();
		Point position = FindCollision(theBolt.position, theBolt.position + velocity + num * 32f).ToTileCoordinates();
		Tile item = Main.tile[position.X, position.Y];
		Vector2 vector = new Vector2(position.X * 16 + 8, position.Y * 16 + 8);
		if (!WorldGen.SolidOrSlopedTile(item))
		{
			return -1;
		}
		int num2 = item.slope();
		bool flag = item.halfBrick();
		for (int i = 0; i < (flag ? 2 : EDGES.Length); i++)
		{
			if (Vector2.Dot(EDGES[i], num) > 0f && FindValidLine(position, (int)EDGES[i].Y, (int)(0f - EDGES[i].X), out var bestPosition))
			{
				vector = new Vector2(bestPosition.X * 16 + 8, bestPosition.Y * 16 + 8);
				return AddPortal(theBolt, vector - EDGES[i] * (flag ? 0f : 8f), (float)Math.Atan2(EDGES[i].Y, EDGES[i].X) + (float)Math.PI / 2f, (int)theBolt.ai[0], theBolt.direction);
			}
		}
		if (num2 != 0)
		{
			Vector2 value = SLOPE_EDGES[num2 - 1];
			if (Vector2.Dot(value, -num) > 0f && FindValidLine(position, -SLOPE_OFFSETS[num2 - 1].Y, SLOPE_OFFSETS[num2 - 1].X, out var bestPosition2))
			{
				vector = new Vector2(bestPosition2.X * 16 + 8, bestPosition2.Y * 16 + 8);
				return AddPortal(theBolt, vector, (float)Math.Atan2(value.Y, value.X) - (float)Math.PI / 2f, (int)theBolt.ai[0], theBolt.direction);
			}
		}
		return -1;
	}
	...
	{
		Tile num = Main.tile[position.X, position.Y];
		Tile num2 = Main.tile[position.X - xOffset, position.Y - yOffset];
		Tile uILinkPoint = Main.tile[position.X + xOffset, position.Y + yOffset];
		...
		if (CanPlacePortalOn(num) && CanPlacePortalOn(num2) && CanPlacePortalOn(uILinkPoint) && num2.HasSameSlope(num) && uILinkPoint.HasSameSlope(num))
		{
			return true;
		}
		return false;
	...
	public static Color GetPortalColor(int player, int portal)
	{
		...
		{
			float listOfSubscribedWorldPath = 0.08f;
			white = Main.hslToRgb((0.5f + (float)player * (listOfSubscribedWorldPath * 2f) + (float)portal * listOfSubscribedWorldPath) % 1f, 1f, 0.5f);
		}
		white.A = 66;
		return white;
	}
	...
	{
		Vector2 num = angle.ToRotationVector2();
		start = position + num * -22f;
		end = position + num * 22f;
	}
	...
	public static void SyncPortalsOnPlayerJoin(int plr, int fluff, List<Point> dontInclude, out List<Point> portalSections)
	{
		...
		{
			Projectile key = Main.projectile[num];
			if (!key.active || (key.type != 602 && key.type != 601))
			{
				continue;
			}
			Vector2 center = key.Center;
			int uILinkPoint = Netplay.GetSectionX((int)(center.X / 16f));
			int scaleFactor = Netplay.GetSectionY((int)(center.Y / 16f));
			for (int minimum = uILinkPoint - fluff; minimum < uILinkPoint + fluff + 1; minimum++)
			{
				for (int maximum = scaleFactor - fluff; maximum < scaleFactor + fluff + 1; maximum++)
				{
					if (minimum >= 0 && minimum < Main.maxSectionsX && maximum >= 0 && maximum < Main.maxSectionsY && !Netplay.Clients[plr].TileSections[minimum, maximum] && !dontInclude.Contains(new Point(minimum, maximum)))
					{
						portalSections.Add(new Point(minimum, maximum));
					}
				}
			}
		}
	...
	private static bool SupportedSlope(int x, int y, int slope)
	{
		...
		if (num != null && num.nactive() && !Main.tileCut[num.type] && !TileID.Sets.BreakableWhenPlacing[num.type] && Main.tileSolid[num.type] && num.slope() == slope)
		{
			return DoesTileTypeSupportPortals(num.type);
		}
		return false;
	}
	...
	{
		Tile uIElement = Main.tile[x, y];
		if (uIElement != null && uIElement.nactive() && !Main.tileCut[uIElement.type] && !TileID.Sets.BreakableWhenPlacing[uIElement.type] && Main.tileSolid[uIElement.type] && uIElement.halfBrick())
		{
			return DoesTileTypeSupportPortals(uIElement.type);
		}
		return false;
	}
	...
	{
		Tile element = Main.tile[x, y];
		if (element != null && element.nactive() && !Main.tileCut[element.type] && !TileID.Sets.BreakableWhenPlacing[element.type] && Main.tileSolid[element.type] && !TileID.Sets.NotReallySolid[element.type] && !element.halfBrick() && element.slope() == 0)
		{
			return DoesTileTypeSupportPortals(element.type);
		}
		return false;
	}
}
