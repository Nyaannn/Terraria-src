...
public class ShopHelper
{
	...
	private float GetTravelingMerchantPrices(NPC npc)
	{
		...
		Vector2 timeCategory = new Vector2(Main.spawnTileX, Main.spawnTileY);
		float list = Vector2.Distance(value, timeCategory) / (float)(Main.maxTilesX / 2);
		list = 1.5f - list;
		return (2f + list) / 3f;
	}
	...
	private void ProcessMood(Player player, NPC npc)
	{
		...
		else
		{
			...
			int personalCategory;
			int list;
			List<NPC> creativePowerUIElementRequestInfo = GetNearbyResidentNPCs(npc, out personalCategory, out list);
			bool request = true;
			float groupOptionButton = 1.05f;
			if (npc.type == 663)
			{
				request = false;
				groupOptionButton = 1f;
				if (personalCategory < 2 && list < 2)
				{
					AddHappinessReportText("HateLonely");
					_currentPriceAdjustment = 1000f;
				}
			}
			if (true && personalCategory > 3)
			{
				for (int i = 3; i < personalCategory; i++)
				{
					_currentPriceAdjustment *= groupOptionButton;
				}
				if (personalCategory > 6)
				{
			...
			helperInfo.npc = npc;
			helperInfo.NearbyNPCs = creativePowerUIElementRequestInfo;
	...
	{
		string groupOptionButton = "TownNPCMood_" + NPCID.Search.GetName(_currentNPCBeingTalkedTo.netID);
		if (_currentNPCBeingTalkedTo.type == 633 && _currentNPCBeingTalkedTo.altTexture == 2)
		{
			groupOptionButton += "Transformed";
		}
		string textValueWith = Language.GetTextValueWith(groupOptionButton + "." + textKeyInCategory, substitutes);
