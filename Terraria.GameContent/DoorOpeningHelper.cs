...
public class DoorOpeningHelper
{
	...
	private class TallGateOpeningInfoProvider : DoorAutoHandler
	{
		public DoorOpenCloseTogglingInfo ProvideInfo(Point tileCoords)
		{
			Tile num = Main.tile[tileCoords.X, tileCoords.Y];
			Point i = tileCoords;
			i.Y -= num.frameY % 90 / 18;
			DoorOpenCloseTogglingInfo bitsByte = default(DoorOpenCloseTogglingInfo);
			bitsByte.handler = this;
			bitsByte.tileCoordsForToggling = i;
			return bitsByte;
		}

		public bool TryOpenDoor(DoorOpenCloseTogglingInfo doorInfo, PlayerInfoForOpeningDoors playerInfo)
		{
			Point packet = doorInfo.tileCoordsForToggling;
			...
			bool flag = false;
			if (WorldGen.ShiftTallGate(packet.X, packet.Y, flag))
			{
				NetMessage.SendData(19, -1, -1, null, 4 + flag.ToInt(), packet.X, packet.Y);
				return true;
			}
			return false;
		}
	...
	private bool TryGetHandler(Point tileCoords, out DoorAutoHandler infoProvider)
	{
		...
		Tile completionPercentText = Main.tile[tileCoords.X, tileCoords.Y];
		if (completionPercentText == null)
		{
			return false;
		}
		if (!_handlerByTileType.TryGetValue(completionPercentText.type, out infoProvider))
		{
			return false;
		}
		return true;
