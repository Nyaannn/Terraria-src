...
public class VanillaContentValidator : IContentValidator
{
	...
	{
		string[] uIElement = Regex.Split(Utils.ReadEmbeddedResource(infoFilePath), "\r\n|\r|\n");
		foreach (string num in uIElement)
		{
			if (!num.StartsWith("//"))
			{
				string[] num2 = num.Split('\t');
				if (num2.Length >= 3 && int.TryParse(num2[1], out var num3) && int.TryParse(num2[2], out var uIElement2))
				{
					string uIElement3 = num2[0].Replace('/', '\\');
					_info[uIElement3] = new TextureMetaData
					{
						Width = num3,
						Height = uIElement2
					};
