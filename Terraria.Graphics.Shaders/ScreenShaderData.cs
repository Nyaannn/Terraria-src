...
public class ScreenShaderData : ShaderData
{
	...
	{
		Vector2 i = new Vector2(Main.offScreenRange, Main.offScreenRange);
		Vector2 value = new Vector2(Main.screenWidth, Main.screenHeight) / Main.GameViewMatrix.Zoom;
		Vector2 vector = new Vector2(Main.screenWidth, Main.screenHeight) * 0.5f;
		Vector2 vector2 = Main.screenPosition + vector * (Vector2.One - Vector2.One / Main.GameViewMatrix.Zoom);
		...
		base.Shader.Parameters["uScreenPosition"].SetValue(vector2 - i);
		base.Shader.Parameters["uTargetPosition"].SetValue(_uTargetPosition - i);
		...
		for (int j = 0; j < _uAssetImages.Length; j++)
		{
			Texture2D texture2D = _uCustomImages[j];
			if (_uAssetImages[j] != null && _uAssetImages[j].get_IsLoaded())
			{
				texture2D = _uAssetImages[j].get_Value();
			}
			if (texture2D != null)
			{
				Main.graphics.GraphicsDevice.Textures[j + 1] = texture2D;
				int width = texture2D.Width;
				int height = texture2D.Height;
				if (_samplerStates[j] != null)
				{
					Main.graphics.GraphicsDevice.SamplerStates[j + 1] = _samplerStates[j];
				}
				else if (Utils.IsPowerOfTwo(width) && Utils.IsPowerOfTwo(height))
				{
					Main.graphics.GraphicsDevice.SamplerStates[j + 1] = SamplerState.LinearWrap;
				}
				else
				{
					Main.graphics.GraphicsDevice.SamplerStates[j + 1] = SamplerState.AnisotropicClamp;
				}
				base.Shader.Parameters["uImageSize" + (j + 1)].SetValue(new Vector2(width, height) * _imageScales[j]);
			}
		}
		base.Apply();
	}
