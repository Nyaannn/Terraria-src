...
public class UISearchBar : UIElement
{
	...
	public void TrimDisplayIfOverElementDimensions(int padding)
	{
		...
		{
			Point item = new Point((int)dimensions.X, (int)dimensions.Y);
			Point point = new Point(item.X + (int)dimensions.Width, item.Y + (int)dimensions.Height);
			Rectangle rectangle = new Rectangle(item.X, item.Y, point.X - item.X, point.Y - item.Y);
			CalculatedStyle dimensions2 = _text.GetDimensions();
			Point point2 = new Point((int)dimensions2.X, (int)dimensions2.Y);
			Point point3 = new Point(point2.X + (int)_text.MinWidth.Pixels, point2.Y + (int)_text.MinHeight.Pixels);
			Rectangle rectangle2 = new Rectangle(point2.X, point2.Y, point3.X - point2.X, point3.Y - point2.Y);
			int num = 0;
			while (rectangle2.Right > rectangle.Right - padding && _text.Text.Length > 0)
			{
				...
				point2 = new Point((int)dimensions2.X, (int)dimensions2.Y);
				point3 = new Point(point2.X + (int)_text.MinWidth.Pixels, point2.Y + (int)_text.MinHeight.Pixels);
				rectangle2 = new Rectangle(point2.X, point2.Y, point3.X - point2.X, point3.Y - point2.Y);
				actualContents = _text.Text;
			}
		}
	}
