...
public class UIColoredSlider : UISliderBase
{
	...
	protected override void DrawSelf(SpriteBatch spriteBatch)
	{
		...
		int item = GetUsageLevel();
		float j = 8f;
		base.DrawSelf(spriteBatch);
		CalculatedStyle colorArmorBody = GetDimensions();
		float num = colorArmorBody.Width + 1f;
		Vector2 vector = new Vector2(colorArmorBody.X, colorArmorBody.Y);
		bool value = false;
		...
		Color value2 = (value ? Color.Gold : (flag ? Color.White : Color.Silver));
		value2 = Color.Lerp(value2, Color.White, flag ? 0.5f : 0f);
		Vector2 vector3 = new Vector2(0.8f);
		vector2.X += 8f;
		vector2.Y += j;
		vector2.X -= 17f;
		TextureAssets.ColorBar.Frame();
		vector2 = new Vector2(colorArmorBody.X + colorArmorBody.Width - 10f, colorArmorBody.Y + 10f + j);
		bool i;
		float obj = DrawValueBar(spriteBatch, vector2, 1f, _getStatusTextAct(), item, out i, _blipFunc);
		if (UISliderBase.CurrentLockedSlider == this || i)
		{
