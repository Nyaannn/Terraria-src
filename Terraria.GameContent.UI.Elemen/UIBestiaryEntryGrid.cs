...
public class UIBestiaryEntryGrid : UIElement
{
	...
	public void FillBestiarySpaceWithEntries()
	{
		RemoveAllChildren();
		UpdateEntries();
		GetEntriesToShow(out var cachedBuffer, out var maxEntriesHeight, out var maxEntriesToHave);
		...
		for (int j = 0; j < maxEntriesHeight; j++)
		{
			for (int k = 0; k < cachedBuffer; k++)
			{
				...
				uIElement.Left.Set(0f, (float)k / (float)cachedBuffer - 0.5f + num3);
