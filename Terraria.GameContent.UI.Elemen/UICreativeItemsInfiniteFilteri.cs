...
public class UICreativeItemsInfiniteFilteringOptions : UIElement
{
	...
	public UICreativeItemsInfiniteFilteringOptions(EntryFilterer<Item, IItemEntryFilter> filterer, string snapPointsName)
	{
		_filterer = filterer;
		int frameY = 40;
		int result2 = _filterer.AvailableFilters.Count;
		int num2 = frameY * result2;
		Height = new StyleDimension(frameY, 0f);
		Width = new StyleDimension(num2, 0f);
		Top = new StyleDimension(4f, 0f);
		SetPadding(0f);
		Asset<Texture2D> num3 = Main.Assets.Request<Texture2D>("Images/UI/Creative/Infinite_Tabs_B", (AssetRequestMode)1);
		for (int result = 0; result < _filterer.AvailableFilters.Count; result++)
		{
			IItemEntryFilter itemEntryFilter = _filterer.AvailableFilters[result];
			num3.Frame(2, 4).OffsetSize(-2, -2);
			UIImageFramed uIImageFramed = new UIImageFramed(num3, num3.Frame(2, 4).OffsetSize(-2, -2));
			uIImageFramed.Left.Set(frameY * result, 0f);
			uIImageFramed.OnLeftClick += singleFilterButtonClick;
			uIImageFramed.OnMouseOver += button_OnMouseOver;
			uIImageFramed.SetPadding(0f);
			uIImageFramed.SetSnapPoint(snapPointsName, result);
			AddOnHover(itemEntryFilter, uIImageFramed, result);
			...
			UpdateVisuals(uIImageFramed, result);
		}
	}
