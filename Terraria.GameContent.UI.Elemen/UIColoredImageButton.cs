...
public class UIColoredImageButton : UIElement
{
	...
	{
		CalculatedStyle drawData = GetDimensions();
		Vector2 item = drawData.Position() + new Vector2(drawData.Width, drawData.Height) / 2f;
		spriteBatch.Draw(_backPanelTexture.get_Value(), item, null, Color.White * (base.IsMouseHovering ? _visibilityActive : _visibilityInactive), 0f, _backPanelTexture.Size() / 2f, 1f, SpriteEffects.None, 0f);
		_ = Color.White;
		if (_hovered)
		{
			spriteBatch.Draw(_backPanelBorderTexture.get_Value(), item, null, Color.White, 0f, _backPanelBorderTexture.Size() / 2f, 1f, SpriteEffects.None, 0f);
		}
		if (_selected)
		{
			spriteBatch.Draw(_backPanelHighlightTexture.get_Value(), item, null, Color.White, 0f, _backPanelHighlightTexture.Size() / 2f, 1f, SpriteEffects.None, 0f);
		}
		if (_middleTexture != null)
		{
			spriteBatch.Draw(_middleTexture.get_Value(), item, null, Color.White, 0f, _middleTexture.Size() / 2f, 1f, SpriteEffects.None, 0f);
		}
		spriteBatch.Draw(_texture.get_Value(), item, null, _color, 0f, _texture.Size() / 2f, 1f, SpriteEffects.None, 0f);
	}
