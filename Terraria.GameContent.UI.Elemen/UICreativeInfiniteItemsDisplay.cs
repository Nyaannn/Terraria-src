...
public class UICreativeInfiniteItemsDisplay : UIElement
{
	...
	{
		UIPanel i = CreateBasicPanel();
		i.VAlign = 0.5f;
		i.Height = new StyleDimension(170f, 0f);
		i.Width = new StyleDimension(170f, 0f);
		i.Top = default(StyleDimension);
		totalContainer.Append(i);
		i.OnUpdate += Hover_OnUpdate;
		i.OnMouseOver += Hover_OnMouseOver;
		i.OnMouseOut += Hover_OnMouseOut;
		AddCogsForSacrificeMenu(i);
		...
		uIElement.Left = new StyleDimension(j, 0f);
		...
		uIPanel.SetSnapPoint("CreativeSacrificeConfirm", 0);
		uIPanel.OnLeftClick += sacrificeButton_OnClick;
		uIPanel.OnMouseOver += FadedMouseOver;
		uIPanel.OnMouseOut += FadedMouseOut;
		uIPanel.OnUpdate += research_OnUpdate;
		i.Append(uIPanel);
		i.OnUpdate += sacrificeWindow_OnUpdate;
	}
	...
	private void UpdateVisualFrame()
	{
		...
		if (_sacrificeAnimationTimeLeft != 0)
		{
			...
			if (_sacrificeAnimationTimeLeft == 56)
			{
				...
				Vector2 accelerationPerFrame = new Vector2(0f, 0.163500011f);
				for (int i = 0; i < 15; i++)
				{
	...
	{
		Main.CreativeMenu.GetSacrificeNumbers(out var itemIdChecked, out var num, out var amountNeededTotal);
		...
		case CreativeUI.ItemSacrificeResult.SacrificedAndDone:
			...
			RememberItemSacrifice(itemIdChecked, num + amountWeSacrificed, amountNeededTotal);
			break;
	...
	{
		RemoveAllChildren();
		CreativeUnlocksTracker num = Main.LocalPlayerCreativeTracker;
		if (_lastTrackerCheckedForEdits != num)
		{
			_lastTrackerCheckedForEdits = num;
			_lastCheckedVersionForEdits = -1;
		}
		int lastEditId = num.ItemSacrifices.LastEditId;
	...
	{
		UserInterface i = UserInterface.ActiveInstance;
		if (Main.gameMenu)
		{
			return Main.MenuUI;
		}
