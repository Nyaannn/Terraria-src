...
public class UIWorkshopPublishWorldListItem : AWorldListItem
{
	...
	protected override void DrawSelf(SpriteBatch spriteBatch)
	{
		...
		CalculatedStyle item = GetInnerDimensions();
		...
		Utils.DrawBorderString(spriteBatch, worldName, new Vector2(num + 6f, item.Y + 3f), color);
		float num2 = (item.Width - 22f - dimensions.Width - _publishButton.GetDimensions().Width) / 2f;
		float height = _publishButton.GetDimensions().Height;
		Vector2 vector = new Vector2(num + 6f, item.Y + item.Height - height);
		float num3 = num2;
		DrawPanel(spriteBatch, vector, num3, height);
		string expertText = "";
		Color gameModeColor = Color.White;
		GetDifficulty(out expertText, out gameModeColor);
		Vector2 vector2 = FontAssets.MouseText.get_Value().MeasureString(expertText);
		float x = vector2.X;
		float y = vector2.Y;
