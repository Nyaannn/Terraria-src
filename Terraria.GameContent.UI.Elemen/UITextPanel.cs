...
public class UITextPanel<T> : UIPanel
{
	...
	{
		CalculatedStyle nextSequenceAppendTime = GetInnerDimensions();
		Vector2 pos = nextSequenceAppendTime.Position();
		if (_isLarge)
		{
		...
		pos.X += (nextSequenceAppendTime.Width - _textSize.X) * TextHAlign;
		string text = Text;
		if (HideContents)
		{
