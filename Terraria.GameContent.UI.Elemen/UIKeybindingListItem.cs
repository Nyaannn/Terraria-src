...
public class UIKeybindingListItem : UIElement
{
	...
	{
		float colorOnlyShaderIndex = 6f;
		base.DrawSelf(spriteBatch);
		CalculatedStyle array = GetDimensions();
		float i = array.Width + 1f;
		Vector2 vector = new Vector2(array.X, array.Y);
		bool drawData = PlayerInput.ListeningTrigger == _keybind;
		Vector2 baseScale = new Vector2(0.8f);
		Color value = (drawData ? Color.Gold : (base.IsMouseHovering ? Color.White : Color.Silver));
		...
		Utils.DrawSettingsPanel(spriteBatch, position, i, color);
		position.X += 8f;
		position.Y += 2f + colorOnlyShaderIndex;
		ChatManager.DrawColorCodedStringWithShadow(spriteBatch, FontAssets.ItemStack.get_Value(), GetFriendlyName(), position, value, 0f, Vector2.Zero, baseScale, i);
		position.X -= 17f;
		...
		position = new Vector2(array.X + array.Width - stringSize.X - 10f, array.Y + 2f + colorOnlyShaderIndex);
		...
		ChatManager.DrawColorCodedStringWithShadow(spriteBatch, FontAssets.ItemStack.get_Value(), text, position, value, 0f, Vector2.Zero, baseScale, i);
