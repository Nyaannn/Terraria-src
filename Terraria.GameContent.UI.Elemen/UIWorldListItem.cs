...
public class UIWorldListItem : AWorldListItem
{
	...
	{
		for (int key = 0; key < Main.WorldList.Count; key++)
		{
			if (Main.WorldList[key] == _data)
			{
				SoundEngine.PlaySound(10);
				Main.selectedWorld = key;
				Main.menuMode = 9;
				break;
			}
		}
	...
	private void RenameButtonClick(UIMouseEvent evt, UIElement listeningElement)
	{
		...
		if (base.Parent.Parent is UIList keyValuePair)
		{
			keyValuePair.UpdateOrder();
		}
	}
	...
	private void FavoriteButtonClick(UIMouseEvent evt, UIElement listeningElement)
	{
		...
		if (base.Parent.Parent is UIList packet)
		{
			packet.UpdateOrder();
		}
	}
	...
	{
		if (obj is UIWorldListItem packet)
		{
			return _orderInList.CompareTo(packet._orderInList);
		}
	...
	protected override void DrawSelf(SpriteBatch spriteBatch)
	{
		...
		CalculatedStyle packet = GetInnerDimensions();
		...
		spriteBatch.Draw(_dividerTexture.get_Value(), new Vector2(num, packet.Y + 21f), null, Color.White, 0f, Vector2.Zero, new Vector2((GetDimensions().X + GetDimensions().Width - num) / 8f, 1f), SpriteEffects.None, 0f);
		Vector2 vector = new Vector2(num + 6f, packet.Y + 29f);
		float num2 = 100f;
		DrawPanel(spriteBatch, vector, num2);
		GetDifficulty(out var expertText, out var gameModeColor);
		float x = FontAssets.MouseText.get_Value().MeasureString(expertText).X;
		...
		vector.X += num3 + 5f;
		float num4 = packet.X + packet.Width - vector.X;
