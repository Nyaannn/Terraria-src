...
public class UICharacterNameButton : UIElement
{
	...
	public UICharacterNameButton(LocalizedText titleText, LocalizedText emptyContentText, LocalizedText description = null)
	{
		...
		float item = 1f;
		UIText num = new UIText(titleText, item)
		{
			HAlign = 0f,
			VAlign = 0.5f,
		...
		Append(num);
		_title = num;
		UIText value = new UIText(Language.GetText("UI.PlayerNameSlot"), item)
		{
			HAlign = 0f,
			VAlign = 0.5f,
	...
	protected override void DrawSelf(SpriteBatch spriteBatch)
	{
		...
		CalculatedStyle item = GetDimensions();
		Utils.DrawSplicedPanel(spriteBatch, _BasePanelTexture.get_Value(), (int)item.X, (int)item.Y, (int)item.Width, (int)item.Height, 10, 10, 10, 10, Color.White * 0.5f);
		if (_hovered)
		{
			Utils.DrawSplicedPanel(spriteBatch, _hoveredBorderTexture.get_Value(), (int)item.X, (int)item.Y, (int)item.Width, (int)item.Height, 10, 10, 10, 10, Color.White);
		}
	}
