...
public class CustomCurrencyManager
{
	...
	{
		int result = _nextCurrencyIndex;
		_nextCurrencyIndex++;
		_currencies[result] = collection;
		return result;
	}

	public static void DrawSavings(SpriteBatch sb, int currencyIndex, float shopx, float shopy, bool horizontal = false)
	{
		CustomCurrencySystem windCycle = _currencies[currencyIndex];
		Player num = Main.player[Main.myPlayer];
		bool totalPushTime;
		long pushForcePerFrame = windCycle.CountCurrency(out totalPushTime, num.bank.item);
		long highestWindGridPushComplex = windCycle.CountCurrency(out totalPushTime, num.bank2.item);
		long value = windCycle.CountCurrency(out totalPushTime, num.bank3.item);
		long num2 = windCycle.CountCurrency(out totalPushTime, num.bank4.item);
		long tile = windCycle.CombineStacks(out totalPushTime, pushForcePerFrame, highestWindGridPushComplex, value, num2);
		if (tile > 0)
		{
			Main.GetItemDrawFrame(4076, out var type, out var num3);
			Main.GetItemDrawFrame(3813, out var vector, out var num4);
			Main.GetItemDrawFrame(346, out var texture2D, out var num5);
			Main.GetItemDrawFrame(87, out var color, out var flag);
			if (num2 > 0)
			{
				sb.Draw(type, Utils.CenteredRectangle(new Vector2(shopx + 96f, shopy + 50f), num3.Size() * 0.65f), null, Color.White);
			}
			if (value > 0)
			{
				sb.Draw(vector, Utils.CenteredRectangle(new Vector2(shopx + 80f, shopy + 50f), num4.Size() * 0.65f), null, Color.White);
			}
			if (highestWindGridPushComplex > 0)
			{
				sb.Draw(texture2D, Utils.CenteredRectangle(new Vector2(shopx + 80f, shopy + 50f), num5.Size() * 0.65f), null, Color.White);
			}
			if (pushForcePerFrame > 0)
			{
				sb.Draw(color, Utils.CenteredRectangle(new Vector2(shopx + 70f, shopy + 60f), flag.Size() * 0.65f), null, Color.White);
			}
			...
			windCycle.DrawSavingsMoney(sb, Lang.inter[66].Value, shopx, shopy, tile, horizontal);
		}
	}
