...
public class EmoteBubble
{
	...
	public static void UpdateAll()
	{
		...
		{
			toClean.Clear();
			foreach (KeyValuePair<int, EmoteBubble> value in byID)
			{
				value.Value.Update();
				if (value.Value.lifeTime <= 0)
				{
					toClean.Add(value.Key);
				}
			}
	...
	private void Draw(SpriteBatch sb)
	{
		...
		SpriteEffects item = SpriteEffects.None;
		Vector2 position = GetPosition(out item);
		...
		{
			origin.Y = 0f;
			item |= SpriteEffects.FlipVertically;
			position = Main.ReverseGravitySupport(position);
		}
		sb.Draw(value, position, value2, Color.White, 0f, origin, 1f, item, 0f);
		if (flag)
		{
			return;
		}
		if (emote >= 0)
		{
			if ((emote == 87 || emote == 89) && item.HasFlag(SpriteEffects.FlipHorizontally))
			{
				item &= ~SpriteEffects.FlipHorizontally;
				position.X += 4f;
			}
			sb.Draw(value, position, value.Frame(8, EMOTE_SHEET_VERTICAL_FRAMES, emote * 2 % 8 + frame, 1 + emote / 4), Color.White, 0f, origin, 1f, item, 0f);
		}
		else if (emote == -1)
		{
			...
			sb.Draw(value, position + new Vector2(item.HasFlag(SpriteEffects.FlipHorizontally) ? 1 : (-1), -value2.Height + 3), null, Color.White, 0f, new Vector2(value.Width / 2, 0f), num, item, 0f);
		}
	}
	...
	{
		EmoteBubble key = byID[bubbleID];
		if (key.anchor.type != 0 || !(key.anchor.entity is Player value))
		{
			return;
		}
		foreach (EmoteBubble value2 in byID.Values)
		{
			if (value2.anchor.type == WorldUIAnchor.AnchorType.Entity && value2.anchor.entity == value && value2.ID != bubbleID)
			{
				value2.lifeTime = 6;
			}
		}
	...
	public void PickNPCEmote(WorldUIAnchor other = null)
	{
		...
		bool packet = false;
		for (int item2 = 0; item2 < 200; item2++)
		{
			if (Main.npc[item2].active && Main.npc[item2].boss)
			{
				packet = true;
			}
		}
		if (!packet)
		{
			...
			if (Main.rand.Next(2) == 0)
			{
				ProbeWeather(item, plr);
			}
			ProbeExceptions(item, plr, other);
		}
		else
		{
			ProbeCombat(item);
		...
		if (item.Count > 0)
		{
			emote = item[Main.rand.Next(item.Count)];
		}
	}
