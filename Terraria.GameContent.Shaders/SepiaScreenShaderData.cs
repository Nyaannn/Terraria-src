...
public class SepiaScreenShaderData : ScreenShaderData
{
	...
	private static void GetDaylightPowers(out float nightlightPower, out float daylightPower, out float moonPower, out float dawnPower)
	{
		...
		Vector2 dimensions = Utils.GetDayTimeAsDirectionIn24HClock(4.5f);
		Vector2 num = Utils.GetDayTimeAsDirectionIn24HClock(0f);
		float fromValue = Vector2.Dot(dayTimeAsDirectionIn24HClock, num);
		float fromValue2 = Vector2.Dot(dayTimeAsDirectionIn24HClock, dimensions);
		...
		if (!Main.dayTime)
		{
			float num2 = (float)(Main.time / 32400.0) * 2f;
			if (num2 > 1f)
			{
				num2 = 2f - num2;
			}
			moonPower = Utils.Remap(num2, 0f, 0.25f, 0f, 1f);
		}
	}
}
