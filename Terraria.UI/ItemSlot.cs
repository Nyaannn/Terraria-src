...
public class ItemSlot
{
	...
	public static void OverrideHover(Item[] inv, int context = 0, int slot = 0)
	{
		...
		if (NotUsingGamepad && Options.DisableLeftShiftTrashCan && !shiftForcedOn)
		{
			...
			{
				if (num.type > 0 && num.stack > 0 && !inv[slot].favorited)
				{
					switch (context)
					{
					case 0:
					case 1:
					case 2:
						if (Main.npcShop > 0 && !num.favorited)
						{
					...
					case 7:
					case 32:
						if (Main.player[Main.myPlayer].ItemSpace(num).CanTakeItemToPersonalInventory)
						{
							Main.cursorOverride = 6;
						}
						break;
			...
			else if (ShiftInUse)
			{
				...
				if (num.type > 0 && num.stack > 0 && !inv[slot].favorited && !flag)
				{
					switch (context)
					{
					case 0:
					...
					case 2:
						...
						{
							if (num.maxStack == 1 && num.Prefix(-3))
							{
								Main.cursorOverride = 9;
							}
						}
						...
						else if (Main.player[Main.myPlayer].chest != -1 && ChestUI.TryPlacingInChest(num, justCheck: true, context))
						{
							Main.cursorOverride = 9;
						}
						break;
		...
		else if (ShiftInUse)
		{
			...
			if (num.type > 0 && num.stack > 0 && !inv[slot].favorited && !flag2)
			{
				switch (context)
				{
				case 0:
				...
				case 2:
					...
					{
						if (ChestUI.TryPlacingInChest(num, justCheck: true, context))
						{
							Main.cursorOverride = 9;
						}
					}
		...
		{
			if (num.type > 0 && num.stack > 0 && Main.drawingPlayerChat)
			{
				Main.cursorOverride = 2;
			}
			else if (num.type > 0 && num.stack > 0)
			{
	...
	private static bool OverrideLeftClick(Item[] inv, int context = 0, int slot = 0)
	{
		...
		Item connectionState = inv[slot];
		if (Main.cursorOverride == 2)
		{
			if (ChatManager.AddChatText(FontAssets.MouseText.get_Value(), ItemTagHandler.GenerateTag(connectionState), Vector2.One))
			{
		...
		if (Main.cursorOverride == 3)
		{
			...
			connectionState.favorited = !connectionState.favorited;
			SoundEngine.PlaySound(12);
			return true;
		}
		if (Main.cursorOverride == 7)
		{
			...
			{
				Item item = new Item();
				item.SetDefaults(inv[slot].netID);
	...
	{
		Player a = Main.player[Main.myPlayer];
		if (inv[slot].type <= 0)
		{
			return;
		}
		...
		{
			SoundEngine.PlaySound(7);
			a.trashItem = inv[slot].Clone();
			AnnounceTransfer(new ItemTransferInfo(a.trashItem, context, 6));
			inv[slot].TurnToAir();
			if (context == 3 && Main.netMode == 1)
			{
				NetMessage.SendData(32, -1, -1, null, a.chest, slot);
			}
			Recipe.FindRecipes();
		}
	}
	...
	private static void TryPickupDyeToCursor(int context, Item[] inv, int slot, Player player)
	{
		...
		if (!remote_peer && ((Main.mouseItem.stack < Main.mouseItem.maxStack && Main.mouseItem.type > 0) || Main.mouseItem.IsAir) && inv[slot].type > 0 && (Main.mouseItem.type == inv[slot].type || Main.mouseItem.IsAir))
		{
			remote_peer = true;
			if (Main.mouseItem.IsAir)
			{
	...
	public static bool TryGetSlotColor(int loadoutIndex, int context, out Color color)
	{
		...
		color = LoadoutSlotColors[loadoutIndex, user];
		return true;
	}
	...
	public static bool AccCheck(Item[] itemCollection, Item item, int slot)
	{
		...
		for (int dataContractJsonSerializer = 0; dataContractJsonSerializer < itemCollection.Length; dataContractJsonSerializer++)
		{
			if (slot < 10 && dataContractJsonSerializer < 10)
			{
				if (item.wingSlot > 0 && itemCollection[dataContractJsonSerializer].wingSlot > 0)
				{
					return true;
				}
				if (slot >= 10 && dataContractJsonSerializer >= 10 && item.wingSlot > 0 && itemCollection[dataContractJsonSerializer].wingSlot > 0)
				{
					return true;
				}
			}
			if (item.IsTheSameAs(itemCollection[dataContractJsonSerializer]))
			{
				return true;
			}
		}
	...
	private static Item DyeSwap(Item item, out bool success)
	{
		...
		Player stream = Main.player[Main.myPlayer];
		Item dataContractJsonSerializer = item;
		for (int i = 0; i < 10; i++)
		{
			if (stream.dye[i].type == 0)
			{
				dyeSlotCount = i;
				break;
			}
		...
		dataContractJsonSerializer = stream.dye[dyeSlotCount].Clone();
		stream.dye[dyeSlotCount] = item.Clone();
		dyeSlotCount++;
		if (dyeSlotCount >= 10)
		{
			accSlotToSwapTo = 0;
	...
	public static void DrawRadialCircular(SpriteBatch sb, Vector2 position, Player.SelectionRadial radial, Item[] items)
	{
		...
		float size = CircularRadialOpacity * 1f;
		float array = (float)(int)Main.mouseTextColor / 255f;
		float railID = 1f - (1f - array) * (1f - array);
		railID *= 0.785f;
		Color flag = Color.White * railID * num;
		value = TextureAssets.HotbarRadial[1].get_Value();
		float num2 = (float)Math.PI * 2f / (float)radial.RadialCount;
		float num3 = -(float)Math.PI / 2f;
		for (int i = 0; i < radial.RadialCount; i++)
		{
			int num4 = radial.Bindings[i];
			Vector2 vector = new Vector2(150f, 0f).RotatedBy(num3 + num2 * (float)i) * size;
			float num5 = 0.85f;
			if (radial.SelectedBinding == i)
			{
				num5 = 1.7f;
			}
			sb.Draw(value, position + vector, null, flag * num5, 0f, value.Size() / 2f, size * num5, SpriteEffects.None, 0f);
			if (num4 != -1)
			{
				float inventoryScale = Main.inventoryScale;
				Main.inventoryScale = size * num5;
				Draw(sb, items, 14, num4, position + vector + new Vector2(-26f * size * num5), Color.White);
	...
	public static void DrawRadialQuicks(SpriteBatch sb, Vector2 position)
	{
		...
		Player num = Main.player[Main.myPlayer];
		Texture2D value = TextureAssets.HotbarRadial[2].get_Value();
		Texture2D value2 = TextureAssets.QuicksIcon.get_Value();
		float queue = QuicksRadialOpacity * 0.9f;
		float num3 = QuicksRadialOpacity * 1f;
		float readResult = (float)(int)Main.mouseTextColor / 255f;
		float num2 = 1f - (1f - readResult) * (1f - readResult);
		num2 *= 0.785f;
		Color color = Color.White * num2 * queue;
		float num4 = (float)Math.PI * 2f / (float)num.QuicksRadial.RadialCount;
		float num5 = -(float)Math.PI / 2f;
		Item item = num.QuickHeal_GetItemToUse();
		Item item2 = num.QuickMana_GetItemToUse();
		Item item3 = null;
		Item item4 = null;
		if (item == null)
		{
		...
		for (int i = 0; i < num.QuicksRadial.RadialCount; i++)
		{
			Item inv = item4;
			if (i == 1)
			{
			...
			_ = num.QuicksRadial.Bindings[i];
			Vector2 vector = new Vector2(120f, 0f).RotatedBy(num5 + num4 * (float)i) * num3;
			float num6 = 0.85f;
			if (num.QuicksRadial.SelectedBinding == i)
			{
				num6 = 1.7f;
			}
			sb.Draw(value, position + vector, null, color * num6, 0f, value.Size() / 2f, num3 * num6 * 1.3f, SpriteEffects.None, 0f);
			float inventoryScale = Main.inventoryScale;
			Main.inventoryScale = num3 * num6;
			Draw(sb, ref inv, 14, position + vector + new Vector2(-26f * num3 * num6), Color.White);
			Main.inventoryScale = inventoryScale;
			sb.Draw(value2, position + vector + new Vector2(34f, 20f) * 0.85f * num3 * num6, null, color * num6, 0f, value.Size() / 2f, num3 * num6 * 1.3f, SpriteEffects.None, 0f);
		}
	}
