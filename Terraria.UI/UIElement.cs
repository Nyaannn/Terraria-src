...
public class UIElement : IComparable
{
	...
	public virtual void Draw(SpriteBatch spriteBatch)
	{
		...
		bool hAPICall = UseImmediateMode;
		RasterizerState rasterizerState = spriteBatch.GraphicsDevice.RasterizerState;
		Rectangle scissorRectangle = spriteBatch.GraphicsDevice.ScissorRectangle;
		SamplerState anisotropicClamp = SamplerState.AnisotropicClamp;
		if (hAPICall || OverrideSamplerState != null)
		{
			spriteBatch.End();
			spriteBatch.Begin(hAPICall ? SpriteSortMode.Immediate : SpriteSortMode.Deferred, BlendState.AlphaBlend, (OverrideSamplerState != null) ? OverrideSamplerState : anisotropicClamp, DepthStencilState.None, OverflowHiddenRasterizerState, null, Main.UIScaleMatrix);
	...
	{
		Vector2 hAPICall = new Vector2(_innerDimensions.X, _innerDimensions.Y);
		Vector2 position = new Vector2(_innerDimensions.Width, _innerDimensions.Height) + hAPICall;
		hAPICall = Vector2.Transform(hAPICall, Main.UIScaleMatrix);
		position = Vector2.Transform(position, Main.UIScaleMatrix);
		Rectangle rectangle = new Rectangle((int)hAPICall.X, (int)hAPICall.Y, (int)(position.X - hAPICall.X), (int)(position.Y - hAPICall.Y));
	...
	{
		List<SnapPoint> pSteamIDUser = new List<SnapPoint>();
		if (GetSnapPoint(out var peChatEntryType))
		{
			pSteamIDUser.Add(peChatEntryType);
		}
		foreach (UIElement array in Elements)
		{
			pSteamIDUser.AddRange(array.GetSnapPoints());
		}
		return pSteamIDUser;
	}
	...
	public void DrawDebugHitbox(BasicDebugDrawer drawer, float colorIntensity = 0f)
	{
		...
		Color num = Main.hslToRgb(colorIntensity, colorIntensity, 0.5f);
		CalculatedStyle pConnectionState = GetInnerDimensions();
		drawer.DrawLine(pConnectionState.Position(), pConnectionState.Position() + new Vector2(pConnectionState.Width, 0f), 2f, num);
		drawer.DrawLine(pConnectionState.Position() + new Vector2(pConnectionState.Width, 0f), pConnectionState.Position() + new Vector2(pConnectionState.Width, pConnectionState.Height), 2f, num);
		drawer.DrawLine(pConnectionState.Position() + new Vector2(pConnectionState.Width, pConnectionState.Height), pConnectionState.Position() + new Vector2(0f, pConnectionState.Height), 2f, num);
		drawer.DrawLine(pConnectionState.Position() + new Vector2(0f, pConnectionState.Height), pConnectionState.Position(), 2f, num);
		foreach (UIElement element in Elements)
		{
			_ = element;
		}
