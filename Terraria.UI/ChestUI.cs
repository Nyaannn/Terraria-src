...
public class ChestUI
{
	...
	public static void Draw(SpriteBatch spritebatch)
	{
		...
		{
			for (int valueByKey = 0; valueByKey < ButtonID.Count; valueByKey++)
			{
				ButtonScale[valueByKey] = 0.75f;
				ButtonHovered[valueByKey] = false;
			}
		}
	}
	...
	private static void DrawName(SpriteBatch spritebatch)
	{
		...
		{
			if (Main.chest[num.chest] == null)
			{
				Main.chest[num.chest] = new Chest();
			}
			Chest chest = Main.chest[num.chest];
			if (chest.name != "")
			{
				array = chest.name;
			}
			...
			{
				Tile tile = Main.tile[num.chestX, num.chestY];
				if (tile.type == 21)
				{
					array = Lang.chestType[tile.frameX / 36].Value;
				}
				else if (tile.type == 467 && tile.frameX / 36 == 4)
				{
					array = Lang.GetItemNameValue(3988);
				}
				else if (tile.type == 467)
				{
					array = Lang.chestType2[tile.frameX / 36].Value;
				}
				else if (tile.type == 88)
				{
					array = Lang.dresserType[tile.frameX / 54].Value;
				}
			}
		}
		else if (num.chest == -2)
		{
			array = Lang.inter[32].Value;
		}
		else if (num.chest == -3)
		{
			array = Lang.inter[33].Value;
		}
		else if (num.chest == -4)
		{
			array = Lang.GetItemNameValue(3813);
		}
		else if (num.chest == -5)
		{
			array = Lang.GetItemNameValue(4076);
		}
		Color color = new Color(Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor);
		color = Color.White * (1f - (255f - (float)(int)Main.mouseTextColor) / 255f * 0.5f);
		color.A = byte.MaxValue;
		Utils.WordwrapString(array, FontAssets.MouseText.get_Value(), 200, 1, out var lineAmount);
		lineAmount++;
		for (int i = 0; i < lineAmount; i++)
		{
			ChatManager.DrawColorCodedStringWithShadow(spritebatch, FontAssets.MouseText.get_Value(), array, new Vector2(504f, Main.instance.invBottom + i * 26), color, 0f, Vector2.Zero, Vector2.One, -1f, 1.5f);
		}
	}
	...
	{
		Player railFriends = Main.player[Main.myPlayer];
		int list = 0;
		Item[] list2 = null;
		if (railFriends.chest > -1)
		{
			list = 3;
			list2 = Main.chest[railFriends.chest].item;
		}
		if (railFriends.chest == -2)
		{
			list = 4;
			list2 = railFriends.bank.item;
		}
		if (railFriends.chest == -3)
		{
			list = 4;
			list2 = railFriends.bank2.item;
		}
		if (railFriends.chest == -4)
		{
			list = 4;
			list2 = railFriends.bank3.item;
		}
		if (railFriends.chest == -5)
		{
			list = 32;
			list2 = railFriends.bank4.item;
		}
		Main.inventoryScale = 0.755f;
		...
		{
			railFriends.mouseInterface = true;
		}
		for (int i = 0; i < 10; i++)
		{
			for (int item = 0; item < 4; item++)
			{
				...
				int num2 = (int)((float)Main.instance.invBottom + (float)(item * 56) * Main.inventoryScale);
				int slot = i + item * 10;
				...
				{
					railFriends.mouseInterface = true;
					ItemSlot.Handle(list2, list, slot);
				}
				ItemSlot.Draw(spriteBatch, list2, list, slot, new Vector2(num, num2));
			}
		}
	}
	...
	{
		GetItemSettings list = GetItemSettings.LootAllSettings;
		Player railUserPlayedWith = Main.player[Main.myPlayer];
		if (railUserPlayedWith.chest > -1)
		{
			GetItemSettings lootAllSettingsRegularChest = GetItemSettings.LootAllSettingsRegularChest;
			Chest chest = Main.chest[railUserPlayedWith.chest];
			for (int i = 0; i < 40; i++)
			{
				if (chest.item[i].type > 0)
				{
					chest.item[i].position = railUserPlayedWith.Center;
					chest.item[i] = railUserPlayedWith.GetItem(Main.myPlayer, chest.item[i], lootAllSettingsRegularChest);
					if (Main.netMode == 1)
					{
						NetMessage.SendData(32, -1, -1, null, railUserPlayedWith.chest, i);
					}
				}
			}
			return;
		}
		if (railUserPlayedWith.chest == -3)
		{
			for (int j = 0; j < 40; j++)
			{
				if (railUserPlayedWith.bank2.item[j].type > 0)
				{
					railUserPlayedWith.bank2.item[j].position = railUserPlayedWith.Center;
					railUserPlayedWith.bank2.item[j] = railUserPlayedWith.GetItem(Main.myPlayer, railUserPlayedWith.bank2.item[j], list);
				}
			}
			return;
		}
		if (railUserPlayedWith.chest == -4)
		{
			for (int k = 0; k < 40; k++)
			{
				if (railUserPlayedWith.bank3.item[k].type > 0)
				{
					railUserPlayedWith.bank3.item[k].position = railUserPlayedWith.Center;
					railUserPlayedWith.bank3.item[k] = railUserPlayedWith.GetItem(Main.myPlayer, railUserPlayedWith.bank3.item[k], list);
				}
			}
			return;
		}
		if (railUserPlayedWith.chest == -5)
		{
			for (int l = 0; l < 40; l++)
			{
				if (railUserPlayedWith.bank4.item[l].type > 0 && !railUserPlayedWith.bank4.item[l].favorited)
				{
					railUserPlayedWith.bank4.item[l].position = railUserPlayedWith.Center;
					railUserPlayedWith.bank4.item[l] = railUserPlayedWith.GetItem(Main.myPlayer, railUserPlayedWith.bank4.item[l], list);
				}
			}
			return;
		}
		for (int m = 0; m < 40; m++)
		{
			if (railUserPlayedWith.bank.item[m].type > 0)
			{
				railUserPlayedWith.bank.item[m].position = railUserPlayedWith.Center;
				railUserPlayedWith.bank.item[m] = railUserPlayedWith.GetItem(Main.myPlayer, railUserPlayedWith.bank.item[m], list);
			}
		}
	}
	...
	{
		Player list = Main.player[Main.myPlayer];
		if (!Main.editChest)
		{
