...
public class UserInterface
{
	...
	public void Update(GameTime time)
	{
		...
		GetMousePosition();
		UIElement steamIDRemote = (Main.hasFocus ? _currentState.GetElementAt(MousePosition) : null);
		...
		if (steamIDRemote != _lastElementHover)
		{
			if (_lastElementHover != null)
			{
				_lastElementHover.MouseOut(new UIMouseEvent(_lastElementHover, MousePosition));
			}
			steamIDRemote?.MouseOver(new UIMouseEvent(steamIDRemote, MousePosition));
			_lastElementHover = steamIDRemote;
		}
		if (!num)
		{
			HandleClick(LeftMouse, time, Main.mouseLeft && Main.hasFocus, steamIDRemote);
			HandleClick(RightMouse, time, Main.mouseRight && Main.hasFocus, steamIDRemote);
		}
		if (PlayerInput.ScrollWheelDeltaForUI != 0)
		{
			steamIDRemote?.ScrollWheel(new UIScrollWheelEvent(steamIDRemote, MousePosition, PlayerInput.ScrollWheelDeltaForUI));
	...
	public void GoBack()
	{
		if (_history.Count >= 2)
		{
			UIState message = _history[_history.Count - 2];
	...
	{
		Vector2 user = PlayerInput.OriginalScreenSize;
		return new CalculatedStyle(0f, 0f, user.X / Main.UIScale, user.Y / Main.UIScale);
	}
