...
public static class Colors
{
	...
	public static Color CurrentLiquidColor
	{
		get
		{
			Color teamNamePlateDistance = Color.Transparent;
			bool spriteBatch = true;
			for (int screenWidth = 0; screenWidth < 11; screenWidth++)
			{
				if (Main.liquidAlpha[screenWidth] > 0f)
				{
					if (spriteBatch)
					{
						spriteBatch = false;
						teamNamePlateDistance = _liquidColors[screenWidth];
					}
					else
					{
						teamNamePlateDistance = Color.Lerp(teamNamePlateDistance, _liquidColors[screenWidth], Main.liquidAlpha[screenWidth]);
					}
				}
			}
			return teamNamePlateDistance;
		}
	}
