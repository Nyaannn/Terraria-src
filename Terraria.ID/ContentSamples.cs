...
public static class ContentSamples
{
	...
	public static void Initialize()
	{
		...
		for (int textKey = -65; textKey < NPCID.Count; textKey++)
		{
			NPC list = new NPC();
			list.SetDefaults(textKey);
			NpcsByNetId[textKey] = list;
			string key = NPCID.Search.GetName(list.netID);
			NpcPersistentIdsByNetIds[textKey] = key;
			NpcBestiaryCreditIdsByNpcNetIds[textKey] = key;
			NpcNetIdsByPersistentIds[key] = textKey;
		}
		ModifyNPCIds();
		...
		for (int i = 0; i < ProjectileID.Count; i++)
		{
			...
			ProjectilesByType[i] = projectile;
		}
		ItemsByType.Clear();
		for (int j = 0; j < ItemID.Count; j++)
		{
			...
			item.SetDefaults(j);
			ItemsByType[j] = item;
			string name = ItemID.Search.GetName(item.netID);
			ItemPersistentIdsByNetIds[j] = name;
			ItemNetIdsByPersistentIds[name] = j;
		}
		foreach (int item3 in ItemID.Sets.ItemsThatAreProcessedAfterNormalContentSample)
		{
			...
			string name2 = ItemID.Search.GetName(item2.netID);
			ItemPersistentIdsByNetIds[item3] = name2;
			ItemNetIdsByPersistentIds[name2] = item3;
		}
	...
	{
		for (int spriteBatch = 0; spriteBatch < itemsThatWillResearchTheItemToUnlock.Length; spriteBatch++)
		{
			AddItemResearchOverride_Inner(itemsThatWillResearchTheItemToUnlock[spriteBatch], itemTypeToUnlock);
		}
	}
	...
	{
		foreach (KeyValuePair<int, Item> localPlayer in ItemsByType)
		{
			localPlayer.Value.material = ItemID.Sets.IsAMaterial[localPlayer.Key];
		}
	}
	...
	{
		NPCSpawnParams num2 = default(NPCSpawnParams);
		num2.gameModeData = Main.RegisteredGameModes[0];
		NPCSpawnParams num = num2;
		for (int t = -65; t < NPCID.Count; t++)
		{
			NPC lerpValue = new NPC();
			lerpValue.SetDefaults(t, num);
			NpcBestiaryRarityStars[t] = GetNPCBestiaryRarityStarsCount(lerpValue);
		}
