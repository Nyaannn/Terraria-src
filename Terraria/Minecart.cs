...
public static class Minecart
{
	...
	public static BitsByte TrackCollision(Player Player, ref Vector2 Position, ref Vector2 Velocity, ref Vector2 lastBoost, int Width, int Height, bool followDown, bool followUp, int fallStart, bool trackOnly, Mount.MountDelegatesData delegatesData)
	{
		...
		Vector2 rectangle = Velocity;
		float destinationRectangle = rectangle.Length();
		rectangle.Normalize();
		Vector2 num = vector;
		Tile num2 = null;
		bool num3 = false;
		...
		while (true)
		{
			int num6 = (int)(num.X / 16f);
			int num7 = (int)(num.Y / 16f);
			int num8 = (int)num.X % 16 / 2;
			if (flag)
			{
				num5 = num8;
			}
			bool flag6 = num8 != num5;
			if ((trackState == TrackState.OnBack || trackState == TrackState.OnTrack || trackState == TrackState.OnFront) && num6 != num4)
			{
				int num9 = ((trackState != TrackState.OnBack) ? num2.FrontTrack() : num2.BackTrack());
				switch ((!(rectangle.X < 0f)) ? _rightSideConnection[num9] : _leftSideConnection[num9])
				{
				case 0:
					num7--;
					num.Y -= 2f;
			...
			if (num6 != num4 || num7 != percent)
			{
				...
				num2 = Main.tile[num6, num7];
				if (num2 == null)
				{
					num2 = new Tile();
					Main.tile[num6, num7] = num2;
				}
				num3 = ((num2.nactive() && num2.type == 314) ? true : false);
			}
			if (num3)
			{
				TrackState trackState3 = TrackState.NoTrack;
				int num10 = num2.FrontTrack();
				int num11 = num2.BackTrack();
				int num12 = _tileHeight[num10][num8];
				switch (num12)
				{
				case -4:
				...
				case -2:
					...
					if (rectangle.X < 0f)
					{
						float num15 = (float)(num6 * 16 + (num8 + 1) * 2) - num.X;
						num.X += num15;
						destinationRectangle += num15 / rectangle.X;
					}
					rectangle.X = 0f - rectangle.X;
					result[1] = true;
					trackState2 = TrackState.OnFront;
					break;
				case -3:
					if (trackState == TrackState.OnFront)
					{
						...
						vector2 = Vector2.Transform(matrix: (Velocity.X > 0f) ? ((_leftSideConnection[num10] != 2) ? Matrix.CreateRotationZ((float)Math.PI / 4f) : Matrix.CreateRotationZ(-(float)Math.PI / 4f)) : ((_rightSideConnection[num10] != 2) ? Matrix.CreateRotationZ(-(float)Math.PI / 4f) : Matrix.CreateRotationZ((float)Math.PI / 4f)), position: new Vector2(Velocity.X, 0f));
				...
				default:
				{
					...
					if (num6 != num4 && trackState == TrackState.NoTrack && num.Y > num13 && num.Y - num13 < 2f)
					{
						flag7 = false;
						trackState = TrackState.AboveFront;
					}
					TrackState trackState4 = ((!(num.Y < num13)) ? ((!(num.Y > num13)) ? TrackState.OnTrack : TrackState.BelowTrack) : TrackState.AboveTrack);
					if (num11 != -1)
					{
						float num14 = num7 * 16 + _tileHeight[num11][num8];
						trackState3 = ((!(num.Y < num14)) ? ((!(num.Y > num14)) ? TrackState.OnTrack : TrackState.BelowTrack) : TrackState.AboveTrack);
					}
					switch (trackState4)
					{
			...
			if (!flag7)
			{
				if (trackState != trackState2)
				{
					...
					if (flag6 || rectangle.Y > 0f)
					{
						...
						case TrackState.OnTrack:
						{
							int num16 = _tileHeight[num2.FrontTrack()][num8];
							int num17 = _tileHeight[num2.BackTrack()][num8];
						...
						case TrackState.OnTrack:
						case TrackState.OnFront:
							num18 = num2.FrontTrack();
							break;
						case TrackState.OnBack:
							num18 = num2.BackTrack();
							break;
						}
						if (num18 != -1)
						{
							if (!flag8 && Velocity.Y > Player.defaultGravity)
							{
							...
							if (trackState == TrackState.AboveFront && _trackType[num18] == 1)
							{
								flag2 = true;
							}
							rectangle.Y = 0f;
							num.Y = num7 * 16 + _tileHeight[num18][num8];
						}
					}
				}
			}
			...
			{
				if (num3 && _trackType[num2.FrontTrack()] == 1)
				{
					flag2 = true;
				}
				rectangle.Y = 0f;
			...
			{
				int num20 = num2.FrontTrack();
				if (_trackType[num20] == 2 && lastBoost.X == 0f && lastBoost.Y == 0f)
				{
			...
			num4 = num6;
			percent = num7;
			if (destinationRectangle > 0f)
			{
				float num21 = num.X % 2f;
				float num22 = num.Y % 2f;
				float num23 = 3f;
				float num24 = 3f;
				if (rectangle.X < 0f)
				{
				...
				float num25 = Math.Abs(num23 / rectangle.X);
				float num26 = Math.Abs(num24 / rectangle.Y);
				float num27 = ((num25 < num26) ? num25 : num26);
				if (num27 > destinationRectangle)
				{
					vector3 = rectangle * destinationRectangle;
					destinationRectangle = 0f;
				}
				else
				{
					vector3 = rectangle * num27;
					destinationRectangle -= num27;
				}
				num += vector3;
				continue;
			}
			if (lastBoost.X != (float)num4 || lastBoost.Y != (float)percent)
			{
		...
		else if (result[1])
		{
			...
			Position.X = num.X - _trackMagnetOffset.X - value.X - Velocity.X;
			if (rectangle.Y == 0f)
			{
				Velocity.Y = 0f;
			}
		...
		Position.Y += num.Y - vector.Y - Velocity.Y;
	...
	{
		Tile text = Main.tile[tileX, tileY];
		if (text.type != 314)
		{
			return false;
		}
		...
		for (int i = num; i < 8; i++)
		{
			num3 = _tileHeight[text.frameX][i];
			if (num3 >= 0)
			{
				num2 = i;
				break;
		...
		if (num2 == -1)
		{
			...
			{
				num3 = _tileHeight[text.frameX][num4];
				if (num3 >= 0)
				{
					num2 = num4;
					break;
	...
	public static Rectangle GetSourceRect(int frameID, int animationFrame = 0)
	{
		...
		Vector2 text = _texturePosition[frameID];
		Rectangle result = new Rectangle((int)text.X, (int)text.Y, 16, 16);
