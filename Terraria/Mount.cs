...
public class Mount
{
	...
	public void StartAbilityCharge(Player mountedPlayer)
	{
		if (Main.myPlayer == mountedPlayer.whoAmI)
		{
			...
			if (num == 9)
			{
				...
				float i = Main.screenPosition.Y + (float)Main.mouseY;
				float vector = num3 - mountedPlayer.position.X;
				float num4 = i - mountedPlayer.position.Y;
				Projectile.NewProjectile(GetProjectileSpawnSource(mountedPlayer), num3, i, 0f, 0f, num2, 0, 0f, mountedPlayer.whoAmI, vector, num4);
	...
	{
		DrillMountData flag = (DrillMountData)_mountSpecificData;
		for (int vector = 0; vector < flag.beams.Length; vector++)
		{
			DrillBeam i = flag.beams[vector];
			if (i.cooldown > 1)
			{
				i.cooldown--;
			}
			...
			{
				i.cooldown = 0;
				i.curTileTarget = Point16.NegativeOne;
			}
		}
		flag.diodeRotation = flag.diodeRotation * 0.85f + 0.15f * flag.diodeRotationTarget;
		if (flag.beamCooldown > 0)
		{
			flag.beamCooldown--;
		}
	}
	...
	public void UseDrill(Player mountedPlayer)
	{
		...
		DrillMountData rectangle = (DrillMountData)_mountSpecificData;
		bool i = mountedPlayer.whoAmI == Main.myPlayer;
		if (mountedPlayer.controlUseItem)
		{
			for (int num = 0; num < amountOfBeamsAtOnce; num++)
			{
				if (rectangle.beamCooldown != 0)
				{
					break;
				}
				for (int num2 = 0; num2 < rectangle.beams.Length; num2++)
				{
					DrillBeam num3 = rectangle.beams[num2];
					if (num3.cooldown != 0)
					{
						continue;
					}
					Point16 num4 = DrillSmartCursor_Blocks(mountedPlayer, rectangle);
					if (num4 == Point16.NegativeOne)
					{
						continue;
					}
					num3.curTileTarget = num4;
					int rectangle2 = drillPickPower;
					if (i)
					{
						bool num7 = true;
						if (WorldGen.InWorld(num4.X, num4.Y) && Main.tile[num4.X, num4.Y] != null && Main.tile[num4.X, num4.Y].type == 26 && !Main.hardMode)
						{
							num7 = false;
						...
						if (num7)
						{
							mountedPlayer.PickTile(num4.X, num4.Y, rectangle2);
						}
					}
					Vector2 num5 = new Vector2((float)(num4.X << 4) + 8f, (float)(num4.Y << 4) + 8f);
					float num6 = (num5 - mountedPlayer.Center).ToRotation();
					for (int num8 = 0; num8 < 2; num8++)
					{
						float num9 = num6 + ((Main.rand.Next(2) == 1) ? (-1f) : 1f) * ((float)Math.PI / 2f);
						float num10 = (float)Main.rand.NextDouble() * 2f + 2f;
						Vector2 vector = new Vector2((float)Math.Cos(num9) * num10, (float)Math.Sin(num9) * num10);
						int num11 = Dust.NewDust(num5, 0, 0, 230, vector.X, vector.Y);
						Main.dust[num11].noGravity = true;
						Main.dust[num11].customData = mountedPlayer;
					}
					if (i)
					{
						Tile.SmoothSlope(num4.X, num4.Y, applyToNeighbors: true, sync: true);
					}
					num3.cooldown = drillPickTime;
					num3.lastPurpose = 0;
					break;
				}
			}
		...
		for (int j = 0; j < amountOfBeamsAtOnce; j++)
		{
			if (rectangle.beamCooldown != 0)
			{
				break;
			}
			for (int k = 0; k < rectangle.beams.Length; k++)
			{
				DrillBeam drillBeam = rectangle.beams[k];
				if (drillBeam.cooldown != 0)
				{
					continue;
				}
				Point16 point = DrillSmartCursor_Walls(mountedPlayer, rectangle);
				...
				if (i)
				{
					...
					if (flag)
					{
						mountedPlayer.PickWall(point.X, point.Y, damage);
					}
				}
				Vector2 vector2 = new Vector2((float)(point.X << 4) + 8f, (float)(point.Y << 4) + 8f);
				float num12 = (vector2 - mountedPlayer.Center).ToRotation();
				for (int l = 0; l < 2; l++)
				{
					float num13 = num12 + ((Main.rand.Next(2) == 1) ? (-1f) : 1f) * ((float)Math.PI / 2f);
					float num14 = (float)Main.rand.NextDouble() * 2f + 2f;
					Vector2 vector3 = new Vector2((float)Math.Cos(num13) * num14, (float)Math.Sin(num13) * num14);
					int num15 = Dust.NewDust(vector2, 0, 0, 230, vector3.X, vector3.Y);
					Main.dust[num15].noGravity = true;
					Main.dust[num15].customData = mountedPlayer;
				}
				drillBeam.cooldown = drillPickTime;
				drillBeam.lastPurpose = 1;
				break;
			}
		}
	}
	...
	{
		Vector2 item = ((mountedPlayer.whoAmI != Main.myPlayer) ? data.crosshairPosition : (Main.screenPosition + new Vector2(Main.mouseX, Main.mouseY)));
		Vector2 num2 = mountedPlayer.Center;
		Vector2 num3 = item - num2;
		...
		num4 += 32f;
		num3.Normalize();
		Vector2 end = num2 + num3 * num4;
		Point16 tilePoint = new Point16(-1, -1);
		if (!Utils.PlotTileLine(num2, end, 65.6f, delegate(int x, int y)
		{
	...
	public void UseAbility(Player mountedPlayer, Vector2 mousePosition, bool toggleOn)
	{
		...
		case 9:
		{
			...
			Vector2 num15 = default(Vector2);
			num15.X = mountedPlayer.position.X + (float)(mountedPlayer.width / 2);
			num15.Y = mountedPlayer.position.Y + (float)mountedPlayer.height;
			int num17 = (_frameExtra - 6) * 2;
			Vector2 num14 = default(Vector2);
			for (int num18 = 0; num18 < 2; num18++)
			{
				num14.Y = num15.Y + scutlixEyePositions[num17 + num18].Y + (float)_data.yOffset;
				if (mountedPlayer.direction == -1)
				{
					num14.X = num15.X - scutlixEyePositions[num17 + num18].X - (float)_data.xOffset;
				}
				else
				{
					num14.X = num15.X + scutlixEyePositions[num17 + num18].X + (float)_data.xOffset;
				}
				Vector2 num19 = mousePosition - num14;
				num19.Normalize();
				num19 *= 14f;
				int num20 = 150;
				num14 += num19;
				Projectile.NewProjectile(GetProjectileSpawnSource(mountedPlayer), num14.X, num14.Y, num19.X, num19.Y, num16, num20, 0f, Main.myPlayer);
			}
			break;
		}
		case 46:
			if (Main.myPlayer == mountedPlayer.whoAmI)
			{
				...
				{
					int torchID = 120;
					Vector2 r = mountedPlayer.Center + new Vector2(mountedPlayer.width * -mountedPlayer.direction, 26f);
					Vector2 g = new Vector2(0f, -4f).RotatedByRandom(0.10000000149011612);
					Projectile.NewProjectile(GetProjectileSpawnSource(mountedPlayer), r.X, r.Y, g.X, g.Y, 930, torchID, 0f, Main.myPlayer);
					SoundEngine.PlaySound(SoundID.Item89.SoundId, (int)r.X, (int)r.Y, SoundID.Item89.Style, 0.2f);
				}
				int num11 = 14;
				int num12 = 100;
				mousePosition = ClampToDeadZone(mountedPlayer, mousePosition);
				Vector2 num10 = default(Vector2);
				num10.X = mountedPlayer.position.X + (float)(mountedPlayer.width / 2);
				num10.Y = mountedPlayer.position.Y + (float)mountedPlayer.height;
				Vector2 num21 = new Vector2(num10.X + (float)(mountedPlayer.width * mountedPlayer.direction), num10.Y - 12f);
				Vector2 num13 = mousePosition - num21;
				num13 = num13.SafeNormalize(Vector2.Zero);
				num13 *= 12f;
				num13 = num13.RotatedByRandom(0.20000000298023224);
				Projectile.NewProjectile(GetProjectileSpawnSource(mountedPlayer), num21.X, num21.Y, num13.X, num13.Y, num11, num12, 0f, Main.myPlayer);
				SoundEngine.PlaySound(SoundID.Item11.SoundId, (int)num21.X, (int)num21.Y, SoundID.Item11.Style, 0.2f);
			}
			break;
		case 8:
			if (Main.myPlayer == mountedPlayer.whoAmI)
			{
				...
				else if (!_abilityActive)
				{
					...
					{
						float b = Main.screenPosition.X + (float)Main.mouseX;
						float r2 = Main.screenPosition.Y + (float)Main.mouseY;
						float g2 = b - mountedPlayer.position.X;
						float b2 = r2 - mountedPlayer.position.Y;
						Projectile.NewProjectile(GetProjectileSpawnSource(mountedPlayer), b, r2, 0f, 0f, 453, 0, 0f, mountedPlayer.whoAmI, g2, b2);
					}
					_abilityActive = true;
				}
			}
	...
	public void UpdateFrame(Player mountedPlayer, int state, Vector2 velocity)
	{
		...
		if (_data.emitsLight)
		{
			Point num = mountedPlayer.Center.ToTileCoordinates();
			Lighting.AddLight(num.X, num.Y, _data.lightColor.X, _data.lightColor.Y, _data.lightColor.Z);
		}
		switch (_type)
		{
		case 52:
		...
		case 49:
		{
			...
			velocity.Length();
			float num2 = mountedPlayer.velocity.Y * 0.05f;
		...
		case 8:
		{
			...
			Vector2 num3 = default(Vector2);
			num3.X = mountedPlayer.position.X;
			num3.Y = mountedPlayer.position.Y + (float)mountedPlayer.height;
			int j = (int)(num3.X / 16f);
			_ = num3.Y / 16f;
			float k = 0f;
			float num8 = mountedPlayer.width;
			while (num8 > 0f)
			{
				float num9 = (float)((j + 1) * 16) - num3.X;
				if (num9 > num8)
				{
					num9 = num8;
				}
				k += Collision.GetTileRotation(num3) * num9;
				num8 -= num9;
				num3.X += num9;
				j++;
			}
			float num10 = k / (float)mountedPlayer.width - mountedPlayer.fullRotation;
			float num11 = 0f;
		...
		case 47:
		{
			bool flag4 = Math.Abs(velocity.X) > DashSpeed - RunSpeed / 2f;
			if (state == 1)
			{
				bool flag5 = false;
				if (flag4)
			...
			if (_type == 47 && flag4 && velocity.Y == 0f)
			{
		...
		case 45:
		{
			...
			bool i = (bool)_mountSpecificData;
			if (i && !flag)
			{
				_mountSpecificData = false;
			}
		...
		case 14:
		{
			bool num4 = Math.Abs(velocity.X) > RunSpeed / 2f;
			float num5 = Math.Sign(mountedPlayer.velocity.X);
			float num6 = 12f;
			float num7 = 40f;
			if (!num4)
			{
			...
			if (num4 && velocity.Y == 0f)
			{
				...
				{
					mountedPlayer.position += new Vector2(num5 * 24f, 0f);
					mountedPlayer.FloorVisuals(Falling: true);
					mountedPlayer.position -= new Vector2(num5 * 24f, 0f);
				}
			}
			if (num5 != (float)mountedPlayer.direction)
			{
				break;
			}
			for (int m = 0; m < (int)(3f * mountedPlayer.basiliskCharge); m++)
			{
				Dust dust = Main.dust[Dust.NewDust(mountedPlayer.BottomLeft, mountedPlayer.width, 6, 6)];
				Vector2 vector = mountedPlayer.Center + new Vector2(num5 * num7, num6);
				dust.position = mountedPlayer.Center + new Vector2(num5 * (num7 - 2f), num6 - 6f + Main.rand.NextFloat() * 12f);
	...
	public void TryBeginningFlight(Player mountedPlayer, int state)
	{
		...
		Vector2 num = Vector2.UnitY * mountedPlayer.gravDir;
		if (Collision.TileCollision(mountedPlayer.position + new Vector2(0f, -0.001f), num, mountedPlayer.width, mountedPlayer.height, fallThrough: false, fall2: false, (int)mountedPlayer.gravDir).Y != 0f)
		{
			...
			float flag = mountedPlayer.gravity * mountedPlayer.gravDir;
			mountedPlayer.position.Y -= mountedPlayer.velocity.Y;
			mountedPlayer.velocity.Y -= flag;
		}
	}
	...
	public void UpdateEffects(Player mountedPlayer)
	{
		...
		if (_shouldSuperCart)
		{
			...
			float flag = 1f + Math.Abs(mountedPlayer.velocity.X) / RunSpeed * 2.5f;
			mountedPlayer.statDefense += (int)(2f * flag);
		}
		switch (_type)
		{
		case 23:
		{
			...
			Vector2 vector10 = mountedPlayer.RotatedRelativePoint(pos3);
			Lighting.AddLight(vector10, rgb);
			if (Main.rand.Next(45) == 0)
			{
				Vector2 vector11 = Main.rand.NextVector2Circular(4f, 4f);
				Dust dust2 = Dust.NewDustPerfect(vector10 + vector11, 43, Vector2.Zero, 254, new Color(255, 255, 0, 255), 0.3f);
				if (vector11 != Vector2.Zero)
				{
					dust2.velocity = vector10.DirectionTo(dust2.position) * 0.2f;
				}
				dust2.fadeIn = 0.3f;
			...
			float num14 = 0.1f;
			num14 += mountedPlayer.velocity.Length() / 30f;
			...
			if (!(Main.rand.NextFloat() <= num14))
			{
				break;
			}
			float num15 = Main.rand.NextFloat();
			for (float num16 = 0f; num16 < 1f; num16 += 0.125f)
			{
				if (Main.rand.Next(15) == 0)
				{
					Vector2 spinningpoint = ((float)Math.PI * 2f * num16 + num15).ToRotationVector2() * new Vector2(0.5f, 1f) * 4f;
		...
		case 46:
		{
			...
			for (int i = 0; i < 200; i++)
			{
				...
				Vector2 v = nPC.Center - num2;
				float num12 = v.Length();
				if (num12 < num11 && ((Vector2.Distance(pre, num2) > num12 && num12 < num10) || !flag2))
				{
					bool flag3 = true;
					float num13 = Math.Abs(v.ToRotation());
					if (mountedPlayer.direction == 1 && (double)num13 > 1.0471975949079879)
					{
						flag3 = false;
					}
					else if (mountedPlayer.direction == -1 && (double)num13 < 2.0943951461045853)
					{
						flag3 = false;
					}
					if (Collision.CanHitLine(num2, 0, 0, nPC.position, nPC.width, nPC.height) && flag3)
					{
						num10 = num12;
						pre = nPC.Center;
						flag2 = true;
			...
			if (flag2)
			{
				...
				{
					AimAbility(mountedPlayer, pre);
					if (_abilityCooldown == 0)
					{
						StopAbilityCharge();
					}
					UseAbility(mountedPlayer, pre, toggleOn: false);
				}
				else
				{
					AimAbility(mountedPlayer, pre);
		...
		case 11:
		{
			...
			float num5 = 1f + Math.Abs(mountedPlayer.velocity.X) / RunSpeed * 2.5f;
			...
			Vector2 vector5 = new Vector2(num7 * (float)num6, -19f).RotatedBy(mountedPlayer.fullRotation);
			...
			{
				int num8 = (int)Vector2.Distance(vector7, vector8) / 3;
				...
				for (float num9 = 1f; num9 <= (float)num8; num9 += 1f)
				{
					...
					obj.position = Vector2.Lerp(vector8, vector7, num9 / (float)num8);
					obj.noGravity = true;
		...
		case 22:
		{
			...
			float num3 = MathHelper.Clamp(mountedPlayer.velocity.Length() / 5f, 0f, 1f);
			for (float num4 = 0f; num4 <= 1f; num4 += 0.1f)
			{
				if (!(Main.rand.NextFloat() < num3))
				{
					Dust dust = Dust.NewDustPerfect(Vector2.Lerp(value, vector, num4), 65, Main.rand.NextVector2Circular(0.5f, 0.5f) * num3);
					dust.scale = 0.6f;
					dust.fadeIn = 0f;
					...
					{
						dust.fadeIn = 1.3f;
						dust.velocity = Main.rand.NextVector2Circular(3f, 3f) * num3;
					}
				}
			}
			break;
		...
		case 8:
		{
			...
			for (int i = 0; i < drillMountData2.beams.Length; i++)
			{
				...
				for (int j = 0; j < 2; j++)
				{
					...
					float y = v.Length();
					DrawData item = new DrawData(scale: new Vector2(2f, y), texture: TextureAssets.MagicPixel.get_Value(), position: vector6 + Position, sourceRect: value7, color: color2, rotation: num12 - (float)Math.PI / 2f, origin: Vector2.Zero, effect: SpriteEffects.None);
