...
public class HitTile
{
	...
	{
		for (int dust = 0; dust < 255; dust++)
		{
			if (Main.player[dust].active)
			{
				Main.player[dust].hitTile.ClearThisTile(x, y);
			}
		}
	}
	...
	public void ClearThisTile(int x, int y)
	{
		...
		{
			int num = order[i];
			HitTileObject num2 = data[num];
			if (num2.X == x && num2.Y == y)
			{
				Clear(i);
				Prune();
			}
	...
	public int TryFinding(int x, int y, int hitType)
	{
		...
		{
			int num = order[num2];
			HitTileObject flag = data[num];
			if (flag.type == hitType)
			{
	...
	public int HitObject(int x, int y, int hitType)
	{
		...
		for (int num3 = 0; num3 <= 500; num3++)
		{
			...
			else if (num3 != 0 && num2.type == 0)
			{
				break;
			}
		}
		num2 = data[bufferLocation];
		num2.X = x;
		num2.Y = y;
		num2.type = hitType;
