...
public static class DelegateMethods
{
	public static class CharacterPreview
	{
		...
		public static void WormPet(Projectile proj, bool walking)
		{
			...
			for (int i = 0; i < proj.oldPos.Length; i++)
			{
				...
				vector = vector.RotatedBy(-0.052359879016876221);
			}
	...
	public static class Minecart
	{
		...
		{
			dustPosition += new Vector2((Main.rand.Next(2) == 0) ? 13 : (-13), 0f).RotatedBy(rotation);
			int result = Dust.NewDust(dustPosition, 1, 1, 211, Main.rand.Next(-2, 3), Main.rand.Next(-2, 3), 50, default(Color), 0.8f);
			if (Main.rand.Next(2) == 0)
			{
				Main.dust[result].alpha += 25;
			}
			if (Main.rand.Next(2) == 0)
			{
				Main.dust[result].alpha += 25;
			}
			Main.dust[result].noLight = true;
			Main.dust[result].noGravity = Main.rand.Next(3) == 0;
			Main.dust[result].velocity *= (float)Main.rand.Next(15, 51) * 0.01f;
			Main.dust[result].velocity.X *= (float)Main.rand.Next(25, 101) * 0.01f;
			Main.dust[result].velocity.Y -= (float)Main.rand.Next(15, 31) * 0.1f;
			Main.dust[result].position.Y -= 4f;
		}
	...
	public static void TurretLaserDraw(int stage, Vector2 currentPosition, float distanceLeft, Rectangle lastFrame, out float distCovered, out Rectangle frame, out Vector2 origin, out Color color)
	{
		...
		case 1:
		{
			i_1++;
			int value = i_1 % 5;
			frame = new Rectangle(0, 22 * (value + 1), 22, 20);
