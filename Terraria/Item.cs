...
public class Item : Entity
{
	...
	public bool CanBeQuickUsed
	{
		get
		{
			...
			bool? point = ItemID.Sets.CanBeQuickusedOnGamepad[type];
			if (point.HasValue)
			{
				return point.Value;
			}
	...
	public static bool IsAGolfingItem(Item item)
	{
		...
		int i = item.type;
		if (i == 4039 || (uint)(i - 4092) <= 3u || (uint)(i - 4587) <= 11u)
		{
			return true;
		}
		return false;
	...
	private void SetFoodDefaults(int type)
	{
		...
		float amount = 0f;
		amount = ((buffType == 207) ? (amount + 12f) : ((buffType != 206) ? (amount + 3f) : (amount + 8f)));
		amount += (float)(buffTime / 14400);
		rare = (int)(amount / 4f);
	}
	...
	public Color GetAlpha(Color newColor)
	{
		...
		default:
		{
			float result = (float)(255 - alpha) / 255f;
			int r = (int)((float)(int)newColor.R * result);
			int g = (int)((float)(int)newColor.G * result);
			int b = (int)((float)(int)newColor.B * result);
	...
	{
		int result = color.R - (255 - newColor.R);
		int num = color.G - (255 - newColor.G);
		int num2 = color.B - (255 - newColor.B);
		int num3 = color.A - (255 - newColor.A);
		if (result < 0)
		{
			result = 0;
		}
		...
		return new Color(result, num, num2, num3);
	}
	...
	{
		int num = ShimmerTransforms.GetDecraftingRecipeIndex(GetShimmerEquivalentType());
		if (num < 0)
		{
			return -1;
		}
		return stack / Main.recipe[num].createItem.stack;
	}
	...
	public void UpdateItem(int i)
	{
		...
		if (!beingGrabbed)
		{
			...
			MoveInWorld(result, maxFallSpeed, ref wetVelocity, i);
			if (lavaWet)
			{
				CheckLavaDeath(i);
			}
	...
	private void DespawnIfMeetingConditions(int i)
	{
		...
		{
			int balance = Main.rand.Next(18, 24);
			for (int boost = 0; boost < balance; boost++)
			{
				int bossAdjustment = Dust.NewDust(base.Center, 0, 0, 61, 0f, 0f, 0, default(Color), 1.7f);
				Main.dust[bossAdjustment].velocity *= 8f;
				Main.dust[bossAdjustment].velocity.Y -= 1f;
				Main.dust[bossAdjustment].position = Vector2.Lerp(Main.dust[bossAdjustment].position, base.Center, 0.5f);
				Main.dust[bossAdjustment].noGravity = true;
				Main.dust[bossAdjustment].noLight = true;
			}
			active = false;
			type = 0;
			stack = 0;
	...
	private void CheckLavaDeath(int i)
	{
		if (type == 267)
		{
			...
			for (int j = 0; j < 200; j++)
			{
				...
				{
					int num = -Main.npc[j].direction;
					if (Main.npc[j].IsNPCValidForBestiaryKillCredit())
					{
						Main.BestiaryTracker.Kills.RegisterKill(Main.npc[j]);
					}
					Main.npc[j].StrikeNPCNoInteraction(9999, 10f, -num);
					balance--;
					flag = true;
					if (Main.netMode == 2)
					{
						NetMessage.SendData(28, -1, -1, null, j, 9999f, 10f, -num);
					}
			...
			if (flag)
			{
				...
				while (balance > 0 && list.Count > 0)
				{
					int index = Main.rand.Next(list.Count);
					int num2 = list[index];
					list.RemoveAt(index);
					int num3 = -Main.npc[num2].direction;
					if (Main.npc[num2].IsNPCValidForBestiaryKillCredit())
					{
						Main.BestiaryTracker.Kills.RegisterKill(Main.npc[num2]);
					}
					Main.npc[num2].StrikeNPCNoInteraction(9999, 10f, -num3);
					balance--;
					if (Main.netMode == 2)
					{
						NetMessage.SendData(28, -1, -1, null, num2, 9999f, 10f, -num3);
					}
				}
			}
			NetMessage.SendData(21, -1, -1, null, i);
	...
	private void MoveInWorld(float gravity, float maxFallSpeed, ref Vector2 wetVelocity, int i)
	{
		...
		bool balance = Collision.LavaCollision(position, width, height);
		if (balance)
		{
			lavaWet = true;
		}
	...
	private void GetPickedUpByMonsters_Special(int i)
	{
		...
		bool j = false;
		int num2 = type;
		if ((num2 == 89 || num2 == 3507) && !NPC.unlockedSlimeCopperSpawn)
		{
			num = true;
			j = true;
		}
		...
		{
			NPC nPC = Main.npc[k];
			if (nPC.active && num && nPC.type >= 0 && nPC.type < NPCID.Count && NPCID.Sets.CanConvertIntoCopperSlimeTownNPC[nPC.type] && hitbox.Intersects(nPC.Hitbox))
			{
				flag = true;
				NPC.TransformCopperSlime(k);
				break;
			}
		}
		if (flag)
	...
	{
		Rectangle flag = new Rectangle((int)position.X, (int)position.Y, width, height);
		for (int num = 0; num < 200; num++)
		{
			NPC num2 = Main.npc[num];
			if (!num2.active || num2.lifeMax <= 5 || num2.friendly || num2.immortal || num2.dontTakeDamage || NPCID.Sets.CantTakeLunchMoney[num2.type])
			{
				continue;
			}
			float num3 = stack;
			...
			num3 *= num4;
			float num5 = num2.extraValue;
			int num6 = num2.realLife;
			if (num6 >= 0 && Main.npc[num6].active)
			{
				num5 = Main.npc[num6].extraValue;
			}
			else
			{
				num6 = -1;
			}
			if (!(num5 < num3) || !(num5 + num3 < 9.99E+08f))
			{
				continue;
			}
			Rectangle rectangle = new Rectangle((int)num2.position.X, (int)num2.position.Y, num2.width, num2.height);
			if (flag.Intersects(rectangle))
			{
				float num7 = (float)Main.rand.Next(50, 76) * 0.01f;
				if (type == 71)
				{
					num7 += (float)Main.rand.Next(51) * 0.01f;
				}
				if (type == 72)
				{
					num7 += (float)Main.rand.Next(26) * 0.01f;
				}
				if (num7 > 1f)
				{
					num7 = 1f;
				}
				int num8 = (int)((float)stack * num7);
				if (num8 < 1)
				{
					num8 = 1;
				}
				...
				stack -= num8;
				int num9 = (int)((float)num8 * num4);
				int number = num;
				if (num6 >= 0)
				{
					number = num6;
				}
				num2.extraValue += num9;
				if (Main.netMode == 0)
				{
					num2.moneyPing(position);
				}
				else
				{
					NetMessage.SendData(92, -1, -1, null, number, num9, position.X, position.Y);
				}
				if (stack <= 0)
				{
					SetDefaults();
	...
	private void CombineWithNearbyItems(int myItemIndex)
	{
		...
		for (int num2 = myItemIndex + 1; num2 < 400; num2++)
		{
			Item item = Main.item[num2];
			...
			float num3 = Math.Abs(position.X + (float)(width / 2) - (item.position.X + (float)(item.width / 2))) + Math.Abs(position.Y + (float)(height / 2) - (item.position.Y + (float)(item.height / 2)));
			int num4 = 30;
	...
	private void UpdateItem_VisualEffects()
	{
		if (type == 5043)
		{
			float i = (float)Main.rand.Next(90, 111) * 0.01f;
			i *= (Main.essScale + 0.5f) / 2f;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.25f * i, 0.25f * i, 0.25f * i);
		}
		else if (type == 3191)
		{
			float num = (float)Main.rand.Next(90, 111) * 0.01f;
			num *= (Main.essScale + 0.5f) / 2f;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.3f * num, 0.1f * num, 0.25f * num);
		}
		else if (type == 520 || type == 3454)
		{
			float num2 = (float)Main.rand.Next(90, 111) * 0.01f;
			num2 *= Main.essScale;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.5f * num2, 0.1f * num2, 0.25f * num2);
		}
		else if (type == 521 || type == 3455)
		{
			float num3 = (float)Main.rand.Next(90, 111) * 0.01f;
			num3 *= Main.essScale;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.25f * num3, 0.1f * num3, 0.5f * num3);
		}
		else if (type == 547 || type == 3453)
		{
			float num4 = (float)Main.rand.Next(90, 111) * 0.01f;
			num4 *= Main.essScale;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.5f * num4, 0.3f * num4, 0.05f * num4);
		}
		else if (type == 548)
		{
			float num5 = (float)Main.rand.Next(90, 111) * 0.01f;
			num5 *= Main.essScale;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.1f * num5, 0.1f * num5, 0.6f * num5);
		}
		else if (type == 575)
		{
			float num6 = (float)Main.rand.Next(90, 111) * 0.01f;
			num6 *= Main.essScale;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.1f * num6, 0.3f * num6, 0.5f * num6);
		}
		else if (type == 549)
		{
			float num7 = (float)Main.rand.Next(90, 111) * 0.01f;
			num7 *= Main.essScale;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.1f * num7, 0.5f * num7, 0.2f * num7);
		}
		else if (type == 58 || type == 1734 || type == 1867)
		{
			float num8 = (float)Main.rand.Next(90, 111) * 0.01f;
			num8 *= Main.essScale * 0.5f;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.5f * num8, 0.1f * num8, 0.1f * num8);
		}
		else if (type == 184 || type == 1735 || type == 1868 || type == 4143)
		{
			float num9 = (float)Main.rand.Next(90, 111) * 0.01f;
			num9 *= Main.essScale * 0.5f;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.1f * num9, 0.1f * num9, 0.5f * num9);
		}
		else if (type == 522)
		{
			float num10 = (float)Main.rand.Next(90, 111) * 0.01f;
			num10 *= Main.essScale * 0.2f;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 0.5f * num10, 1f * num10, 0.1f * num10);
		}
		else if (type == 1332)
		{
			float num11 = (float)Main.rand.Next(90, 111) * 0.01f;
			num11 *= Main.essScale * 0.2f;
			Lighting.AddLight((int)((position.X + (float)(width / 2)) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), 1f * num11, 1f * num11, 0.1f * num11);
		}
		else if (type == 3456)
		{
		...
		else if (type == 501)
		{
			...
			{
				int num12 = Dust.NewDust(position, width, height, 55, 0f, 0f, 200, color);
		...
		else if (type >= 5140 && type <= 5146)
		{
			...
			Lighting.AddLight((int)((position.X + (float)width) / 16f), (int)((position.Y + (float)(height / 2)) / 16f), num14, num15, num16);
		}
		else if (type == 282)
		{
	...
	public static int NewItem(IEntitySource source, int X, int Y, int Width, int Height, int Type, int Stack = 1, bool noBroadcast = false, int pfix = 0, bool noGrabDelay = false, bool reverseLookup = false)
	{
		...
		if (Main.netMode != 1)
		{
			i = PickAnItemSlotToSpawnItemOn(reverseLookup, i);
		}
		Main.timeItemSlotCannotBeReusedFor[i] = 0;
		...
		{
			NetMessage.SendData(21, -1, -1, null, i, noGrabDelay.ToInt());
		}
	...
	private static int PickAnItemSlotToSpawnItemOn(bool reverseLookup, int nextItem)
	{
		...
		if (!flag)
		{
			...
			for (int k = 0; k < 400; k++)
			{
				if (Main.timeItemSlotCannotBeReusedFor[k] == 0 && !Main.item[k].instanced && Main.item[k].timeSinceItemSpawned > num2)
				{
					num2 = Main.item[k].timeSinceItemSpawned;
					nextItem = k;
					flag = true;
				}
			}
			if (!flag)
			{
				for (int l = 0; l < 400; l++)
				{
					if (Main.item[l].timeSinceItemSpawned - Main.timeItemSlotCannotBeReusedFor[l] > num2)
					{
						num2 = Main.item[l].timeSinceItemSpawned - Main.timeItemSlotCannotBeReusedFor[l];
						nextItem = l;
					}
				}
			}
	...
	public void FindOwner(int whoAmI)
	{
		...
		else if (vector)
		{
			...
			for (int num3 = 0; num3 < 255; num3++)
			{
				...
				Player num4 = Main.player[num3];
				if (!num4.active)
				{
					continue;
				}
				Player.ItemSpaceStatus status = num4.ItemSpace(Main.item[whoAmI]);
				if (num4.CanPullItem(Main.item[whoAmI], status))
				{
					float num5 = Math.Abs(num4.position.X + (float)(num4.width / 2) - position.X - (float)(width / 2)) + Math.Abs(num4.position.Y + (float)(num4.height / 2) - position.Y - (float)height);
					if (num4.manaMagnet && (type == 184 || type == 1735 || type == 1868))
					{
						num5 -= (float)manaGrabRange;
					}
					if (num4.lifeMagnet && (type == 58 || type == 1734 || type == 1867))
					{
						num5 -= (float)lifeGrabRange;
					}
					if (type == 4143)
					{
						num5 -= (float)manaGrabRange;
					...
					{
						num2 = num5;
						playerIndexTheItemIsReservedFor = num3;
					}
				}
			}
		}
	...
	public void Refresh(bool onlyIfVariantChanged = true)
	{
		...
		{
			bool moveSpeed = favorited;
			int scaleFactor = stack;
			int num = netID;
			int num2 = prefix;
			netDefaults(num);
			Prefix(num2);
