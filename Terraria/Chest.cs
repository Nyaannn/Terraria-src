...
public class Chest : IFixLoadedData
{
	...
	public static void ServerPlaceItem(int plr, int slot)
	{
		...
		{
			int ret = slot - PlayerItemSlotID.Bank4_0;
			Main.player[plr].bank4.item[ret] = PutItemInNearbyChest(Main.player[plr].bank4.item[ret], Main.player[plr].Center);
			NetMessage.SendData(5, -1, -1, null, plr, slot, (int)Main.player[plr].bank4.item[ret].prefix);
		}
		else if (slot < 58)
		{
	...
	public static bool Unlock(int X, int Y)
	{
		...
		for (int i = X; i <= X + 1; i++)
		{
			for (int ele = Y; ele <= Y + 1; ele++)
			{
				Tile txt = Framing.GetTileSafely(i, ele);
				if (txt.type == response)
				{
					txt.frameX -= ret;
					Main.tile[i, ele] = txt;
					for (int j = 0; j < 4; j++)
					{
						Dust.NewDust(new Vector2(i * 16, ele * 16), 16, 16, errorCode);
					}
				}
			}
		}
	...
	{
		bool flag = Main.LocalPlayer.currentShoppingSettings.PriceAdjustment <= 0.89999997615814209;
		Item[] array = item;
		for (int i = 0; i < 40; i++)
		{
			array[i] = new Item();
