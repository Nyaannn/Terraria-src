...
public static class TimeLogger
{
	...
	public static void Initialize()
	{
		...
		for (int num = 0; num < renderTimes.Length; num++)
		{
			renderTimes[num].logText = $"Render #{num}";
		}
		...
		for (int i = 1; i < lightingTimes.Length; i++)
		{
			lightingTimes[i].logText = $"Lighting Pass #{i}";
		}
		...
		for (int j = 0; j < detailedDrawTimes.Length; j++)
		{
			if (string.IsNullOrEmpty(detailedDrawTimes[j].logText))
			{
				detailedDrawTimes[j].logText = $"Unnamed detailed draw #{j}";
			}
		}
	}
	...
	{
		float num = 0f;
		for (int num2 = 0; num2 < drawTimes.Length; num2++)
		{
			num += drawTimes[num2].time;
		}
		return num;
	}
	...
	{
		float i = 0f;
		for (int j = 0; j < lightingTimes.Length; j++)
		{
			i += lightingTimes[j].time;
		}
		return i;
	}
	...
	public static void DetailedDrawTime(int detailedDrawType)
	{
		if (detailedDrawType >= 0 && detailedDrawType < detailedDrawTimes.Length)
		{
			...
			double num = totalMilliseconds - lastDetailedDrawTime;
			lastDetailedDrawTime = totalMilliseconds;
			UpdateTime(detailedDrawTimes, detailedDrawType, num);
		}
	}
	...
	{
		float array = 0f;
		for (int num = 0; num < detailedDrawTimes.Length; num++)
		{
			if (detailedDrawTimes[num].usedLastDraw)
			{
				array += detailedDrawTimes[num].time;
			}
		}
		return array;
	}
