...
public class NetMessage
{
	...
	public static void SendData(int msgType, int remoteClient = -1, int ignoreClient = -1, NetworkText text = null, int number = 0, float number2 = 0f, float number3 = 0f, float number4 = 0f, int number5 = 0, int number6 = 0, int number7 = 0)
	{
		...
		lock (buffer[obj])
		{
			BinaryWriter writer = buffer[obj].writer;
			if (writer == null)
			{
				buffer[obj].ResetWriter();
				writer = buffer[obj].writer;
			}
			...
			case 2:
				...
				if (Main.dedServ)
				{
					Console.WriteLine(Language.GetTextValue("CLI.ClientWasBooted", Netplay.Clients[obj].Socket.GetRemoteAddress().ToString(), text));
				}
				break;
			case 3:
			...
			case 7:
			{
				...
				{
					writer.Write((byte)Main.treeStyle[num7]);
				}
				for (int num8 = 0; num8 < 3; num8++)
				{
					writer.Write(Main.caveBackX[num8]);
				}
			...
			case 33:
			{
				...
				if (number > -1)
				{
					num4 = Main.chest[number].x;
					num5 = Main.chest[number].y;
				}
				if (number2 == 1f)
				{
					string text3 = text.ToString();
					num6 = (byte)text3.Length;
					if (num6 == 0 || num6 > 20)
					{
						num6 = 255;
					}
			...
			case 72:
			{
				...
				{
					writer.Write((short)Main.travelShop[num19]);
				}
				break;
			}
			case 73:
			...
			{
				writer.Write((byte)number);
				Player obj2 = Main.player[number];
				obj2.piggyBankProjTracker.Write(writer);
				obj2.voidLensChest.Write(writer);
				break;
			}
			case 146:
				writer.Write((byte)number);
			...
			if (num20 > 65535)
			{
				throw new Exception("Maximum packet length exceeded. id: " + msgType + " length: " + num20);
			}
			...
			if (Main.netMode == 1)
			{
				if (Netplay.Connection.Socket.IsConnected())
				{
					try
					{
						buffer[obj].spamCount++;
						Main.ActiveNetDiagnosticsUI.CountSentMessage(msgType, num20);
						Netplay.Connection.Socket.AsyncSend(buffer[obj].writeBuffer, 0, num20, Netplay.Connection.ClientWriteCallBack);
					}
					catch
					{
					}
			...
			else if (remoteClient == -1)
			{
				...
				case 69:
				{
					...
					{
						if (num22 != ignoreClient && buffer[num22].broadcast && Netplay.Clients[num22].IsConnected())
						{
							try
							{
								buffer[num22].spamCount++;
								Main.ActiveNetDiagnosticsUI.CountSentMessage(msgType, num20);
								Netplay.Clients[num22].Socket.AsyncSend(buffer[obj].writeBuffer, 0, num20, Netplay.Clients[num22].ServerWriteCallBack);
							}
							catch
							{
							}
				...
				case 20:
				{
					...
					{
						if (num26 != ignoreClient && buffer[num26].broadcast && Netplay.Clients[num26].IsConnected() && Netplay.Clients[num26].SectionRange((int)Math.Max(number3, number4), number, (int)number2))
						{
							try
							{
								buffer[num26].spamCount++;
								Main.ActiveNetDiagnosticsUI.CountSentMessage(msgType, num20);
								Netplay.Clients[num26].Socket.AsyncSend(buffer[obj].writeBuffer, 0, num20, Netplay.Clients[num26].ServerWriteCallBack);
							}
							catch
							{
							}
				...
				case 23:
				{
					...
					for (int num27 = 0; num27 < 256; num27++)
					{
						...
						{
							Rectangle rect5 = Main.player[num27].getRect();
						...
						if (flag6)
						{
							try
							{
								...
								Main.ActiveNetDiagnosticsUI.CountSentMessage(msgType, num20);
								Netplay.Clients[num27].Socket.AsyncSend(buffer[obj].writeBuffer, 0, num20, Netplay.Clients[num27].ServerWriteCallBack);
							}
							catch
							{
							}
					...
					for (int num24 = 0; num24 < 256; num24++)
					{
						...
						if (flag5)
						{
							try
							{
								...
								Main.ActiveNetDiagnosticsUI.CountSentMessage(msgType, num20);
								Netplay.Clients[num24].Socket.AsyncSend(buffer[obj].writeBuffer, 0, num20, Netplay.Clients[num24].ServerWriteCallBack);
							}
							catch
							{
							}
					...
					for (int num25 = 0; num25 < 256; num25++)
					{
						if (num25 != ignoreClient && buffer[num25].broadcast && Netplay.Clients[num25].IsConnected())
						{
							try
							{
								...
								Main.ActiveNetDiagnosticsUI.CountSentMessage(msgType, num20);
								Netplay.Clients[num25].Socket.AsyncSend(buffer[obj].writeBuffer, 0, num20, Netplay.Clients[num25].ServerWriteCallBack);
							}
							catch
							{
							}
					...
					for (int num23 = 0; num23 < 256; num23++)
					{
						...
						if (flag4)
						{
							try
							{
								...
								Main.ActiveNetDiagnosticsUI.CountSentMessage(msgType, num20);
								Netplay.Clients[num23].Socket.AsyncSend(buffer[obj].writeBuffer, 0, num20, Netplay.Clients[num23].ServerWriteCallBack);
							}
							catch
							{
							}
				...
				default:
				{
					...
					{
						if (num21 != ignoreClient && (buffer[num21].broadcast || (Netplay.Clients[num21].State >= 3 && msgType == 10)) && Netplay.Clients[num21].IsConnected())
						{
							try
							{
								buffer[num21].spamCount++;
								Main.ActiveNetDiagnosticsUI.CountSentMessage(msgType, num20);
								Netplay.Clients[num21].Socket.AsyncSend(buffer[obj].writeBuffer, 0, num20, Netplay.Clients[num21].ServerWriteCallBack);
							}
							catch
							{
							}
			...
			else if (Netplay.Clients[remoteClient].IsConnected())
			{
				try
				{
					...
					Main.ActiveNetDiagnosticsUI.CountSentMessage(msgType, num20);
					Netplay.Clients[remoteClient].Socket.AsyncSend(buffer[obj].writeBuffer, 0, num20, Netplay.Clients[remoteClient].ServerWriteCallBack);
				}
				catch
				{
				}
			...
			if (Main.verboseNetplay)
			{
				...
				{
					_ = buffer[obj].writeBuffer[num29];
				}
			}
			buffer[obj].writeLocked = false;
			if (msgType == 2 && Main.netMode == 2)
			{
				Netplay.Clients[obj].PendingTermination = true;
				Netplay.Clients[obj].PendingTerminationApproved = true;
			}
		}
	}
	...
	public static void CompressTileBlock(int xStart, int yStart, short width, short height, Stream stream)
	{
		...
		DeflateStream obj = new DeflateStream(stream, (CompressionMode)0, true);
		try
		{
			BinaryWriter binaryWriter = new BinaryWriter((Stream)(object)obj);
		...
		for (int i = yStart; i < yStart + height; i++)
		{
			for (int j = xStart; j < xStart + width; j++)
			{
				...
				if (tile != null)
				{
					if (num3 > 0)
					{
						array[num4] = (byte)((uint)num3 & 0xFFu);
						num4++;
						if (num3 > 255)
						{
							b = (byte)(b | 0x80u);
							array[num4] = (byte)((num3 & 0xFF00) >> 8);
							num4++;
						}
						else
						{
					...
					array[num5] = b;
					writer.Write(array, num5, num4 - num5);
					num3 = 0;
				}
				num4 = 4;
				byte b3;
				...
				{
					b = (byte)(b | 2u);
					array[num4] = (byte)tile2.type;
					num4++;
					if (tile2.type > 255)
					{
						array[num4] = (byte)(tile2.type >> 8);
						num4++;
						b = (byte)(b | 0x20u);
					}
					if (TileID.Sets.BasicChest[tile2.type] && tile2.frameX % 36 == 0 && tile2.frameY % 36 == 0)
					{
						short num6 = (short)Chest.FindChest(j, i);
						if (num6 != -1)
						{
							_compressChestList[obj] = num6;
							obj = (short)(obj + 1);
						}
					}
					if (tile2.type == 88 && tile2.frameX % 54 == 0 && tile2.frameY % 36 == 0)
					{
						...
						if (num7 != -1)
						{
							_compressChestList[obj] = num7;
							obj = (short)(obj + 1);
						}
					}
					if (tile2.type == 85 && tile2.frameX % 36 == 0 && tile2.frameY % 36 == 0)
					{
						short num8 = (short)Sign.ReadSign(j, i);
						if (num8 != -1)
						{
							_compressSignList[num++] = num8;
						}
					}
					...
					if (tile2.type == 378 && tile2.frameX % 36 == 0 && tile2.frameY == 0)
					{
						...
						if (num12 != -1)
						{
							_compressEntities[num2++] = (short)num12;
						}
					}
					if (tile2.type == 395 && tile2.frameX % 36 == 0 && tile2.frameY == 0)
					{
						...
						if (num13 != -1)
						{
							_compressEntities[num2++] = (short)num13;
						}
					}
					if (tile2.type == 520 && tile2.frameX % 18 == 0 && tile2.frameY == 0)
					{
						int num14 = TEFoodPlatter.Find(j, i);
						if (num14 != -1)
						{
							_compressEntities[num2++] = (short)num14;
						}
					}
					if (tile2.type == 471 && tile2.frameX % 54 == 0 && tile2.frameY == 0)
					{
						int num15 = TEWeaponsRack.Find(j, i);
						if (num15 != -1)
						{
							_compressEntities[num2++] = (short)num15;
						}
					}
					if (tile2.type == 470 && tile2.frameX % 36 == 0 && tile2.frameY == 0)
					{
						int num16 = TEDisplayDoll.Find(j, i);
						if (num16 != -1)
						{
							_compressEntities[num2++] = (short)num16;
						}
					}
					if (tile2.type == 475 && tile2.frameX % 54 == 0 && tile2.frameY == 0)
					{
						int num17 = TEHatRack.Find(j, i);
						if (num17 != -1)
						{
							_compressEntities[num2++] = (short)num17;
						}
					}
					if (tile2.type == 597 && tile2.frameX % 54 == 0 && tile2.frameY % 72 == 0)
					{
						int num18 = TETeleportationPylon.Find(j, i);
						if (num18 != -1)
						{
							_compressEntities[num2++] = (short)num18;
						}
					}
					if (Main.tileFrameImportant[tile2.type])
					{
						array[num4] = (byte)((uint)tile2.frameX & 0xFFu);
						num4++;
						array[num4] = (byte)((tile2.frameX & 0xFF00) >> 8);
						num4++;
						array[num4] = (byte)((uint)tile2.frameY & 0xFFu);
						num4++;
						array[num4] = (byte)((tile2.frameY & 0xFF00) >> 8);
						num4++;
					}
					if (tile2.color() != 0)
					{
						b3 = (byte)(b3 | 8u);
						array[num4] = tile2.color();
						num4++;
					}
				}
				if (tile2.wall != 0)
				{
					b = (byte)(b | 4u);
					array[num4] = (byte)tile2.wall;
					num4++;
					if (tile2.wallColor() != 0)
					{
						b3 = (byte)(b3 | 0x10u);
						array[num4] = tile2.wallColor();
						num4++;
					}
				}
				if (tile2.liquid != 0)
				...
				int num19 = (tile2.halfBrick() ? 16 : ((tile2.slope() != 0) ? (tile2.slope() + 1 << 4) : 0));
				b4 = (byte)(b4 | (byte)num19);
				if (tile2.actuator())
				{
					b3 = (byte)(b3 | 2u);
				}
	...
	public static void DecompressTileBlock(Stream stream)
	{
		...
		DeflateStream obj = new DeflateStream(stream, (CompressionMode)1, true);
		try
		{
			BinaryReader binaryReader = new BinaryReader((Stream)(object)obj);
		...
		for (int i = yStart; i < yStart + height; i++)
		{
			for (int j = xStart; j < xStart + width; j++)
			{
				if (num != 0)
				{
					...
					{
						Main.tile[j, i] = new Tile(obj);
					}
					else
					{
						Main.tile[j, i].CopyFrom(obj);
					}
					continue;
				}
				byte b2;
				...
				byte startsWith = reader.ReadByte();
				bool flag = false;
				if ((startsWith & 1) == 1)
				{
					flag = true;
				...
				bool flag3 = obj.active();
				byte b4;
				if ((startsWith & 2) == 2)
				{
					obj.active(active: true);
					ushort type = obj.type;
					int num2;
					if ((startsWith & 0x20) == 32)
					{
						b4 = reader.ReadByte();
					...
					obj.type = (ushort)num2;
					if (Main.tileFrameImportant[num2])
					{
						obj.frameX = reader.ReadInt16();
						obj.frameY = reader.ReadInt16();
					}
					else if (!flag3 || obj.type != type)
					{
						obj.frameX = -1;
						obj.frameY = -1;
					}
					if ((b2 & 8) == 8)
					{
						obj.color(reader.ReadByte());
					}
				}
				if ((startsWith & 4) == 4)
				{
					obj.wall = reader.ReadByte();
					if ((b2 & 0x10) == 16)
					{
						obj.wallColor(reader.ReadByte());
					}
				}
				b4 = (byte)((startsWith & 0x18) >> 3);
				if (b4 != 0)
				{
					obj.liquid = reader.ReadByte();
					if ((b2 & 0x80) == 128)
					{
						obj.shimmer(shimmer: true);
					}
					else if (b4 > 1)
					{
						if (b4 == 2)
				...
				if (b3 > 1)
				{
					...
					if ((b3 & 4) == 4)
					{
						obj.wire2(wire2: true);
					}
					if ((b3 & 8) == 8)
					{
						obj.wire3(wire3: true);
					}
					b4 = (byte)((b3 & 0x70) >> 4);
					if (b4 != 0 && Main.tileSolid[obj.type])
					{
						if (b4 == 1)
						{
							obj.halfBrick(halfBrick: true);
						}
						else
						{
							obj.slope((byte)(b4 - 1));
				...
				if (b2 > 1)
				{
					if ((b2 & 2) == 2)
					{
						obj.actuator(actuator: true);
					}
					if ((b2 & 4) == 4)
					{
						obj.inActive(inActive: true);
					}
					if ((b2 & 0x20) == 32)
					{
						obj.wire4(wire4: true);
					}
					if ((b2 & 0x40) == 64)
					{
						b4 = reader.ReadByte();
						obj.wall = (ushort)((b4 << 8) | obj.wall);
					}
				}
				if (b > 1)
				{
					if ((b & 2) == 2)
					{
						obj.invisibleBlock(invisibleBlock: true);
					}
					if ((b & 4) == 4)
					{
						obj.invisibleWall(invisibleWall: true);
					}
					if ((b & 8) == 8)
					{
						obj.fullbrightBlock(fullbrightBlock: true);
					}
					if ((b & 0x10) == 16)
					{
						obj.fullbrightWall(fullbrightWall: true);
					}
				}
				num = (byte)((startsWith & 0xC0) >> 6) switch
				{
					0 => 0, 
					1 => reader.ReadByte(), 
					_ => reader.ReadInt16(), 
	...
	public static void SendObjectPlacement(int whoAmi, int x, int y, int type, int style, int alternative, int random, int direction)
	{
		...
		SendData(79, obj, ignoreClient, null, x, y, type, style, alternative, random, direction);
	}
	...
	{
		int i = (centeredSquareSize - 1) / 2;
		SendTileSquare(whoAmi, tileX - i, tileY - i, centeredSquareSize, centeredSquareSize, changeType);
	}
	...
	public static void SendTileSquare(int whoAmi, int tileX, int tileY, TileChangeType changeType = TileChangeType.None)
	{
		int i = 1;
		int j = (i - 1) / 2;
		SendTileSquare(whoAmi, tileX - j, tileY - j, i, i, changeType);
	}
	...
	public static void SendAnglerQuest(int remoteClient)
	{
		...
		if (remoteClient == -1)
		{
			for (int array = 0; array < 255; array++)
			{
				if (Netplay.Clients[array].State == 10)
				{
					SendData(74, array, -1, NetworkText.FromLiteral(Main.player[array].name), Main.anglerQuest);
				}
			}
		}
	...
	public static void SendSection(int whoAmi, int sectionX, int sectionY)
	{
		...
		try
		{
			...
			int str = sectionX * 200;
			int num = sectionY * 150;
			int num2 = 150;
			for (int num3 = num; num3 < num + 150; num3 += num2)
			{
				SendData(10, whoAmi, -1, null, str, num3, 200f, num2);
			}
			for (int i = 0; i < 200; i++)
			{
