...
public class Gore
{
	...
	public byte numFrames
	{
		...
		set
		{
			SpriteFrame i = new SpriteFrame(Frame.ColumnCount, value);
			i.CurrentColumn = Frame.CurrentColumn;
			i.CurrentRow = Frame.CurrentRow;
			SpriteFrame num = (Frame = i);
		}
	}
	...
	private void UpdateAmbientFloorCloud()
	{
		...
		bool i = false;
		Point deprecatedClassLeftInForLoading = (position + new Vector2(15f, 0f)).ToTileCoordinates();
		Tile tile = Main.tile[deprecatedClassLeftInForLoading.X, deprecatedClassLeftInForLoading.Y];
		Tile tile2 = Main.tile[deprecatedClassLeftInForLoading.X, deprecatedClassLeftInForLoading.Y + 1];
		Tile tile3 = Main.tile[deprecatedClassLeftInForLoading.X, deprecatedClassLeftInForLoading.Y + 2];
	...
	{
		Vector2 dust = position + new Vector2(12f) / 2f - new Vector2(4f) / 2f;
		dust.Y -= 4f;
		Vector2 vector = position - dust;
		if (velocity.Y < 0f)
		{
			Vector2 vector2 = new Vector2(velocity.X, -0.2f);
			int num = 4;
			num = (int)((float)num * 0.9f);
			Point point = (new Vector2(num, num) / 2f + dust).ToTileCoordinates();
			if (!WorldGen.InWorld(point.X, point.Y))
			{
				active = false;
				return;
			...
			Rectangle value = new Rectangle((int)dust.X, (int)dust.Y + num2, num, num);
			...
			vector2 = Collision.TileCollision(dust, vector2, num, num);
			if (flag)
			{
		...
		Vector2 vector3 = new Vector2(Vector2.UnitY.RotatedBy(velocity.Y).X * 1f, Math.Abs(Vector2.UnitY.RotatedBy(velocity.Y).Y) * 1f);
		int num3 = 4;
		if ((double)position.Y < Main.worldSurface * 16.0)
		{
		...
		vector3 = Collision.TileCollision(dust, vector3, num3, num3);
		Vector4 vector5 = Collision.SlopeCollision(dust, vector3, num3, num3, 1f);
		...
		Rectangle value2 = new Rectangle((int)dust.X, (int)dust.Y + num4, num3, num3);
	...
	private void Gore_UpdateSail()
	{
		if (velocity.Y < 0f)
		{
			Vector2 num = new Vector2(velocity.X, 0.6f);
			int rectangle = 32;
			if (TextureAssets.Gore[type].get_IsLoaded())
			{
				rectangle = TextureAssets.Gore[type].Width();
				if (TextureAssets.Gore[type].Height() < rectangle)
				{
					rectangle = TextureAssets.Gore[type].Height();
				}
			}
			rectangle = (int)((float)rectangle * 0.9f);
			num = Collision.TileCollision(position, num, (int)((float)rectangle * scale), (int)((float)rectangle * scale));
			num.X *= 0.97f;
			if ((double)num.X > -0.01 && (double)num.X < 0.01)
			{
				num.X = 0f;
			}
			if (timeLeft > 0)
		...
		Vector2 value = new Vector2(Vector2.UnitY.RotatedBy(velocity.Y).X * 2f, Math.Abs(Vector2.UnitY.RotatedBy(velocity.Y).Y) * 3f);
		value *= 2f;
		int result = 32;
		if (TextureAssets.Gore[type].get_IsLoaded())
		{
			result = TextureAssets.Gore[type].Width();
			if (TextureAssets.Gore[type].Height() < result)
			{
				result = TextureAssets.Gore[type].Height();
			}
		}
		Vector2 vector = value;
		value = Collision.TileCollision(position, value, (int)((float)result * scale), (int)((float)result * scale));
		if (value != vector)
		{
			velocity.Y = -1f;
		}
		position += value;
		rotation = value.ToRotation() + (float)Math.PI;
		if (timeLeft > 0)
		{
			timeLeft--;
		}
