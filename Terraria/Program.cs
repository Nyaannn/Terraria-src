...
public static class Program
{
	...
	{
		Type[] num = assembly.GetTypes();
		foreach (Type type in num)
		{
	...
	private static void ForceLoadAssembly(string name, bool initializeStaticMembers)
	{
		...
		if (flag == null)
		{
			flag = Assembly.Load(name);
		}
		ForceLoadAssembly(flag, initializeStaticMembers);
	}
	...
	private static void SetupLogging()
	{
		...
		{
			string num = LaunchParameters["-logfile"];
			num = ((num != null && !(num.Trim() == "")) ? Path.Combine(num, $"Log_{DateTime.Now:yyyyMMddHHmmssfff}.log") : Path.Combine(SavePath, "Logs", $"Log_{DateTime.Now:yyyyMMddHHmmssfff}.log"));
			ConsoleOutputMirror.ToFile(num);
		}
