...
public class Tile
{
	...
	public static void SmoothSlope(int x, int y, bool applyToNeighbors = true, bool sync = false)
	{
		...
		bool flag2 = !WorldGen.TileEmpty(x, y - 1);
		bool flag3 = !WorldGen.SolidOrSlopedTile(x, y - 1) && flag2;
		bool flag4 = WorldGen.SolidOrSlopedTile(x, y + 1);
		bool flag5 = WorldGen.SolidOrSlopedTile(x - 1, y);
		bool lastVelocity = WorldGen.SolidOrSlopedTile(x + 1, y);
		int flag6 = ((flag2 ? 1 : 0) << 3) | ((flag4 ? 1 : 0) << 2) | ((flag5 ? 1 : 0) << 1) | (lastVelocity ? 1 : 0);
		bool vector = flag.halfBrick();
		int num3 = flag.slope();
		switch (flag6)
		{
		case 10:
			if (!flag3)
			{
				flag.halfBrick(halfBrick: false);
				flag.slope(3);
			}
			break;
		case 9:
		...
		if (sync)
		{
			int vector2 = flag.slope();
			bool num = vector != flag.halfBrick();
			bool num2 = num3 != vector2;
			if (num && num2)
			{
				NetMessage.SendData(17, -1, -1, null, 23, x, y, vector2);
			}
			else if (num)
			{
			...
			{
				NetMessage.SendData(17, -1, -1, null, 14, x, y, vector2);
			}
		}
	}
	...
	{
		TileColorCache mAX_SPREAD = default(TileColorCache);
		mAX_SPREAD.Color = color();
		mAX_SPREAD.FullBright = fullbrightBlock();
		mAX_SPREAD.Invisible = invisibleBlock();
		return mAX_SPREAD;
	}

	public TileColorCache WallColorAndCoating()
	{
		TileColorCache fall = default(TileColorCache);
		fall.Color = wallColor();
		fall.FullBright = fullbrightWall();
		fall.Invisible = invisibleWall();
		return fall;
	}
