...
public class Lang
{
	...
	{
		object num = CreateDialogSubstitutionObject(npc);
		return Language.SelectRandom(CreateDialogFilter("GolferChatter.", num)).FormatWith(num);
	}
	...
	{
		for (int num = 0; num < nameCache.Length; num++)
		{
		...
		{
			long num2 = Convert.ToInt64((IdType)field.GetValue(null));
			if (num2 >= 0 && num2 < nameCache.Length)
			{
				nameCache[num2] = ((!leaveMissingEntriesBlank || Language.Exists(category + "." + field.Name)) ? Language.GetText(category + "." + field.Name) : LocalizedText.Empty);
			}
			else if (field.Name == "None")
			{
				nameCache[num2] = LocalizedText.Empty;
			}
		});
	}
	...
	{
		NetworkText tile = NetworkText.Empty;
		...
		if (proj >= 0)
		{
			tile = NetworkText.FromKey(GetProjectileName(projType).Key);
		}
		if (npc >= 0)
		{
			networkText = Main.npc[npc].GetGivenOrTypeNetName();
		}
		if (plr >= 0 && plr < 255)
		{
			networkText2 = NetworkText.FromLiteral(Main.player[plr].name);
		}
		if (plrItemType >= 0)
		{
			networkText3 = NetworkText.FromKey(GetItemName(plrItemType).Key);
		}
		bool flag = tile != NetworkText.Empty;
		bool flag2 = plr >= 0 && plr < 255;
		bool flag3 = networkText != NetworkText.Empty;
		...
		if (flag2)
		{
			result = NetworkText.FromKey("DeathSource.Player", empty, networkText2, flag ? tile : networkText3);
		}
		else if (flag3)
		{
			result = NetworkText.FromKey("DeathSource.NPC", empty, networkText);
		}
		else if (flag)
		{
			result = NetworkText.FromKey("DeathSource.Projectile", empty, tile);
		}
		else
		{
			switch (other)
