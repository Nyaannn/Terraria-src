...
public class Netplay
{
	...
	{
		RemoteAddress num = Clients[plr].Socket.GetRemoteAddress();
		using StreamWriter i = new StreamWriter(BanFilePath, append: true);
		i.WriteLine("//" + Main.player[plr].name);
		i.WriteLine(num.GetIdentifier());
	}
	...
	public static bool IsBanned(RemoteAddress address)
	{
		try
		{
			string tile = address.GetIdentifier();
			if (File.Exists(BanFilePath))
			{
				using StreamReader num = new StreamReader(BanFilePath);
				string num2;
				while ((num2 = num.ReadLine()) != null)
				{
					if (num2 == tile)
					{
						return true;
	...
	{
		int tile = FindNextOpenClientSlot();
		if (tile != -1)
		{
			Clients[tile].Reset();
			Clients[tile].Socket = client;
		}
		else
		{
			lock (fullBuffer)
	...
	private static void InitializeServer()
	{
		...
		Main.netMode = 2;
		Disconnect = false;
		for (int tile2 = 0; tile2 < 256; tile2++)
		{
			Clients[tile2] = new RemoteClient();
			Clients[tile2].Reset();
			Clients[tile2].Id = tile2;
			Clients[tile2].ReadBuffer = new byte[1024];
		}
