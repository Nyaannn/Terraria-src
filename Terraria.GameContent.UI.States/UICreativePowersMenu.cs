...
public class UICreativePowersMenu : UIState
{
	...
	{
		MenuTree<TimeSubcategory> i = _timeCategory;
		List<UIElement> j = new List<UIElement>();
		CreativePowerUIElementRequestInfo tile = default(CreativePowerUIElementRequestInfo);
		tile.PreferredButtonWidth = 40;
		tile.PreferredButtonHeight = 40;
		CreativePowerUIElementRequestInfo num = tile;
		CreativePowerManager.Instance.GetPower<CreativePowers.FreezeTime>().ProvidePowerButtons(num, j);
		CreativePowerManager.Instance.GetPower<CreativePowers.StartDayImmediately>().ProvidePowerButtons(num, j);
		CreativePowerManager.Instance.GetPower<CreativePowers.StartNoonImmediately>().ProvidePowerButtons(num, j);
		CreativePowerManager.Instance.GetPower<CreativePowers.StartNightImmediately>().ProvidePowerButtons(num, j);
		CreativePowerManager.Instance.GetPower<CreativePowers.StartMidnightImmediately>().ProvidePowerButtons(num, j);
		GroupOptionButton<int> num2 = CreateSubcategoryButton<CreativePowers.ModifyTimeRate>(ref num, 2, "strip 2", 1, 0, i.Buttons, i.Sliders);
		num2.OnLeftClick += TimeCategoryButtonClick;
		j.Add(num2);
		return j;
	}
	...
	{
		MenuTree<PersonalSubcategory> num = _personalCategory;
		List<UIElement> i = new List<UIElement>();
		CreativePowerUIElementRequestInfo y = default(CreativePowerUIElementRequestInfo);
		y.PreferredButtonWidth = 40;
		y.PreferredButtonHeight = 40;
		CreativePowerUIElementRequestInfo holeRadius = y;
		CreativePowerManager.Instance.GetPower<CreativePowers.GodmodePower>().ProvidePowerButtons(holeRadius, i);
		CreativePowerManager.Instance.GetPower<CreativePowers.FarPlacementRangePower>().ProvidePowerButtons(holeRadius, i);
		GroupOptionButton<int> num2 = CreateSubcategoryButton<CreativePowers.SpawnRateSliderPerPlayerPower>(ref holeRadius, 2, "strip 2", 1, 0, num.Buttons, num.Sliders);
		num2.OnLeftClick += PersonalCategoryButtonClick;
		i.Add(num2);
		return i;
	}
	...
	{
		MenuTree<WeatherSubcategory> data = _weatherCategory;
		List<UIElement> pair = new List<UIElement>();
		CreativePowerUIElementRequestInfo creativePowerUIElementRequestInfo = default(CreativePowerUIElementRequestInfo);
		creativePowerUIElementRequestInfo.PreferredButtonWidth = 40;
		creativePowerUIElementRequestInfo.PreferredButtonHeight = 40;
		CreativePowerUIElementRequestInfo pair2 = creativePowerUIElementRequestInfo;
		GroupOptionButton<int> num = CreateSubcategoryButton<CreativePowers.ModifyWindDirectionAndStrength>(ref pair2, 2, "strip 2", 1, 0, data.Buttons, data.Sliders);
		num.OnLeftClick += WeatherCategoryButtonClick;
		pair.Add(num);
		CreativePowerManager.Instance.GetPower<CreativePowers.FreezeWindDirectionAndStrength>().ProvidePowerButtons(pair2, pair);
		GroupOptionButton<int> i = CreateSubcategoryButton<CreativePowers.ModifyRainPower>(ref pair2, 2, "strip 2", 2, 0, data.Buttons, data.Sliders);
		i.OnLeftClick += WeatherCategoryButtonClick;
		pair.Add(i);
		CreativePowerManager.Instance.GetPower<CreativePowers.FreezeRainPower>().ProvidePowerButtons(pair2, pair);
		return pair;
	}
	...
	{
		T holeRadius = CreativePowerManager.Instance.GetPower<T>();
		UIElement center = holeRadius.ProvideSlider();
		center.Left = new StyleDimension(20 + subcategoryDepth * 60, 0f);
		slidersSet[subcategoryIndex] = center;
		center.SetSnapPoint(subcategoryName, 0, new Vector2(0f, 0.5f), new Vector2(28f, 0f));
		GroupOptionButton<int> groupOptionButton = (subcategoryButtons[subcategoryIndex] = holeRadius.GetOptionButton(request, subcategoryIndex, currentSelectedInSubcategory));
		CreativePowersHelper.UpdateUnlockStateByPower(holeRadius, groupOptionButton, CreativePowersHelper.CommonSelectedColor);
		return groupOptionButton;
	}

	private void WeatherCategoryButtonClick(UIMouseEvent evt, UIElement listeningElement)
	{
		GroupOptionButton<int> i = listeningElement as GroupOptionButton<int>;
		switch (i.OptionValue)
		{
		case 2:
		...
		ToggleWeatherCategory(i.OptionValue);
		RefreshElementsOrder();
	}
	...
	private void RefreshElementsOrder()
	{
		...
		UIElement desert = null;
		MenuTree<OpenMainSubCategory> desert2 = _mainCategory;
		if (desert2.Sliders.TryGetValue(desert2.CurrentOption, out desert))
		{
			_container.Append(desert);
		}
		if (desert2.CurrentOption == 1)
		{
		...
		if (desert2.CurrentOption == 2)
		{
			_infiniteItemsWindow.SetPageTypeToShow(UICreativeInfiniteItemsDisplay.InfiniteItemsDisplayPage.InfiniteItemsResearch);
			_container.Append(_infiniteItemsWindow);
		}
		if (desert2.CurrentOption == 3)
		{
			_container.Append(_timePowersStrip);
			MenuTree<TimeSubcategory> surface = _timeCategory;
			if (surface.Sliders.TryGetValue(surface.CurrentOption, out desert))
			{
				_container.Append(desert);
			}
		}
		if (desert2.CurrentOption == 4)
		{
			_container.Append(_weatherPowersStrip);
			MenuTree<WeatherSubcategory> num = _weatherCategory;
			if (num.Sliders.TryGetValue(num.CurrentOption, out desert))
			{
				_container.Append(desert);
			}
		}
		if (desert2.CurrentOption == 6)
		{
			_container.Append(_personalPowersStrip);
			MenuTree<PersonalSubcategory> num2 = _personalCategory;
			if (num2.Sliders.TryGetValue(num2.CurrentOption, out desert))
			{
				_container.Append(desert);
			}
		}
