...
public class UIWorldSelect : UIState
{
	...
	private void UpdateWorldsList()
	{
		...
		{
			_worldList.Add(new UIWorldListItem(item, value++, CanWorldBePlayed(item)));
		}
	}
	...
	{
		List<WorldFileData> result = new List<WorldFileData>(Main.WorldList);
		result.Sort(delegate(WorldFileData x, WorldFileData y)
		{
		...
		bool flag = false;
		if (!flag && result.Count != favoritesCache.Count)
		{
			flag = true;
		}
		if (!flag)
		{
			for (int i = 0; i < favoritesCache.Count; i++)
			{
				...
				if (!(result[i].Name == tuple.Item1) || result[i].IsFavorite != tuple.Item2)
				{
					flag = true;
					break;
				}
		...
		if (flag)
		{
			...
			foreach (WorldFileData item in result)
			{
