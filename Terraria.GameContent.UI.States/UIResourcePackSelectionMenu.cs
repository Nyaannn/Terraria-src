...
public class UIResourcePackSelectionMenu : UIState, IHaveBackButtonCommand
{
	...
	private void OffsetResourcePackPriority(ResourcePack resourcePack, int offset)
	{
		if (resourcePack.IsEnabled)
		{
			List<ResourcePack> uIText = _packsList.EnabledPacks.ToList();
			int num = uIText.IndexOf(resourcePack);
			int num2 = Utils.Clamp(num + offset, 0, uIText.Count - 1);
			if (num2 != num)
			{
				int sortingOrder = uIText[num].SortingOrder;
				uIText[num].SortingOrder = uIText[num2].SortingOrder;
				uIText[num2].SortingOrder = sortingOrder;
			}
		}
	}
	...
	{
		if (listeningElement is UIResourcePackInfoButton<string> i)
		{
			SoundEngine.PlaySound(10);
			Main.MenuUI.SetState(new UIResourcePackInfoMenu(this, i.ResourcePack));
		}
	}
	...
	{
		UITextPanel<LocalizedText> entryUICollectionInfo = new UITextPanel<LocalizedText>(Language.GetText("UI.Back"), 0.7f, large: true)
		{
		...
		entryUICollectionInfo.OnMouseOver += FadedMouseOver;
		entryUICollectionInfo.OnMouseOut += FadedMouseOut;
		entryUICollectionInfo.OnLeftClick += GoBackClick;
		entryUICollectionInfo.SetSnapPoint("GoBack", 0);
		outerContainer.Append(entryUICollectionInfo);
		UITextPanel<LocalizedText> unlockState = new UITextPanel<LocalizedText>(Language.GetText("GameUI.OpenFileFolder"), 0.7f, large: true)
		{
			Width = new StyleDimension(-8f, 0.5f),
			Height = new StyleDimension(50f, 0f),
			VAlign = 1f,
		...
		unlockState.OnMouseOver += FadedMouseOver;
		unlockState.OnMouseOut += FadedMouseOut;
		unlockState.OnLeftClick += OpenFoldersClick;
		unlockState.SetSnapPoint("OpenFolder", 0);
		outerContainer.Append(unlockState);
	}
