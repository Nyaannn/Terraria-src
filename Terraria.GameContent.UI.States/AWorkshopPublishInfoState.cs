...
public abstract class AWorkshopPublishInfoState<TPublishedObjectType> : UIState, IHaveBackButtonCommand
{
	...
	private void SetTagsFromFoundEntry()
	{
		...
		if (i.tags != null)
		{
			...
			{
				bool flag = i.tags.Contains(tagOption.OptionValue.InternalNameForAPIs);
		...
		GroupOptionButton<WorkshopItemPublicSettingId>[] publicityOptions = _publicityOptions;
		for (int j = 0; j < publicityOptions.Length; j++)
		{
			publicityOptions[j].SetCurrentOption(i.publicity);
		}
	}
	...
	private UIElement CreateSteamDisclaimer(string tagGroup)
	{
		...
		groupOptionButton.Height = StyleDimension.FromPixelsAndPercent(num + 4f, 0f);
		...
		UIElement uIElement = new UIElement
		{
			...
			VAlign = 1f,
			Width = new StyleDimension(0f, 1f),
			Height = new StyleDimension(location, 0f)
		};
	...
	{
		List<WorkshopTagOption> mouseItem = GetTagsToShow();
		int amountNeeded = 3;
		int num = (int)Math.Ceiling((float)mouseItem.Count / (float)amountNeeded);
		int num2 = heightPerRow * num;
		CreateStylizedCategoryPanel(num2, "Workshop.CategoryTitleTags", out var entirePanel, out var innerPanel);
		float num3 = 0.98f;
		List<GroupOptionButton<WorkshopTagOption>> list = new List<GroupOptionButton<WorkshopTagOption>>();
		for (int i = 0; i < mouseItem.Count; i++)
		{
			WorkshopTagOption workshopTagOption = mouseItem[i];
			...
			int num4 = i / amountNeeded;
			int num5 = i - num4 * amountNeeded;
			groupOptionButton.Width = StyleDimension.FromPixelsAndPercent(-4 * (amountNeeded - 1), 1f / (float)amountNeeded * num3);
			groupOptionButton.HAlign = (float)num5 / (float)(amountNeeded - 1);
			groupOptionButton.Left = StyleDimension.FromPercent((1f - num3) * (1f - groupOptionButton.HAlign * 2f));
			groupOptionButton.Top.Set(num4 * heightPerRow, 0f);
	...
	private void CreateStylizedCategoryPanel(float height, string titleTextKey, out UIElement entirePanel, out UIElement innerPanel)
	{
		...
		UISlicedImage uISlicedImage = new UISlicedImage(Main.Assets.Request<Texture2D>("Images/UI/CharCreation/CategoryPanel", (AssetRequestMode)1))
		{
			...
			Height = StyleDimension.FromPixelsAndPercent(height + item + 4f, 0f),
		...
		AddHorizontalSeparator(uISlicedImage, item, 4);
		UIText uIText = new UIText(Language.GetText(titleTextKey))
		{
			HAlign = 0f,
			VAlign = 0f,
			Width = StyleDimension.FromPixelsAndPercent(-40f, 1f),
			Height = StyleDimension.FromPixelsAndPercent(item, 0f),
	...
	{
		GroupOptionButton<WorkshopTagOption> amountNeeded = (GroupOptionButton<WorkshopTagOption>)listeningElement;
		amountNeeded.SetCurrentOption(amountNeeded.IsSelected ? null : amountNeeded.OptionValue);
		amountNeeded.SetColor(amountNeeded.IsSelected ? new Color(152, 175, 235) : Colors.InventoryDefaultColor, 1f);
	}
