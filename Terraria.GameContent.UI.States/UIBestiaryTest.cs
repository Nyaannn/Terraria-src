...
public class UIBestiaryTest : UIState
{
	...
	{
		UIImageButton num = new UIImageButton(Main.Assets.Request<Texture2D>("Images/UI/Bestiary/Button_Search", (AssetRequestMode)1))
		{
			Left = new StyleDimension(0f - infoSpace.Width.Pixels, 1f),
			VAlign = 0.5f
		};
		num.OnLeftClick += Click_SearchArea;
		num.SetHoverImage(Main.Assets.Request<Texture2D>("Images/UI/Bestiary/Button_Search_Border", (AssetRequestMode)1));
		num.SetVisibility(1f, 1f);
		num.SetSnapPoint("SearchButton", 0);
		innerTopContainer.Append(num);
		UIPanel vector = (_searchBoxPanel = new UIPanel
		{
			Left = new StyleDimension(0f - infoSpace.Width.Pixels + num.Width.Pixels + 3f, 1f),
			Width = new StyleDimension(infoSpace.Width.Pixels - num.Width.Pixels - 3f, 0f),
			Height = new StyleDimension(0f, 1f),
			VAlign = 0.5f
		});
		vector.BackgroundColor = new Color(35, 40, 83);
		vector.BorderColor = new Color(35, 40, 83);
		vector.SetPadding(0f);
		innerTopContainer.Append(vector);
		UISearchBar vector2 = (_searchBar = new UISearchBar(Language.GetText("UI.PlayerNameSlot"), 0.8f)
		{
			Width = new StyleDimension(0f, 1f),
			Height = new StyleDimension(0f, 1f),
			HAlign = 0f,
		...
		vector.OnLeftClick += Click_SearchArea;
		vector2.OnContentsChanged += OnSearchContentsChanged;
		vector.Append(vector2);
		vector2.OnStartTakingInput += OnStartTakingInput;
		vector2.OnEndTakingInput += OnEndTakingInput;
		vector2.OnNeedingVirtualKeyboard += OpenVirtualKeyboardWhenNeeded;
		UIImageButton num2 = new UIImageButton(Main.Assets.Request<Texture2D>("Images/UI/SearchCancel", (AssetRequestMode)1))
		{
			HAlign = 1f,
			VAlign = 0.5f,
			Left = new StyleDimension(-2f, 0f)
		};
		num2.OnMouseOver += searchCancelButton_OnMouseOver;
		num2.OnLeftClick += searchCancelButton_OnClick;
		vector.Append(num2);
	}
	...
	{
		int result = 40;
		UIVirtualKeyboard result2 = new UIVirtualKeyboard(Language.GetText("UI.PlayerNameSlot").Value, _searchString, OnFinishedSettingName, GoBackHere, 0, allowEmpty: true);
		result2.SetMaxInputLength(result);
		UserInterface.ActiveInstance.SetState(result2);
	}
	...
	public BestiaryUnlockProgressReport GetUnlockProgress()
	{
		...
		for (int k = 0; k < originalEntriesList.Count; k++)
		{
			int num = ((originalEntriesList[k].UIInfoProvider.GetEntryUICollectionInfo().UnlockState > BestiaryEntryUnlockState.NotKnownAtAll_0) ? 1 : 0);
			j++;
			i += (float)num;
		}
		BestiaryUnlockProgressReport result = default(BestiaryUnlockProgressReport);
		result.EntriesTotal = j;
		result.CompletionAmountTotal = i;
		return result;
	}
	...
	private void MoveToVisuallyClosestPoint(List<UILinkPoint> lostrefpoints)
	{
		...
		Vector2 i = Main.MouseScreen;
		UILinkPoint j = null;
		foreach (UILinkPoint k in lostrefpoints)
		{
			if (j == null || Vector2.Distance(i, j.Position) > Vector2.Distance(i, k.Position))
			{
				j = k;
			}
		}
		if (j != null)
		{
			UILinkPointNavigator.ChangePoint(j.ID);
		}
	}
	...
	{
		List<SnapPoint> dictionary = GetOrderedPointsByCategoryName(pts, "Entries");
		...
		{
			int num3 = num2 % gridWidth;
			int num4 = num2 / gridWidth;
			gridPoints[num3, num4] = MakeLinkPointFromSnapPoint(currentID++, dictionary[num2]);
		}
		for (int num5 = 0; num5 < gridWidth; num5++)
		{
			for (int i = 0; i < gridHeight; i++)
			{
				UILinkPoint uILinkPoint = gridPoints[num5, i];
				if (num5 < gridWidth - 1)
				{
					UILinkPoint uILinkPoint2 = gridPoints[num5 + 1, i];
				...
				if (i < gridHeight - 1)
				{
					UILinkPoint uILinkPoint3 = gridPoints[num5, i + 1];
	...
	{
		List<SnapPoint> num = GetOrderedPointsByCategoryName(pts, "SortSteps");
		_sortingGrid.GetEntriesToShow(out gridWidth, out gridHeight, out var _);
		gridPoints = new UILinkPoint[gridWidth, gridHeight];
		for (int holeRadius = 0; holeRadius < num.Count; holeRadius++)
		{
			int num2 = holeRadius % gridWidth;
			int y = holeRadius / gridWidth;
			gridPoints[num2, y] = MakeLinkPointFromSnapPoint(currentID++, num[holeRadius]);
		}
		for (int j = 0; j < gridWidth; j++)
		{
			for (int k = 0; k < gridHeight; k++)
