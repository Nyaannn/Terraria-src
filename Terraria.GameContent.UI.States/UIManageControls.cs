...
public class UIManageControls : UIState
{
	...
	{
		List<UIElement> flag = _bindsKeyboard;
		if (!OnKeyboard)
		{
			flag = _bindsGamepad;
		}
		if (!OnGameplay)
		{
			flag = (OnKeyboard ? _bindsKeyboardUI : _bindsGamepadUI);
		}
		_uilist.Clear();
		foreach (UIElement item in flag)
		{
			_uilist.Add(item);
		}
	}
	...
	{
		string bestiaryCreditId = PlayerInput.CurrentProfile.Name;
		List<string> list = PlayerInput.Profiles.Keys.ToList();
		int num = list.IndexOf(bestiaryCreditId);
		num++;
		if (num >= list.Count)
		{
			num -= list.Count;
	...
	private void SetupGamepadPoints(SpriteBatch spriteBatch)
	{
		...
		int i = num;
		UILinkPoint uILinkPoint = UILinkPointNavigator.Points[i];
		uILinkPoint.Unlink();
		uILinkPoint.Up = num + 6;
		i = num + 1;
		UILinkPoint uILinkPoint2 = UILinkPointNavigator.Points[i];
		uILinkPoint2.Unlink();
		uILinkPoint2.Right = num + 2;
		uILinkPoint2.Down = num + 6;
		i = num + 2;
		UILinkPoint uILinkPoint3 = UILinkPointNavigator.Points[i];
		uILinkPoint3.Unlink();
		uILinkPoint3.Left = num + 1;
		uILinkPoint3.Right = num + 4;
		uILinkPoint3.Down = num + 6;
		i = num + 4;
		UILinkPoint uILinkPoint4 = UILinkPointNavigator.Points[i];
		uILinkPoint4.Unlink();
		uILinkPoint4.Left = num + 2;
		uILinkPoint4.Right = num + 5;
		uILinkPoint4.Down = num + 6;
		i = num + 5;
		UILinkPoint uILinkPoint5 = UILinkPointNavigator.Points[i];
		uILinkPoint5.Unlink();
		uILinkPoint5.Left = num + 4;
		uILinkPoint5.Right = num + 3;
		uILinkPoint5.Down = num + 6;
		i = num + 3;
		UILinkPoint uILinkPoint6 = UILinkPointNavigator.Points[i];
		uILinkPoint6.Unlink();
		...
		{
			if (!snapPoints[j].Position.Between(minimum, maximum))
			{
				_ = snapPoints[j].Position;
				snapPoints.Remove(snapPoints[j]);
				j--;
			}
		}
		...
		{
			i = num + 6 + k;
			if (snapPoints[k].Name == "Thin")
			{
				UILinkPoint uILinkPoint7 = UILinkPointNavigator.Points[i];
				uILinkPoint7.Unlink();
				UILinkPointNavigator.SetPosition(i, snapPoints[k].Position);
				uILinkPoint7.Right = i + 1;
				uILinkPoint7.Down = ((k < snapPoints.Count - 2) ? (i + 2) : num);
				uILinkPoint7.Up = ((k < 2) ? (num + 1) : ((snapPoints[k - 1].Name == "Wide") ? (i - 1) : (i - 2)));
				UILinkPointNavigator.Points[num].Up = i;
				UILinkPointNavigator.Shortcuts.FANCYUI_HIGHEST_INDEX = i;
				k++;
				if (k < snapPoints.Count)
				{
					i = num + 6 + k;
					UILinkPoint uILinkPoint8 = UILinkPointNavigator.Points[i];
					uILinkPoint8.Unlink();
					UILinkPointNavigator.SetPosition(i, snapPoints[k].Position);
					uILinkPoint8.Left = i - 1;
					uILinkPoint8.Down = ((k >= snapPoints.Count - 1) ? num : ((snapPoints[k + 1].Name == "Wide") ? (i + 1) : (i + 2)));
					uILinkPoint8.Up = ((k < 2) ? (num + 1) : (i - 2));
					UILinkPointNavigator.Shortcuts.FANCYUI_HIGHEST_INDEX = i;
				}
			}
			else
			{
				UILinkPoint uILinkPoint9 = UILinkPointNavigator.Points[i];
				uILinkPoint9.Unlink();
				UILinkPointNavigator.SetPosition(i, snapPoints[k].Position);
				uILinkPoint9.Down = ((k < snapPoints.Count - 1) ? (i + 1) : num);
				uILinkPoint9.Up = ((k < 1) ? (num + 1) : ((snapPoints[k - 1].Name == "Wide") ? (i - 1) : (i - 2)));
				UILinkPointNavigator.Shortcuts.FANCYUI_HIGHEST_INDEX = i;
				UILinkPointNavigator.Points[num].Up = i;
			}
		}
