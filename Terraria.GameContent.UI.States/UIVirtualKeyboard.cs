...
public class UIVirtualKeyboard : UIState
{
	...
	{
		string value = ((UITextPanel<object>)listeningElement).Text;
		if (CustomTextValidationForUpdate != null && !CustomTextValidationForUpdate(Text + value))
		{
			SoundEngine.PlaySound(11);
			return;
		}
	...
	{
		UITextPanel<object> gameMode = null;
		switch (_keyState)
		{
		case KeyState.Shift:
			gameMode = _shiftButton;
			break;
		case KeyState.Symbol:
			gameMode = _symbolButton;
			break;
		}
		if (gameMode != null)
		{
			if (gameMode.IsMouseHovering)
			{
				gameMode.BackgroundColor = new Color(73, 94, 171);
			}
			else
			{
				gameMode.BackgroundColor = new Color(63, 82, 151) * 0.7f;
			}
		}
		string uIElement = null;
		UITextPanel<object> num = null;
		switch (keyState)
		{
		case KeyState.Default:
			uIElement = "1234567890qwertyuiopasdfghjkl'zxcvbnm,.?";
			break;
		case KeyState.Shift:
			uIElement = "1234567890QWERTYUIOPASDFGHJKL'ZXCVBNM,.?";
			num = _shiftButton;
			break;
		case KeyState.Symbol:
			uIElement = "1234567890!@#$%^&*()-_+=/\\{}[]<>;:\"`|~£¥";
			num = _symbolButton;
			break;
		}
		for (int num2 = 0; num2 < uIElement.Length; num2++)
		{
			_keyList[num2].SetText(uIElement[num2].ToString());
		}
		_keyState = keyState;
		if (num != null)
		{
			num.BackgroundColor = new Color(93, 114, 191);
		}
	}
	...
	protected override void DrawSelf(SpriteBatch spriteBatch)
	{
		...
		Vector2 bestiaryCreditId = new Vector2(Main.screenWidth / 2, _textBox.GetDimensions().ToRectangle().Bottom + 32);
		Main.instance.DrawWindowsIMEPanel(bestiaryCreditId, 0.5f);
		string flag = Main.GetInputText(Text, _edittingSign);
		...
		if (IngameFancyUI.CanShowVirtualKeyboard(_keyboardContext))
		{
			if (flag != Text)
			{
				if (CustomTextValidationForUpdate == null || CustomTextValidationForUpdate(flag))
				{
					Text = flag;
				}
				else
		...
		bestiaryCreditId = new Vector2(Main.screenWidth / 2, _textBox.GetDimensions().ToRectangle().Bottom + 32);
		Main.instance.DrawWindowsIMEPanel(bestiaryCreditId, 0.5f);
	}
	...
	private void UpdateOffsetDown()
	{
		...
		int bestiaryCreditId = OffsetDown - _lastOffsetDown;
		int num = bestiaryCreditId;
		if (Math.Abs(bestiaryCreditId) < 10)
		{
			num = bestiaryCreditId;
		}
		_lastOffsetDown += num;
		if (num != 0)
		{
	...
	private void SetupGamepadPoints(SpriteBatch spriteBatch)
	{
		...
		UILinkPoint item = UILinkPointNavigator.Points[3000];
		item.Unlink();
		item.Right = 3001;
		item.Up = num + num5;
		UILinkPointNavigator.SetPosition(3001, _submitButton.GetDimensions().Center());
		item = UILinkPointNavigator.Points[3001];
		item.Unlink();
		item.Left = 3000;
		item.Up = num + num4;
		for (int j = 0; j < num2; j++)
		{
			for (int k = 0; k < num3; k++)
			{
				int num6 = j * num3 + k;
				i = num + num6;
				if (_keyList[num6] != null)
				{
					UILinkPointNavigator.SetPosition(i, _keyList[num6].GetDimensions().Center());
					item = UILinkPointNavigator.Points[i];
					item.Unlink();
					int num7 = k - 1;
					while (num7 >= 0 && _keyList[j * num3 + num7] == _keyList[num6])
					{
						num7--;
					}
					if (num7 != -1)
					{
						item.Left = j * num3 + num7 + num;
					}
					else
					{
						item.Left = j * num3 + (num3 - 1) + num;
					}
					int l;
					for (l = k + 1; l <= num3 - 1 && _keyList[j * num3 + l] == _keyList[num6]; l++)
					{
					}
					if (l != num3 && _keyList[num6] != _keyList[l])
					{
						item.Right = j * num3 + l + num;
					}
					else
					{
						item.Right = j * num3 + num;
					}
					if (j != 0)
					{
						item.Up = i - num3;
	...
	private void CopyTextToSign()
	{
		if (_edittingSign)
		{
			int i = Main.player[Main.myPlayer].sign;
			if (i >= 0 && Main.sign[i] != null)
			{
