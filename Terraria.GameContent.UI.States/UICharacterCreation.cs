...
public class UICharacterCreation : UIState
{
	...
	{
		float value = 70f;
		for (float num = 0f; num <= 1f; num += 1f)
		{
			UICharacter element = new UICharacter(_player, animated: true, hasBackPanel: false, 1.5f)
			{
				...
				Top = StyleDimension.FromPixelsAndPercent(0f - value, 0f),
				VAlign = 0f,
				HAlign = 0.5f
			};
	...
	private void MakeHairsylesMenu(UIElement middleInnerPanel)
	{
		...
		entry.SetPadding(4f);
		i.Append(entry);
		UIScrollbar addedText = new UIScrollbar
		{
			HAlign = 1f,
			Height = StyleDimension.FromPixelsAndPercent(-30f, 1f),
			Top = StyleDimension.FromPixels(10f)
		};
		addedText.SetView(100f, 1000f);
		entry.SetScrollbar(addedText);
		i.Append(addedText);
		...
		entry.Add(uIElement);
		uIElement.SetPadding(0f);
		for (int buff = 0; buff < count; buff++)
		{
			UIHairStyleButton uIHairStyleButton = new UIHairStyleButton(_player, Main.Hairstyles.AvailableHairstyles[buff])
			{
				Left = StyleDimension.FromPixels((float)(buff % 10) * 46f + 6f),
				Top = StyleDimension.FromPixels((float)(buff / 10) * 48f + 1f)
			};
			uIHairStyleButton.SetSnapPoint("Middle", buff);
			uIHairStyleButton.SkipRenderingContent(buff);
			uIElement.Append(uIHairStyleButton);
		}
		_hairstylesContainer = i;
	}

	private void MakeClothStylesMenu(UIElement middleInnerPanel)
	{
		UIElement bestiaryEntry = new UIElement
		{
			Width = StyleDimension.FromPixelsAndPercent(-10f, 1f),
			Height = StyleDimension.FromPixelsAndPercent(0f, 1f),
			HAlign = 0.5f,
		...
		middleInnerPanel.Append(bestiaryEntry);
		bestiaryEntry.SetPadding(0f);
		int list = 15;
		for (int i = 0; i < _validClothStyles.Length; i++)
		{
			...
			{
				Left = StyleDimension.FromPixels((float)i * 46f + (float)num + 6f),
				Top = StyleDimension.FromPixels(list)
			};
			uIClothStyleButton.OnLeftMouseDown += Click_CharClothStyle;
			uIClothStyleButton.SetSnapPoint("Middle", i);
			bestiaryEntry.Append(uIClothStyleButton);
		}
		for (int j = 0; j < 2; j++)
		{
			int num2 = 0;
			...
			{
				Left = StyleDimension.FromPixels((float)j * 230f + (float)num2 + 6f),
				Top = StyleDimension.FromPixels(list + 86),
				Width = StyleDimension.FromPixelsAndPercent(230f, 0f),
				Color = Color.Lerp(Color.White, new Color(63, 65, 151, 255), 0.85f) * 0.9f
			};
			bestiaryEntry.Append(element);
			...
			uIColoredImageButton.Top = StyleDimension.FromPixelsAndPercent(list + 92, 0f);
			uIColoredImageButton.Left = StyleDimension.FromPixels((float)j * 230f + 92f + (float)num2 + 6f);
			uIColoredImageButton.HAlign = 0f;
			uIColoredImageButton.VAlign = 0f;
			bestiaryEntry.Append(uIColoredImageButton);
			if (j == 0)
			{
		...
		UIElement ratesInfo = new UIElement
		{
		...
		bestiaryEntry.Append(ratesInfo);
		...
		ratesInfo.Append(uIColoredImageButton2);
		...
		ratesInfo.Append(uIColoredImageButton3);
		_pasteTemplateButton = uIColoredImageButton3;
		UIColoredImageButton item2 = new UIColoredImageButton(Main.Assets.Request<Texture2D>("Images/UI/CharCreation/Randomize", (AssetRequestMode)1), isSmall: true)
		{
			VAlign = 0.5f,
			HAlign = 1f
		};
		item2.OnLeftMouseDown += Click_RandomizePlayer;
		ratesInfo.Append(item2);
		_randomizePlayerButton = item2;
		uIColoredImageButton2.SetSnapPoint("Low", 1);
		uIColoredImageButton3.SetSnapPoint("Low", 2);
		item2.SetSnapPoint("Low", 3);
		_clothStylesContainer = bestiaryEntry;
	}
	...
	private void MakeCategoriesBar(UIElement categoryContainer)
	{
		...
		categoryContainer.Append(CreateColorPicker(CategoryId.HairColor, "Images/UI/CharCreation/ColorHair", i, xPositionPerId));
		categoryContainer.Append(CreateColorPicker(CategoryId.Eye, "Images/UI/CharCreation/ColorEye", i, xPositionPerId));
		categoryContainer.Append(CreateColorPicker(CategoryId.Skin, "Images/UI/CharCreation/ColorSkin", i, xPositionPerId));
		categoryContainer.Append(CreateColorPicker(CategoryId.Shirt, "Images/UI/CharCreation/ColorShirt", i, xPositionPerId));
		categoryContainer.Append(CreateColorPicker(CategoryId.Undershirt, "Images/UI/CharCreation/ColorUndershirt", i, xPositionPerId));
		categoryContainer.Append(CreateColorPicker(CategoryId.Pants, "Images/UI/CharCreation/ColorPants", i, xPositionPerId));
		categoryContainer.Append(CreateColorPicker(CategoryId.Shoes, "Images/UI/CharCreation/ColorShoes", i, xPositionPerId));
		_colorPickers[4].SetMiddleTexture(Main.Assets.Request<Texture2D>("Images/UI/CharCreation/ColorEyeBack", (AssetRequestMode)1));
		_clothingStylesCategoryButton = CreatePickerWithoutClick(CategoryId.Clothing, "Images/UI/CharCreation/ClothStyleMale", i, xPositionPerId);
		...
		_hairStylesCategoryButton = CreatePickerWithoutClick(CategoryId.HairStyle, "Images/UI/CharCreation/HairStyle_Hair", i, xPositionPerId);
		...
		_charInfoCategoryButton = CreatePickerWithoutClick(CategoryId.CharInfo, "Images/UI/CharCreation/CharInfo", i, xPositionPerId);
	...
	{
		float dictionary = UILinksInitializer.HandleSliderHorizontalInput(_currentColorHSL.Y, 0f, 1f, PlayerInput.CurrentProfile.InterfaceDeadzoneX, 0.35f);
		UpdateHSLValue(HSLSliderId.Saturation, dictionary);
	}

	private void UpdateHSL_L()
	{
		float spawnConditionBestiaryInfoElement = UILinksInitializer.HandleSliderHorizontalInput(_currentColorHSL.Z, 0f, 1f, PlayerInput.CurrentProfile.InterfaceDeadzoneX, 0.35f);
		UpdateHSLValue(HSLSliderId.Luminance, spawnConditionBestiaryInfoElement);
	}
	...
	private void MakeBackAndCreatebuttons(UIElement outerContainer)
	{
		...
		UITextPanel<LocalizedText> item = new UITextPanel<LocalizedText>(Language.GetText("UI.Create"), 0.7f, large: true)
		{
			Width = StyleDimension.FromPixelsAndPercent(-10f, 0.5f),
			Height = StyleDimension.FromPixels(50f),
			VAlign = 1f,
		...
		item.OnMouseOver += FadedMouseOver;
		item.OnMouseOut += FadedMouseOut;
		item.OnLeftMouseDown += Click_NamingAndCreating;
		item.SetSnapPoint("Create", 0);
		outerContainer.Append(item);
	}
	...
	public override void Draw(SpriteBatch spriteBatch)
	{
		...
		string unlockStateByKillCount = null;
		if (_copyHexButton.IsMouseHovering)
		{
			unlockStateByKillCount = Language.GetTextValue("UI.CopyColorToClipboard");
		}
		if (_pasteHexButton.IsMouseHovering)
		{
			unlockStateByKillCount = Language.GetTextValue("UI.PasteColorFromClipboard");
		}
		if (_randomColorButton.IsMouseHovering)
		{
			unlockStateByKillCount = Language.GetTextValue("UI.RandomizeColor");
		}
		if (_copyTemplateButton.IsMouseHovering)
		{
			unlockStateByKillCount = Language.GetTextValue("UI.CopyPlayerToClipboard");
		}
		if (_pasteTemplateButton.IsMouseHovering)
		{
			unlockStateByKillCount = Language.GetTextValue("UI.PastePlayerFromClipboard");
		}
		if (_randomizePlayerButton.IsMouseHovering)
		{
			unlockStateByKillCount = Language.GetTextValue("UI.RandomizePlayer");
		}
		if (unlockStateByKillCount != null)
		{
			float result = FontAssets.MouseText.get_Value().MeasureString(unlockStateByKillCount).X;
			Vector2 vector = new Vector2(Main.mouseX, Main.mouseY) + new Vector2(16f);
			if (vector.Y > (float)(Main.screenHeight - 30))
			{
				vector.Y = Main.screenHeight - 30;
			}
			if (vector.X > (float)Main.screenWidth - result)
			{
				vector.X = Main.screenWidth - 460;
			}
			Utils.DrawBorderStringFourWay(spriteBatch, FontAssets.MouseText.get_Value(), unlockStateByKillCount, vector.X, vector.Y, new Color(Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor, Main.mouseTextColor), Color.Black, Vector2.Zero);
		}
	...
	{
		Vector3 iconbox = Main.rgbToHsl(color);
		iconbox.Z = (iconbox.Z - 0.15f) / 0.85f;
		return Vector3.Clamp(iconbox, Vector3.Zero, Vector3.One);
	}
}
