...
public class UIWorkshopHub : UIState, IHaveBackButtonCommand
{
	...
	{
		UIElement shapeData = MakeFancyButton("Images/UI/Workshop/HubPublishResourcepacks", "Workshop.HubPublishResourcePacks");
		shapeData.OnLeftClick += Click_OpenResourcePackPublishMenu;
		_buttonPublishResourcePacks = shapeData;
		return shapeData;
	}
	...
	{
		UIHorizontalSeparator num = new UIHorizontalSeparator
		{
	...
	{
		float x = 0f;
		UISlicedImage num = new UISlicedImage(Main.Assets.Request<Texture2D>("Images/UI/CharCreation/CategoryPanelHighlight", (AssetRequestMode)1))
		{
			HAlign = 0.5f,
			VAlign = 1f,
			Width = StyleDimension.FromPixelsAndPercent((0f - x) * 2f, 1f),
			Left = StyleDimension.FromPixels(0f - x),
			Height = StyleDimension.FromPixelsAndPercent(height, 0f),
			Top = StyleDimension.FromPixels(2f)
		};
		num.SetSliceDepths(10);
		num.Color = Color.LightGray * 0.7f;
		container.Append(num);
		UIText num2 = new UIText(Language.GetText("Workshop.HubDescriptionDefault"))
		{
			HAlign = 0f,
			VAlign = 0f,
		...
		num2.PaddingLeft = 20f;
		num2.PaddingRight = 20f;
		num2.PaddingTop = 6f;
		num2.IsWrapped = true;
		num.Append(num2);
		_descriptionText = num2;
	}
	...
	private UIElement MakeFancyButton(string iconImagePath, string textKey)
	{
		...
		int x = -3;
		int y = -3;
		uIPanel.Width = StyleDimension.FromPixelsAndPercent(x, 0.5f);
		uIPanel.Height = StyleDimension.FromPixelsAndPercent(y, 0.5f);
		...
		UIImage num = new UIImage(Main.Assets.Request<Texture2D>(iconImagePath, (AssetRequestMode)1))
		{
