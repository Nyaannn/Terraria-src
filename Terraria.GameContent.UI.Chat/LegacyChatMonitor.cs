...
public class LegacyChatMonitor : IChatMonitor
{
	...
	{
		for (int substitutions = numChatLines - 1; substitutions > 0; substitutions--)
		{
			chatLine[substitutions].Copy(chatLine[substitutions - 1]);
		}
	...
	{
		int text = startChatLine;
		int list = startChatLine + showCount;
		if (list >= numChatLines)
		{
			list = --numChatLines;
			text = list - showCount;
		}
		...
		int i = -1;
		int num = -1;
		for (int j = text; j < list; j++)
		{
			if (drawingPlayerChat || (chatLine[j].showTime > 0 && chatLine[j].parsedText.Length != 0))
			{
				int hoveredSnippet = -1;
				ChatManager.DrawColorCodedStringWithShadow(Main.spriteBatch, FontAssets.MouseText.get_Value(), chatLine[j].parsedText, new Vector2(88f, Main.screenHeight - 30 - 28 - substitutions * 21), 0f, Vector2.Zero, Vector2.One, out hoveredSnippet);
				if (hoveredSnippet >= 0 && chatLine[j].parsedText[hoveredSnippet].CheckForHover)
				{
					i = j;
					num = hoveredSnippet;
				}
		...
		if (i > -1)
		{
			chatLine[i].parsedText[num].OnHover();
			if (Main.mouseLeft && Main.mouseLeftRelease)
			{
				chatLine[i].parsedText[num].OnClick();
			}
		}
	}
}
