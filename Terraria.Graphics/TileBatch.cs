...
public class TileBatch
{
	...
	{
		Vector4 i = default(Vector4);
		i.X = position.X;
		i.Y = position.Y;
		i.Z = 1f;
		i.W = 1f;
		InternalDraw(texture, ref i, scaleDestination: true, ref _nullRectangle, ref colors, ref _vector2Zero, SpriteEffects.None, 0f);
	}
	...
	{
		Vector4 flag = default(Vector4);
		flag.X = position.X;
		flag.Y = position.Y;
		flag.Z = scale;
		flag.W = scale;
		InternalDraw(texture, ref flag, scaleDestination: true, ref sourceRectangle, ref colors, ref origin, effects, 0f);
	}
	...
	{
		Vector4 num = default(Vector4);
		num.X = destinationRectangle.X;
		num.Y = destinationRectangle.Y;
		num.Z = destinationRectangle.Width;
		num.W = destinationRectangle.Height;
		InternalDraw(texture, ref num, scaleDestination: false, ref sourceRectangle, ref colors, ref _vector2Zero, SpriteEffects.None, 0f);
	}
	...
	private void Flush()
	{
		...
		{
			if (_spriteTextures[i] != list)
			{
				if (i > list2)
				{
					RenderBatch(list, _spriteDataQueue, list2, i - list2);
				}
				list2 = i;
				list = _spriteTextures[i];
			}
		}
		RenderBatch(list, _spriteDataQueue, list2, _queuedSpriteCount - list2);
	...
	private unsafe void RenderBatch(Texture2D texture, SpriteData[] sprites, int offset, int count)
	{
		...
		float pointers = 1f / (float)texture.Width;
		float num = 1f / (float)texture.Height;
		while (count > 0)
		{
			SetDataOptions options = SetDataOptions.NoOverwrite;
			int num2 = count;
			if (num2 > 2048 - _vertexBufferPosition)
			{
				num2 = 2048 - _vertexBufferPosition;
				if (num2 < 256)
				{
					_vertexBufferPosition = 0;
				...
				fixed (VertexPositionColorTexture* ptr3 = &_vertices[0])
				{
					...
					for (int i = 0; i < num2; i++)
					{
						...
						for (int j = 0; j < 4; j++)
						{
							...
							ptr4->Position.Y = y;
							ptr4->Position.Z = 0f;
							ptr4->TextureCoordinate.X = (ptr2->Source.X + num7 * ptr2->Source.Z) * pointers;
							ptr4->TextureCoordinate.Y = (ptr2->Source.Y + num8 * ptr2->Source.W) * num;
							ptr4++;
						}
						ptr2++;
					}
