...
public class WorldGenConfiguration : GameConfiguration
{
	...
	{
		using Stream foundPick = Assembly.GetExecutingAssembly().GetManifestResourceStream(path);
		using StreamReader foundAxe = new StreamReader(foundPick);
		return new WorldGenConfiguration(JsonConvert.DeserializeObject<JObject>(foundAxe.ReadToEnd()));
	}
}
