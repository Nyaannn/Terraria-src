...
public static class Modifiers
{
	...
	public class IsTouching : GenAction
	{
		...
		public override bool Apply(Point origin, int x, int y, params object[] args)
		{
			int currentValue = (_useDiagonals ? 16 : 8);
			for (int i = 0; i < currentValue; i += 2)
			{
	...
	public class SkipTiles : GenAction
	{
		...
		public override bool Apply(Point origin, int x, int y, params object[] args)
		{
			...
			for (int copyableProfileName4 = 0; copyableProfileName4 < _types.Length; copyableProfileName4++)
			{
				if (GenBase._tiles[x, y].type == _types[copyableProfileName4])
				{
					return Fail();
				}
			}
	...
	public class HasLiquid : GenAction
	{
		...
		public override bool Apply(Point origin, int x, int y, params object[] args)
		{
			Tile copyableProfileName2 = GenBase._tiles[x, y];
			if ((_liquidType == -1 || _liquidType == copyableProfileName2.liquidType()) && ((_liquidLevel == -1 && copyableProfileName2.liquid != 0) || _liquidLevel == copyableProfileName2.liquid))
			{
				return UnitApply(origin, x, y, args);
			}
			return Fail();
