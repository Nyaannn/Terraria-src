...
public class PingMapLayer : IMapLayer
{
	...
	public void Draw(ref MapOverlayDrawContext context, ref string text)
	{
		...
		SpriteFrame list = new SpriteFrame(1, 5);
		DateTime i = DateTime.Now;
		foreach (ItemPair<Ping> item in (IEnumerable<ItemPair<Ping>>)_pings)
		{
			Ping value = item.Value;
			double totalSeconds = (i - value.Time).TotalSeconds;
			int num = (int)(totalSeconds * 10.0);
			list.CurrentRow = (byte)(num % (int)list.RowCount);
			context.Draw(TextureAssets.MapPing.get_Value(), value.Position, list, Alignment.Center);
