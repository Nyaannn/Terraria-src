...
public static class MapHelper
{
	...
	{
		Color type2 = WorldGen.paintColor(colorType);
		float tileObjectData = (float)(int)oldColor.R / 255f;
		float num = (float)(int)oldColor.G / 255f;
		float num2 = (float)(int)oldColor.B / 255f;
		if (num > tileObjectData)
		{
			float num5 = tileObjectData;
			tileObjectData = num;
			num = num5;
		}
		if (num2 > tileObjectData)
		{
			float num6 = tileObjectData;
			tileObjectData = num2;
			num2 = num6;
		}
		switch (colorType)
		...
		case 29:
		{
			...
			oldColor.R = (byte)((float)(int)type2.R * num3);
			oldColor.G = (byte)((float)(int)type2.G * num3);
			oldColor.B = (byte)((float)(int)type2.B * num3);
			break;
		}
		case 30:
		...
		default:
		{
			float num4 = tileObjectData;
			oldColor.R = (byte)((float)(int)type2.R * num4);
			oldColor.G = (byte)((float)(int)type2.G * num4);
			oldColor.B = (byte)((float)(int)type2.B * num4);
			break;
		}
		}
	}
	...
	public static MapTile CreateMapTile(int i, int j, byte Light)
	{
		...
		if (tileData.active())
		{
			int num3 = tileData.type;
			num2 = tileLookup[num3];
			bool flag = tileData.invisibleBlock();
			if (tileData.fullbrightBlock() && !flag)
			{
				num = 255;
			}
			if (flag)
			...
			else if (num3 == 5)
			{
				...
				postPlaceMyPlayer = tileData.color();
			}
			else
			{
				switch (num3)
				...
				case 51:
					...
					break;
				case 19:
					if (tileData.frameY == 864)
					{
						num2 = 0;
					}
					break;
				case 184:
					if (tileData.frameX / 22 == 10)
					{
						num3 = 627;
						num2 = tileLookup[num3];
					}
				...
				if (num2 != 0)
				{
					GetTileBaseOption(i, j, num3, tileData, ref baseOption);
					postPlaceMyPlayer = ((num3 != 160) ? tileData.color() : 0);
				}
			}
		}
		if (num2 == 0)
		{
			bool flag2 = tileData.invisibleWall();
			if (tileData.wall > 0 && tileData.fullbrightWall() && !flag2)
			{
				num = 255;
			}
			if (tileData.liquid > 32)
			{
				int num4 = tileData.liquidType();
				num2 = liquidPosition + num4;
			}
			else if (!tileData.invisibleWall() && tileData.wall > 0 && tileData.wall < WallID.Count)
			{
				int wall = tileData.wall;
				num2 = wallLookup[wall];
				postPlaceMyPlayer = tileData.wallColor();
				switch (wall)
				{
				case 21:
				case 88:
		...
		if (num2 == 0)
		{
			...
			else if (j < Main.UnderworldLayer)
			{
				...
				num2 = ((!((double)j < Main.rockLayer)) ? (rockPosition + b) : (dirtPosition + b));
			}
			else
			{
				num2 = hellPosition;
		...
		return MapTile.Create((ushort)(num2 + baseOption), (byte)num, (byte)postPlaceMyPlayer);
	}
	...
	public static void GetTileBaseOption(int x, int y, int tileType, Tile tileCache, ref int baseOption)
	{
		...
		{
			int num = y + 1;
			WorldGen.GetBiomeInfluence(x, x, num, num, out var corruptCount, out var crimsonCount, out var hallowedCount);
			int num2 = corruptCount;
			if (num2 < crimsonCount)
			{
				num2 = crimsonCount;
			}
			if (num2 < hallowedCount)
			{
				num2 = hallowedCount;
			}
			int num3 = 0;
			num3 = (baseOption = ((corruptCount == 0 && crimsonCount == 0 && hallowedCount == 0) ? ((x < WorldGen.beachDistance || x > Main.maxTilesX - WorldGen.beachDistance) ? 1 : 0) : ((hallowedCount == num2) ? 2 : ((crimsonCount != num2) ? 4 : 3))));
			break;
		}
		case 530:
		{
			int num7 = y - tileCache.frameY % 36 / 18 + 2;
			int num8 = x - tileCache.frameX % 54 / 18;
			WorldGen.GetBiomeInfluence(num8, num8 + 3, num7, num7, out var corruptCount2, out var crimsonCount2, out var hallowedCount2);
			int num9 = corruptCount2;
			if (num9 < crimsonCount2)
			{
				num9 = crimsonCount2;
			}
			if (num9 < hallowedCount2)
			{
				num9 = hallowedCount2;
			}
			int num10 = 0;
			num10 = (baseOption = ((corruptCount2 != 0 || crimsonCount2 != 0 || hallowedCount2 != 0) ? ((hallowedCount2 == num9) ? 1 : ((crimsonCount2 != num9) ? 3 : 2)) : 0));
			break;
		}
		case 19:
		{
			int num5 = tileCache.frameY / 18;
			baseOption = 0;
			if (num5 == 48)
			{
				baseOption = 1;
		...
		case 468:
		{
			int tileData = tileCache.frameX / 36;
			switch (tileData)
			{
			case 0:
			case 1:
		...
		{
			int tileData = tileCache.frameX / 36;
			int num11 = tileCache.frameY / 18 - 1;
			tileData += num11 * 18;
			if (tileData < 6 || tileData == 19 || tileData == 20 || tileData == 21 || tileData == 22 || tileData == 23 || tileData == 24 || tileData == 33 || tileData == 38 || tileData == 39 || tileData == 40)
			{
				baseOption = 0;
			}
			else if (tileData < 16)
			...
			else if (tileData < 19 || tileData == 31 || tileData == 32)
			{
				baseOption = 1;
			}
			else if (tileData < 31)
		...
		case 649:
		{
			int tileData = tileCache.frameX / 18;
			if (tileData < 6 || tileData == 28 || tileData == 29 || tileData == 30 || tileData == 31 || tileData == 32)
			{
				baseOption = 0;
			}
			else if (tileData < 12 || tileData == 33 || tileData == 34 || tileData == 35)
			{
				baseOption = 1;
			}
			else if (tileData < 28)
		...
		case 647:
		{
			int tileData = tileCache.frameX / 54;
			if (tileData < 7)
			{
				baseOption = 2;
			}
			else if (tileData < 22 || tileData == 33 || tileData == 34 || tileData == 35)
			{
				baseOption = 0;
			}
			else if (tileData < 25)
		...
		case 648:
		{
			...
			tileData += num4 * 36;
			if (tileData < 3 || tileData == 14 || tileData == 15 || tileData == 16)
			{
				baseOption = 0;
			}
			else if (tileData < 6)
		...
		{
			int tileData = tileCache.frameX / 54;
			int num13 = tileCache.frameY / 54;
			tileData += num13 * 36;
			if ((tileData < 0 || tileData > 11) && (tileData < 47 || tileData > 53))
			{
				switch (tileData)
				{
				case 72:
		...
		case 242:
		{
			int tileData = tileCache.frameY / 72;
			if (tileCache.frameX / 106 == 0 && tileData >= 22 && tileData <= 24)
			{
				baseOption = 1;
			}
			else
		...
		case 428:
		{
			int tileData = tileCache.frameY / 18;
			if (tileData > 3)
			{
				tileData = 3;
			}
	...
	private static void InternalSaveMap()
	{
		...
		bool array = Main.ActivePlayerFileData.IsCloudSave;
			...
			try
			{
				...
				for (int j = 0; j < Main.maxTilesY; j++)
				{
					...
					for (num3 = 0; num3 < Main.maxTilesX; num3++)
					{
						...
						if (flag)
						{
							array2[num] = (byte)num8;
							num++;
							if (num8 > 255)
							{
								array2[num] = (byte)(num8 >> 8);
								num++;
							}
						}
						if (flag2)
						{
							array2[num] = mapTile.Light;
							num++;
						}
						if (num4 > 0)
						{
							array2[num] = (byte)num4;
							num++;
							if (num4 > 255)
							{
								array2[num] = (byte)(num4 >> 8);
								num++;
							}
						}
						for (int k = num5; k < num6; k++)
						{
							array2[num] = Main.Map[k, j].Light;
							num++;
						}
						num3 += num4;
						if (num >= 4096)
						{
							((Stream)(object)val).Write(array2, 0, num);
							num = 0;
						}
					}
				}
				...
				FileUtilities.WriteAllBytes(text, memoryStream.ToArray(), array);
			}
			finally
			{
