...
public class MoonlordDeathDrama
{
	...
	{
		Rectangle i = Utils.CenteredRectangle(Main.screenPosition + new Vector2(Main.screenWidth, Main.screenHeight) * 0.5f, new Vector2(Main.screenWidth + 1000, Main.screenHeight + 1000));
		for (int j = 0; j < _explosions.Count; j++)
		{
			if (_explosions[j].InDrawRange(i))
			{
				_explosions[j].Draw(spriteBatch);
			}
		}
	}
