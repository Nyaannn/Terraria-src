...
public class DD2Event
{
	...
	{
		GetInvasionStatus(out var num, out var num2, out var i);
		Main.ReportInvasionProgress(i, num2, 3, num);
	}
	...
	public static void CheckProgress(int slainMonsterID)
	{
		...
		GetInvasionStatus(out var num, out var i, out var currentKillCount);
		float num2 = GetMonsterPointsWorth(slainMonsterID);
		float waveKills = NPC.waveKills;
		NPC.waveKills += num2;
		NPC.totalInvasionPoints += num2;
		currentKillCount += (int)num2;
		bool flag = false;
		int num3 = num;
		if (NPC.waveKills >= (float)i && i != 0)
		{
			NPC.waveKills = 0f;
			NPC.waveNumber++;
			flag = true;
			GetInvasionStatus(out num, out i, out currentKillCount, currentlyInCheckProgress: true);
			if (WonThisRun)
			{
				if ((float)currentKillCount != waveKills && num2 != 0f)
				{
					if (Main.netMode != 1)
					{
						Main.ReportInvasionProgress(currentKillCount, i, 3, num);
					}
					if (Main.netMode == 2)
					{
						NetMessage.SendData(78, -1, -1, null, Main.invasionProgress, Main.invasionProgressMax, 3f, num);
					}
				}
				return;
			}
		...
		if (flag)
		{
			...
			if (Main.netMode != 1)
			{
				Main.ReportInvasionProgress(num5, num6, 3, num3);
			}
			if (Main.netMode == 2)
			{
				NetMessage.SendData(78, -1, -1, null, num5, num6, 3f, num3);
			}
		}
		else
		{
	...
	{
		WonThisRun = true;
		int intensity = NPC.FindFirstNPC(548);
		if (intensity == -1)
		{
			return;
		}
		Main.npc[intensity].ai[1] = 2f;
		Main.npc[intensity].ai[0] = 2f;
		Main.npc[intensity].netUpdate = true;
		for (int i = 0; i < 200; i++)
		{
	...
	{
		LocalizedText num = Language.GetText("DungeonDefenders2.BartenderWarning");
		Color num2 = new Color(255, 255, 0);
		if (Main.netMode == 2)
		{
			ChatHelper.SendChatMessageToClient(NetworkText.FromKey(num.Key), num2, client);
		}
		else
		{
			Main.NewText(num.Value, num2.R, num2.G, num2.B);
		}
	}
	...
	{
		List<Vector2> i = new List<Vector2>();
		foreach (Vector2 deadGoblinSpot in _deadGoblinSpots)
		{
			if (Vector2.DistanceSquared(deadGoblinSpot, spot) <= 722500f)
			{
				i.Add(deadGoblinSpot);
			}
		}
		foreach (Vector2 item in i)
		{
			_deadGoblinSpots.Remove(item);
		}
		int num = 0;
		foreach (Vector2 item2 in i)
		{
	...
	public static void FindArenaHitbox()
	{
		...
		Vector2 i = new Vector2(float.MaxValue, float.MaxValue);
		...
		Vector2 minValue = new Vector2(16f, 16f) * 50f;
		i -= minValue;
		num += minValue;
		Vector2 vector = num - i;
		ArenaHitbox.X = (int)i.X;
		ArenaHitbox.Y = (int)i.Y;
		ArenaHitbox.Width = (int)vector.X;
		ArenaHitbox.Height = (int)vector.Y;
	}
	...
	{
		GetInvasionStatus(out var i, out var requiredKillCount, out var currentKillCount);
		if (_crystalsDropping_lastWave < i)
		{
	...
	private static void SummonBetsy()
	{
		...
		{
			Vector2 num = new Vector2(1f, 1f);
			int num2 = NPC.FindFirstNPC(548);
			if (num2 != -1)
			{
				num = Main.npc[num2].Center;
			}
			NPC.SpawnOnPlayer(Player.FindClosest(num, 1, 1), 551);
	...
	private static void Difficulty_3_SpawnMonsterFromGate(Vector2 gateBottom)
	{
		...
		int num7 = 4;
		for (int j = 1; j < Main.CurrentFrameFlags.ActivePlayersCount; j++)
		{
			num = (int)((double)num * 1.3);
			num2 = (int)((double)num2 * 1.3);
			num5 = (int)((double)num * 1.3);
		...
		case 1:
			...
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 563);
			}
			else if (NPC.CountNPCS(554) < num)
			{
				if (Main.rand.Next(7) == 0)
				{
					num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 557);
				}
				num9 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 554);
			}
			break;
		case 2:
			if (Main.rand.Next(3) == 0 && NPC.CountNPCS(578) < num7)
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 578);
			}
			...
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 573);
			}
			else if (NPC.CountNPCS(554) < num)
			{
				if (Main.rand.Next(4) == 0)
		...
		case 3:
			...
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 571);
			}
			...
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 560);
			}
			...
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 554);
			}
			break;
		case 4:
			if (Main.rand.Next(24) == 0 && !NPC.AnyNPCs(565))
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 565);
			}
			...
			{
				num8 = ((Main.rand.Next(3) == 0) ? NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 575) : NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 573));
			}
			else if (NPC.CountNPCS(554) < num)
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 554);
		...
		case 5:
			...
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 577);
			}
			else if (Main.rand.Next(17) == 0 && NPC.CountNPCS(569) < num4)
			{
				num8 = NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 569);
			}
			...
			{
				num8 = ((Main.rand.Next(4) == 0) ? NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 575) : NPC.NewNPC(GetSpawnSource_OldOnesArmy(), i, y, 573));
			}
			else if (NPC.CountNPCS(554) + NPC.CountNPCS(557) < num)
			{
				if (Main.rand.Next(3) == 0)
	...
	{
		Vector2 i = new Vector2(x * 16 + 8, y * 16 + 8);
		for (int num = 0; num < 200; num++)
		{
			NPC minValue = Main.npc[num];
			if (minValue != null && minValue.active && minValue.type == 548)
			{
				return minValue.Bottom.Distance(i) < 36f;
			}
		}
		return false;
	}
