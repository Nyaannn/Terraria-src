...
public class HiveBiome : MicroBiome
{
	public override bool Place(Point origin, StructureMap structures)
	{
		...
		Ref<int> result = new Ref<int>(0);
		Ref<int> @ref = new Ref<int>(0);
		Ref<int> ref2 = new Ref<int>(0);
		WorldUtils.Gen(origin, new Shapes.Circle(15), Actions.Chain(new Modifiers.IsSolid(), new Actions.Scanner(result), new Modifiers.OnlyTiles(60, 59), new Actions.Scanner(@ref), new Modifiers.OnlyTiles(60), new Actions.Scanner(ref2)));
		if ((double)@ref.Value / (double)result.Value < 0.75 || ref2.Value < 2)
		{
			return false;
		}
		int num = 0;
	...
	private static void FrameOutAllHiveContents(Point origin, int squareHalfWidth)
	{
		...
		int info3 = Math.Max(10, origin.Y - squareHalfWidth);
		int info2 = Math.Min(Main.maxTilesY - 10, origin.Y + squareHalfWidth);
		for (int info = num; info < num2; info++)
		{
			for (int i = info3; i < info2; i++)
			{
				Tile tile = Main.tile[info, i];
				if (tile.active() && tile.type == 225)
				{
					WorldGen.SquareTileFrame(info, i);
				}
				if (tile.wall == 86)
				{
					WorldGen.SquareWallFrame(info, i);
				}
			}
		}
	}
	...
	private static bool TooCloseToImportantLocations(Point origin)
	{
		...
		int y = origin.Y;
		int num = 150;
		for (int i = result - num; i < result + num; i += 10)
		{
	...
	{
		for (int result = x - 1; result <= x + 2; result++)
		{
			for (int i = y - 1; i <= y + 2; i++)
			{
				if (result >= x && result <= x + 1 && i >= y && i <= y + 1)
				{
					Main.tile[result, i].active(active: false);
					Main.tile[result, i].liquid = byte.MaxValue;
					Main.tile[result, i].honey(honey: true);
				}
				else
				{
					Main.tile[result, i].active(active: true);
					Main.tile[result, i].type = 225;
				}
			}
		}
	}
	...
	{
		for (int messageAuthor = x - 1; messageAuthor <= x + 2; messageAuthor++)
		{
			for (int color = y - 1; color <= y + 2; color++)
			{
				if (messageAuthor < 10 || messageAuthor > Main.maxTilesX - 10)
				{
					return true;
				}
				if (Main.tile[messageAuthor, color].active() && Main.tile[messageAuthor, color].type != 225)
				{
					return true;
				}
			}
