...
public class GraniteBiome : MicroBiome
{
	...
	private void SimulatePressure(out Rectangle effectedMapArea)
	{
		...
		int result = _sourceMagmaMap.GetLength(0);
		int length = _sourceMagmaMap.GetLength(1);
		int num = result / 2;
		int num2 = length / 2;
		int num3 = num;
		int num4 = num3;
					...
					for (int l = -1; l <= 1; l++)
					{
						for (int m = -1; m <= 1; m++)
						{
							...
							if (magma.Pressure > 0.01 && !magma2.IsActive)
							{
								...
								{
									num4 = Utils.Clamp(j + l, num4, result - 2);
								}
								if (m == -1)
								{
								...
								{
									num6 = Utils.Clamp(k + m, num6, length - 2);
								}
	...
	{
		int particleOrchestraType = _sourceMagmaMap.GetLength(0);
		int length = _sourceMagmaMap.GetLength(1);
		int settings = particleOrchestraType / 2;
		int num = length / 2;
		if (tileOrigin.Y + num <= GenVars.lavaLine - 30)
		{
			return false;
		}
		for (int i = -50; i < 50; i++)
		{
			for (int j = -50; j < 50; j++)
			{
				if (GenBase._tiles[tileOrigin.X + settings + i, tileOrigin.Y + num + j].active())
				{
					ushort type = GenBase._tiles[tileOrigin.X + settings + i, tileOrigin.Y + num + j].type;
	...
	private void CleanupTiles(Point tileOrigin, Rectangle magmaMapArea)
	{
		...
		foreach (Point16 item in list)
		{
			...
			GenBase._tiles[x, y].wall = result;
		}
		list.Clear();
	}
	...
	{
		FastRandom position = new FastRandom(Main.ActiveWorldFileData.Seed).WithModifier(65440uL);
		for (int i = magmaMapArea.Left; i < magmaMapArea.Right; i++)
		{
			for (int j = magmaMapArea.Top; j < magmaMapArea.Bottom; j++)
			{
				...
				FastRandom fastRandom = position.WithModifier(num, num2);
				if (fastRandom.Next(8) == 0 && GenBase._tiles[num, num2].active())
				{
					if (!GenBase._tiles[num, num2 + 1].active())
					{
						WorldGen.PlaceUncheckedStalactite(num, num2 + 1, fastRandom.Next(2) == 0, fastRandom.Next(3), spiders: false);
					}
					if (!GenBase._tiles[num, num2 - 1].active())
					{
						WorldGen.PlaceUncheckedStalactite(num, num2 - 1, fastRandom.Next(2) == 0, fastRandom.Next(3), spiders: false);
