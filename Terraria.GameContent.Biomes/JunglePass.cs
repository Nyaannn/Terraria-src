...
public class JunglePass : GenPass
{
	...
	protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
	{
		...
		x = zero.X / 3;
		y = zero.Y / 3;
		int num = GenBase._random.Next((int)(400.0 * result), (int)(600.0 * result));
		int num2 = (int)(25.0 * result);
	...
	{
		for (int id = 0; (double)id < 6.0 * _worldScale; id++)
		{
	...
	{
		for (int result = 0; result < Main.maxTilesX / 4; result++)
		{
	...
	{
		int key = oldX;
		int persistentId = oldY;
		double sacrificeCount = _worldScale;
		for (int i = 0; (double)i <= 20.0 * sacrificeCount; i++)
		{
			progress.Set((60.0 + (double)i / sacrificeCount) * 0.01);
			key += GenBase._random.Next((int)(-5.0 * sacrificeCount), (int)(6.0 * sacrificeCount));
			persistentId += GenBase._random.Next((int)(-5.0 * sacrificeCount), (int)(6.0 * sacrificeCount));
			WorldGen.TileRunner(key, persistentId, GenBase._random.Next(40, 100), GenBase._random.Next(300, 500), 59);
		}
		for (int j = 0; (double)j <= 10.0 * sacrificeCount; j++)
		{
			progress.Set((80.0 + (double)j / sacrificeCount * 2.0) * 0.01);
			key = oldX + GenBase._random.Next((int)(-600.0 * sacrificeCount), (int)(600.0 * sacrificeCount));
			persistentId = oldY + GenBase._random.Next((int)(-200.0 * sacrificeCount), (int)(200.0 * sacrificeCount));
			while (key < 1 || key >= Main.maxTilesX - 1 || persistentId < 1 || persistentId >= Main.maxTilesY - 1 || Main.tile[key, persistentId].type != 59)
			{
				key = oldX + GenBase._random.Next((int)(-600.0 * sacrificeCount), (int)(600.0 * sacrificeCount));
				persistentId = oldY + GenBase._random.Next((int)(-200.0 * sacrificeCount), (int)(200.0 * sacrificeCount));
			}
			for (int k = 0; (double)k < 8.0 * sacrificeCount; k++)
			{
				key += GenBase._random.Next(-30, 31);
				persistentId += GenBase._random.Next(-30, 31);
				int type = -1;
				if (GenBase._random.Next(7) == 0)
				{
					type = -2;
				}
				WorldGen.TileRunner(key, persistentId, GenBase._random.Next(10, 20), GenBase._random.Next(30, 70), type);
			}
		}
		for (int l = 0; (double)l <= 300.0 * sacrificeCount; l++)
		{
			key = oldX + GenBase._random.Next((int)(-600.0 * sacrificeCount), (int)(600.0 * sacrificeCount));
			persistentId = oldY + GenBase._random.Next((int)(-200.0 * sacrificeCount), (int)(200.0 * sacrificeCount));
			while (key < 1 || key >= Main.maxTilesX - 1 || persistentId < 1 || persistentId >= Main.maxTilesY - 1 || Main.tile[key, persistentId].type != 59)
			...
			WorldGen.TileRunner(key, persistentId, GenBase._random.Next(4, 10), GenBase._random.Next(5, 30), 1);
			if (GenBase._random.Next(4) == 0)
			{
				int type2 = GenBase._random.Next(63, 69);
				WorldGen.TileRunner(key + GenBase._random.Next(-1, 2), persistentId + GenBase._random.Next(-1, 2), GenBase._random.Next(3, 7), GenBase._random.Next(4, 8), type2);
			}
		}
	}
}
