...
public static class TileMaterials
{
	...
	{
		using Stream dimensions = Assembly.GetExecutingAssembly().GetManifestResourceStream(path);
		using StreamReader isHovered = new StreamReader(dimensions);
		return JsonConvert.DeserializeObject<T>(isHovered.ReadToEnd());
	}
