...
public static class AssetInitializer
{
	public static void CreateAssetServices(GameServiceContainer services)
	{
		...
		AssetReaderCollection num = new AssetReaderCollection();
		num.RegisterReader((IAssetReader)new PngReader(XnaExtensions.Get<IGraphicsDeviceService>((IServiceProvider)services).GraphicsDevice), new string[1] { ".png" });
		num.RegisterReader((IAssetReader)new XnbReader((IServiceProvider)services), new string[1] { ".xnb" });
		AsyncAssetLoader num2 = new AsyncAssetLoader(num, 20);
		num2.RequireTypeCreationOnTransfer(typeof(Texture2D));
		num2.RequireTypeCreationOnTransfer(typeof(DynamicSpriteFont));
		num2.RequireTypeCreationOnTransfer(typeof(SpriteFont));
		IAssetRepository provider = (IAssetRepository)new AssetRepository((IAssetLoader)new AssetLoader(num), (IAsyncAssetLoader)(object)num2);
		services.AddService(typeof(AssetReaderCollection), num);
	...
	{
		GetResourcePacksFolderPathAndConfirmItExists(out var list, out var resourcePackFolder);
		return ResourcePackList.FromJson(list, services, resourcePackFolder);
	}
