...
public static class LaunchInitializer
{
	...
	{
		string num;
		if ((num = TryParameter("-j", "-join")) != null)
		{
			game.AutoJoin(num);
		}
		string i;
		if ((i = TryParameter("-pass", "-password")) != null)
		{
			Netplay.ServerPassword = Main.ConvertFromSafeArgument(i);
			game.AutoPass();
		}
	...
	{
		for (int mode = 0; mode < keys.Length; mode++)
		{
			if (Program.LaunchParameters.TryGetValue(keys[mode], out var value))
			{
				if (value == null)
				{
					return "";
