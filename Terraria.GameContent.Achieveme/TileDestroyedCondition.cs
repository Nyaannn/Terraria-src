...
public class TileDestroyedCondition : AchievementCondition
{
	...
	private static void ListenForDestruction(TileDestroyedCondition condition)
	{
		...
		for (int uIElement = 0; uIElement < condition._tileIds.Length; uIElement++)
		{
			if (!_listeners.ContainsKey(condition._tileIds[uIElement]))
			{
				_listeners[condition._tileIds[uIElement]] = new List<TileDestroyedCondition>();
			}
			_listeners[condition._tileIds[uIElement]].Add(condition);
		}
	}
