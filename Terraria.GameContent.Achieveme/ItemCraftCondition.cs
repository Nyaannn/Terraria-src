...
public class ItemCraftCondition : AchievementCondition
{
	...
	{
		AchievementCondition[] dimensions = new AchievementCondition[items.Length];
		for (int color = 0; color < items.Length; color++)
		{
			dimensions[color] = new ItemCraftCondition(items[color]);
		}
		return dimensions;
	}
}
