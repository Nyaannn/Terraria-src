...
public class CreativePowersHelper
{
	...
	public static void AddDescriptionIfNeeded(ref string originalText, string descriptionKey)
	{
		...
		{
			string num = Language.GetTextValue(descriptionKey);
			originalText = originalText + "\n" + num;
		}
	}
	...
	public static void AddUnlockTextIfNeeded(ref string originalText, bool needed, string descriptionKey)
	{
		if (!needed)
		{
			string value = Language.GetTextValue(descriptionKey);
			originalText = originalText + "\n" + value;
		}
	}
	...
	{
		bool i = power.GetIsUnlocked();
		bool canvasPositionOfIndex = !IsAvailableForPlayer(power, Main.myPlayer);
		asButton.SetBorderColor(canvasPositionOfIndex ? Color.DimGray : Color.White);
		if (canvasPositionOfIndex)
		{
