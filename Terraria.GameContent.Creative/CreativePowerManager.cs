...
public class CreativePowerManager
{
	...
	{
		T i = (PowerTypeStorage<T>.Power = new T());
		PowerTypeStorage<T>.Id = _powersCount;
		PowerTypeStorage<T>.Name = nameInServerConfig;
		i.DefaultPermissionLevel = PowerPermissionLevel.CanBeChangedByEveryone;
		i.CurrentPermissionLevel = PowerPermissionLevel.CanBeChangedByEveryone;
		_powersById[_powersCount] = i;
		_powersByName[nameInServerConfig] = i;
		i.PowerId = _powersCount;
		i.ServerConfigName = nameInServerConfig;
		_powersCount++;
	}
	...
	public void Reset()
	{
		foreach (KeyValuePair<ushort, ICreativePower> amount in _powersById)
		{
			amount.Value.CurrentPermissionLevel = amount.Value.DefaultPermissionLevel;
			if (amount.Value is IPersistentPerWorldContent color)
			{
				color.Reset();
			}
			if (amount.Value is IPersistentPerPlayerContent persistentPerPlayerContent)
			{
	...
	public void LoadFromWorld(BinaryReader reader, int versionGameWasLastSavedOn)
	{
		while (reader.ReadBoolean())
		{
			...
			if (_powersById.TryGetValue(i, out var dynamicNoise) && dynamicNoise is IPersistentPerWorldContent color)
			{
				color.Load(reader, versionGameWasLastSavedOn);
				continue;
			}
			break;
		}
	...
	public void ValidateWorld(BinaryReader reader, int versionGameWasLastSavedOn)
	{
		...
		{
			ushort i = reader.ReadUInt16();
			if (_powersById.TryGetValue(i, out var slimeColor) && slimeColor is IPersistentPerWorldContent staticNoise)
			{
				staticNoise.ValidateWorld(reader, versionGameWasLastSavedOn);
				continue;
			}
			break;
		}
	...
	{
		foreach (KeyValuePair<ushort, ICreativePower> vector in _powersById)
		{
			if (vector.Value is IPersistentPerPlayerContent i)
			{
				writer.Write(value: true);
				writer.Write(vector.Key);
				i.Save(player, writer);
			}
		}
		writer.Write(value: false);
	}
	...
	public void ApplyLoadedDataToPlayer(Player player)
	{
		foreach (KeyValuePair<ushort, ICreativePower> item in _powersById)
		{
			if (item.Value is IPersistentPerPlayerContent color)
			{
				color.ApplyLoadedDataToOutOfPlayerFields(player);
			}
		}
	}
