...
public class CreativePowers
{
	public abstract class APerPlayerTogglePower : ICreativePower, IOnPlayerJoining
	{
		...
		public void SetEnabledState(int playerIndex, bool state)
		{
			...
			if (Main.netMode == 2)
			{
				NetPacket num20 = NetCreativePowersModule.PreparePacket(PowerId, 3);
				num20.Writer.Write((byte)1);
				num20.Writer.Write((byte)playerIndex);
				num20.Writer.Write(state);
				NetManager.Instance.Broadcast(num20);
			}
		}
		...
		public void Reset()
		{
			for (int num19 = 0; num19 < _perPlayerIsEnabled.Length; num19++)
			{
				_perPlayerIsEnabled[num19] = _defaultToggleState;
			}
		}
		...
		{
			bool num18 = _perPlayerIsEnabled[Main.myPlayer];
			GroupOptionButton<bool> groupOptionButton = affectedElement as GroupOptionButton<bool>;
			groupOptionButton.SetCurrentOption(num18);
	...
	public abstract class APerPlayerSliderPower : ICreativePower, IOnPlayerJoining, IProvideSliderElement, IPowerSubcategoryElement
	{
		...
		{
			float num11 = GetSliderValue();
			float num12 = UILinksInitializer.HandleSliderVerticalInput(num11, 0f, 1f, PlayerInput.CurrentProfile.InterfaceDeadzoneX, 0.35f);
			if (num12 != num11)
			{
				SetValueKeyboard(num12);
			}
		}
		...
		private void categoryButton_OnUpdate(UIElement affectedElement)
		{
			if (affectedElement.IsMouseHovering)
			{
				GroupOptionButton<int> num10 = affectedElement as GroupOptionButton<int>;
				string originalText = Language.GetTextValue(_powerNameKey + (num10.IsSelected ? "_Opened" : "_Closed"));
				CreativePowersHelper.AddDescriptionIfNeeded(ref originalText, _powerNameKey + "_Description");
				CreativePowersHelper.AddUnlockTextIfNeeded(ref originalText, GetIsUnlocked(), _powerNameKey + "_Unlock");
				CreativePowersHelper.AddPermissionTextIfNeeded(this, ref originalText);
				Main.instance.MouseTextNoOverride(originalText, 0, 0);
		...
		{
			for (int num9 = 0; num9 < _cachePerPlayer.Length; num9++)
			{
				ResetForPlayer(num9);
			}
		}
	...
	public abstract class ASharedButtonPower : ICreativePower
	{
		...
		public void ProvidePowerButtons(CreativePowerUIElementRequestInfo info, List<UIElement> elements)
		{
			GroupOptionButton<bool> num3 = CreativePowersHelper.CreateSimpleButton(info);
			CreativePowersHelper.UpdateUnlockStateByPower(this, num3, CreativePowersHelper.CommonSelectedColor);
			num3.Append(CreativePowersHelper.GetIconImage(_iconLocation));
			num3.OnLeftClick += button_OnClick;
			num3.OnUpdate += button_OnUpdate;
			elements.Add(num3);
		}
	...
	public abstract class ASharedTogglePower : ICreativePower, IOnPlayerJoining
	{
		...
		public void DeserializeNetMessage(BinaryReader reader, int userId)
		{
			...
			if (Main.netMode != 2 || CreativePowersHelper.IsAvailableForPlayer(this, userId))
			{
				...
				if (Main.netMode == 2)
				{
					NetPacket indexesSortable29 = NetCreativePowersModule.PreparePacket(PowerId, 1);
					indexesSortable29.Writer.Write(Enabled);
					NetManager.Instance.Broadcast(indexesSortable29);
				}
			}
		}
		...
		private void button_OnUpdate(UIElement affectedElement)
		{
			...
			GroupOptionButton<bool> indexesSortable26 = affectedElement as GroupOptionButton<bool>;
			indexesSortable26.SetCurrentOption(enabled);
			if (affectedElement.IsMouseHovering)
			{
				string buttonTextKey = GetButtonTextKey();
				string item13 = Language.GetTextValue(buttonTextKey + (indexesSortable26.IsSelected ? "_Enabled" : "_Disabled"));
				CreativePowersHelper.AddDescriptionIfNeeded(ref item13, buttonTextKey + "_Description");
				CreativePowersHelper.AddUnlockTextIfNeeded(ref item13, GetIsUnlocked(), buttonTextKey + "_Unlock");
				CreativePowersHelper.AddPermissionTextIfNeeded(this, ref item13);
				Main.instance.MouseTextNoOverride(item13, 0, 0);
			}
		}
	...
	public abstract class ASharedSliderPower : ICreativePower, IOnPlayerJoining, IProvideSliderElement, IPowerSubcategoryElement
	{
		...
		public void DeserializeNetMessage(BinaryReader reader, int userId)
		{
			...
			if (Main.netMode != 2 || CreativePowersHelper.IsAvailableForPlayer(this, userId))
			{
				...
				if (Main.netMode == 2)
				{
					NetPacket indexesSortable12 = NetCreativePowersModule.PreparePacket(PowerId, 4);
					indexesSortable12.Writer.Write(num);
					NetManager.Instance.Broadcast(indexesSortable12);
				}
			}
		}
		...
		internal void SetValueGamepad()
		{
			...
			float indexesSortable3 = UILinksInitializer.HandleSliderVerticalInput(sliderValue, 0f, 1f, PlayerInput.CurrentProfile.InterfaceDeadzoneX, 0.35f);
			if (indexesSortable3 != sliderValue)
			{
				SetValueKeyboard(indexesSortable3);
			}
		}
		...
		private void categoryButton_OnUpdate(UIElement affectedElement)
		{
			if (affectedElement.IsMouseHovering)
			{
				...
				string indexesSortable = Language.GetTextValue(_powerNameKey + (groupOptionButton.IsSelected ? "_Opened" : "_Closed"));
				CreativePowersHelper.AddDescriptionIfNeeded(ref indexesSortable, _powerNameKey + "_Description");
				CreativePowersHelper.AddUnlockTextIfNeeded(ref indexesSortable, GetIsUnlocked(), _powerNameKey + "_Unlock");
				CreativePowersHelper.AddPermissionTextIfNeeded(this, ref indexesSortable);
				Main.instance.MouseTextNoOverride(indexesSortable, 0, 0);
			}
			AttemptPushingChange();
		}
	...
	public class FarPlacementRangePower : APerPlayerTogglePower, IPersistentPerPlayerContent
	{
		...
		public void Save(Player player, BinaryWriter writer)
		{
			bool hAPICall = IsEnabledForPlayer(Main.myPlayer);
			writer.Write(hAPICall);
		}
