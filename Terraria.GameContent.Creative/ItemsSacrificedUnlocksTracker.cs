...
public class ItemsSacrificedUnlocksTracker : IPersistentPerWorldContent, IOnPlayerJoining
{
	...
	{
		foreach (KeyValuePair<int, int> mousevec in _sacrificesCountByItemIdCache)
		{
			if (TryGetSacrificeNumbers(mousevec.Key, out var _, out var flag) && mousevec.Value >= flag)
			{
				toObtainInfinitely.Add(mousevec.Key);
			}
		}
	}
