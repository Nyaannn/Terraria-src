...
public static class NPCUtils
{
	...
	public static TargetSearchResults SearchForTarget(NPC searcher, Vector2 position, TargetSearchFlag flags = TargetSearchFlag.All, SearchFilter<Player> playerFilter = null, SearchFilter<NPC> npcFilter = null)
	{
		...
		return new TargetSearchResults(searcher, num, (float)Math.Sqrt(list), nearestTankIndex, nearestTankDistance, num2, tankType);
	}

	public static void TargetClosestOldOnesInvasion(NPC searcher, bool faceTarget = true, Vector2? checkPosition = null)
	{
		TargetSearchResults localPlayer = SearchForTarget(searcher, TargetSearchFlag.All, SearchFilters.OnlyPlayersInCertainDistance(searcher.Center, 200f), SearchFilters.OnlyCrystal);
		if (localPlayer.FoundTarget)
		{
			searcher.target = localPlayer.NearestTargetIndex;
			searcher.targetRect = localPlayer.NearestTargetHitbox;
			if (searcher.ShouldFaceTarget(ref localPlayer) && faceTarget)
			{
				searcher.FaceTarget();
			}
		}
	...
	{
		TargetSearchResults localPlayer = SearchForTarget(searcher, TargetSearchFlag.Players, SearchFilters.DownwindFromNPC(searcher, distanceMaxX));
		if (localPlayer.FoundTarget)
		{
			searcher.target = localPlayer.NearestTargetIndex;
			searcher.targetRect = localPlayer.NearestTargetHitbox;
	...
	{
		TargetSearchResults uIPointForBuildingMode = SearchForTarget(searcher, TargetSearchFlag.All, null, SearchFilters.OnlyCrystal);
		if (uIPointForBuildingMode.FoundTarget)
		{
			TargetType value = uIPointForBuildingMode.NearestTargetType;
			if (uIPointForBuildingMode.FoundTank && !uIPointForBuildingMode.NearestTankOwner.dead)
			{
				value = TargetType.Player;
			}
			searcher.target = uIPointForBuildingMode.NearestTargetIndex;
			searcher.targetRect = uIPointForBuildingMode.NearestTargetHitbox;
			if (searcher.ShouldFaceTarget(ref uIPointForBuildingMode, value) && faceTarget)
			{
				searcher.FaceTarget();
			}
		}
