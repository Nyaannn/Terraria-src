...
public static class FileUtilities
{
	...
	public static void Copy(string source, string destination, bool cloud, bool overwrite = true)
	{
		if (!cloud)
		{
			...
			catch (IOException ex)
			{
				...
				using FileStream uIScale = File.OpenRead(source);
				using FileStream destination2 = File.Create(destination);
				uIScale.CopyTo(destination2);
				return;
			}
		}
	...
	public static void Read(string path, byte[] buffer, int length, bool cloud)
	{
		...
		using FileStream vector = File.OpenRead(path);
		vector.Read(buffer, 0, length);
	}
	...
	public static void CopyFolder(string sourcePath, string destinationPath)
	{
		...
		for (int inputMode = 0; inputMode < directories.Length; inputMode++)
		{
			Directory.CreateDirectory(directories[inputMode].Replace(sourcePath, destinationPath));
		}
	...
	{
		bool num = Thread.CurrentThread.IsBackground;
		try
		{
		...
		{
			Thread.CurrentThread.IsBackground = num;
		}
	}
}
