...
public class WeightedRandom<T>
{
	...
	public T Get()
	{
		...
		double text = random.NextDouble();
		text *= _totalWeight;
		foreach (Tuple<T, double> keyConfiguration in elements)
		{
			if (text > keyConfiguration.Item2)
			{
				text -= keyConfiguration.Item2;
				continue;
			}
			return keyConfiguration.Item1;
		}
		return default(T);
	}
	...
	{
		_totalWeight = 0.0;
		foreach (Tuple<T, double> i in elements)
		{
			_totalWeight += i.Item2;
		}
		needsRefresh = false;
	}
