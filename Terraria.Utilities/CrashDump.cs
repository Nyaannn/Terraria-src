...
public static class CrashDump
{
	...
	{
		DateTime text = DateTime.Now.ToLocalTime();
		return string.Format("{0}_{1}_{2}_{3}.dmp", Main.dedServ ? "TerrariaServer" : "Terraria", Main.versionNumber, text.ToString("MM-dd-yy_HH-mm-ss-ffff", CultureInfo.InvariantCulture), Thread.CurrentThread.ManagedThreadId);
	}
	...
	private static bool Write(Options options, ExceptionInfo exceptionInfo, string outputDirectory)
	{
		...
		string text = Path.Combine(outputDirectory, CreateDumpName());
		if (!Utils.TryCreatingDirectory(outputDirectory))
		{
			return false;
		}
		using FileStream i = File.Create(text);
		return Write(i.SafeFileHandle, options, exceptionInfo);
	}
